---V4.2 白龙战斗
local XLevelScript90002 = XDlcScriptManager.RegLevelLogicScript(90002, "XLevelLogicScript90002") --注册脚本类到管理器（逻辑脚本注册
...
function XLevelScript90002:InitPhase()
    --初始化关卡各个阶段的相关变量
end
---@param phase number
function XLevelScript90002:SetPhase(phase)    --跳转关卡阶段
...
function XLevelScript90002:OnEnterPhase(phase)
...
---@param dt number @ delta time
function XLevelScript90002:OnUpdatePhase(dt)
...
function XLevelScript90002:OnExitPhase(phase)
    --退出一个关卡阶段时需要做的事情在这里实现（最好不要在这里跳转关卡阶段

end
function XLevelScript90002:OnPhaseChanged(lastPhase, nextPhase)
    --关卡阶段改变时需要执行的逻辑，一般用于通知外部
end
function XLevelScript90002:HandlePhaseEvent(eventType, eventArgs)

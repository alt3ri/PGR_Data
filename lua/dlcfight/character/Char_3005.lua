...
function XCharTest:ProcessSwitchNpc()
    ...
    elseif self._proxy:IsKeyDown(ENpcOperationKey.SwitchNpc4) then
        self._proxy:SwitchPlayerNpc(self._uuid, ENpcOperationKey.SwitchNpc4)
    end
end
--endregion
...
function XCharTest:TryDoChangeMoveState()
    ...
    elseif curMoveType == ENpcMoveType.Sprint then
        if moveNormalizedDist <= normalizedWalk2Run and moveNormalizedDist > normalizedWalk2Run then                -- 摇杆小于慢走阈值且不为0则切换为慢走
            nextMoveType = ENpcMoveType.Walk
        elseif self._proxy:IsKeyDown(ENpcOperationKey.SwitchWalk) then                                                  -- 点击了慢走切换按键则切换为慢走
            nextMoveType = ENpcMoveType.Walk

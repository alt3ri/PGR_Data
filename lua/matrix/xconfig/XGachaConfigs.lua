...
function XGachaConfigs.GetClientConfigList(key)
    if not GachaClientConfig[key] then
        XLog.ErrorTableDataNotFound("XGachaConfigs.GetClientConfig", "GachaClientConfig", TABLE_GACHA_CLIENT_CONFIG, "key", tostring(key))
        return nil
    end
...
function XGachaConfigs.GetClientConfigNumberList(key)
    if not GachaClientConfig[key] then
        XLog.ErrorTableDataNotFound("XGachaConfigs.GetClientConfig", "GachaClientConfig", TABLE_GACHA_CLIENT_CONFIG, "key", tostring(key))
        return 0
    end
    ...
    for i, v in ipairs(GachaClientConfig[key].Value) do
        if string.IsFloatNumber(v) then
            table.insert(numberList, tonumber(v))
        else
            XLog.Error('GachaClientConfig.tab中，key：'..tostring(key)..'index:'..tostring(i)..'的配置不是个数值：'..tostring(v))

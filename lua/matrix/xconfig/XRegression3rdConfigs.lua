...
local TABLE_SURVEY_PATH = "Share/Regression3/Regression3Survey.tab"
...
function XRegression3rdConfigs.Init()
    ...
    --- 问卷
    TableSurvey = XTableManager.ReadByIntKey(TABLE_SURVEY_PATH, XTable.XTableRegression3Survey, "Id")
end
...
--region   ------------------问卷 start-------------------
function XRegression3rdConfigs.GetSurveyCfgIds()
    local ids = {}

    if not XTool.IsTableEmpty(TableSurvey) then
        for i, v in pairs(TableSurvey) do
...
---@return XTableRegression3Survey
function XRegression3rdConfigs.GetSurveyCfgById(id)
    local cfg = TableSurvey[id]

    if cfg then
        return cfg
    else
        XLog.ErrorTableDataNotFound("XRegression3rdConfigs->GetSurveyCfgById", "TableSurvey", TABLE_SURVEY_PATH, "id", id)
    end
end
--endregion------------------问卷 finish------------------
...
function XSkyGardenShoppingStreetModel:OnInit()
    ...
    self._TotalStageStarNum = self:GetMaxStageId() * 3
    self.CurrentStarNum = 0
...
function XSkyGardenShoppingStreetModel:CalculateProperty()
    ...
    self:UpdateMusicPart()
end

-- satisfactionNum2MusicPart
function XSkyGardenShoppingStreetModel:UpdateMusicPart(inputPart)
    if not XMVCA.XSkyGardenShoppingStreet:IsInGameUi() then return end

    if inputPart and inputPart ~= self._lastPart then
        XLuaAudioManager.SetMusicSourceFirstBlockIndex(inputPart)
        self._lastPart = inputPart
        return
    end
    local clientType = XMVCA.XSkyGardenShoppingStreet.StageResType
...
function XSkyGardenShoppingStreetModel:CalculateSatisfaction(isCache, isCacheAndUpdateShow)
    ...
    if isCache then
        ...
        if isCacheAndUpdateShow then
            self._StartSatisfactionNum = self._StartSatisfactionNum + self._InitFriendlyCache - lastFriendlyCache
...
function XSkyGardenShoppingStreetModel:FoodChefIdSwitchX3cId(id)
    if not self._FoodChiefSwitchIdMap then
        self._FoodChiefSwitchIdMap = { 1, 2, 3, }
    end
    return self._FoodChiefSwitchIdMap[id]
end

function XSkyGardenShoppingStreetModel:FoodIdSwitchX3cId(id)
    if not self._FoodSwitchIdMap then
        self._FoodSwitchIdMap = { 3, 1, 2, }
    end
    return self._FoodSwitchIdMap[id]
end

function XSkyGardenShoppingStreetModel:DessertIdSwitchX3cId(id)
    if not self._DessertSwitchIdMap then
        self._DessertSwitchIdMap = { 2, 1, 3, 4, }
    end
    return self._DessertSwitchIdMap[id]
end
...
function XSkyGardenShoppingStreetModel:GetX3CSceneData(data)
    ...
    for i = 1, self._InsideShowMaxNum + self._OutsideShowMaxNum do
        ...
        if shopData then
            ...
            local shopShowData
            local funcType = shopCfg.FuncType
            local funcData = XMVCA.XSkyGardenShoppingStreet:GetFuncDataByShopData(shopData)
            local XSgStreetShopFuncType = XMVCA.XSkyGardenShoppingStreet.XSgStreetShopFuncType
            if funcType == XSgStreetShopFuncType.Food then
                shopShowData = { Ints = {}, }
                for key, value in pairs(funcData.GoodsCountList) do
                    shopShowData.Ints[self:FoodIdSwitchX3cId(key)] = value
                end
                shopShowData.Ints[4] = self:FoodChefIdSwitchX3cId(funcData.ChefId)
            elseif funcType == XSgStreetShopFuncType.Grocery then
                shopShowData = { Ints = {}, }
                for _, shelfData in pairs(funcData.ShelfDatas) do
                    table.insert(shopShowData.Ints, shelfData.GoodsId)
                end
                for i= #shopShowData.Ints + 1, 3 do
                    table.insert(shopShowData.Ints, 0)
                end
            elseif funcType == XSgStreetShopFuncType.Dessert then
                shopShowData = { Ints = {}, }
                local count = #funcData.GoodsIdList
                for goodIndex, goodId in pairs(funcData.GoodsIdList) do
                    shopShowData.Ints[count - goodIndex + 1] = self:DessertIdSwitchX3cId(goodId)
                end
            end

            table.insert(shopList, {
                PlaceId = placeId,
                ShopResId = shopCfg.ShopResId,
                Level = showLevel,
                FuncType = funcType,
                ShopShowData = shopShowData,
            })
        end
    end
...
function XSkyGardenShoppingStreetModel:SetPassedStageIds(passedStageIds)
    ...
    self.CurrentStarNum = 0
    for _, getList in pairs(passedStageIds) do
        for _, _ in pairs(getList.RewardIndexRecord or {}) do
            self.CurrentStarNum = self.CurrentStarNum + 1
        end
    end
end

function XSkyGardenShoppingStreetModel:IsComplete()
    return self:GetCurrentStarNum() >= self:GetTotalStarNum()
end

function XSkyGardenShoppingStreetModel:GetInsideShowMaxNum()
    return self._InsideShowMaxNum
end

function XSkyGardenShoppingStreetModel:GetTotalStarNum()
    return self._TotalStageStarNum
end

function XSkyGardenShoppingStreetModel:GetCurrentStarNum()
    return self.CurrentStarNum
end
...
function XSkyGardenShoppingStreetModel:GetRewardIndexRecord(stageId)
    if not self.PassedStageIds then return {} end
    local data = self.PassedStageIds[stageId]
    if not data then return {} end
    return data.RewardIndexRecord or {}
end
...
function XSkyGardenShoppingStreetModel:_UpdateStageData(stageData, isUpdate)
    ...
    self:SetFirstOutsideBuildTurn(self.StageData.FirstOutsideBuildTurn)
    if isUpdate then
...
function XSkyGardenShoppingStreetModel:GetDefaultShopShowData(shopConfig)
    if shopConfig.FuncType == XMVCA.XSkyGardenShoppingStreet.XSgStreetShopFuncType.Food then
        local shopConfig = self:GetShopFoodConfigsByShopId(shopConfig.Id)
        local x3cId = {}
        for i = 1, #shopConfig.Goods do
            local goodId = shopConfig.Goods[i]
            local cfg = self:GetShopFoodGoodsConfigsByGoodId(goodId)
            x3cId[self:FoodIdSwitchX3cId(goodId)] = cfg.GoodsInit
        end
        x3cId[4] = self:FoodChefIdSwitchX3cId(shopConfig.Chef[1])
        return { Ints = x3cId, }
    elseif shopConfig.FuncType == XMVCA.XSkyGardenShoppingStreet.XSgStreetShopFuncType.Grocery then
        return { Ints = {0,0,0}, }
    elseif shopConfig.FuncType == XMVCA.XSkyGardenShoppingStreet.XSgStreetShopFuncType.Dessert then
        local shopConfig = self:GetShopDessertConfigsByShopId(shopConfig.Id)
        local goodsIdList = {}
        local dessertCount = #shopConfig.Goods
        for i = 1, dessertCount do
            goodsIdList[i] = shopConfig.Goods[dessertCount - i + 1]
        end
        return { Ints = goodsIdList, }
    end
    return {}
end
...
function XSkyGardenShoppingStreetModel:UpdateShopAreaData(shopData, isSkipEvent, isSkipDataUpdate)
    if not shopData or not self.StageDataNow then return end
    ...
    if not isSkipEvent or isOutside then
        local isInGame = XMVCA.XBigWorldGamePlay:IsInGame()
        if isInGame then
            local areaId = self:GetAreaIdByShopId(shopId)
            if not hasShop then
                local shopConfig = self:GetShopConfigById(shopId)
                local shopShowData = self:GetDefaultShopShowData(shopConfig)
                XMVCA.X3CProxy:Send(CS.X3CCommand.CMD_SHOPSTREET_CREATE_BUILDING, {
                    PlaceId = self:GetPlaceIdByAreaId(areaId),
                    ShopResId = areaData:GetShopResId(),
                    FuncType = shopConfig.FuncType,
                    ShopShowData = shopShowData,
                })
            end
            if areaData:IsLevelChange() then
...
function XSkyGardenShoppingStreetModel:ClientSatisfactionGrow(growValue)
    ...
    if self._TotalFinishCount <= 0 then return end
    ...
    if growValue then
        ...
        growSatisfaction = currentNum + add
    else
        self._FinishCount = self._FinishCount + 1
        growSatisfaction = self._StartSatisfactionNum + (self._InitFriendlyCache - self._StartSatisfactionNum) * (self._FinishCount / self._TotalFinishCount)
        growSatisfaction = math.floor(growSatisfaction)
    end
    ...
    growSatisfaction = XMath.Clamp(growSatisfaction, 0, self:GetMaxSatisfactionSatisfaction())
...
-- 获取buff显示
function XSkyGardenShoppingStreetModel:_AddBuffShow(buffData)
    local buffCfg = self:GetBuffConfigById(buffData.BuffConfigId)
    ...
    table.insert(self._ShowGetBuffList, buffData)
...
-- 增加buff数据
function XSkyGardenShoppingStreetModel:_AddBuffData(buffData, isNew)
    table.insert(self._GameBuffs, buffData)
    if isNew then self:_AddBuffShow(buffData) end
end
...
function XSkyGardenShoppingStreetModel:SetBuffsData(buffDatas, isUpdate, isForceCache)
    ...
    if isUpdate and buffDatas and self._GameBuffs then
        for _, buffData in pairs(self._GameBuffs) do
            ...
            hasBuffList[buffData.BuffUid] = true
        end
    end
    ...
    if buffDatas then
        for _, buffData in pairs(buffDatas) do
            local isNew = false
            if hasBuffList and not hasBuffList[buffData.BuffUid] then
                if not self._clientAddBuff or not self._clientAddBuff[buffData.BuffConfigId] then
                    isNew = true
                end
            end
...
function XSkyGardenShoppingStreetModel:GetCurrentStageId(isSafe)
    ...
    if not self.StageData then return 0 end
...
function XSkyGardenShoppingStreetModel:SetTaskDatas(taskDatas, isUpdate)
    ...
    if isUpdate and self.StageDataNow and self.StageDataNow.TaskDatas then
        local nowTaskDatas = self.StageDataNow.TaskDatas
        if not nowTaskDatas then nowTaskDatas = {} end

        for _, data in pairs(taskDatas) do
            local id = data.Id
            local isFound = false
            for i = 1, #nowTaskDatas do
                if nowTaskDatas[i].Id == id then
                    nowTaskDatas[i] = data
                    isFound = true
                    break
                end
            ...
            if not isFound then
                table.insert(nowTaskDatas, data)
            end
        end
        self.StageDataNow.TaskDatas = nowTaskDatas
    end
    ...
    XEventManager.DispatchEvent(XMVCA.XBigWorldService.DlcEventId.EVENT_BUSINESS_STREET_TASK_REFRESH)
end
...
function XSkyGardenShoppingStreetModel:SetFinishComfirm(isComfirm)
    self._IsFinishComfirm = isComfirm
end

function XSkyGardenShoppingStreetModel:IsFinishComfirm()
    return self._IsFinishComfirm
end
...
-- 获取资源数据
function XSkyGardenShoppingStreetModel:GetStageResById(resId, isReal)
    if isReal then
        for _, v in pairs(self.StageDataNow.ResourceDatas) do
            if v.ResourceId == resId then
                return v.Count
            end
        end
    end
...
function XSkyGardenShoppingStreetModel:GetStageGameBuff(buffId)
    ...
    for _, v in pairs(self._GameBuffs) do
        if v.BuffConfigId == buffId then return v end
    end
end
...
function XSkyGardenShoppingStreetModel:GetStageGameBuffs(getCount)
    ...
    for _, serverBuff in pairs(self._GameBuffs) do
        if self:_StageGameBuffDefaultFilterFunc(serverBuff.BuffConfigId) then
...
function XSkyGardenShoppingStreetModel:ShopUpgradeCostReduceBySubType(subType, cost)
    if not self._ShopAttrAdds then return cost end
    local shopAttrAdds = self._ShopAttrAdds[subType]
    if not shopAttrAdds then return cost end
...
-- 突发事件启用特殊奖励buff
function XSkyGardenShoppingStreetModel:HasEmergencyRewardSpOptionBuff()
    local num = self._AttrAdds[XMVCA.XSkyGardenShoppingStreet.XSgStreetAttrType.EmergencyRewardSpOptionBuff] or 0
    return num > 0
end

-- 获取商店上次累计到店人数
function XSkyGardenShoppingStreetModel:GetLastTotalCustomerNumByShopId(shopId)
    if not self.StageDataNow then return 0 end
    local statisticsData = self.StageDataNow.StatisticsData
    if not statisticsData then return 0 end
    return statisticsData.CustomerNums[shopId] or 0
end
...
-- 获取累计流水
function XSkyGardenShoppingStreetModel:GetAccumulativeGold()
    if not self.StatisticsData then return 0 end
    return self.StatisticsData.AccumulativeGold
end

-- 不满事件处理
function XSkyGardenShoppingStreetModel:DoDiscontentEvent(Id, AwardGold)
    if self._IdCheck[Id] then
        XLog.Warning("XSkyGardenShoppingStreetModel:DoDiscontentEvent", "重复事件", Id)
        return
    end
    self._IdCheck[Id] = true
...
function XSkyGardenShoppingStreetModel:DoEmergencyEvent(Id, EmergencyOptionIndex, buffId)
    if self._IdCheck[Id] then
        XLog.Warning("XSkyGardenShoppingStreetModel:DoEmergencyEvent", "重复事件", Id)
        return
    end
    self._IdCheck[Id] = true
    table.insert(self._EventResults, {
...
-- 反馈处理
function XSkyGardenShoppingStreetModel:DoFeedbackEvent(Id, FeedBackData)
    if self._IdCheck[Id] then
        XLog.Warning("XSkyGardenShoppingStreetModel:DoFeedbackEvent", "重复事件", Id)
        return
    end
    self._IdCheck[Id] = true
    table.insert(self._EventResults, { Id = Id, })
    table.insert(self._RunningFeedbackList, FeedBackData)
end

-- 反馈列表
function XSkyGardenShoppingStreetModel:GetRunningFeedbackList()
    return self._RunningFeedbackList
end
...
function XSkyGardenShoppingStreetModel:IsMaxTurn()
    return self:GetRunRound() >= self:GetMaxRunRound()
end

-- 获取当前显示任务id
function XSkyGardenShoppingStreetModel:GetDisplayTaskConfigId()
    local XSgStreetTaskState = XMVCA.XSkyGardenShoppingStreet.XSgStreetTaskState
    local XSgStreetTaskSource = XMVCA.XSkyGardenShoppingStreet.XSgStreetTaskSource
    for _, data in pairs(self.TaskDatas) do
        if data.Source == XSgStreetTaskSource.StageTarget and data.State == XSgStreetTaskState.Activated then
...
function XSkyGardenShoppingStreetModel:StartRunRound(operatingData)
    ...
    self._TotalFinishCount = #datas
    for i = 1, self._TotalFinishCount do
        local index = i
...
function XSkyGardenShoppingStreetModel:GetMaxRunRound()
    return self._MaxTurn or 0
end

function XSkyGardenShoppingStreetModel:GetFirstOutsideBuildTurn()
    return self.FirstOutsideBuildTurn or 0
end

function XSkyGardenShoppingStreetModel:SetFirstOutsideBuildTurn(FirstOutsideBuildTurn)
    if not FirstOutsideBuildTurn then return end
    self.FirstOutsideBuildTurn = FirstOutsideBuildTurn
end

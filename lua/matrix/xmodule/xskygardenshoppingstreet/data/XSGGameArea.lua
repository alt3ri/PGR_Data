...
function XSGGameArea:SetDataCommonInfo(data)
    self._Data = data
    if self._Data.LastCollectFeedBacks then
        self._feedbackList = {}
        for _, feedbackData in pairs(self._Data.LastCollectFeedBacks) do
            local feedBackDataTemp = feedbackData.FeedBackData
            feedBackDataTemp.CustomerId = feedbackData.CustomerId
            table.insert(self._feedbackList, feedBackDataTemp)
        end
    end
end
...
-- 获取商店显示等级
function XSGGameArea:IsLevelChange()
    if self:GetShopLevel() <= 0 then return false end
    if self._Data.Level == 1 then return true end
    local curConfig = XMVCA.XSkyGardenShoppingStreet:GetShopLevelConfigById(self._Data.ShopId, self._Data.Level, self._IsInside)
    local lastConfig = XMVCA.XSkyGardenShoppingStreet:GetShopLevelConfigById(self._Data.ShopId, self._Data.Level - 1, self._IsInside)
    return curConfig.ShowLevel ~= lastConfig.ShowLevel
end
...
function XSGGameArea:CanShowUpgradeTips()
    ...
    if self:IsInside() then
        ...
        return XMVCA.XSkyGardenShoppingStreet:CanShowUpgradeTips(self._Data.ShopId, self._Data.Level, self._IsInside)
    else
        if self._Data.Level <= 0 then return false end
        local nextLevel = self._Data.Level + 1
        local maxLevel = XMVCA.XSkyGardenShoppingStreet:GetShopMaxLevel(self._Data.ShopId)
...
function XSGGameArea:GetFeedbackDatas()
    if self._Data.LastCollectFeedBacks then
        return self._feedbackList or {}
    end
    if self._Data.LastFeedBacks then

...
function XBigWorldMessageControl:GetContactsMessageList()
    ...
    for _, message in pairs(messageList) do
        ...
        if XTool.IsNumberValid(contactsId) then
            if not contactsMap[contactsId] then
                contactsMap[contactsId] = {}
            end

            table.insert(contactsMap[contactsId], message)
        end
    end
    for _, messages in pairs(contactsMap) do
...
function XBigWorldMessageControl:PeekForceMessageData()
    return self._Model:PeekForceMessageData()
end

---@return XBWMessageData
function XBigWorldMessageControl:DequeueForceMessageData()
...
function XBigWorldMessageControl:_SortContactsHandle(contactsDataA, contactsDataB)
    local messageListA = contactsDataA.Messages
    local messageListB = contactsDataB.Messages
    local isNonCompleteA = self:_CheckMessageListHasNonComplete(messageListA)
    local isNonCompleteB = self:_CheckMessageListHasNonComplete(messageListB)

    if isNonCompleteA ~= isNonCompleteB then
        return isNonCompleteA
    end

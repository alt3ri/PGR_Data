...
function XBWMessageEntity:IsRecord()
    if not self:IsNil() then
        return self._Model:GetMessageReadRecord(self:GetMessageId())
    end
...
function XBWMessageEntity:IsForce()
    return self:GetMessageType() == XEnumConst.BWMessage.MessageType.ForcePlay
end

function XBWMessageEntity:GetMessageType()
    if not self:IsNil() then
        return self._Model:GetBigWorldMessageTypeById(self:GetMessageId())
    end

    return XEnumConst.BWMessage.MessageType.Normal
end
...
function XBWMessageEntity:GetCurrentText()
    if not self:IsNil() then
        ...
        while content and not content:IsEnd() do
            if not content:IsCompleteWithOption() then
...
function XBWMessageEntity:Record()
    if not self:IsNil() then
        self._Model:SetMessageRecord(self:GetMessageId())
    end
end

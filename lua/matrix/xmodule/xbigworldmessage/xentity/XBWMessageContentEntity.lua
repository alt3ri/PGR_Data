...
--- 如果是选项节点判断这个选项是否选择
function XBWMessageContentEntity:IsCompleteWithOption()
...
function XBWMessageContentEntity:GetText()
    if not self:IsOptions() and not self:IsMemes() then
        ...
        return XUiHelper.ReplaceTextNewLine(self._OwnControl:ReplaceMessageContentPlayerName(text))
    end
...
function XBWMessageContentEntity:GetSpeakerIcon()
    ...
    elseif self:IsSend() then
        return self._Model:GetPlayerHeadIcon()
    end
...
function XBWMessageContentEntity:GetNextStepId()
    ...
    elseif self:IsOptions() and self:IsCompleteWithOption() then
        local count = self:GetOprionsCount()

...
local Value2FuncName = {
    ...
    --     SetFunc = "SetRenderScaleLevel",
    -- },
    _SceneQualityLevel = {
        GetFunc = "GetSceneQualityLevel",
        SetFunc = "SetSceneQualityLevel",
    },
    _RoleQualityLevel = {
        GetFunc = "GetRoleQualityLevel",
        SetFunc = "SetRoleQualityLevel",
    },
    _LightingQualityLevel = {
        GetFunc = "GetLightingQualityLevel",
        SetFunc = "SetLightingQualityLevel",
    },
    _CharacterShadowLevel = {
        GetFunc = "GetCharacterShadowLevel",
        SetFunc = "SetCharacterShadowLevel",
    },
}

function XBWGraphicsSetting:InitValue()
    for key, config in pairs(Value2FuncName) do
        self[config.GetFunc] = function(setting)
            return setting[key]:GetValue()
        end
        self[config.SetFunc] = function(setting, value)
            return setting[key]:SetValue(value)
        end
    end
...
function XBWGraphicsSetting:Reset()
    ...
    for key, _ in pairs(Value2FuncName) do
        self[key]:Reset()
    end
...
function XBWGraphicsSetting:IsChanged()
    ...
    for key, _ in pairs(Value2FuncName) do
        if self[key] and self[key]:IsChanged() then
            return true
        end
    end
...
function XBWGraphicsSetting:_InitValueByName(defaultValue, value, name)
    if not self[name] then
        ---@type XBWSettingValue
        self[name] = XBWSettingValue.New(defaultValue, value)
        self[name]:RegisterValueChangedEvent(Handler(self, self.__OnQualitySettingChanged))
    else
        self[name]:Init(defaultValue, value)
    end
end
...
function XBWGraphicsSetting:_InitOther(defaultQuality, quality)
    ...
    for key, config in pairs(Value2FuncName) do
        local getFunc = config.GetFunc
        self:_InitValueByName(defaultValue[getFunc](defaultValue), value[getFunc](value), key)
    end
...
function XBWGraphicsSetting:_InitQualityValue(quality)
    ...
    for key, config in pairs(Value2FuncName) do
        local getFunc = config.GetFunc
        self[key]:SetValueWithoutEvent(value[getFunc](value))
    end
...
function XBWGraphicsSetting:__ToCsQualitySetting()
    ...
    for _, config in pairs(Value2FuncName) do
        quality[config.SetFunc](quality, self[config.GetFunc](self))
    end

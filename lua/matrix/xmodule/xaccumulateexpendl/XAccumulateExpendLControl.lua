---@class XAccumulateExpendControlL : XControl
---@field private _Model XAccumulateExpendModelL
local XAccumulateExpendControlL = XClass(XControl, "XAccumulateExpendControlL")
local XAccumulateExpendReward = require("XModule/XAccumulateExpendL/XEntity/XAccumulateExpendReward")
local XAccumulateExpendRuler = require("XModule/XAccumulateExpendL/XEntity/XAccumulateExpendRuler")

function XAccumulateExpendControlL:OnInit()
    -- 初始化内部变量
...
function XAccumulateExpendControlL:AddAgencyEvent()
    -- control在生命周期启动的时候需要对Agency及对外的Agency进行注册
end

function XAccumulateExpendControlL:RemoveAgencyEvent()

end

function XAccumulateExpendControlL:OnRelease()
    self._RewardList = nil
    -- XLog.Error("这里执行Control的释放")
end

function XAccumulateExpendControlL:GetCurrentRewardIndex()
...
function XAccumulateExpendControlL:GetCurrentRewardCount()
    local rewardList = self:GetRewardList()
...
function XAccumulateExpendControlL:GetNextSpecialReward(index)
    local rewardList = self:GetRewardList()
...
function XAccumulateExpendControlL:GetAtLastSpecialRewardIndex()
    local rewardList = self:GetRewardList()
...
function XAccumulateExpendControlL:GetStartTime()
    local activityId = self._Model:GetActivityId()
    local timeId = self._Model:GetActivityTimeIdById(activityId)
...
function XAccumulateExpendControlL:GetEndTimeStr()
    local time = 0
...
function XAccumulateExpendControlL:AutoCloseHandler(isClose)
    if isClose then
...
function XAccumulateExpendControlL:GetRewardList(isForceRefresh)
...
function XAccumulateExpendControlL:GetRewardCount()
    local rewardList = self:GetRewardList()
...
function XAccumulateExpendControlL:RefreshRewardList()
...
function XAccumulateExpendControlL:ReceiveAllReward()
    local rewardList = self:GetRewardList()
    local taskIds = {}
...
function XAccumulateExpendControlL:GetRulerList()
...
function XAccumulateExpendControlL:CheckAllFinish()
    local rewardList = self:GetRewardList()
...
return XAccumulateExpendControlL

...
function XFangKuaiActivity:UpdateStageData(chapterId, serverData)
    ---@type XFangKuaiStageData
    local stageData = require("XModule/XFangKuai/XEntity/XFangKuaiStageData").New()
    stageData:UpdateStageData(serverData)
    self._StageDatas[chapterId] = stageData
end
...
function XFangKuaiActivity:GetAllMaxRound()
    local maxRound = 0
    for _, data in pairs(self._StageHistroyDict) do
        if maxRound < data.MaxRound then
...
---@return XFangKuaiStageData[]
function XFangKuaiActivity:GetStageDatas()
    return self._StageDatas
end
...
function XFangKuaiActivity:IsNoStagePassed()
    return XTool.IsTableEmpty(self._FinishedStageIds)
end

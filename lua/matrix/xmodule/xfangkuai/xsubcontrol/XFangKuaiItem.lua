...
function XFangKuaiItem:ExecuteLengthReduce(itemIdx, color)
    ...
    for blockData, _ in pairs(blockDatas) do
        if (not color or blockData:GetColor() == color) and not blockData:IsBoss() then
...
function XFangKuaiItem:ExecuteBecomeOneGrid(itemIdx, color, chapterId)
    ...
    local blockDatas = self._MainControl:GetBlockMap()
    for blockData, _ in pairs(blockDatas) do
        if (not color or blockData:GetColor() == color) and not blockData:IsBoss() and blockData:GetLen() > 1 then
            -- 对1长度的方块不生效
...
function XFangKuaiItem:ExecuteBorn(itemIdx, color, maxX, maxY, chapterId)
    ...
    for y = 1, maxY do
        ...
        for blockData, _ in pairs(blocks) do
            ...
            if not blockData:IsBoss() and (not color or blockData:GetColor() == color) then
                isCheck = true
            end
        end
        if not isCheck then
...
function XFangKuaiItem:IsNoBossAndSameColor(blockData, dimColorId)
    return blockData and not blockData:IsBoss() and (not dimColorId or blockData:GetColor() == dimColorId)
end
...
---@param chiefBlockData XFangKuaiBlock

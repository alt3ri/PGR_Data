...
local TableKey = {
    ...
    Theatre3Event = { CacheType = XConfigUtil.CacheType.Normal },
    ...
    Theatre3EventNode = { CacheType = XConfigUtil.CacheType.Normal, DirPath = XConfigUtil.DirectoryType.Client, Identifier = "EventId" },
...
function XTheatre3Model:GetClientConfigs(key)
    local config = self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.Theatre3ClientConfig, key)
    if not config then
        return nil
    end
...
local function TeamGeneralSkillCacheKey()
    return 'XTheatre3'..tostring(XPlayer.Id)
end

function XTheatre3Model:GetTeamGeneralSkillId()
    if self._TeamGeneralSkillId == nil then
        self._TeamGeneralSkillId = XSaveTool.GetData(TeamGeneralSkillCacheKey()) or 0
    end
    
    return self._TeamGeneralSkillId
end

function XTheatre3Model:SetTeamGeneralSkillId(generalSkillId)
    self._TeamGeneralSkillId = generalSkillId
end

function XTheatre3Model:SaveTeamGeneralSkillId()
    XSaveTool.SaveData(TeamGeneralSkillCacheKey(), self._TeamGeneralSkillId)
end

function XTheatre3Model:GetTeamData()
    return self.ActivityData:GetTeamData()
end
--endregion
...
function XTheatre3Model:InitCharacterConfig()
    ...
    -- tab表是无序的 这里按Id从小到大排序
    for _, tb in pairs(self.CharacterIdToCharacterEndingIdList) do
        table.sort(tb)
    end
...
function XTheatre3Model:IsHaveAdventure()
    if XTool.IsNumberValid(self.ActivityData:GetDifficultyId()) then
        return true
    end
    if XTool.IsNumberValid(self.ActivityData:GetCurChapterId()) then
        return true
    end
    return false
end

function XTheatre3Model:IsAdventureALine()
    if not self:IsHaveAdventure() then
        return true
    end
    return self:CheckChapterIsALine(self.ActivityData:GetCurChapterId())
end
--endregion
...
function XTheatre3Model:GetEventCfgByIdAndStep(eventId, stepId)
    for _, cfg in ipairs(self:GetEventConfigs()) do
...
---@return XTableTheatre3ItemGroup[]
function XTheatre3Model:GetItemGroupConfigs()
    return self._ConfigUtil:GetByTableKey(TableKey.Theatre3ItemGroup)
end

---@return XTableTheatre3ItemGroup
function XTheatre3Model:GetItemGroupConfigById(id)
    return self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.Theatre3ItemGroup, id)
end

---@return XTableTheatre3ItemGroup[]
function XTheatre3Model:GetItemsByGroupId(groupId)
    if not self._ItemGroupMap then
        self._ItemGroupMap = {}
    end
    local items = self._ItemGroupMap[groupId]
    if not items then
        items = {}
        local configs = self:GetItemGroupConfigs()
        for _, v in pairs(configs) do

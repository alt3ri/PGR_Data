...
function XDlcRoomData:GetNodeId()
...
function XDlcRoomData:SetLevelId(value)
    self._LevelId = value
end
...
function XDlcRoomData:AddPlayerDataBySource(playerData)
    if playerData then
        local player = XDlcPlayerData.New(XMVCA.XDlcWorld:GetWorldTypeById(self:GetWorldId()), playerData)
    ...
    if data then
        ...
        if not XTool.IsTableEmpty(playerDataList) then
            for i = 1, #playerDataList do
                ...
                local worldType = XMVCA.XDlcWorld:GetWorldTypeById(self:GetWorldId())

                if not playerData then
                    playerData = XDlcPlayerData.New(worldType, playerDataList[i])
                else
                    playerData:SetDataWithRoomData(playerDataList[i], worldType)
                end

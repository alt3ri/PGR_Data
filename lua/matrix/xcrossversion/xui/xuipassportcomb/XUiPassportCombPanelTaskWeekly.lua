...
---@class XUiPassportCombPanelTaskWeekly:XUiNode
local XUiPassportCombPanelTaskWeekly = XClass(XUiNode, "XUiPassportCombPanelTaskWeekly")

--周任务
function XUiPassportCombPanelTaskWeekly:Ctor(ui, rootUi)
    self.RootUi = rootUi
    ...
    self:AddRedPointEvent(self.BtnTongBlack, self.OnCheckTaskRedPoint, self, { XRedPointConditions.Types.CONDITION_PASSPORT_COMB_TASK_WEEKLY_RED })
end

function XUiPassportCombPanelTaskWeekly:Refresh()
...
function XUiPassportCombPanelTaskWeekly:UpdateTime()
...
function XUiPassportCombPanelTaskWeekly:OnDynamicTableEvent(event, index, grid)
...
--一键领取
function XUiPassportCombPanelTaskWeekly:OnBtnTongBlackClick()
    self._Control:FinishMultiTaskRequest(XEnumConst.PASSPORT.TASK_TYPE.WEEKLY)
end

function XUiPassportCombPanelTaskWeekly:OnCheckTaskRedPoint(count)
    self.BtnTongBlack:ShowReddot(count >= 0)
end

function XUiPassportCombPanelTaskWeekly:Show()
    self.GameObject:SetActiveEx(true)
    self:Refresh()
end

function XUiPassportCombPanelTaskWeekly:Hide()
    self.GameObject:SetActiveEx(false)
end

function XUiPassportCombPanelTaskWeekly:IsShow()
...
return XUiPassportCombPanelTaskWeekly
...
function XConfigUtil:InitConfigByTableKey(parentPath, tableKey, forceAllCacheType)
    ...
    for k, v in pairs(tableKey) do
        local path
        if v.DirPath == XConfigUtil.DirectoryType.Client then
            path = "Client/".. parentPath .."/" .. k ..".tab"
            
        elseif v.DirPath == XConfigUtil.DirectoryType.Custom then
            path = v.Path
            
        else
            path = "Share/".. parentPath .."/" .. k ..".tab"
        end
        ...
        if func then
            ...
            if IsWindowsEditor then --在编辑器状态下检查如果是私有的表格, 然后界面没有引用的话可能是有问题的
                ...
                if args[4] == XConfigUtil.CacheType.Temp then --临时配置表
                    local time = os.time()
                    if self._TempTimeTabs[path] and time - self._TempTimeTabs[path] <= 10 then
                        XLog.Error(string.format("Temp配置表短期内连续访问, 请考虑更改读表方式, 配置表: %s", path))
                    end
                    self._TempTimeTabs[path] = time
...
function XConfigUtil:HasArgs(path)
    return self._ConfigArgs[path] and true or false
end

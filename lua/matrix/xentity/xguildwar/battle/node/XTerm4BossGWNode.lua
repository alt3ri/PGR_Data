...
function XTerm4BossGWNode:GetMaxDamage()
    ...
    if aliveType == XGuildWarConfig.FightRecordAliveType.Alive and not self:GetIsRuinsStatus() then
        ...
        local node1MaxDamage = not XTool.IsTableEmpty(node1) and node1:GetMaxDamage() or 0
        local node2MaxDamage = not XTool.IsTableEmpty(node2) and node2:GetMaxDamage() or 0

        return math.max(node1MaxDamage, node2MaxDamage)
...
function XTerm4BossGWNode:GetShowMonsterName()
    ...
    -- 使用XNormalGWNode而不是self.Super是因为它只支持直接父类的调用，不支持自底向上多层调用，会因为传参的self实例没有变化而导致无限递归
    return XNormalGWNode.GetShowMonsterName(self) .. " Lv." .. lv
end

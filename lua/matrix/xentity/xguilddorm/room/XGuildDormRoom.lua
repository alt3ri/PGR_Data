...
function XGuildDormRoom:CreateRole(playerData, containerGo)
    ...
    local XGDInteractCheckAudioFloorComponent = require("XEntity/XGuildDorm/Components/XGDInteractCheckAudioFloorComponent")
    ...
    local XGDFurnitureRandomBoxInteractComponent = require("XEntity/XGuildDorm/Components/XGDFurnitureRandomBoxInteractComponent")
    ...
    if XPlayer.Id == playerData.PlayerId then
        ...
        role:AddComponent(XGDInteractCheckAudioFloorComponent.New(role, self))
        role:AddComponent(XGDFurnitureInteractComponent.New(role))
        role:AddComponent(XGDNpcInteractComponent.New(role, self))
        role:AddComponent(XGDFurnitureSpecialInteractComponent.New(role, self))
        role:AddComponent(XGDFurnitureRandomBoxInteractComponent.New(role, self))
        role:AddComponent(XGDActionPlayComponent.New(role))
        -- 设置摄像机跟随角色
        rlRole:UpdateCameraFollow()
        rlRole:SetTransparent(XDataCenter.GuildDormManager.GetHideOwnPlayer() and 0 or 1)
    else
...
function XGuildDormRoom:SetIsShow(value)
    ...
    for _, role in pairs(self.Roles) do
        ...
        if not role:CheckIsSelfPlayer() then
            ...
            role:GetRLRole():SetTransparent(XDataCenter.GuildDormManager.GetHideOwnPlayer() and 0 or 1)
        end
    end
...
-- 刷新画板模型贴图
function XGuildDormRoom:RefreshPaint()
    local cfgs = XGuildDormConfig.GetPaintCfgs()
    if not XTool.IsTableEmpty(cfgs) then
        for i, cfg in pairs(cfgs) do
            local furnitureData = self:GetFurnitureById(cfg.Id)
            if furnitureData then
                local imageAddress = self:GetPaintImage(cfg)
                -- 设置贴图
                local rawImage = furnitureData.FurnitureModel.Transform:GetComponentInChildren(typeof(CS.UnityEngine.UI.RawImage))
                if rawImage then
                    rawImage:SetRawImage(imageAddress)
                end
            end
        end
    end
...
function XGuildDormRoom:GetPaintImage(cfg)
    local startTime = 0
    local paintCfgs = nil
    
    if XTool.IsNumberValid(cfg.TimeId) and XFunctionManager.CheckInTimeByTimeId(cfg.TimeId) then
        -- 使用特殊贴图
        startTime = XFunctionManager.GetStartTimeByTimeId(cfg.TimeId)
        paintCfgs = cfg.SpeicalPaints
    else
        -- 使用普通贴图
        -- 获取公会创建时间
        startTime = XDataCenter.GuildDormManager.GetGuildCreateTime() or 0
        paintCfgs = cfg.DefaultPaints
    end

    -- 修正回退到5点（如果开始时间不是5点整）
    startTime = self:FixedStartTime(startTime)
    -- 计算经过了几天 (86400 = 24 * 3600)
    local curTimestamp = XTime.GetServerNowTimestamp()
    local passDays = math.floor((curTimestamp - startTime) / 86400)
    -- 计算显示图片的索引
    local paintCount = paintCfgs and #paintCfgs or 0
    local index = XTool.IsNumberValid(paintCount) and math.floor((passDays % paintCount) + 1) or 0

    return paintCfgs and paintCfgs[index] or ''
end

-- 修正开始时间到刷新时间点
function XGuildDormRoom:FixedStartTime(time)
    -- 修正回退到5点（如果开始时间不是5点整）
    local dt = CS.XDateUtil.GetGameDateTime(time)
    if dt.Hour == 5 and (dt.Minute > 0 or dt.Second > 0) then
        time = time - dt.Second - dt.Minute * 60
    elseif dt.Hour > 5 then
        time = time - (dt.Hour - 5) * 3600 - dt.Minute * 60 - dt.Second
    elseif dt.Hour < 5 then
        -- 先补足到5点
        time = time + (5 - dt.Hour) * 3600 - dt.Minute * 60 - dt.Second
        -- 再回退12个小时 （43200 = 12 * 3600）
        time = time - 43200
    end

...
function XAdventureManager:UpdateTeamByServerData(team, data)
    ...
    -- 入场结算动画角色自选
    team:SetEnterCgIndex(data.EnterCgIndex)
    team:SetSettleCgIndex(data.SettleCgIndex)
end
...
function XAdventureManager:RequestSetSingleTeam(callback)
    ...
    local requestBody = {
        TeamData = {
            ...
            CardIds = cardIds,
            RobotIds = robotIds,
            EnterCgIndex = self.SingleTeam:GetEnterCgIndex(),
            SettleCgIndex = self.SingleTeam:GetSettleCgIndex(),
        }
    }

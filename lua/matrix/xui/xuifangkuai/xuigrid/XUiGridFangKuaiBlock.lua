...
function XUiGridFangKuaiBlock:OnStart()
    ...
    self._ScaleVec3 = CS.UnityEngine.Vector3(1, 1, 1)
    self._ClearBlockTime = tonumber(self._Control:GetClientConfig("ClearBlockTime"))
    self._WaneBlockTime = tonumber(self._Control:GetClientConfig("WaneBlockTime"))
end
...
function XUiGridFangKuaiBlock:Init(blockData)
    ...
    self._ScaleVec3.x = self.BlockData:IsFacingLeft() and 1 or -1
    self:ChangeAlpha(1)
    self:UpdateBlock()
    self:UpdatePosition()
...
function XUiGridFangKuaiBlock:Recycle()
    ...
    if not XTool.UObjIsNil(self._ChiefAnimEnable) then
        self._ChiefAnimEnable:StopTimelineAnimation()
    end
    self:Close()
end
...
function XUiGridFangKuaiBlock:UpdatePosition()
    ...
    self.Transform.localScale = self._ScaleVec3
...
function XUiGridFangKuaiBlock:UpdateBlock()
    ...
    if self._Control:IsDebug() then
        ...
        self.TxtDebug.text = self.BlockData:GetScore()
        self.TxtDebug.transform.localScale = self._ScaleVec3
    else
...
function XUiGridFangKuaiBlock:UpdateChiefAnimation()
    if self.BlockData:GetBlockType() == XEnumConst.FangKuai.BlockType.Chief and self.BlockData:GetRemainHitTimes() == 1 then
        if not self._ChiefAnimEnable then
            self._ChiefAnimEnable = self.Transform:Find("Content/ImgOne/ImgOneRoot/Animation/CommanderLoop")
            self._ChiefAnimDisable = self.Transform:Find("Content/ImgOne/ImgOneRoot/Animation/CommanderDisable")
        end
        self._ChiefAnimEnable:PlayTimelineAnimation()
        self._ChiefAnimDisable:StopTimelineAnimation()
    elseif not XTool.UObjIsNil(self._ChiefAnimEnable) then
        self._ChiefAnimEnable:StopTimelineAnimation()
        self._ChiefAnimDisable:PlayTimelineAnimation()
    end
end
...
function XUiGridFangKuaiBlock:RemoveTimer()
    ...
    if self._WaneTimer then
        XScheduleManager.UnSchedule(self._WaneTimer)
        self._WaneTimer = nil
    end
end
...
function XUiGridFangKuaiBlock:PlayClearUp(cb)
    ...
    self._RecycleTimer = XScheduleManager.ScheduleOnce(function()
        cb()
        self:Recycle()
    end, self._ClearBlockTime * 1000)
end

function XUiGridFangKuaiBlock:PlayBossWane(isClear)
    -- 等整行消除特效播完再缩短长度
    local delayTime = isClear and (self._WaneBlockTime * 1000 - 10) or 300

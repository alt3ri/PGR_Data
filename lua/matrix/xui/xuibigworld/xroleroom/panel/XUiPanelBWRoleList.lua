...
function XUiGridBWRoleVTeam:Refresh(id, selectId, pos, index)
    self._Id = id
    self._Index = index
...
function XUiGridBWRoleVTeam:OnBtnClick()
    self.Parent:OnSelect(self._Index, self)
end
...
function XUiPanelBWRoleList:InitView()
    ...
    self._DynamicTable:SetProxy(XUiGridBWRoleVTeam, self)
...
function XUiPanelBWRoleList:OnDynamicTableEvent(evt, index, grid)
    ...
    if evt == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_ATINDEX then
        ...
        grid:Refresh(entityId, self._SelectEntityId, pos, index)

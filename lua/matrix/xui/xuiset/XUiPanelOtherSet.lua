...
function XUiPanelOtherSet:InitUi()
    ...
    --region DamageStyleSetting
    local damageStyle = {self.TogType1, self.TogType2}
    self.GroupGraphicsType:Init(damageStyle, handler(self, self.OnDamageStyleChanged))
    self.TxtType1.text = CsXTextManagerGetText("NumStyleOld")
    self.TxtType2.text = CsXTextManagerGetText("NumStyleNew")
    --endregion DamageStyleSetting
    ...
    local captionTypes = {self.TogCaptionType1, self.TogCaptionType2}
    self.TGroupCaptionType:Init(captionTypes, handler(self, self.OnCaptionTypeChanged))

    self.TxtCaptionType1.text = CsXTextManagerGetText("CaptionType1")
    self.TxtCaptionType2.text = CsXTextManagerGetText("CaptionType2")
...
function XUiPanelOtherSet:ShowFilings()
    ...
    if XFunctionManager.CheckFunctionFitter(XFunctionManager.FunctionName.ICPFiling) then
        self.Filings.gameObject:SetActiveEx(false)
        return
    end
...
function XUiPanelOtherSet:GetCache()
    ...
    self.CurNumStyle = XSaveTool.GetData(self.SetConfigs.NumStyleKey) or self.SetConfigs.NumStyleConfig.New
    ...
    self.GroupGraphicsType:SelectIndex(self.CurNumStyle)
    ...
    self.SaveNumStyle = self.CurNumStyle
    ...
    self.CaptionType = XDataCenter.SetManager.CaptionType
    self.TGroupCaptionType:SelectIndex(self.CaptionType)

    self.FightCameraVibration = XDataCenter.SetManager.FightCameraVibration
    self:SetUIVibrationSliderValue(self.FightCameraVibration)
end
...
function XUiPanelOtherSet:ResetToDefault()
    ...
    ---重置飘字样式设定，需要需要判定一下场合
    local inFight = XFightUtil.IsFighting();
    ---非战中默认设置成新的。 其余情况保持原本设定
    if (not inFight) then
        self.CurNumStyle = self.SetConfigs.NumStyleConfig.New
    end
    ...
    self:SetUIVibrationSliderValue(self.SetConfigs.DefaultFightCameraVibration)
end
...
function XUiPanelOtherSet:SaveChange()
    ...
    XDataCenter.SetManager.SaveNumStyle(self.CurNumStyle)
    ...
    XDataCenter.SetManager.SetNumStyle(self.CurNumStyle)
    ...
    XDataCenter.SetManager.SetCaptionType(self.CaptionType)
    
    XDataCenter.SetManager.SetFightCameraVibration(self.FightCameraVibration)
...
function XUiPanelOtherSet:CheckDataIsChange()
    return
            ...
            self.SaveNumStyle ~= self.CurNumStyle or
            ...
            self.WeaponTransType ~= XDataCenter.SetManager.WeaponTransType or
            self.FightCameraVibration ~= XDataCenter.SetManager.FightCameraVibration
end
...
function XUiPanelOtherSet:OnDamageStyleChanged(index)
    self.CurNumStyle = index
    self.GraphicsTypeOld.gameObject:SetActiveEx(index  == 1)
    self.GraphicsTypeNew.gameObject:SetActiveEx(index  == 2)
end
...
function XUiPanelOtherSet:OnCaptionTypeChanged(index)
    if index == 0 then
        return
    end
    self.CaptionType = index
    self.DescriptionCaptionType1.gameObject:SetActiveEx(index == 1)
    self.DescriptionCaptionType2.gameObject:SetActiveEx(index == 2)
end
...
function XUiPanelOtherSet:ShowPanel()
    ...
    if self.GraphicsSet then
        local inFight = XFightUtil.IsFighting();
        local congfigState = CS.XGame.ClientConfig:GetInt("HideNewNum") or 0;
        self.GraphicsSet.gameObject:SetActiveEx(not inFight and congfigState == 0);
    end
end
...
--region Fight Camera Vibration

function XUiPanelOtherSet:InitFightVibration()
    local uiObj = self.Vibration
    uiObj:GetObject("BtnInput").CallBack = function()
        self:OnVibrationBtnInputClick()
    end

    uiObj:GetObject("Slider").onValueChanged:AddListener(function(value)
        value = math.ceil(value)
        self.FightCameraVibration = value
        self.Vibration:GetObject("BtnInput"):SetName(value)
    end)
end

function XUiPanelOtherSet:OnVibrationBtnInputClick()
    XLuaUiManager.Open("UiSetNumber", 0, 100, 3, function(num)
        self:SetUIVibrationSliderValue(num)
    end)
end

function XUiPanelOtherSet:SetUIVibrationSliderValue(value)
    value = math.max(0, math.ceil(value))
    self.FightCameraVibration = value
    local slider = self.Vibration:GetObject("Slider")
    slider.value = value
    self.Vibration:GetObject("BtnInput"):SetName(value)
end

...
function XUiGuildWarTerm4PanelDetail:Update()
    ...
    local rootNode = node:GetNodeType() == XGuildWarConfig.NodeType.NodeBoss7 and node or node:GetParentNode()
    -- 血量
...
function XUiGuildWarTerm4PanelDetail:RefreshRewardList()
    if self._IsIngoreBossReward then
...
function XUiGuildWarTerm4PanelDetail:SetIsIngoreBossReward(ignoreBossReward)
    self._IsIngoreBossReward = ignoreBossReward
end

---@class XUiGridSkillEffectBall3D:XUiNode
...
function XUiGridSkillEffectBall3D:LoadQualityEffectIcon()
    -- 因为PrefabLoader，是在start以后才加载完成，所以需要注册callback，等待完成
    local prefabLoader = XUiHelper.TryGetComponent(self.EffectBallSmallQuality, "", "XPrefabLoader")
    if not prefabLoader then
        XLog.Error("[XUiGridSkillEffectBall3D] 找不到品质图标的PrefabLoader")
        return
    end
    ...
    if prefabLoader.RegisterCallback then
        prefabLoader:RegisterCallback(function()
            local icon = XMVCA.XCharacter:GetCharacterQualityEffectIcon(self.CurQuality)
            local resource = self._Control:GetLoader():Load(icon)
            if resource then
                local child = self.EffectBallSmallQuality:GetChild(0)

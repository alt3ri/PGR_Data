...
function XUiPanelCharInfoWithEquipOther:UpdateCharacter(data)
    ...
    table.sort(self.Awareness, function (a, b)
        local aSite = XMVCA.XEquip:GetEquipSiteByEquip(a)
        local bSite = XMVCA.XEquip:GetEquipSiteByEquip(b)
        return aSite < bSite
    end)
...
function XUiPanelCharInfoWithEquipOther:UpdateRoleView()
    ...
    local initColor = XMVCA.XCharacter:GetModelCharacterQualityIconInitColor(initQuality)
    ...
    -- 机制
    local generalSkillIds = XMVCA.XCharacter:GetOtherPlayerCharactersActiveGeneralSkillIdList(character)
...
function XUiPanelCharInfoWithEquipOther:UpdateAwarenessView()
    ...
    for _, equipSite in pairs(XEnumConst.EQUIP.EQUIP_SITE.AWARENESS) do
        ...
        --     if XMVCA.XEquip:GetEquipSiteByEquip(v) == equipSite then
        --         equip = v
        --     end
        -- end

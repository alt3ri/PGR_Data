...
function XUiPanelSGWallOp:OnFurnitureSlotMoving(index, id, x, y)
    local furniture = self._Control:CloneContainerFurnitureData(self._AreaType):GetFurniture(id, false)
    if furniture then
...
function XUiPanelSGWallOp:RotateByHandle(z)
end

function XUiPanelSGWallOp:FocusSlot(slot)
    self._FocusSlot = slot
end

function XUiPanelSGWallOp:OutFocusSlot()
    self._FocusSlot = nil
end

...
function XUiSkyGardenShoppingStreetAsset:OnStart(passUpdate)
    self:_RegisterButtonClicks()
    self._PassUpdate = passUpdate or false
...
function XUiSkyGardenShoppingStreetAsset:OnNotify(event, resType, add)
    if event == XMVCA.XBigWorldService.DlcEventId.EVENT_BUSINESS_STREET_RES_REFRESH then
        if self._ComponentDic[resType] then
            self._CacheType = resType
            self._CacheNum = add
        end
        if self._PassUpdate then return end
...
function XUiSkyGardenShoppingStreetAsset:GetResUiPos(resType)
...
function XUiSkyGardenShoppingStreetAsset:ShowAssetInfo()
    ...
    XUiManager.MoveToTop(self.BubbleAssetDetail.gameObject, self.BtnAssetClose, function ()
...
function XUiSkyGardenShoppingStreetAsset:ShowBuffInfo()
    ...
    XUiManager.MoveToTop(self.PanelBuffDetail.gameObject, self.BtnBuffClose, function ()
    ...
    if resType then
        ...
        if coms then
            ...
            if add and add ~= 0 and self.TxtGoldChange then
                if not self._textMap then
                    self._textMap = {}
                end
                local com = self._textMap[resType]
                if not com then
                    com = self:_AddTextGameObject()
                    self._textMap[resType] = com
                end
                ...
                if animationTr and animationTr.gameObject.activeSelf then
                    animationTr.gameObject:PlayTimelineAnimation(function()
                        com.gameObject:SetActive(false)
...
function XUiSkyGardenShoppingStreetAsset:UpdatePanel()
    self._PassUpdate = false
    self:_RefreshRes(self._CacheType, self._CacheNum)
    self:_RefreshBuffs()
    self._PassUpdate = true
end

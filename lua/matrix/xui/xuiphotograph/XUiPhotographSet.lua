...
local XUiPhotographSet = XLuaUiManager.Register(XLuaUi, "UiPhotographSet")
local UiButtonState = CS.UiButtonState
local CsDropDown = CS.UnityEngine.UI.Dropdown

function XUiPhotographSet:OnAwake()
    self:InitCb()
end 

function XUiPhotographSet:OnStart(setData)
    self.SetData = setData
    self:InitView()
end

function XUiPhotographSet:InitCb()
...
function XUiPhotographSet:InitView()
...
function XUiPhotographSet:OnCardDrdSortValueChanged(value)
    if value == self.LogoDrdSort.value and (value ~= 0 or self.LogoDrdSort.value ~= 0) then
        XUiManager.TipText("PhotoModeLogoInfoMutex")
        self.CardDrdSort.value = self.LastCardValue
        return
    end
...
function XUiPhotographSet:OnLogoDrdSortValueChanged(value)
...
function XUiPhotographSet:SaveSet()

...
function XUiPanelFubenKoroStage:Ctor(uiRoot, ui, cfg, panelType)
...
function XUiPanelFubenKoroStage:InitPanel(panelType)
    ...
    for i = 1, #self.StageIds do
        ...
        self.Stages[i] = XUiFubenKoroStageItem.New(self, itemStage)
...
--初始化挑战目标
function XUiPanelFubenKoroStage:InitTreasureGrade()
...
--初始化收集进度
function XUiPanelFubenKoroStage:InitStarts()
    local curStars
    local totalStars
...
function XUiPanelFubenKoroStage:OnShow(type)
    self.PanelType = type
...
function XUiPanelFubenKoroStage:OnHide()
    self.GameObject:SetActiveEx(false)
end

--选中关卡
function XUiPanelFubenKoroStage:UpdateNodesSelect(stageId)
...
--取消选中关卡
function XUiPanelFubenKoroStage:ClearNodesSelect()
    local stageIds = self.StageIds
    for i = 1, #stageIds do
        if self.Stages[i] then
...
function XUiPanelFubenKoroStage:OpenStageDetails(stageId, id)
...
--关闭战斗详情
function XUiPanelFubenKoroStage:CloseStageDetails()
    self.IsOpenDetails = false
...
function XUiPanelFubenKoroStage:PlayScrollViewMove(gridTransform)
...
--点击关闭详情按钮
function XUiPanelFubenKoroStage:OnBtnCloseDetailClick()
    self:CloseStageDetails()
end

function XUiPanelFubenKoroStage:CheckCanClose()
...
function XUiPanelFubenKoroStage:OnBtnTreasureBgClick()
...
function XUiPanelFubenKoroStage:OnBtnTreasureClick()
...
--拖拽事件处理
function XUiPanelFubenKoroStage:OnDragProxy(dragType)
    if self.IsOpenDetails and dragType == 0 then
        self:CloseStageDetails()
    end
end

return XUiPanelFubenKoroStage
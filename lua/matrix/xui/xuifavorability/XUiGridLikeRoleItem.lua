...
function XUiGridLikeRoleItem:OnRefresh(data)
    ...
    local isOwn = XMVCA.XCharacter:IsOwnCharacter(data.Id)
...
function XUiGridLikeRoleItem:IsRed()
    if self.CharacterData then
        ...
        local storyReddot = XMVCA.XFavorability:HasStroyToBeUnlock(characterId) or XMVCA.XFavorability:HasStroyTaskCanFinish(characterId)

...
---@class XUiRiftAttributeSlider:XUiNode
---@field Parent XUiRiftAttribute
---@field _Control XRiftControl
local XUiRiftAttributeSlider = XClass(XUiNode, "UiRiftAttributeSlider")

function XUiRiftAttributeSlider:OnStart(index)
	self.Index = index
	self.CurLevel = 0
	self.Pool = {}
	---@type XTableRiftTeamAttribute
	self.Config = self._Control:GetTeamAttributeConfig(self.Index)
	self.LastValue = 0
	self.isInit = true
	...
	self.Bubble = XUiPanelRiftSysAttribute.New(self.PanelBuffBubble, self.Parent, index)
end
...
function XUiRiftAttributeSlider:RegisterEvent()
	...
	self.Slider.onValueChanged:AddListener(function(value)
		if not XTool.IsNumberValid(self.LimitLevel) then
			return
		end
...
function XUiRiftAttributeSlider:OnBtnHelpClick()
	self.Bubble:Open()
	self:SetPosition()
	if self.Parent.OpenBubbleCloseBtn then
		self.Parent:OpenBubbleCloseBtn()
	end
end
...
function XUiRiftAttributeSlider:InitSystemPropNode()
	local attrs = self._Control:GetSystemAttr(self.Index)
...
function XUiRiftAttributeSlider:GetMaxPreviewLevel()
	local previewAllLevel = self._Control:GetCanPreviewAttrAllLevel()
	local otherSliderLevel = 0
	for index, slider in ipairs(self.Parent.AttrSliderList) do
...
function XUiRiftAttributeSlider:SetPosition()
	local pos = self.Parent.Transform:InverseTransformPoint(self.PanelBuffBubble.position)
	local maxPosY = self.PanelBuffBubble.rect.height - self.Parent.Transform.rect.height / 2

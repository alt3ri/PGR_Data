...
function XUiGridFashionStoryTrialStage:RefreshData(id)
    self.Id=id
    --判断是否解锁
    local isOpen,unOpenReason=XMVCA.XFashionStory:CheckFashionStoryStageIsOpen(self.Id)
    self.IsOpen=isOpen
    if self.IsOpen then
        self.GridFitting:SetButtonState(XUiButtonState.Normal)
    else
        self.GridFitting:SetButtonState(XUiButtonState.Disable)
        if unOpenReason==XMVCA.XFashionStory.TrialStageUnOpenReason.PreStageUnPass then
            self.TxtLock.text=XUiHelper.GetText('FashionStoryTrialPreStagePassTip',XDataCenter.FubenManager.GetStageName(XMVCA.XFashionStory:GetPreStageId(self.Id)))
        elseif unOpenReason==XMVCA.XFashionStory.TrialStageUnOpenReason.OutOfTime then
            self.TxtLock.text=XUiHelper.GetText('FashionStoryTrialOutTime',XUiHelper.GetTimeYearMonthDay(XMVCA.XFashionStory:GetStageTimeId(self.Id)))
        end
    end
    --设置基本信息
    self.GridFitting:SetNameByGroup(0,XDataCenter.FubenManager.GetStageName(self.Id))
    self.GridFitting:SetRawImage(XMVCA.XFashionStory:GetTrialFace(self.Id))
    self.RImgIconMask:SetRawImage(XMVCA.XFashionStory:GetTrialLockIcon(self.Id))
end

...
local XUiTeamPrefabEquipOverrunSelect = XLuaUiManager.Register(XLuaUi, "UiTeamPrefabEquipOverrunSelect")
local XUiEquipOverrunSelectGrid = require("XUi/XUiEquipOverrun/XUiEquipOverrunSelectGrid")
local CSInstantiate = CS.UnityEngine.Object.Instantiate

function XUiTeamPrefabEquipOverrunSelect:OnAwake()
...
function XUiTeamPrefabEquipOverrunSelect:OnStart(teamPrefab, equipId, currentPos, selectCallBack, isPreview)
    ...
    self.LastSelectSuitId = self.TeamPrefab:GetWeaponOverrunSuitId(self.CurrentPos)
    self.CurSelectSuitId = self.LastSelectSuitId
    self:Refresh()
end

function XUiTeamPrefabEquipOverrunSelect:OnEnable()
    self:RefreshCost()
end

function XUiTeamPrefabEquipOverrunSelect:SetButtonCallBack()
...
function XUiTeamPrefabEquipOverrunSelect:OnBtnActive()
...
function XUiTeamPrefabEquipOverrunSelect:OnBtnChange()
    if self.SelectSuitData.Id == self.LastSelectSuitId then
        return
    end

    -- 更新预设中的超频套装ID
    self.TeamPrefab:CopyRealWeaponWeaponOverrunSuitId(self.CurSelectSuitId, self.CurrentPos)
...
function XUiTeamPrefabEquipOverrunSelect:OnBtnRImgSpendIcon()
...
-- 刷新意识列表
function XUiTeamPrefabEquipOverrunSelect:Refresh()
...
-- 意识激活回调
function XUiTeamPrefabEquipOverrunSelect:OnActiveSuit()
    self.SuitDataList = self:GetSuitDataList()
    self.DynamicTable:SetDataSource(self.SuitDataList)
    self.DynamicTable:ReloadDataASync(1)
...
function XUiTeamPrefabEquipOverrunSelect:GetSuitDataList()
    local overrunSuitCfg = XMVCA.XEquip:GetWeaponOverrunSuitCfgByTemplateId(self.Equip.TemplateId)
    ...
    for _, suitId in ipairs(suitIdList) do
        local suitData = {}
        suitData.Id = suitId
        suitData.Quality = XMVCA.XEquip:GetSuitQuality(suitId)
        suitData.IsCharWear = false
        suitData.CharWearCnt = 0 -- 角色装备该套装意识数量
        suitData.SuitMaxCnt = XMVCA.XEquip:GetSuitMaxCnt(suitId) -- 套装效果最大数量
    ...
    if characterSuitPriorityId ~= 0 then
        local suitPriorityCfg = XMVCA.XEquip:GetConfigCharacterSuitPriority(characterSuitPriorityId)
...
function XUiTeamPrefabEquipOverrunSelect:InitDynamicTable()
...
function XUiTeamPrefabEquipOverrunSelect:OnDynamicTableEvent(event, index, grid)
...
-- 刷新意识套装详情
function XUiTeamPrefabEquipOverrunSelect:RefreshSuitDetail(suitData)
...
-- 刷新消耗
function XUiTeamPrefabEquipOverrunSelect:RefreshCost()
...
return XUiTeamPrefabEquipOverrunSelect
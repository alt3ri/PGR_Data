...
function XUiMultiplayerRoom:InitMultiDim()
    ...
    --end
    self.BtnMultiDimDifficultySelect.gameObject:SetActiveEx(false)

    if self.PanelRecommendType then
        self.PanelRecommendType.gameObject:SetActiveEx(false)
    end
end
...
function XUiMultiplayerRoom:OnBtnFightClick()
    ...
    if not self:CheckEnoughActionPoint() then
        XUiManager.TipText('MultiplayerRoomActionPointNotEnough')
        return
    end
...
function XUiMultiplayerRoom:CheckEnoughActionPoint()
    local roomData = XDataCenter.RoomManager.RoomData
    ...
    if roomData then
        ...
        if XTool.IsNumberValid(stageId) then
            local requireCnt = XDataCenter.FubenManager.GetStageActionPointConsume(stageId) or 0
            local curCnt = XDataCenter.ItemManager.GetActionPointsNum()

            return curCnt >= requireCnt
        end
    end

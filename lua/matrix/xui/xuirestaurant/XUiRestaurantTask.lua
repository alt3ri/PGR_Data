...
---@class XUiRestaurantTask : XLuaUi
...
function XUiRestaurantTask:RefreshRedPoint()
    self.BtnDaily:ShowReddot(self._Control:CheckDailyTaskRedPoint())
    self.BtnActivity:ShowReddot(self._Control:CheckAchievementTaskRedPoint())
    self.BtnRecipe:ShowReddot(self._Control:CheckRecipeTaskRedPoint())
end
...
function XUiRestaurantTask:InitBtnTab()
    ...
    local typeEnum = XMVCA.XRestaurant.TaskType
...
function XUiRestaurantTask:GetTaskShowDataList()
    local timeLimitTaskIds = self._Control:GetTimeLimitTaskIds()
    --local recipeId = self._Control:GetRecipeTaskId()
...
function XUiRestaurantTask:UpdateTime()
    local endTime = self._Control:GetShopEndTime()
    ...
    if leftTime and leftTime > 0 then
        ...
        self.TxtTime.text = self._Control:GetShopTimeTxt(timeStr)

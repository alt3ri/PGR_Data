...
function XUiGuildDormPanelGITopInfo:Refresh()
    ...
    if XTool.IsNumberValid(curGuildHeadId) then
        ...
        if guildHeadPortrait.IsSpecial then
            self.ImgSpecialIconBg:SetRawImage(guildHeadPortrait.NewGuildRoomBg)
            self.ImgSpecialIconBg.gameObject:SetActiveEx(true)
        else
            self.ImgSpecialIconBg.gameObject:SetActiveEx(false)
        end
    end

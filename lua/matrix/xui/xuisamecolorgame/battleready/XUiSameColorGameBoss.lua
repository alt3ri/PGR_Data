...
function XUiSameColorGameBoss:UpdateStartBtnStatus()
    local isLock = self.CurrentRole:GetIsLock()
    self.BtnStart:SetButtonState(isLock and CS.UiButtonState.Disable or CS.UiButtonState.Normal)
end
...
function XUiSameColorGameBoss:UpdateChildPanel(panelType)
    ...
    local childPanelType = XEnumConst.SAME_COLOR_GAME.UI_BOSS_CHILD_PANEL_TYPE
    self.BtnStart.gameObject:SetActiveEx(panelType == childPanelType.MAIN)
...
function XUiSameColorGameBoss:InitSelectRole()
    local roles = self.RoleManager:GetRoles()
    local lastRoleId = self.CurrentBoss:GetLastRoleId()
    local defaultRoleId = self.CurrentBoss.Config.RoleId
    if lastRoleId ~= nil and lastRoleId > 0 then
        self.CurrentRole = self.RoleManager:GetRole(lastRoleId)
        if self.CurrentRole:GetIsLock() then
            -- 锁住的换回默认第一个
            self.CurrentRole = roles[1]
        end
    elseif XTool.IsNumberValid(defaultRoleId) then
        for _, v in pairs(roles) do
            if v:GetId() == defaultRoleId then
                self.CurrentRole = v
            end
        end
        if not self.CurrentRole then
            XLog.Error("不存在角色：" .. defaultRoleId)
            self.CurrentRole = roles[1]
        end
    else
        self.CurrentRole = roles[1]
...
function XUiSameColorGameBoss:OnBtnStartClicked()
    ...
    XDataCenter.SameColorActivityManager.RequestEnterStage(self.CurrentRole:GetId(), self.CurrentBoss:GetId(), self.CurrentRole:GetUsingSkillGroupIds(true), function(actionsList)
        --XDataCenter.SameColorActivityManager:ClearMainUiModelInfo()
        XLuaUiManager.PopThenOpen("UiSameColorGameBattle", self.CurrentRole, self.CurrentBoss)
    end)
end

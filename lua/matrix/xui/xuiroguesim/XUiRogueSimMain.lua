...
function XUiRogueSimMain:RegisterUiEvents()
    ...
    self:RegisterClickEvent(self.BtnRetreat, self.OnBtnRetreatClick)
...
function XUiRogueSimMain:OnBtnRetreatClick()
    ...
    local isPass = self._Control:CheckCurStageStarConditions()
    local title = self._Control:GetClientConfig("BattleEarlySettlementTitle", isPass and 1 or 2)
    local content = self._Control:GetClientConfig("BattleEarlySettlementContent", isPass and 1 or 2)
...
function XUiRogueSimMain:RefreshBtnBattle()
    ...
    if not isEmpty then
        local isPass = self._Control:CheckCurStageStarConditions()
        local btnName = self._Control:GetClientConfig("BattleEarlySettlementBtnName", isPass and 1 or 2)
        self.BtnRetreat:SetNameByGroup(0, btnName)
    end
    -- 蓝点
    self:RefreshBtnStartRed()
end

function XUiRogueSimMain:RefreshBtnStartRed()
    local isRed = XMVCA.XRogueSim:IsShowStagesRedPoint()
    self.BtnStart:ShowReddot(isRed)
end

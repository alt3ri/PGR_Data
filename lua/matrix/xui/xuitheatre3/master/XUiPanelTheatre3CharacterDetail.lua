...
function XUiPanelTheatre3CharacterDetail:OnStart()
    ...
    self._HiddenEndingIndex = self._Control:GetClientConfigNumber("HiddenEndingIndex")
end
...
function XUiPanelTheatre3CharacterDetail:RefreshEnding()
    ...
    local isHiddenEndingUnlock = #characterEndingIdList >= self._HiddenEndingIndex
    for index, id in pairs(characterEndingIdList) do
        local endIconName = "Part" .. index
        if self.EndingUiObjDir[endIconName] then
            local isUnlock = self._Control:CheckCharacterEnding(id)
            self.EndingUiObjDir[endIconName].gameObject:SetActiveEx(isUnlock)
            if not isUnlock then
                isHiddenEndingUnlock = false
            end
        end
    end
    if isHiddenEndingUnlock then
        for i = 1, self._HiddenEndingIndex - 1 do
        ...
        for i = #characterEndingIdList, 6 do
            local endIconName = "Part" .. i
            if self.EndingUiObjDir[endIconName] then
                self.EndingUiObjDir[endIconName].gameObject:SetActiveEx(false)
            end

local XUiPracticeBasicsStage = XClass(nil, "XUiPracticeBasicsStage")
...
function XUiPracticeBasicsStage:UpdateNode(stageId, stageMode)
    ...
    elseif self.StageMode == XPracticeConfigs.PracticeType.Boss then
        self.IsLock = false
    else
...
function XUiPracticeBasicsStage:OnBtnStageClick()
    ...
    if self.IsLock then
        ...
        if self.RootUi.SetSelectStageId then
            self.RootUi:SetSelectStageId(self.StageId)
        end

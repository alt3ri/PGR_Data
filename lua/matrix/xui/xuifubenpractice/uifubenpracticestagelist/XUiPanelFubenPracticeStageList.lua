local XUiPanelFubenPracticeStageList = XClass(XUiNode, "XUiPanelFubenPracticeStageList")
local XUiGridFubenPracticeStage = require("XUi/XUiFubenPractice/UiFubenPracticeStageList/XUiGridFubenPracticeStage")
local XUguiDragProxy = CS.XUguiDragProxy

function XUiPanelFubenPracticeStageList:OnStart(cfg)
    self.Cfg = cfg
    self:InitPanel()
end

function XUiPanelFubenPracticeStageList:OnEnable()
    self:OnShow()
end

function XUiPanelFubenPracticeStageList:SetSubDetailPanelKey(detailPanelKey)
    self._DetailPanelKey = detailPanelKey
end

function XUiPanelFubenPracticeStageList:InitPanel()
    ...
    for i = 1, #self.StageIds do
        ...
        self.Stages[i] = XUiGridFubenPracticeStage.New(itemStage, self)
        self.Stages[i]:Open()
    end
...
function XUiPanelFubenPracticeStageList:OnShow()
    for i = 1, #self.StageIds do
        self.Stages[i]:Open()
...
function XUiPanelFubenPracticeStageList:OnHide()
    self:Close()
end

--选中关卡
function XUiPanelFubenPracticeStageList:UpdateNodesSelect(stageId)
...
--取消选中关卡
function XUiPanelFubenPracticeStageList:ClearNodesSelect()
    local stageIds = self.StageIds
    for i = 1, #stageIds do
        if self.Stages[i] then
...
function XUiPanelFubenPracticeStageList:OpenStageDetails(stageId, id)
...
--关闭战斗详情
function XUiPanelFubenPracticeStageList:CloseStageDetails()
    self.IsOpenDetails = false
...
function XUiPanelFubenPracticeStageList:PlayScrollViewMove(gridTransform)
...
--自动定位
function XUiPanelFubenPracticeStageList:SetPanelInNewStage(gridTransform)
    local gridRect = gridTransform:GetComponent("RectTransform")
    local diffX = gridRect.localPosition.x + self.PanelStageContent.localPosition.x
    if diffX < XDataCenter.FubenMainLineManager.UiGridChapterMoveMinX or diffX > XDataCenter.FubenMainLineManager.UiGridChapterMoveMaxX then
...
--点击关闭详情按钮
function XUiPanelFubenPracticeStageList:OnBtnCloseDetailClick()
    self:CloseStageDetails()
end

function XUiPanelFubenPracticeStageList:CheckCanClose()
...
--拖拽事件处理
function XUiPanelFubenPracticeStageList:OnDragProxy(dragType)
    if self.IsOpenDetails and dragType == 0 then
        self:CloseStageDetails()
    end
end

return XUiPanelFubenPracticeStageList
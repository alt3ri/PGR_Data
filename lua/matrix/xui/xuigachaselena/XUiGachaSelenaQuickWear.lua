...
---@class XUiGachaSelenaQuickWear : XLuaUi 使用历程奖励弹框
local XUiGachaSelenaQuickWear = XLuaUiManager.Register(XLuaUi, "UiGachaSelenaQuickWear")

function XUiGachaSelenaQuickWear:OnAwake()
    XUiHelper.RegisterClickEvent(self, self.BtnClose, self.Close)
    XUiHelper.RegisterClickEvent(self, self.BtnWear, self.OnBtnWearClick)
end

function XUiGachaSelenaQuickWear:OnStart(templateId, rewardId, isConvertFrom, rewardName, callBack)
    self._CallBack = callBack
    self._TemplateId = templateId
    self.TxtDesc.text = XUiHelper.GetText(isConvertFrom and "GachaSelenaRewardTitle1" or "GachaSelenaRewardTitle2", rewardName)
...
function XUiGachaSelenaQuickWear:Close()
...
function XUiGachaSelenaQuickWear:OnBtnWearClick()
    ...
    if XDataCenter.HeadPortraitManager.IsHeadPortraitValid(self._TemplateId) then
        ...
        if config.Type == 1 then
            ...
            XDataCenter.HeadPortraitManager.ChangeHeadPortrait(self._TemplateId, function()
                XUiManager.TipText("GachaSelenaSettingSuccess")
                self._IsDone = true
            end)
        else
            -- 更换头像框
...
return XUiGachaSelenaQuickWear
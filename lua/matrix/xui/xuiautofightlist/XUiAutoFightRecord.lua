local XUiAutoFightRecord = XClass(nil, "XUiAutoFightRecord")
...
function XUiAutoFightRecord:BindTimer()
    ...
    XCountDown.BindTimer(self.GameObject, self.TimerName, function(v)
        if v == 0 then
            self.Parent:PlayAnimation(AnimName, function()
                self:SetState(true)
            end)
            self:RemoveTimer()
        else
...
function XUiAutoFightRecord:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiAutoFightRecord:RegisterClickEvent函数参数错误：参数func不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiAutoFightRecord:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end
...
-- auto
function XUiAutoFightRecord:OnBtnObtainClick()

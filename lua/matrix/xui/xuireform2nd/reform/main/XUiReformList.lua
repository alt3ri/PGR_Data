...
function XUiReformList:OnDisable()
    ...
    if XLuaUiManager.IsMaskShow("Reform") then
        XLuaUiManager.SetMask(false, "Reform")
    end
end
...
function XUiReformList:PlayListAnimation()
    ...
    -- 由于还可以点击词缀等等，所以直接用遮罩一了百了
    XLuaUiManager.SetMask(true, "Reform")
    local playAnimationTime = 6 * gap + 0.34
    self:Tween(playAnimationTime, nil, function()
        XLuaUiManager.SetMask(false, "Reform")
    end)
end

...
function XUiCerberusGameMainV2P9:OnBtnFashionStory1Click()
    local chapterId = XMVCA.XCerberusGame:GetChapterIdList()[XEnumConst.CerberusGame.ChapterIdIndex.FashionStory]
    local chapterConfig = XMVCA.XCerberusGame:GetModelCerberusGameChapter()[chapterId]
    local condition = chapterConfig.OpenCondition
    local ispass, desc = XConditionManager.CheckCondition(condition)
    if condition and not ispass then
        XUiManager.TipMsg(desc)
        return
    end
    XLuaUiManager.Open("UiCerberusGameChapterV2P9", chapterId)
    XMVCA.XCerberusGame:SetBtnFashionStoryClick()
end

function XUiCerberusGameMainV2P9:OnBtnFashionStory2Click()
    local chapterId = XMVCA.XCerberusGame:GetChapterIdList()[XEnumConst.CerberusGame.ChapterIdIndex.FashionChallenge]
    local chapterConfig = XMVCA.XCerberusGame:GetModelCerberusGameChapter()[chapterId]
    local condition = chapterConfig.OpenCondition
    local ispass, desc = XConditionManager.CheckCondition(condition)
    if condition and not ispass then

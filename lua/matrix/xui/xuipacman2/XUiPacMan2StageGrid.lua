...
function XUiPacMan2StageGrid:OnStart()
    ...
    self._IsPlayAnimationStar = false
    self._IsPassed = nil
    self._StarBegin = 1
    self._StarEnd = 1
end
...
function XUiPacMan2StageGrid:Update(data)
    ...
    if self._IsPassed == nil then
        self._IsPassed = data.IsPassed
    end
    if self._IsPassed ~= data.IsPassed then
        self._IsPassed = data.IsPassed
        self._IsPlayAnimationStar = true
    else
        self._IsPlayAnimationStar = false
    end

    local star = data.Star
    self._StarBegin = self._StarEnd
    self._StarEnd = star
    for i = 1, 3 do
...
function XUiPacMan2StageGrid:PlayAnimationStar()
    if self._IsPlayAnimationStar then
        if self._StarEnd > 0 then
            for i = self._StarBegin, self._StarEnd do
                local star = self._GridStar[i]
                if star then
                    star:PlayAnimation("ImgStarUnlock")
                end
            end
        end
    end

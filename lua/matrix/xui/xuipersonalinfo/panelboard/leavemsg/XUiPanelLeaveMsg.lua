local XUiPanelLeaveMsg = XClass(nil, "XUiPanelLeaveMsg")
...
function XUiPanelLeaveMsg:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiPanelLeaveMsg:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end
...
function XUiPanelLeaveMsg:Refresh( parent )
    XDataCenter.PersonalInfoManager.CreateItems(self.RootUi, self.PanelLeaveMsgItem, self.PanelLeaveMsgItem.gameObject.transform.parent,parent.pageNum,parent.data.Id,XDataCenter.PersonalInfoManager.ItemPanelType.DailyleaveMsg,function()
        self:SetIsShow(true)
    end)
end

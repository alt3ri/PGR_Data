XInputManagerPcCreator = function ()
    ...
    ---切换当前操作模式
    XInputManagerPc.SetCurInputMap = function(operationType)
        CSXInputManager.SetCurInputMap(operationType)
    end

    ---还原当前操作模式
    XInputManagerPc.ResumeCurInputMap = function()
        CSXInputManager.SetCurInputMap(CSXInputManager.BeforeInputMapID)
    end
    ...
    XInputManagerPc.OnSystemClick = function(inputDevice, key, type, operationType)
    ...
    XInputManagerPc.OnSystemKeyPress = function(inputDevice, key, operationType)
        if CS.XUiManagerExtension.Masked then
            return
        end
        local keyCode = CS.XUiPc.XUiPcCustomKeyEnum.__CastFrom(key)
    ...
    ---当活动游戏按键被按时
    XInputManagerPc._OnActivityGameKeyClick = function(inputDevice, key, type, operationType)
        if not key then
            return
        end
        if type == XOperationClickType.KeyDown then
    ...
    ---当活动游戏按键被按住时
    XInputManagerPc._OnActivityGameKeyPress = function(inputDevice, key, operationType)

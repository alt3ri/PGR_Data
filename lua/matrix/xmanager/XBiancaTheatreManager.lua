...
XBiancaTheatreManagerCreator = function()
    ...
    function XBiancaTheatreManager.CheckEndBgmPlay(adventureEnd)
        ...
        XLuaAudioManager.PlayAudioByType(XLuaAudioManager.SoundType.Music, adventureEnd:GetBgmCueId())
    end
    ------------- 背景音乐相关 end -----------
    ...
    function XBiancaTheatreManager.OpenAudioFilter(audioFilterVisionId)
        ...
        if XTool.IsNumberValid(cueId) then
            ...
            XScheduleManager.Schedule(function ()
                local audioInfo = XLuaAudioManager.FindByCueId(cueId)
    ...
    function XBiancaTheatreManager.ResetAudioFilter(audioFilterVisionId)
        ...
        if XTool.IsNumberValid(visionId) then
            ...
            --     XLuaAudioManager.PlayAudioByType(XLuaAudioManager.SoundType.Sound, closeCueId)
            --     XLuaAudioManager.StopAudioByCueId(closeCueId)
            -- end
            if XTool.IsNumberValid(openCueId) then
                XLuaAudioManager.StopAudioByCueId(openCueId)
            end
            XLuaAudioManager.ResetSystemAudioVolume()
        end
    ...
    ------------------副本入口扩展 start-------------------------
    function XBiancaTheatreManager:OpenMainUi()

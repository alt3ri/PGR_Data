...
XSlotMachineManagerCreator = function()
    ...
    function XSlotMachineManager.CheckRedPoint()
        ...
        if XTool.IsTableEmpty(SlotMachineDataEntityAllList) then
            return false
        end
        
        for actId, machineDataList in pairs(SlotMachineDataEntityAllList) do
            for _, machineData in ipairs(machineDataList) do
    ...
    function XSlotMachineManager.GetBestIconCfgId(rockResults)
        local resultsCount = XTool.GetTableCount(rockResults)
        if resultsCount == 1 then
            if rockResults[1].IconList then
                return rockResults[1].IconList[1]
            end
        else
            for _, rockResult in ipairs(rockResults) do
                if rockResult.IconList then
                    local iconCfgId = rockResult.IconList[1]
                    if XSlotMachineConfigs.GetSlotMachinesIconTemplateById(iconCfgId).IsPrix then
                        return iconCfgId
                    end
                end
            end

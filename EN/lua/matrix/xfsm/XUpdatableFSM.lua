...
local Update = function(self)
    ...
    if self.on_pre_update_event then
        self.on_pre_update_event(self.XController, self.RefProxy, (self.ProxyArgs~=nil) and table.unpack(self.ProxyArgs))
    end
    ...
    for i = 1, transitions_count do
        ...
        if nil ~= e.conditions and true == e.conditions(self.XController, self.RefProxy, (self.ProxyArgs~=nil) and table.unpack(self.ProxyArgs)) then
            self[e.name]((self.ProxyArgs~=nil) and table.unpack(self.ProxyArgs))
            break
        end
    end
end

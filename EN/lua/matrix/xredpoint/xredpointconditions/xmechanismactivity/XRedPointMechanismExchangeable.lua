local XRedPointMechanismExchangeable = {}

function XRedPointMechanismExchangeable.Check()

    if not XMVCA.XMechanismActivity:ExCheckInTime() or not XFunctionManager.DetectionFunction(XFunctionManager.FunctionName.MechanismActivity, false, true) then
        return false
    end
    
    local timeId = XMVCA.XMechanismActivity:GetMechanismClientConfigNum('ShopShowReddotTimeId')

    if XTool.IsNumberValid(timeId) and not XFunctionManager.CheckInTimeByTimeId(timeId) then
        return false
    end
...
return XRedPointMechanismExchangeable
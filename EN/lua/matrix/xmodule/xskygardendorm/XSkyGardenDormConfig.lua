...
local TableKey = {
    ...
    SgDormFurnitureHandbook = {
        ReadFunc = XConfigUtil.ReadType.IntAll,
        DirPath = XConfigUtil.DirectoryType.Client,
        Identifier = "FurnitureId"
    },
    SgDormFurnitureHandbookType = {
        ReadFunc = XConfigUtil.ReadType.IntAll,
        DirPath = XConfigUtil.DirectoryType.Client,
        Identifier = "Type"
    },
...
function XSkyGardenDormConfig:ResetAll() 
    ...
    self._HandBookTypeList = nil
    self._HandBookType2FurnitureList = nil
end
...
---@return XTableSgDormFurnitureHandbookType
function XSkyGardenDormConfig:GetHandBookTypeTemplate(typeId)
    return self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.SgDormFurnitureHandbookType, typeId)
end

function XSkyGardenDormConfig:GetHandBookTypeList()
    if self._HandBookTypeList then
        return self._HandBookTypeList
    end
    local list = {}
    ---@type table<number, XTableSgDormFurnitureHandbookType>
    local templates = self._ConfigUtil:GetByTableKey(TableKey.SgDormFurnitureHandbookType)
    for type, _ in pairs(templates) do
        list[#list + 1] = type
    end
    table.sort(list, function(a, b) 
        local pA = self:GetHandBookTypeTemplate(a).Priority
        local pB = self:GetHandBookTypeTemplate(b).Priority
        if pA ~= pB then
            return pA < pB
        end
        return a < b
    end)
    self._HandBookTypeList = list
    
    return list
end

---@return XTableSgDormFurnitureHandbook
function XSkyGardenDormConfig:GetHandBookFurnitureTemplate(furnitureId)
    return self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.SgDormFurnitureHandbook, furnitureId)
end

function XSkyGardenDormConfig:GetHandBookFurnitureListByType(type)
    if self._HandBookType2FurnitureList and self._HandBookType2FurnitureList[type] then
        return self._HandBookType2FurnitureList[type]
    end
    local dict = {}
    ---@type table<number, XTableSgDormFurnitureHandbook>
    local templates = self._ConfigUtil:GetByTableKey(TableKey.SgDormFurnitureHandbook)
    for fId, t in pairs(templates) do
        local list = dict[t.HandbookType]
        if not list then
            list = {}
            dict[t.HandbookType] = list
        end
        list[#list + 1] = fId
    end
    for _, list in pairs(dict) do
        table.sort(list, function(a, b) 
            local pA = self:GetHandBookFurnitureTemplate(a).Priority
            local pB = self:GetHandBookFurnitureTemplate(b).Priority
            if pA ~= pB then
                return pA < pB
            end
            return a < b

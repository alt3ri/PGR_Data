...
function XSkyGardenDormControl:GetAreaType(typeId)
    local t = self._Model:GetFurnitureTypeTemplate(typeId)
...
function XSkyGardenDormControl:GetFurnitureTypeIcon(typeId)
    local t = self._Model:GetFurnitureTypeTemplate(typeId)
...
function XSkyGardenDormControl:GetNotPutFurnitureIdList(cfgId, containerFurnitureData, isAlbumPhoto)
    return self._Model:GetDormData():GetNotPutFurnitureIdList(cfgId, containerFurnitureData, isAlbumPhoto)
end
...
function XSkyGardenDormControl:GetPutCountDictByMajorType(typeList, containerData)
    ...
    local fDict = containerData:GetFurnitureDict(false)
    ...
    local pDict = containerData:GetFurnitureDict(true)
    local albumMajorType = self:GetMajorType(self:GetAlbumPhotoTypeId())
    for _, _ in pairs(pDict) do
        if dict[albumMajorType] then
            dict[albumMajorType] = dict[albumMajorType] + 1
        end
    end
    return dict
end
...
function XSkyGardenDormControl:CreateDynamicContainer(transform, prefab)
    ...
    local sceneTransform = wall:GetTransform()
...
function XSkyGardenDormControl:AddFightFurnitureData(id, data, index, isAlbumPhoto)
    self._Model:AddFightFurnitureData(id, data, index, isAlbumPhoto)
end

function XSkyGardenDormControl:UpdateWallFightData(data)
    self._Model:GetWallFightData():UpdateData(data.ActorRef)
end

function XSkyGardenDormControl:UpdateGiftFightData(data)
    self._Model:GetGiftShelfFightData():UpdateData(data.ActorRef, data.FrameGridSizeList)
end

function XSkyGardenDormControl:RemoveFightFurnitureData(id, isAlbumPhoto)
    self._Model:RemoveFightFurnitureData(id, isAlbumPhoto)
end

---@return XSgDormFightFurnitureData
function XSkyGardenDormControl:GetFightFurnitureData(id, isAlbumPhoto)
    return self._Model:GetFightFurnitureData(id, isAlbumPhoto)
end

--- 恢复到服务器的数据
---@param areaType number 区域
---@param serverData XSgContainerFurnitureData 服务端记录的家具
function XSkyGardenDormControl:RevertDecoration(areaType, serverData)
    XMVCA.XSkyGardenDorm:RevertDecoration(areaType, serverData)
...
function XSkyGardenDormControl:GetAlbumPhotoList()
    return self._Model:GetDormData():GetAlbumPhotoList()
end

function XSkyGardenDormControl:GetAlbumPhotoSalt(id)
    return self._Model:GetDormData():GetAlbumPhotoSalt(id)
end
...
function XSkyGardenDormControl:RequestSaveAndApplyLayout(areaType, saveId, applyId, containerDataList, func)
    ...
    XNetwork.Call("SgDormSaveAndApplyLayoutRequest", req, function(res)
        ...
        self:Notify(XDormEventId.EVENT_DORM_LAYOUT_REFRESH)

        if applyNew then
            self:Notify(XDormEventId.EVENT_DORM_APPLY_NEW_LAYOUT)
        end
    end)
end
...
function XSkyGardenDormControl:GetHandBookTypeList()
    return self._Model:GetHandBookTypeList()
end

function XSkyGardenDormControl:GetHandBookTypeName(type)
    local t = self._Model:GetHandBookTypeTemplate(type)
    return t and t.Name or ""
end

function XSkyGardenDormControl:GetHandBookFurnitureListByType(type)
    return self._Model:GetHandBookFurnitureListByType(type)
end

function XSkyGardenDormControl:GetHandBookFurnitureType(furnitureId)
    local t = self._Model:GetHandBookFurnitureTemplate(furnitureId)
    return t and t.HandbookType or 0
end

function XSkyGardenDormControl:GetHandBookFurnitureScale(furnitureId)
    local t = self._Model:GetHandBookFurnitureTemplate(furnitureId)
    return t and t.Scale / 10000 or 0
end

function XSkyGardenDormControl:GetHandBookFurnitureRotationX(furnitureId)
    local t = self._Model:GetHandBookFurnitureTemplate(furnitureId)
    return t and t.RotationX or 0
end

function XSkyGardenDormControl:GetHandBookFurnitureRotationY(furnitureId)
    local t = self._Model:GetHandBookFurnitureTemplate(furnitureId)
    return t and t.RotationY or 0
end

function XSkyGardenDormControl:GetHandBookFurniturePositionY(furnitureId)
    local t = self._Model:GetHandBookFurnitureTemplate(furnitureId)
    return t and t.PositionY / 10000 or 0
end
...
function XSkyGardenDormControl:GetAlbumPhotoTypeId()
    return self._Model:GetAlbumPhotoTypeId()
end

function XSkyGardenDormControl:GetDefaultAlbumPhotoIcon()
    return self._Model:GetConfigValue("DefaultAlbumPhotoIcon", 1)
end

function XSkyGardenDormControl:GetDragSpeed()
    local value = self._Model:GetConfigValue("DragSpeed", 1)
    return tonumber(value)
end

function XSkyGardenDormControl:GetDefaultCameraFOV()
    local value = self._Model:GetConfigValue("DefaultCameraFOV", 1)
    return tonumber(value)
end

function XSkyGardenDormControl:GetHandleRotateSpeed()
    local value = self._Model:GetConfigValue("HandleRotateSpeed", 1)
    return tonumber(value)
end
...
function XSkyGardenDormControl:IsPositiveSequence(areaType)
    local key = self._Model:GetCookieKey("SORT_ORDER_%s", areaType)
...
function XSkyGardenDormControl:SetPositiveSequence(areaType, value)
    local key = self._Model:GetCookieKey("SORT_ORDER_%s", areaType)
    local data = XSaveTool.GetData(key)
    if data == value then
        return
...
function XSkyGardenDormControl:CheckHandBookNewMark(type, furnitureId)
    return self._Model:CheckHandBookNewMark(type, furnitureId)
end

function XSkyGardenDormControl:MarkHandBookNewMark(type, furnitureId)
    self._Model:MarkHandBookNewMark(type, furnitureId)
end

function XSkyGardenDormControl:SaveHandBookMark()
    self._Model:SaveHandBookMark()
end
...
function XSkyGardenDormControl:Subscribe(eventId, func, caller)
    XMVCA.XSkyGardenDorm:Subscribe(eventId, func, caller)
end

function XSkyGardenDormControl:Unsubscribe(eventId, func, caller)
    XMVCA.XSkyGardenDorm:Unsubscribe(eventId, func, caller)
end

function XSkyGardenDormControl:Notify(eventId, ...)
    XMVCA.XSkyGardenDorm:Notify(eventId, ...)
end

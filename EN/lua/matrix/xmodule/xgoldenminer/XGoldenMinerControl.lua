...
function XGoldenMinerControl:RequestGoldenMinerExitGame(stageId, cb, settlementInfo, curMapScore, beforeScore)
    ...
    if not settlementInfo then
        ...
        settlementInfoReq.SlotMachineCount = 0
    else
...
function XGoldenMinerControl:RequestGoldenMinerSaveStage(curPlayStageId)
    ...
    if self:TryExitToCollection(false) then
        return
    end
...
function XGoldenMinerControl:_GiveUpGame()
    self:RequestGoldenMinerExitGame(0, function()
        if self:TryExitToCollection(true) then
...
function XGoldenMinerControl:TryExitToCollection(isSettled)
    ...
    XMVCA.XGameCollection:OnGameExitToCollection(gameType, {
        Score = score,
        IsSettled = isSettled and true or false,
    })
...
function XGoldenMinerControl:HandleActivityEndTime()
    if XLuaUiManager.IsUiShow("UiGoldenMinerSuspend") then
        XLuaUiManager.Remove("UiGoldenMinerSuspend")
    end
...
function XGoldenMinerControl:GetPartnerRadarScore(time)
    local score = 0
    ---@type XGoldenMinerGameControl
    score = self._GameControl.SystemPartner:GetPartnerRadarScore(time)
    return score
end
--endregion
...
function XGoldenMinerControl:CheckIsCanOpenRelicModule(mapId)
    local mapCfg = self._Model:GetMapCfg(mapId)
    if XTool.IsTableEmpty(mapCfg) then
        return false
    end

    -- 目前和老虎机功能互斥
    return not mapCfg.IsOpenSlotsScore
end

--endregion

--region Check - SlotsScore

function XGoldenMinerControl:CheckIsCanOpenSlotsScore(mapId)
    local mapCfg = self._Model:GetMapCfg(mapId)
    if XTool.IsTableEmpty(mapCfg) then
        return false
    end

    return mapCfg.IsOpenSlotsScore
end

--endregion

--region Check - SunAndMoon

function XGoldenMinerControl:CheckIsSunAndMoonMap(mapId)
    local mapCfg = self._Model:GetMapCfg(mapId)
    if XTool.IsTableEmpty(mapCfg) then
        return false
    end

    return mapCfg.SunMoonInitialType == XEnumConst.GOLDEN_MINER.MAP_SUN_MOON_TYPE.SUN or
            mapCfg.SunMoonInitialType == XEnumConst.GOLDEN_MINER.MAP_SUN_MOON_TYPE.MOON
end
...
function XGoldenMinerControl:GetClientSlotScoreAnyIcon()
    return self._Model:GetClientCfgValue("SlotScoreAnyIcon", 1)
end

function XGoldenMinerControl:GetClientSlotsScores(type)
    return self._Model:GetClientCfgNumberValue("SlotsScores", type)
end

function XGoldenMinerControl:GetClientSlotsScoreIcon(type)
    return self._Model:GetClientCfgValue("SlotsScoresIcon", type)
end
...
function XGoldenMinerControl:GetClientGuideTipsText(isAimHookTips)
    if isAimHookTips then
        return self._Model:GetClientCfgValue("GuidePanelMoveShipTips", 2)
    else
        return self._Model:GetClientCfgValue("GuidePanelMoveShipTips", 1)
    end
end

function XGoldenMinerControl:GetClientTextPCControl(isAimHookTips)
    if isAimHookTips then
        return self._Model:GetClientCfgValue("TextPCControl", 2)
    else
...
function XGoldenMinerControl:GetMapSunMoonInitialType(id)
    local cfg = self._Model:GetMapCfg(id)

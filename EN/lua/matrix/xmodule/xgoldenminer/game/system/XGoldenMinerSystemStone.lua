...
---@param stoneEntity XGoldenMinerEntityStone
function XGoldenMinerSystemStone:_SetComponentSunMoonCanInteraction(stoneEntity, isCanInteraction)
    local stoneComponent = stoneEntity:GetComponentSunMoon()
...
---@param stoneEntity XGoldenMinerEntityStone
function XGoldenMinerSystemStone:_OnStoneInteractionStateChanged(stoneEntity)
    local stoneComponent = stoneEntity:GetComponentStone()
    local active = stoneComponent.CanInteraction

    local sunMoonComponent = stoneEntity:GetComponentSunMoon()
    if sunMoonComponent then
        active = active and sunMoonComponent.CanInteraction
    end
...
function XGoldenMinerSystemStone:SetStoneSunMoonReal(stoneEntity, isReal, isNeedChangeSurface)
    isNeedChangeSurface = isNeedChangeSurface or true
    local sunMoonComponent = stoneEntity:GetComponentSunMoon()
    if not sunMoonComponent then
        return
    end

    local realType = isReal and XEnumConst.GOLDEN_MINER.STONE_SUN_MOON_REAL_TYPE.REAL or XEnumConst.GOLDEN_MINER.STONE_SUN_MOON_REAL_TYPE.VIRTUAL

    --if self._MainControl.SystemBuff:CheckHasAliveBuffByType(XEnumConst.GOLDEN_MINER.BUFF_TYPE.HOOK_EX_GRAB_VIRTUAL) then
    --    self:_SetComponentSunMoonCanInteraction(stoneEntity, true) -- 受Buff影响虚拟的抓取物也能抓
    --else
    --    self:_SetComponentSunMoonCanInteraction(stoneEntity, isReal)
    --end
    
    self:_SetComponentSunMoonCanInteraction(stoneEntity, true) -- 改变日月状态不再改变Collider的Enable 不论什么状态都能交互

    if isNeedChangeSurface then
        if sunMoonComponent.RealRoot then
            sunMoonComponent.RealRoot.gameObject:SetActiveEx(isReal)
        end

        if sunMoonComponent.VirtualRoot then
            sunMoonComponent.VirtualRoot.gameObject:SetActiveEx(not isReal)
        end
    end

    sunMoonComponent.StoneRealType = realType
end

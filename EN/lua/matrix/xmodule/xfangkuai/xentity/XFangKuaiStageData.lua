...
function XFangKuaiStageData:UpdateStageData(data)
    ...
    if not XTool.IsTableEmpty(data.HistoryFrenzyRecords) then
        for _, record in ipairs(data.HistoryFrenzyRecords) do
            table.insert(self.HistoryFevRecords, {
                EntryFrenzyRound = record.EntryFrenzyRound,
                FrenzyMaxLevel = record.FrenzyMaxLevel,
            })
        end
    end
...
function XFangKuaiStageData:RecordFeverRound()
    table.insert(self.HistoryFevRecords, {
        EntryFrenzyRound = self.Round,
        FrenzyMaxLevel = 0,
    })
end

function XFangKuaiStageData:RecordFeverMaxLevel()
    local idx = #self.HistoryFevRecords
    if idx == 0 then
        XLog.Error("记录狂热最大等级失败 因为进入狂热回合数尚未记录.")
        return
    end
    self.HistoryFevRecords[idx].FrenzyMaxLevel = self.FevLevel
end
...
function XFangKuaiStageData:SetFevValue(value)
    self.FevValue = value
end

function XFangKuaiStageData:SetFevStep(value)
    self.FevStep = value
end

function XFangKuaiStageData:SetFevLevel(value)
    self.FevLevel = value
end
...
function XFangKuaiStageData:AddPoint(score, combo, comboCount, fevRadio)
    -- 提前÷1000再相乘的话会有精度的问题 230会变成229！
    self.Point = self.Point + math.floor(score * fevRadio * combo / 10000)
...
function XFangKuaiStageData:AddFevValue(add)
    self.FevValue = self.FevValue + add
    return self.FevValue
end

function XFangKuaiStageData:AddFevLevel(addLevel, maxLevel)
    self.FevLevel = math.min(maxLevel, self.FevLevel + addLevel)
end

function XFangKuaiStageData:ReduceFevStep()
    if self.FevStep > 0 then
        self.FevStep = self.FevStep - 1
        XLog.Debug("狂热步数减少 当前剩余:" .. self.FevStep)
        return true
    end
    return false
end
...
function XFangKuaiStageData:ReduceDropBlockCd()
    if self.DropBlockCd > 0 then
        ...
        XLog.Debug(string.format("顶部方块掉落Cd：%s", self.DropBlockCd))
    end
end
...
function XFangKuaiStageData:GetFevValue()
    return self.FevValue
end

function XFangKuaiStageData:GetFevStep()
    return self.FevStep
end

function XFangKuaiStageData:GetFevLevel()
    return self.FevLevel
end

function XFangKuaiStageData:GetFevRecord()
    return self.HistoryFevRecords
end

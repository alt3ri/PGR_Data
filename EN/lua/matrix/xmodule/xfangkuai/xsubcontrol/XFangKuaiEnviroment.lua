...
function XFangKuaiEnviroment:CreateDropBlockData(stageId)
    ...
    local randomIdx = XMath.RandByWeights(config.Weight)
    local blockLen = config.BlockLength[randomIdx]
    local blockType = config.BlockType[randomIdx]
    local blockX = XTool.Random(1, stageConfig.SizeX - blockLen + 1)
    local blockData = self._MainControl:CreateBlock(stageData, blockLen, blockX, stageConfig.SizeY, blockType)
    local itemId = self._MainControl:GetItemIdByBlockOnCreate(blockType)
    if XTool.IsNumberValid(itemId) then
        blockData:SetItemId(itemId)
    end

...
function XFangKuaiControl:GetNewBlockPool()
    return self._Game:GetNewBlockPool()
end
...
function XFangKuaiControl:CreateBlock(stageData, length, x, y, blockType)
    return self._Create:CreateBlock(stageData, length, x, y, blockType)
end
...
function XFangKuaiControl:GetCollectionReward()
    local rewardId = self:GetActivityConfig().RewardId
    return XRewardManager.GetRewardList(rewardId)
end

function XFangKuaiControl:IsNoStagePassed()
    return self._Model.ActivityData:IsNoStagePassed()
end
...
---@return XTableFangKuaiArchieve[]
function XFangKuaiControl:GetArchieveByBlockTypes(stageId)
    return self._Model:GetArchieveByBlockTypes(stageId)
end
...
-- 首席方块消除后会清除上方所有方块
function XFangKuaiControl:ExecuteChief(chiefBlockData, maxY)
    self._Item:ExecuteChief(chiefBlockData, maxY)
end

-- 真意斩（效果同【单行消除】）
function XFangKuaiControl:ExecuteUltimaSlash(gridY)
    self._Item:ExecuteUltimaSlash(gridY)
end
...
function XFangKuaiControl:GetItemIdByBlockOnCreate(blockKind)
    return self._Model:GetItemIdByBlockOnCreate(blockKind)
end
...
function XFangKuaiControl:FevMoveY(tran, gridY, isMoveUp, addLine, finishCallBack)
    self._BlockMove:FevMoveY(tran, gridY, isMoveUp, addLine, finishCallBack)
end
...
function XFangKuaiControl:GetFevMoveDownTime(lineCount)
    return self._BlockMove:GetFevMoveDownTime(lineCount)
end
...
function XFangKuaiControl:GetFevSpeed()
    return tonumber(self._Model:GetClientConfig("FevSpeed"))
end
...
function XFangKuaiControl:GetBlockExpression(expression, isBoss, colorId)
    if expression == XEnumConst.FangKuai.Expression.Standby then
        ...
        if isBoss and #config.BossStandby == 0 then
            return ""
        elseif not isBoss and #config.Standby == 0 then
            return ""
        end
...
function XFangKuaiControl:GetFevComboGain(combo)
    combo = math.min(combo, self._Model:GetMaxCombo())
    return self._Model:GetComboConfig(combo).FevComboGain 
end
...
function XFangKuaiControl:PlayComboSound(combo)
    local cueIds = self._Model:GetClientConfigs(self:IsFever() and "FevComboCueId" or "ComboCueId")
...
function XFangKuaiControl:PlayDropSound()
    local cueId = tonumber(self._Model:GetClientConfig(self:IsFever() and "FevDropCueId" or "DropCueId"))
    XLuaAudioManager.PlayAudioByType(XLuaAudioManager.SoundType.SFX, cueId)
end
...
function XFangKuaiControl:UpdateCueSpeed()
    if not self._BasicCueSpeed or not self._FevCueSpeed then
        self._BasicCueSpeed = tonumber(self:GetClientConfig("BasicCueSpeed"))
        self._FevCueSpeed = tonumber(self:GetClientConfig("FevCueSpeed"))
    end
    local speed = self:IsFever() and self._FevCueSpeed or self._BasicCueSpeed
    CS.XAudioManager.SetMusicSourcePlayerSpeed(speed)
end
...
function XFangKuaiControl:GetFevExcitedValue()
    local stageId = self:GetCurStageData():GetStageId()
    return self:GetStageConfig(stageId).FevExctiedValue / 10000
end

function XFangKuaiControl:GetMaxFevLevel()
    return self:GetActivityConfig().MaxFevLevel
end

function XFangKuaiControl:GetMaxEnhanceTimes()
    return self:GetActivityConfig().MaxEnhanceTimes
end

function XFangKuaiControl:GetFevScoreGainCoefficient()
    return self:GetActivityConfig().FevScoreGainCoefficient
end

function XFangKuaiControl:GetFevScoreGainCoefficientRate()
    return self:GetActivityConfig().FevScoreGainCoefficientRate
end
...
function XFangKuaiControl:GetSingleLineRemoveEffect(isBigMap, clearType)
    local key = clearType == XEnumConst.FangKuai.ClearType.Chief and "SlashLineRemoveEffect" or "SingleLineRemoveEffect"
    return self._Model:GetClientConfig(key, isBigMap and 2 or 1)
end
...
function XFangKuaiControl:GetCollectionRound()
    return self._Model:GetClientConfigs("CollectionRound")
end
...
function XFangKuaiControl:GetCollectionRecordData()
    return self._Model:GetCollectionRecordData()
end

function XFangKuaiControl:ClearCollection()
    self._Model:ClearCollection()
end

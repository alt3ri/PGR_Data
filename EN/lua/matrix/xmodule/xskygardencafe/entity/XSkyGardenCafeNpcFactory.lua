...
function XSkyGardenCafeNpc:OnInit(uuid, card, posId, transform)
    self._NpcUUId = uuid
    self._Card = card
    self._PosId = posId
    self._Transform = transform
    self._IsPlayLoadEffect = false

    if card then
        self._Card:SetNpcUUId(uuid)
...
function XSkyGardenCafeNpc:ShowDialog()
    ...
    XMVCA.XSkyGardenCafe:DispatchInnerEvent(EventId.EVENT_CAFE_HUD_REFRESH, self._NpcUUId, self._Transform, 
...
function XSkyGardenCafeNpc:HideCoffee()
    XMVCA.XSkyGardenCafe:DispatchInnerEvent(EventId.EVENT_CAFE_HUD_HIDE, self._NpcUUId, HudType.CoffeeHud)
end
...
function XSkyGardenCafeNpc:HideReview()
    XMVCA.XSkyGardenCafe:DispatchInnerEvent(EventId.EVENT_CAFE_HUD_HIDE, self._NpcUUId, HudType.ReviewHud)
end
...
function XSkyGardenCafeNpc:HideEmoji()
    XMVCA.XSkyGardenCafe:DispatchInnerEvent(EventId.EVENT_CAFE_HUD_HIDE, self._NpcUUId, HudType.EmojiHud)
end

function XSkyGardenCafeNpc:SetShowLoadEffect(value)
    self._IsPlayLoadEffect = value
end

function XSkyGardenCafeNpc:GetShowLoadEffect()
    return self._IsPlayLoadEffect
end
...
function XSkyGardenCafeNpcFactory:CreateNpc(card, posId)
    local cardId = card:GetCardId()
    local npcId = self._Model:GetCustomerNpcId(card:GetCardId())
    ...
    npc = self:AddEntity(XSkyGardenCafeNpc, uuid, card, posId, root)
...
function XSkyGardenCafeNpcFactory:LoadNpc(card)
    ...
    --已经加载在吧台的角色，直接飞特效
    if card:IsMaxQuality() and self._BarNpcUUId and self._BarNpcUUId > 0 then
    ...
    local uuid = self:CreateNpc(card, self._Model:RandomPosId())
    if not uuid or uuid <= 0 then
        XMVCA.XSkyGardenCafe:DispatchInnerEvent(XMVCA.XBigWorldService.DlcEventId.EVENT_CAFE_EFFECT_BEGIN_FLY)
        XMVCA.XSkyGardenCafe:DispatchInnerEvent(XMVCA.XBigWorldService.DlcEventId.EVENT_CAFE_EFFECT_FLY_COMPLETE)
        return
    end
    local npc = self:GetNpc(uuid)
    if npc then
        npc:ShowCoffee(card:GetTotalCoffee())
        npc:ShowReview(card:GetTotalReview())
        npc:SetShowLoadEffect(true)
    end
end

---@param card XSkyGardenCafeCardEntity
function XSkyGardenCafeNpcFactory:PlayResourceChange(card, changeCoffee, changeReview)
    if not card then
        return
    end
    local npcUUId = card:GetNpcUUId()
    if not npcUUId or npcUUId <= 0 then
        return
    end
    local npc = self:GetNpc(npcUUId)
    if npc and npc:GetShowLoadEffect() then
        npc:ShowCoffee(changeCoffee)
        npc:ShowReview(changeReview)
    end
...
function XSkyGardenCafeNpcFactory:LoadNpcWhenDrawCard(deckCards)
    ...
    if not XTool.IsTableEmpty(deckCards) then
        for _, card in pairs(deckCards) do
            ...
            self._BarNpcUUId = self:CreateNpc(card, posId)
            XMVCA.XSkyGardenCafe:DispatchInnerEvent(XMVCA.XBigWorldService.DlcEventId.EVENT_CAFE_BAR_COUNTER_NPC_CHANGED)
            ::continue::
        end
    end
end
...
---@param deckCards XSkyGardenCafeCardEntity[]
function XSkyGardenCafeNpcFactory:RemoveNpcWhenRoundEnd(deckCards, force)
    local keepBarNpc = false
    if self._BarNpcUUId and not force then
        if not keepBarNpc and not XTool.IsTableEmpty(self._KeepStayNpcId) then
            for _, npc in pairs(self._NpcDict) do
                local npcId = self._Model:GetCustomerNpcId(npc:GetCardId())
                if self._KeepStayNpcId[npcId] then
                    keepBarNpc = true
                    break
                end
            end
        end
        if not keepBarNpc and not XTool.IsTableEmpty(deckCards) then
            for _, card in pairs(deckCards) do
                local uuid = card:GetNpcUUId()
                if uuid == self._BarNpcUUId and uuid > 0 then
                    keepBarNpc = true
                    break
                end
            end

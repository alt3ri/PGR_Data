...
local TriggerDictInDeck = {
    [XMVCA.XSkyGardenCafe.EffectTriggerId.Deck2Deal] = true
}
...
function XSGCafePriorityLotteryBuff:OnApply()
...
function XSGCafePriorityLotteryBuff:OnDismiss()
    if not self._Card then
        return
    end
    local cardId = self._Card:GetCardId()
...
function XSGCafeStayInHandBuff:OnApply()
...
function XSGCafeStayInHandBuff:OnDismiss()
...
function XSGCafeRemoveBuff:OnApply()
    local param1 = self._Params[1]
    --指定当前卡牌
    if param1 == 1 then
        if not self._Card then
...
function XSGCafeReplaceBuff:OnApply()
    local param1 = self._Params[1]
    ...
    if param1 == 1 then
        --指定类型
        deckCards = roundEntity:GetDeckCardsWithType(dict, true)
    elseif param1 == 2 then
        --指定非类型
        deckCards = roundEntity:GetDeckCardsWithType(dict, false)
    elseif param1 == 3 then
        --指定稀有度
        deckCards = roundEntity:GetDeckCardsWithQuality(dict, true)
    elseif param1 == 4 then
        --指定非稀有度
        deckCards = roundEntity:GetDeckCardsWithQuality(dict, false)
    end
    ...
    --直接牌库抽
    if param2 == 1 then
        libCards = roundEntity:GetPoolCardsWithOrder(#deckCards, isRandom)
    elseif param2 == 2 then
        --读取配置抽
        local targets = { self._Params[4] }
        libCards = roundEntity:GetPoolCardsWithTargets(targets, #deckCards, isRandom)
    elseif param2 == 3 then
        --根据类型抽
        libCards = roundEntity:GetPoolCardsWithType(self._Params[4], #deckCards, isRandom)
    end
    
    self:ChangeEffectCount(false, 1)
    self._AddDeckCount = roundEntity:ReplacePoolInDeck(deckCards, libCards, self._Card)
end

function XSGCafeReplaceBuff:OnDismiss()
    if not self._AddDeckCount then
        return
    end
    self._Model:GetBattleInfo():AddDeckCount(-self._AddDeckCount)
    self:ChangeEffectCount(false, 1)
end
...
function XSGCafeNonDirectedLotteryBuff:OnApply()
    ...
    local libs = roundEntity:GetPoolCardsWithOrder(count, false)
    self:ChangeEffectCount(false, 1)
    self._AddDeckCount = roundEntity:ReplacePoolInDeck(nil, libs, self._Card)
end

function XSGCafeNonDirectedLotteryBuff:OnDismiss()
    if not self._AddDeckCount then
        return
    end
    self._Model:GetBattleInfo():AddDeckCount(-self._AddDeckCount)
...
function XSGCafeTargetedLotteryBuff:OnApply()
    if self:TryAddNextRoundBuff() then
        return
    end
    ...
    local roundEntity = self._OwnControl:GetMainControl():GetRoundEntity()
    local libIds = roundEntity:GetPoolCardsWithTargets(targets, self._Params[3], isRandom)

    self:ChangeEffectCount(false, 1)
    self._AddDeckCount = roundEntity:ReplacePoolInDeck(nil, libIds, self._Card)
end


function XSGCafeTargetedLotteryBuff:OnDismiss()
    if not self._AddDeckCount then
        return
    end
    self._Model:GetBattleInfo():AddDeckCount(-self._AddDeckCount)
...
function XSGCafeInsertBuff:OnApply()
    if self:TryAddNextRoundBuff() then
        return
    end
    ...
    elseif param3 == 2 then
        ...
        cardIds = self:GetParamList(index, check)
    end
    self._OwnControl:GetMainControl():GetRoundEntity():InsertPoolNewCardsWithEdge(cardIds, pos)
    self:ChangeEffectCount(false, 1)
end
...
function XSGCafePool2DeckBuff:OnApply()
    if self:TryAddNextRoundBuff() then
        return
    end
...
function XSGCafeDealCountChangeBuff:OnApply()
    if self:TryAddNextRoundBuff() then
        return
    end
...
function XSGCafeDealCountChangeBuff:OnDismiss()
    if not self._Count or self._Count == 0 then
        return
    end
...
function XSGCafeCopyBuff:OnApply()
    if not self._Card then
        return
    end
    ...
    --指定位置的卡
    local roundEntity = self._OwnControl:GetMainControl():GetRoundEntity()
    local index = roundEntity:GetDealCardIndexWithCard(self._Card)
    local targetIndex
    if param1 == 1 then
        --前一张卡
        targetIndex = index - 1
...
function XSGCafeDealIndexBuff:OnApply()
    ...
    buff:DoApply(TriggerDictInDeck)
    self:ChangeEffectCount(false, 1)
end

function XSGCafeDealIndexBuff:OnPreview()
    local param1 = self._Params[1]
    local count = 0
    local roundEntity = self._OwnControl:GetMainControl():GetRoundEntity()
    ...
    self._PreviewBuff = buff
    self._AttachCount = count
    self._PreviewBuff:ChangeMaxEffectCount(true, count)
    self._PreviewBuff:DoPreview(TriggerDictInDeck)
    self:ChangeEffectCount(true, 1)
end

function XSGCafeDealIndexBuff:AddBuffArgs()
    local buff = self._AttachBuff or self._PreviewBuff
    if not self._Card or not buff then
        return
    end
    self._Card:AddBuffArgs(1301, self._AttachCount)
    self._Card:AddBuffArgs(1399, self._AttachCount * buff:GetExtraValue())
end

function XSGCafeDealIndexBuff:OnPreviewReset()
    if not self._PreviewBuff then
        return
    end
    self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._PreviewBuff)
    self._PreviewBuff = nil
end

function XSGCafeDealIndexBuff:OnDestroy()
    if self._AttachBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._AttachBuff)
    end
    self._AttachBuff = nil
end
...
function XSGCafeCardCountBuff:OnApply()
    local param1 = self._Params[1]
    ...
    if param4 == 0 then
        ...
        if not XTool.IsTableEmpty(cards) then
            ...
            elseif param2 == 4 then
                --指定稀有度
                local temp = {}
                local cardQuality = self:GetParamDict(5, function(v)
                    return v and v > 0
                end)
                for _, card in pairs(cards) do
                    if cardQuality[card:GetCardQuality()] then
                        temp[#temp + 1] = card
                    end
                end
                cards = temp
...
function XSGCafeCardCountBuff:OnPreview()
    local param1 = self._Params[1]
    local param4 = self._Params[4]
    local count
    if param4 == 0 then
    ...
    buff:ChangeMaxEffectCount(true, count)
    for _ = 1, count do
        buff:DoPreview(TriggerDictInDeck)
    end
    self._PreviewBuff = buff
    self._AttachCount = count
    self:ChangeEffectCount(true, 1)
...
function XSGCafeCardCountBuff:OnPreviewReset()
    if self._PreviewBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._PreviewBuff)
    end
    self._PreviewBuff = nil
end

function XSGCafeCardCountBuff:OnDestroy()
    if self._AttachBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._AttachBuff)
    end
    self._AttachBuff = nil
...
function XSGCafeCarryWhenUseBuff:OnApply()
    local param1 = self._Params[1]
    local roundEntity = self._OwnControl:GetMainControl():GetRoundEntity()
    ---@type XSkyGardenCafeCardEntity[]
    local deckCards = roundEntity:GetDeckCardEntities()
    local libCards = roundEntity:GetPoolCardEntities()
...
function XSGCafeCardUseCountBuff:OnApply()
...
function XSGCafeCardUseCountBuff:OnPreview()
    local count = self._Model:GetBattleInfo():GetCardUseCount(self._Card:GetCardId())
    --预览的需要+1
    count = count + 1
    local buffId
...
function XSGCafeCardUseCountBuff:AddBuffArgs()
    ...
    self._Card:AddBuffArgs(1699, self._AttachCount * buff:GetExtraValue())
end

function XSGCafeCardUseCountBuff:OnPreviewReset()
    if self._PreviewBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._PreviewBuff)
    end
    self._PreviewBuff = nil
...
function XSGCafeCardUseCountBuff:OnDestroy()
    if self._AttachBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._AttachBuff)
    end
    self._AttachBuff = nil
...
function XSGCafeGameRoundBuff:OnApply()
...
function XSGCafeGameRoundBuff:OnPreview()
    local round = self._Model:GetBattleInfo():GetRound()
    local buffId = self._Params[2]
    local value = self._Params[1]
    if value == 0 then
...
function XSGCafeGameRoundBuff:AddBuffArgs()
    ...
    self._Card:AddBuffArgs(1799, self._AttachCount * buff:GetExtraValue())
end

function XSGCafeGameRoundBuff:OnPreviewReset()
    if self._PreviewBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._PreviewBuff)
    end
    self._PreviewBuff = nil
...
function XSGCafeGameRoundBuff:OnDestroy()
    if self._AttachBuff then
        self._OwnControl:GetMainControl():GetBuffFactory():RemoveEntity(self._AttachBuff)
    end
    self._AttachBuff = nil
...
function XSGCafeApplyOtherBuff:OnApply()
    if not self._Card then
        XLog.Error("该Buff必须配置在卡上, EffectId = " .. self._BuffId)
        return
    end
    local buffIds = self:GetParamList(1, function(value) 
...
function XSGCafeGameRoundSustainBuff:OnApply()
    if self._Card then
...
function XSGCafeCreateNewBuff:OnApply()
    if self:TryAddNextRoundBuff() then
        return
    end
    ...
    local targets = self:GetParamList(index, check)
    local roundEntity = self._OwnControl:GetMainControl():GetRoundEntity()
    local libIds = roundEntity:InsertPoolNewCards(targets, self._Params[3], isRandom)
    
    self:ChangeEffectCount(false, 1)
    self._AddDeckCount = roundEntity:ReplacePoolInDeck(nil, libIds, self._Card)
end

function XSGCafeCreateNewBuff:OnDismiss()
    if not self._AddDeckCount then
        return
    end
    self._Model:GetBattleInfo():AddDeckCount(-self._AddDeckCount)
...
function XSGCafeBuffFactory:CreateBuff(id, card)
    ...
    if not cls then
        XLog.Error(string.format("创建Buff失败! BuffId = %s BuffType = %s", id. buffType))
        return
    end

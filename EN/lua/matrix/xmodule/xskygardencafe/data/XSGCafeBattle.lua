...
function XSGCafeBattle:NewBattle(battleInfo)
    self:UpdateDataFromServer(battleInfo)

    self._CardForeverValueCache = {}
    self._CardUseCountCache = {}
    self._AbandonCardsCache = {}
    self._PrecedeCardCache = {}
    self._StayInHandCardCache = {}

    self:ResetWhenDataSync(true)
end

function XSGCafeBattle:UpdateData(battleInfo)
    self:UpdateDataFromServer(battleInfo)
    
    self._CardForeverValueCache = XTool.Clone(self._CardForeverValue)
    self._CardUseCountCache = XTool.Clone(self._CardUseCount)
    self._AbandonCardsCache = XTool.Clone(self._AbandonCards)
    self._PrecedeCardCache = XTool.Clone(self._PrecedeCard)
    self._StayInHandCardCache = XTool.Clone(self._StayInHandCard)
    self:ResetWhenDataSync(true)
end

function XSGCafeBattle:UpdateDataFromServer(battleInfo)
    ...
    self._NextRoundBuffs = battleInfo.NextRoundBuffs or {}
end
...
function XSGCafeBattle:Reset()
    ...
    self._TriggerRoundResourceDict = {
        [XMVCA.XSkyGardenCafe.EffectTriggerId.RoundResourceChanged] = true
    }
    
    self._TriggerCardResourceDict = {
        [XMVCA.XSkyGardenCafe.EffectTriggerId.CardResourceChanged] = true
    }
...
function XSGCafeBattle:ResetWhenDataSync(ignoreClone)
    ...
    if not ignoreClone then
        self._CardForeverValue = XTool.Clone(self._CardForeverValueCache)
        self._CardUseCount = XTool.Clone(self._CardUseCountCache)
        self._AbandonCards = XTool.Clone(self._AbandonCardsCache)
        self._PrecedeCard = XTool.Clone(self._PrecedeCardCache)
        self._StayInHandCard = XTool.Clone(self._StayInHandCardCache)
    end
...
function XSGCafeBattle:SyncBill()
    --回合初先同步上回合的分数
    self._Score = self:GetTotalScore()
    self._Review = self:GetTotalReview()
    --再清空本回合增加分数
    self:ResetAddCardScore()
    self:ResetAddCardReview()
end

function XSGCafeBattle:GetNextRoundBuffs()
    return self._NextRoundBuffs
end
...
function XSGCafeBattle:NextRound()
    ...
    XMVCA.XSkyGardenCafe:DispatchInnerEvent(XMVCA.XBigWorldService.DlcEventId.EVENT_CAFE_ROUND_CHANGED)
end
...
function XSGCafeBattle:ChangeCardForeverData(cardId, isCoffee, value)
    ...
    XMVCA.XSkyGardenCafe:DispatchInnerEvent(XMVCA.XBigWorldService.DlcEventId.EVENT_CAFE_APPLY_BUFF, true, true, false, self._TriggerCardResourceDict)
end

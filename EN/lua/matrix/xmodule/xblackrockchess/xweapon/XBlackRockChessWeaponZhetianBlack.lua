...
---@class XBlackRockChessWeaponZhetianBlack
---@field _Boss XChessBoss
local XBlackRockChessWeaponZhetianBlack = XClass(XBlackRockChessWeapon, "XBlackRockChessWeaponZhetianBlack")

-- 初始化
function XBlackRockChessWeaponZhetianBlack:OnInit()
    -- 普攻范围
    self._ATTACK_RANGE = {
        {x = 0, y = 1},
        {x = 1, y = 0},
...
function XBlackRockChessWeaponZhetianBlack:OnRelease()
...
-- 由C#调用
-- 获取移动范围
function XBlackRockChessWeaponZhetianBlack:Search(csharpList)
...
-- C#调用，由继承类实现
-- 获取进攻范围
function XBlackRockChessWeaponZhetianBlack:SearchAttack(csharpList)
    -- 存在延迟生效的技能，使用延迟技能的攻击范围
    if self._Boss:IsExitSkillDelay() then
        ...
        if skillInfo.DelayRound == 0 then
            ...
            if skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_SKILL then
                self:AddNormalSkillRange(csharpList)
            elseif skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL then
                local ATTACK_TYPE = 2
                if skillInfo.Count == ATTACK_TYPE then
                    self:AddSpecialSkillRange(csharpList)
                end
            elseif skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_ATTACK then
...
-- 获取可移动的点位，用于计算最佳移动点位
function XBlackRockChessWeaponZhetianBlack:GetMovePoints()
    local points = {}
    for x = 1, XEnumConst.BLACK_ROCK_CHESS.BOARD_WIDTH do
        for y = 1, XEnumConst.BLACK_ROCK_CHESS.BOARD_HEIGHT do
            local isBlack = CS.XBlackRockChess.XBlackRockChessManager.Instance:IsBlackBlock(x, y)
            if isBlack then
...
-- 添加移动范围
function XBlackRockChessWeaponZhetianBlack:AddMoveRange(csharpList)
...
-- 检测能否攻击到这个点位
function XBlackRockChessWeaponZhetianBlack:CheckAttackPoint(point)
    -- 存在延迟生效的技能，使用延迟技能的攻击范围
    if self._Boss:IsExitSkillDelay() then
        local skillInfo = self._Boss:GetDelaySkillInfo()
        local skillId = skillInfo.SkillId
        if skillInfo.DelayRound == 0 then
            ...
            -- 黑雷击：对所有黑格造成伤害
            if skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_SKILL then
                local isBlack = CS.XBlackRockChess.XBlackRockChessManager.Instance:IsBlackBlock(point.x, point.y)
                return isBlack
            -- 大招：1阶段召唤、2阶段全屏伤害
            elseif skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL then
                local ATTACK_TYPE = 2
...
-- 是否可以释放技能
function XBlackRockChessWeaponZhetianBlack:IsCanPlaySkill()
    self._SkillId = nil
...
-- 是否可以使用大招
function XBlackRockChessWeaponZhetianBlack:IsCanPlaySpecialSkill()
    if self._Boss:IsSkillInCD(XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL) then
        return false
    end

    self._SkillId = XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL
...
-- 获取技能对应的召唤怪物信息
function XBlackRockChessWeaponZhetianBlack:GetSkillSummonParams(skillId)
    -- 召唤怪物信息
...
-- 添加大招技能的攻击范围
function XBlackRockChessWeaponZhetianBlack:AddSpecialSkillRange(csharpList, luaList)
    for x = 1, XEnumConst.BLACK_ROCK_CHESS.BOARD_WIDTH do
        for y = 1, XEnumConst.BLACK_ROCK_CHESS.BOARD_HEIGHT do
...
-- 是否可以使用普通技能
function XBlackRockChessWeaponZhetianBlack:IsCanPlayNormalSkill()
    if self._Boss:IsSkillInCD(XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_SKILL) then
        return false
    end
    ...
    for _, role in pairs(roleDic) do
        if role:IsInBoard() then
            ...
            local isBlack = CS.XBlackRockChess.XBlackRockChessManager.Instance:IsBlackBlock(rolePoint.x, rolePoint.y)
            haveTarget = haveTarget or isBlack
        end
    end
    if not haveTarget then return false end

    self._SkillId = XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_SKILL
    self._ActionType = XEnumConst.BLACK_ROCK_CHESS.ACTION_TYPE.ATTACK
    self._SkillTargetPoint = self._Boss:GetMovedPoint()
    self._SkillAttackTimes = 1
    return true
...
-- 添加普通技能的攻击范围
function XBlackRockChessWeaponZhetianBlack:AddNormalSkillRange(csharpList, luaList)
    for x = 1, XEnumConst.BLACK_ROCK_CHESS.BOARD_WIDTH do
        for y = 1, XEnumConst.BLACK_ROCK_CHESS.BOARD_HEIGHT do
            local isBlack = CS.XBlackRockChess.XBlackRockChessManager.Instance:IsBlackBlock(x, y)
            if isBlack then
...
-- 是否可以使用切换技能
function XBlackRockChessWeaponZhetianBlack:IsCanPlayChangeSkill()
    if self._Boss:IsSkillInCD(XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_CHANGE) then
        return false
    end

    self._SkillId = XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_CHANGE
    self._ActionType = XEnumConst.BLACK_ROCK_CHESS.ACTION_TYPE.ATTACK
    self._SkillTargetPoint = self._Boss:GetMovedPoint()
    self._SkillAttackTimes = 1
    self._SwitchWeaponId = XEnumConst.BLACK_ROCK_CHESS.WEAPON_TYPE.ZHETIAN_WHITE
    return true
end

-- 是否可以使用普攻
function XBlackRockChessWeaponZhetianBlack:IsCanPlayAttackSkill()
    if self._Boss:IsSkillInCD(XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_ATTACK) then
        return false
    end
    ...
    for _, offset in ipairs(self._ATTACK_RANGE) do
        ...
        if isValid then
            ...
            if piece and piece.ChessRole then
                self._SkillId = XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_ATTACK
                self._ActionType = XEnumConst.BLACK_ROCK_CHESS.ACTION_TYPE.ATTACK
...
-- 添加普攻范围
function XBlackRockChessWeaponZhetianBlack:AddAttackRange(csharpList, luaList)
    local curPoint = self._Boss:GetMovedPoint()
    for _, offset in ipairs(self._ATTACK_RANGE) do
        local targetX = curPoint.x + offset.x
        local targetY = curPoint.y + offset.y
...
-- 获取延迟释放技能的攻击点位
function XBlackRockChessWeaponZhetianBlack:GetDelayDamagePoints()
    local points = {}
    -- 存在延迟生效的技能，使用延迟技能的攻击范围
    if self._Boss:IsExitSkillDelay() then
        local skillInfo = self._Boss:GetDelaySkillInfo()
...
-- 释放技能
function XBlackRockChessWeaponZhetianBlack:PlaySkill()
    local animTime = 0
    ...
    -- 大招动画表现
    if skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL then
    ...
    -- 技能动画表现
    elseif skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_SKILL then
        self._Boss:GetImp():PlayAnimation("Aim")
        
    -- 切换状态动画表现
    elseif skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_CHANGE then
        ...
        self._Boss:GetImp():PlayAnimation("Skill01", 0, function()
            self._Boss:ReloadBossModel(XEnumConst.BLACK_ROCK_CHESS.WEAPON_TYPE.ZHETIAN_WHITE)
            self._Boss:GetImp():LoadEffectInternal(XEnumConst.BLACK_ROCK_CHESS.EFFECT_ID.ZHETIAN_CHANGE_EFFECT_WHITE, false)
        end)
...
-- 检测处理延迟技能
function XBlackRockChessWeaponZhetianBlack:CheckPlayDelaySkill()
    ...
    for skillId, skillInfo in pairs(self._Boss:GetWeaponSkillDict()) do
        if skillInfo.DelayRound == 0 then
            ...
            -- 普攻技能结算
            if skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_ATTACK then
...
-- 是否在延迟技能之后眩晕
function XBlackRockChessWeaponZhetianBlack:IsDizzyAfterDelaySkill()
    for skillId, skillInfo in pairs(self._Boss:GetWeaponSkillDict()) do
        if skillInfo.DelayRound == 0 then
            if skillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL then
                local END_COUNT = 2
...
-- 刷新技能特效
function XBlackRockChessWeaponZhetianBlack:RefreshSkillEffects()
    if self._Boss:IsExitSkillDelay() then
        local skillInfo = self._Boss:GetDelaySkillInfo()
        -- 跳劈技能中
        if skillInfo.SkillId == XEnumConst.BLACK_ROCK_CHESS.WEAPON_SKILL_TYPE.ZHETIAN_BLACK_BIGSKILL and not self.IsRecoverEffect then
            if skillInfo.Count == 1 then
                self._Boss:GetImp():PlayAnimation("Aim")
            else
...
return XBlackRockChessWeaponZhetianBlack
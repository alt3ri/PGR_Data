...
local CharacterEnhanceSkillTableKey = 
{
    ...
    EnhanceSkillLevelEffect = { Identifier = "SkillLevelId" },
...
local CharacterSkillTableKey = 
{
    ...
    CharacterSkillGroup = {},
    -- CharacterSkillPos = {},
    -- CharacterSkillUpgrade = { CacheType = XConfigUtil.CacheType.Preload },  -- 已被优化
    CharacterSkillUpgradeIdOptimize = { DirPath = XConfigUtil.DirectoryType.Client, Identifier = "SkillLevelId", CacheType = XConfigUtil.CacheType.Preload },
    CharacterSkillUpgradeMinMaxLevel = { DirPath = XConfigUtil.DirectoryType.Client, Identifier = "SkillId", CacheType = XConfigUtil.CacheType.Preload },
    ...
    CharacterSkillLevelEffect = { Identifier = "SkillLevelId", CacheType = XConfigUtil.CacheType.Normal },
    CharacterSkillLevelEffectMinMaxLevel = { DirPath = XConfigUtil.DirectoryType.Client, Identifier = "SkillId", CacheType = XConfigUtil.CacheType.Normal },
    CharacterSkillType = {},
...
function XCharacterModel:OnInit()
    ...
    self.EnhanceSkillIdGroupDic = {}            -- 角色补强技能id和组Id字典
    ...
    self.SkillIdGeneralSkillIdsDic = {}         -- 记录技能id拥有的效应，解锁对应效应需要的技能等级
    self.EnhanceSkillIdGeneralSkillIdsDic = {}  -- 记录跃升技能id拥有的效应，解锁对应效应需要的技能等级
    self.TempWholeDic = {}                      -- 临时存放的字典，只记录不会变化的数据，重登不需要清除
    self.TempWholeResetDic = {}                 -- 临时存放的字典，重登会清除
...
--- @return XTableCharacterSkillGroup[]
...
function XCharacterModel:GetCharacterSkillUpgradeIdOptimize()
    return self._ConfigUtil:GetByTableKey(CharacterSkillTableKey.CharacterSkillUpgradeIdOptimize)
end

function XCharacterModel:GetCharacterSkillUpgradeMinMaxLevel()
    return self._ConfigUtil:GetByTableKey(CharacterSkillTableKey.CharacterSkillUpgradeMinMaxLevel)
end
...
function XCharacterModel:GetCharacterSkillLevelEffectMinMaxLevel()
    return self._ConfigUtil:GetByTableKey(CharacterSkillTableKey.CharacterSkillLevelEffectMinMaxLevel)
end
...
function XCharacterModel:GetCharacterGeneralSkill()
    return self._ConfigUtil:GetByTableKey(CharacterTableKey.CharacterGeneralSkill)
end
...
function XCharacterModel:InitEnhanceSkillIdGroupDic()
    local config = self:GetEnhanceSkillGroup()
    for skillGroupId, v in pairs(config) do
        for k2, skillId in pairs(v.SkillId) do
            self.EnhanceSkillIdGroupDic[skillId] = skillGroupId
        end
    end
end

function XCharacterModel:GetEnhanceSkillIdGroupDic()
    if XTool.IsTableEmpty(self.EnhanceSkillIdGroupDic) then
        self:InitEnhanceSkillIdGroupDic()
    end
    return self.EnhanceSkillIdGroupDic
end

function XCharacterModel:InitEnhanceGroupIdCharacterIdDic()
    local config = self:GetEnhanceSkill()
    for characterId, v in pairs(config) do
        for i, skillGroupId in pairs(v.SkillGroupId) do
            self.EnhanceGroupIdCharacterIdDic[skillGroupId] = characterId
        end
    end
end

function XCharacterModel:GetEnhanceGroupIdCharacterIdDic()
    if XTool.IsTableEmpty(self.EnhanceGroupIdCharacterIdDic) then
        self:InitEnhanceGroupIdCharacterIdDic()
    end

    return self.EnhanceGroupIdCharacterIdDic
end
...
function XCharacterModel:InitSkillIdAndEnhanceSkillIdGeneralSkillIdsDic()
    local allConfig = self:GetCharacterGeneralSkill()
    for generalSKillId, v in pairs(allConfig) do
        for k, skillId in pairs(v.SkillId) do
            if not self.SkillIdGeneralSkillIdsDic[skillId] then
                self.SkillIdGeneralSkillIdsDic[skillId] = { }
            end
            table.insert(self.SkillIdGeneralSkillIdsDic[skillId], {GeneralSkillId = generalSKillId, NeedLevel = v.SkillLevel[k]})
        end

        for k, enhanceSkillId in pairs(v.EnhanceSkillId) do
            if not self.EnhanceSkillIdGeneralSkillIdsDic[enhanceSkillId] then
                self.EnhanceSkillIdGeneralSkillIdsDic[enhanceSkillId] = {}
            end
            table.insert(self.EnhanceSkillIdGeneralSkillIdsDic[enhanceSkillId], {GeneralSkillId = generalSKillId, NeedLevel = v.EnhanceSkillLevel[k]})
        end
    end
end

function XCharacterModel:GetSkillIdGeneralSkillIdsDic()
    if XTool.IsTableEmpty(self.SkillIdGeneralSkillIdsDic) or XTool.IsTableEmpty(self.EnhanceSkillIdGeneralSkillIdsDic) then
        self:InitSkillIdAndEnhanceSkillIdGeneralSkillIdsDic()
    end

    return self.SkillIdGeneralSkillIdsDic
end

function XCharacterModel:GetEnhanceSkillIdGeneralSkillIdsDic()
    if XTool.IsTableEmpty(self.EnhanceSkillIdGeneralSkillIdsDic) then
        self:InitSkillIdAndEnhanceSkillIdGeneralSkillIdsDic()
    end

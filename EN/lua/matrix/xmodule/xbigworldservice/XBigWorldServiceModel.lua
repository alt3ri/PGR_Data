...
function XBigWorldServiceModel:OnInit()
    ...
    local config = {
        ...
        [TablePath.BigWorldRewardGoodDetails] = {
            ReadIntType,
            XTable.XTableBigWorldRewardGoodDetails,
            identifier,
            NormalCacheType,
        },
    }
    ...
    self._QualityPathKey = {
        [1] = "BigWorldSkyGardenQuality1",
        [2] = "BigWorldSkyGardenQuality2",
        [3] = "BigWorldSkyGardenQuality3",
        [4] = "BigWorldSkyGardenQuality4",
        [5] = "BigWorldSkyGardenQuality5",
...
---@return XTableBigWorldRewardGoodDetails
function XBigWorldServiceModel:GetRewardGoodDetailsTemplate(rewardGoodsId, isNoTip)
    return self._ConfigUtil:GetCfgByPathAndIdKey(TablePath.BigWorldRewardGoodDetails, rewardGoodsId, isNoTip)
end
...
function XBigWorldServiceModel:GetItemsByType(itemType, isIncludeZero)
    ...
    for itemId, itemConfig in pairs(itemConfigs) do
        if itemConfig.ItemType == itemType then
            local item = self:GetItem(itemId)
...
function XBigWorldServiceModel:GetQualityPathKey(quality)
    return self._QualityPathKey[quality] or ""
end

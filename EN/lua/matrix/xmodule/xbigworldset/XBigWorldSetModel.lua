...
local XBWInputSetting = require("XModule/XBigWorldSet/XSetting/XBWInputSetting")
...
function XBigWorldSetModel:GetBigWorldShowInputMapIds()
    local config = self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.BigWorldShowInputMap, 1)
    return config.InputMapId
end
...
function XBigWorldSetModel:CreateSettingBySetType(setType)
    ...
    elseif setType == XEnumConst.BWSetting.SetType.Input then
        return XBWInputSetting.New()
    end
end

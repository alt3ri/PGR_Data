...
function XUrlAgency:InitEvent()
    XEventManager.AddEventListener(XEventId.EVENT_WEBVIEW_CLOSE, self.OnWebViewCloseEvent, self)
end

function XUrlAgency:OnRelease()
    XEventManager.RemoveEventListener(XEventId.EVENT_WEBVIEW_CLOSE, self.OnWebViewCloseEvent, self)
end
...
function XUrlAgency:SkipByUrlId(id)
    ...
    if isEmbed then
        ...
        XHeroSdkManager.OpenWebview(url, cfg.Title or "", cfg.IsTransparent, isLandscape, function() 
            -- 内置浏览器在Editor及debugPC包打不开，此时不处理
            if XMain.IsEditorDebug then
                return
            end
            
            -- 内置浏览器关闭音量
            if cfg.IsDisableAudio then
                self._DisableAudio = true
                self:_SetAllBaseSourceMute(true)
            end
        end)
    else
...
--- 浏览器关闭事件回调
function XUrlAgency:OnWebViewCloseEvent()
    -- 恢复音量
    if self._DisableAudio then
        self:_SetAllBaseSourceMute(false)
        self._DisableAudio = nil
    end
end

--- 设置游戏内音量的静音状态
function XUrlAgency:_SetAllBaseSourceMute(isMute)
    XLuaAudioManager.MuteAisacByPlayType(XLuaAudioManager.SoundType.Music, isMute)
    XLuaAudioManager.MuteAisacByPlayType(XLuaAudioManager.SoundType.SFX, isMute)
    XLuaAudioManager.MuteAisacByPlayType(XLuaAudioManager.SoundType.Voice, isMute)
end

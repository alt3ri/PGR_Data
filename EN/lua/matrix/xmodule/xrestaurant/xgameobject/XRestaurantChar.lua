...
function XRestaurantChar:OnInit(root, id)
    XRestaurantIScene.OnInit(self, root, id)
    self._Id = id
    self._ReferenceCount = 0
    self._BubbleOffset = XMVCA.XRestaurant:StrPos2Vector3(self._Model:GetClientConfigValue("CharacterProperty", 2))
    self:InitData()
end

function XRestaurantChar:Init()
    self._MoveSpeed = tonumber(self._Model:GetClientConfigValue("CharacterProperty", 1))
    self._BubbleDuration = self._OwnControl:GetBubbleProperty()
end
...
function XRestaurantChar:Born(bornCb)
    ...
    --存在 且 显示
    if exist and not self:IsHide() then
...
function XRestaurantChar:CheckAfterLoad()
    if XTool.UObjIsNil(self._Root) then
        return false
    end
    return true
end
...
function XRestaurantChar:Dispose()
    ...
    self:Hide()
    self:StopDelayTimer()
    self.DelayTimer = XScheduleManager.ScheduleOnce(self._DelayReleaseCb, ResourceUnloadTime)
end

function XRestaurantChar:StopDelayTimer()
...
function XRestaurantChar:OnLoadSuccess()
    ...
    if not self._Model or not self:CheckAfterLoad() then
        self:Release()
        return
    end
    -- 加载自定义状态机
    ...
    if not string.IsNilOrEmpty(path) then
        ...
        self._CtrlPath = path
        self._Animator.runtimeAnimatorController = asset
    end
    --取消动画自带的位移
    ...
    if self:IsHideWhenStopBusiness() and not self._OwnControl:GetRoom():IsInBusinessScene() then
        self:Hide()
    end
end

function XRestaurantChar:IsHideWhenStopBusiness()
    return false
end

function XRestaurantChar:Release()
    if not string.IsNilOrEmpty(self._CtrlPath) then
        local loader = self._OwnControl:GetLoader()
        loader:Unload(self._CtrlPath)
        self._CtrlPath = nil
    end
    ...
    if not self._RandomBubbleHandler then
        self._RandomBubbleHandler = function()
            ...
            XScheduleManager.ScheduleOnce(function() 
                self:DoHideBubble()
                self._IsShowDelayBubble = false
            end, self._BubbleDuration * XScheduleManager.SECOND)
        end
    end
...
--加载特效
function XRestaurantChar:DoLoadEffect(effectId, initPos)
    if XTool.UObjIsNil(self._GameObject) then
        return
    end
    local path = self._Model:GetEffectUrl(effectId)

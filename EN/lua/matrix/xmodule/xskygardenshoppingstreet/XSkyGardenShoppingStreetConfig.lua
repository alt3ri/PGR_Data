...
local TableKey = {
    ...
    SgStreetFeedbackType = { Identifier = "FeedbackType", },
    SgStreetGrapevine = {},
...
---@return bool 是否是好反馈
function XSkyGardenShoppingStreetConfig:IsSgStreetFeedbackTypeGood(feedbackType)
    local config = self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.SgStreetFeedbackType, feedbackType)
    if not config then return false end
    return config.FeedbackAttitude == 1
end

...
function XSkyGardenShoppingStreetControl:OnInit()
    ...
    self._ResetData = XPlayer.Id.. "_SS_ResetData"
    self._FeedbackTipsKey = XPlayer.Id.. "_SS_FeedbackTipsKey"
end
...
function XSkyGardenShoppingStreetControl:SgStreetStageStartRequest(stageId, cb)
    ...
    XNetwork.Call(
        ...
        function(res)
            ...
            self._Model:SetFinishComfirm(false)
            -- 存储商店设置缓存
            self:SetResetData({})
            XMVCA.XSkyGardenShoppingStreet:SetFinishStageConfirmId(0)
            if cb then cb() end
        end
    )
end
...
function XSkyGardenShoppingStreetControl:SgStreetOperatingSettleRequest(cb)
    ...
    XNetwork.Call(
        ...
        function(res)
            ...
            local resetData = {}
            local areas = self:GetShopAreas(true)
            for _, area in pairs(areas) do
                if not area:IsEmpty() then
                    local shopId = area:GetShopId()
                    local funcType = XMVCA.XSkyGardenShoppingStreet:GetFuncTypeByShopId(shopId)
                    if funcType then
                        local shopData = area:GetData()
                        local shopResetData = XMVCA.XSkyGardenShoppingStreet:GetFuncDataByShopData(shopData)
                        if shopResetData then
                            resetData[funcType] = shopResetData
                        end
                    end
                end
            end
            -- 存储商店设置缓存
            self:SetResetData(resetData)
            self:RemoveAllFeedbackTips()
            if cb then cb() end
        end
    )
end
        ...
        function(res)
            ...
            if not isInside then
                ...
                if self._Model:GetFirstOutsideBuildTurn() == 0 then
                    self._Model:SetFirstOutsideBuildTurn(self._Model:GetRunRound())
                end
                if self:HasPromotion() then
...
function XSkyGardenShoppingStreetControl:SgStreetShopRemoveRequest(ShopId, cb)
    XNetwork.Call(
        ...
        function(res)
            ...
            self:X3CSetVirtualCamera(areaId, XMVCA.XSkyGardenShoppingStreet.X3CCameraPosIndex.Middle, nil, nil, false)
            XScheduleManager.ScheduleOnce(function()
                local X3CEShopEffectType = XMVCA.XSkyGardenShoppingStreet.X3CEShopEffectType
...
function XSkyGardenShoppingStreetControl:SgStreetShopUpgradeRequest(ShopId, BranchId, cb)
    XNetwork.Call(
        ...
        function(res)
            ...
            if shopAreaData:IsLevelChange() then
                local X3CEShopEffectType = XMVCA.XSkyGardenShoppingStreet.X3CEShopEffectType
            ...
            if cb then
                cb(function()
                    XMVCA.XSkyGardenShoppingStreet:Toast(XMVCA.XBigWorldService:GetText("SG_SS_UpgradeSucess"))
                end)
            end
        end
    )
...
function XSkyGardenShoppingStreetControl:SgStreetShopSetupFoodRequest(ShopId, ChefId, GoodsCountList, GoldCount, cb)
    XNetwork.Call(
        ...
        function(res)
            ...
            self._Model:UpdateShopAreaData(res.ShopData)
            if cb then cb() end
            XMVCA.XSkyGardenShoppingStreet:Toast(XMVCA.XBigWorldService:GetText("SG_SS_SaveSucess"))
        end
    )
end
    ...
    XNetwork.Call(
        ...
        function(res)
            ...
            if shopId then
                ...
                CS.XAudioManager.PlayAudio(5700032)
            end
            if cb then cb() end
        end
    )
...
function XSkyGardenShoppingStreetControl:X3CBuildingCreate(areaId, sceneObjectBaseId, funcType, shopShowData)
    local isEnterLevel = XMVCA.XSkyGardenShoppingStreet:IsEnterLevel()
    if not isEnterLevel then return end
...
--- 设置虚拟摄像机通过id
function XSkyGardenShoppingStreetControl:X3CSetVirtualCameraByCameraIndex(cameraIndex, Index, Duration, isFoce, cb)
    local isEnterLevel = XMVCA.XSkyGardenShoppingStreet:IsEnterLevel()
    if not isEnterLevel then return end
...
function XSkyGardenShoppingStreetControl:X3CSetVirtualCamera(AreaId, Index, shopResId, shopVCamIndex, isFromBase)
    local isEnterLevel = XMVCA.XSkyGardenShoppingStreet:IsEnterLevel()
    if not isEnterLevel then return end
    if self._LastCameraPlaceId == AreaId and self._LastCameraIndex == Index and self._LastShopVCamIndex == shopVCamIndex then return end

    local IsFromBase
    if isFromBase ~= nil then
        IsFromBase = isFromBase
    else
        IsFromBase = AreaId <= self._Model:GetInsideShowMaxNum() or self._LastCameraPlaceId ~= AreaId
    end
    ...
    if self._LastCameraPlaceId == AreaId then
        ...
        needMask = true
        durationTime = 0.5--tonumber(self:GetGlobalConfigByKey("CameraSwitchTime")) or 0
    end

    self._LastCameraPlaceId = AreaId
    self._LastCameraIndex = Index
    self._LastShopVCamIndex = shopVCamIndex
    ...
    XMVCA.X3CProxy:Send(CS.X3CCommand.CMD_SHOPSTREET_CHANGE_VIRTUAL_CAMERA, {
        ...
        VirtualCameraId = 0,
        IsFromBase = IsFromBase,
        ShopResId = shopResId or 0,
        ShopVCamIndex = shopVCamIndex or 1,
    })
end
...
-- 设置商铺展示内容
function XSkyGardenShoppingStreetControl:X3CChangeShowShowGoods(areaId, funcType, ids)
    local isEnterLevel = XMVCA.XSkyGardenShoppingStreet:IsEnterLevel()
    if not isEnterLevel then return end
    local placeId = self._Model:GetPlaceIdByAreaId(areaId)
    return XMVCA.X3CProxy:Send(CS.X3CCommand.CMD_SHOPSTREET_CHANGE_SHOP_SHOW_GOODS, {
        PlaceId = placeId,
        FuncType = funcType,
        ShopShowData = {
            Ints = ids,
...
function XSkyGardenShoppingStreetControl:FoodChefIdSwitchX3cId(id)
    return self._Model:FoodChefIdSwitchX3cId(id)
end

function XSkyGardenShoppingStreetControl:FoodIdSwitchX3cId(id)
    return self._Model:FoodIdSwitchX3cId(id)
end

function XSkyGardenShoppingStreetControl:DessertIdSwitchX3cId(id)
    return self._Model:DessertIdSwitchX3cId(id)
end
...
function XSkyGardenShoppingStreetControl:GetRewardIndexRecord(stageId)
    return self._Model:GetRewardIndexRecord(stageId)
end

function XSkyGardenShoppingStreetControl:GetRewardIndexRecordAndIndex(stageId, taskIndex)
    return table.contains(self._Model:GetRewardIndexRecord(stageId), taskIndex - 1)
end
...
function XSkyGardenShoppingStreetControl:GetMaxRunRound()
    return self._Model:GetMaxRunRound()
end

function XSkyGardenShoppingStreetControl:GetFirstOutsideBuildTurn()
    return self._Model:GetFirstOutsideBuildTurn()
end
...
function XSkyGardenShoppingStreetControl:ShowTeachInfo()
    local teachId = tonumber(self:GetGlobalConfigByKey("StreetTeachId"))
    if not teachId then return end
    XMVCA.XBigWorldTeach:OpenTeachTipUi(teachId)
end

function XSkyGardenShoppingStreetControl:ShowTeachInfoStreetTeachIdInside()
    local teachId = tonumber(self:GetGlobalConfigByKey("StreetTeachIdInside"))
    if not teachId then return end
    XMVCA.XBigWorldTeach:OpenTeachTipUi(teachId)
end

function XSkyGardenShoppingStreetControl:ShowTeachInfoStreetTeachIdOuside()
    local teachId = tonumber(self:GetGlobalConfigByKey("StreetTeachIdOuside"))
    if not teachId then return end
    XMVCA.XBigWorldTeach:OpenTeachTipUi(teachId)
end

function XSkyGardenShoppingStreetControl:ShowTeachInfoStreetTeachIdLight()
    local teachId = tonumber(self:GetGlobalConfigByKey("StreetTeachIdLight"))
    if not teachId then return end
    XMVCA.XBigWorldTeach:OpenTeachTipUi(teachId)
end
...
function XSkyGardenShoppingStreetControl:GetResetData(resetType)
    local resetData = XSaveTool.GetData(self._ResetData)
    if not resetData then return end
    return resetData[resetType]
end

function XSkyGardenShoppingStreetControl:SetResetData(data)
    XSaveTool.SaveData(self._ResetData, data)
end
...
function XSkyGardenShoppingStreetControl:IsMaxTurn()
    return self._Model:IsMaxTurn()
end

function XSkyGardenShoppingStreetControl:GetAccumulativeGold()
    return self._Model:GetAccumulativeGold()
end
...
function XSkyGardenShoppingStreetControl:UpdateMusicPart(part)
    return self._Model:UpdateMusicPart(part)
end

function XSkyGardenShoppingStreetControl:GetTotalStarNum()
    return self._Model:GetTotalStarNum()
end

function XSkyGardenShoppingStreetControl:GetCurrentStarNum()
    return self._Model:GetCurrentStarNum()
end

function XSkyGardenShoppingStreetControl:InitFeedbackTipsCache()
    if self._FeedbackCache then return end
    self._FeedbackCache = XSaveTool.GetData(self._FeedbackTipsKey) or {}
end

function XSkyGardenShoppingStreetControl:HasFeedbackTips(pos)
    self:InitFeedbackTipsCache()
    local shopAreaData = self:GetShopAreaByUiPos(pos, true)
    local feedbacks = shopAreaData:GetFeedbackDatas()
    if not feedbacks or #feedbacks <= 0 then return false end

    local foundBad = false
    for _, feedBackDataTemp in ipairs(feedbacks) do
        if not self._Model:IsSgStreetFeedbackTypeGood(feedBackDataTemp.FeedbackType) then
            foundBad = true
        end
        if foundBad then break end
    end
    if not foundBad then return false end
    return not self._FeedbackCache[pos]
end

function XSkyGardenShoppingStreetControl:RemoveFeedbackTips(pos, isInside)
    self:InitFeedbackTipsCache()
    self._FeedbackCache[pos] = true
    XSaveTool.SaveData(self._FeedbackTipsKey, self._FeedbackCache)
end

function XSkyGardenShoppingStreetControl:RemoveAllFeedbackTips()
    self._FeedbackCache = {}
    XSaveTool.SaveData(self._FeedbackTipsKey, self._FeedbackCache)
end

function XSkyGardenShoppingStreetControl:GetDefaultShopShowData(shopConfig)
    return self._Model:GetDefaultShopShowData(shopConfig)
end
...
function XSkyGardenShoppingStreetControl.AttrSort(a, b)
...
function XSkyGardenShoppingStreetControl:GetShopAttributes(shopId, lv, isIniside, isConfig)
    ...
    if not isConfig and shopArea and shopArea:HasShop() then
        ...
        table.sort(attrData, XSkyGardenShoppingStreetControl.AttrSort)
        return attrData
    else
        local attrData = self:GetAttributes(self:GetShopLevelConfigById(shopId, lv or 1, isIniside), shopId)
        table.sort(attrData, XSkyGardenShoppingStreetControl.AttrSort)
        return attrData
    end
end
...
function XSkyGardenShoppingStreetControl:_InitParseData()
    ...
    self._FeedbackTypeParseFunc = {
        ...
        [7] = function(parseData, shopId, desc, key)
            local effectDesc
            if parseData.Difference > 0 then
                effectDesc = XMVCA.XBigWorldService:GetText("SG_SS_GoodPosDown")
            else
                effectDesc = XMVCA.XBigWorldService:GetText("SG_SS_GoodPosUp")
            end
            return string.gsub(desc, key, effectDesc)
        end,
    }
...
function XSkyGardenShoppingStreetControl:ParseFeedback(feedbackData, shopId)
    ...
    local descIndex = feedbackData.DescIndex or 0
    local desc = feedbackConfig.Desc[descIndex + 1]
...
function XSkyGardenShoppingStreetControl:_EnterGame(stageId, isOpenTarget)
    self:SgStreetStageStartRequest(stageId, function ()
        if isOpenTarget then
            XMVCA.XBigWorldUI:Open("UiSkyGardenShoppingStreetVideoRecording")
        end
        -- 进入关卡场景
...
function XSkyGardenShoppingStreetControl:UpgradeShop(uiPos, isInside, upgradeIndex, cb)
    ...
    if not upgradeIndex or not branchIds or upgradeIndex > #branchIds then return end
...
function XSkyGardenShoppingStreetControl:GetRunningFeedbackList()
    return self._Model:GetRunningFeedbackList()
end
...
-- 突发事件启用特殊奖励buff
function XSkyGardenShoppingStreetControl:HasEmergencyRewardSpOptionBuff()
    return self._Model:HasEmergencyRewardSpOptionBuff()
end

-- 获取商店上次累计到店人数
function XSkyGardenShoppingStreetControl:GetLastTotalCustomerNumByShopId(shopId)
    return self._Model:GetLastTotalCustomerNumByShopId(shopId)
end
...
-- 获取当前显示任务id
function XSkyGardenShoppingStreetControl:GetDisplayTaskConfigId()
    return self._Model:GetDisplayTaskConfigId()
end

function XSkyGardenShoppingStreetControl:SetUiSelectShopId(shopId)
    self._UiSelectShopId = shopId
end

function XSkyGardenShoppingStreetControl:GetUiSelectShopId()
    return self._UiSelectShopId
end

local TableKey = {
    Stage = { DirPath = XConfigUtil.DirectoryType.Share, ReadFunc = XConfigUtil.ReadType.Normal, CacheType = XConfigUtil.CacheType.Normal, Identifier = "StageId" },
    ...
    StageLineupType = { DirPath = XConfigCenter.DirectoryType.Share, CacheType = XConfigUtil.CacheType.Normal, Identifier = "StageId" },
}
...
function XFubenModel:OnInit()
    ...
    ---@class StageInfo
    ---@field Type number
    ---@field HaveAssist boolean
    ---@field IsMultiplayer boolean
    ---@field Passed boolean
    ---@field Stars number
    ---@field StarsMap table
    ---@field Unlock boolean
    ---@field IsOpen boolean
    ---@field TotalStars number
    ---@field ChapterId number  这个以下需要整改，不要使用
    ---@field OrderId number
    ---@field Difficult number
    ---@field mode number
    ---@type table<number, StageInfo>
...
function XFubenModel:GetStageInfo(stageId)
    if self._StageInfo[stageId] then
        return self._StageInfo[stageId]
    end

    local cfg = self:GetStageCfg(stageId, true)
    if not cfg then
        return nil
    end
...
function XFubenModel:GetStageCfg(stageId, ignoreError)
    ignoreError = (ignoreError) or (CS.XFight.Instance and CS.XFight.Instance.IsFubenDebug)  --黑幕进战斗不配置Stage表，忽略报错
...
function XFubenModel:GetStageCharacterLimitType(stageId)
    if not stageId then
        return XEnumConst.FuBen.CharacterLimitType.All
    end
...
function XFubenModel:IsStageRecommendCharacterType(stageId, id)
    ...
    local element = XMVCA.XCharacter:GetCharacterElements(characterId)
    ...
    return XTool.IsNumberValid(recommendType) and
            ...
            (table.contains(element, recommendElement) or recommendElement == 0)
end
...
function XFubenModel:GetStageIdByBeginStoryId(storyId)
    return self._BeginStory2StageIdDic[storyId]
end
---
--- 获得战前剧情ID
function XFubenModel:GetBeginStoryId(stageId)
    ...
    if XMain.IsEditorDebug then
        -- 检查无配置storyId却配置了condition
        for i, condition in pairs(config.BeginConditions) do
            if not config.BeginStoryIds[i] then
                local tips = string.format("请策划老师检查Stage.tab，StageId：%s，BeginStoryIds[%s]为空，BeginConditions[%s]为%s", stageId, i, i, condition)
                XLog.Error(tips)
            end
        end
        ...
        -- 检查多个剧情可同时播放
        local storyIds = {}
        for i, storyId in ipairs(config.BeginStoryIds) do
            local condition = config.BeginConditions[i]
        ...
        if #storyIds > 1 then
            local tips = string.format("请策划老师检查Stage.tab，StageId：%s，BeginStoryIds当前有多个剧情可播放：%s", stageId, XLog.Dump(storyIds))
            XLog.Error(tips)
        end
    end
    ...
    for i, storyId in ipairs(config.BeginStoryIds) do
        local condition = config.BeginConditions[i]
        if not condition then
            self._BeginStory2StageIdDic[storyId] = stageId
            return storyId
        else
            local isOpen, desc = XConditionManager.CheckCondition(condition)
            if isOpen then
...
function XFubenModel:GetEndStoryId(stageId)
    ...
    if XMain.IsEditorDebug then
        -- 检查无配置storyId却配置了condition
        for i, condition in pairs(config.EndConditions) do
            if not config.EndStoryIds[i] then
                local tips = string.format("请策划老师检查Stage.tab，StageId：%s，EndStoryIds[%s]为空，EndConditions[%s]为%s", stageId, i, i, condition)
                XLog.Error(tips)
            end
        end
        ...
        -- 检查多个剧情可同时播放
        local storyIds = {}
        for i, storyId in ipairs(config.EndStoryIds) do
            local condition = config.EndConditions[i]
            if not condition then
                table.insert(storyIds, storyId)
            else
                local isOpen, desc = XConditionManager.CheckCondition(condition)
        ...
        if #storyIds > 1 then
            local tips = string.format("请策划老师检查Stage.tab，StageId：%s，EndStoryIds当前有多个剧情可播放：%s", stageId, XLog.Dump(storyIds))
            XLog.Error(tips)
        end
    end
...
function XFubenModel:GetChallengeGridShowConditionList()
    local result = {}
    for i, value in ipairs(self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.FubenClientConfig, "ChallengeGridShowCondition").Values) do
        result[#result + 1] = tonumber(value)
    end
    return result
end
...
function XFubenModel:GetConfigStageLineupType(stageId)
    local config = self._ConfigUtil:GetCfgByTableKeyAndIdKey(TableKey.StageLineupType, stageId, true)

    return not XTool.IsTableEmpty(config), config
end
----------config end----------
...
function XFubenModel:SetFightCgEnable(isEnable)
    self._IsFightCgEnable = isEnable
    XEventManager.DispatchEvent(XEventId.EVENT_FIGHT_ANIM_ENABLE)
end

function XFubenModel:IsFightCgEnable()
    return self._IsFightCgEnable
end

--endregion

function XFubenModel:IsPassed(stageId)
    local dataFromServer = self:GetPlayerStageDataById(stageId)
...
function XFubenModel:GetStageType(stageId)
    local stageCfg = self:GetStageCfg(stageId, true)
    if stageCfg then
        return stageCfg.Type
    end
end

function XFubenModel:GetFubenActivityConfigs()
    local activityConfigs = self._ConfigUtil:GetByTableKey(TableKey.FubenActivity)
    return activityConfigs
end

function XFubenModel:GetFubenTabConfig()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FubenTabConfig)
    return configs
end

function XFubenModel:GetFubenSecondTag()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FubenSecondTag)
    return configs
end

function XFubenModel:GetFubenStoryLine()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FubenStoryLine)
    return configs
end

function XFubenModel:GetFubenActivityTimeTips()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FubenActivityTimeTips)
    return configs
end

function XFubenModel:GetFubenCollegeBanner()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FubenCollegeBanner)
    return configs
end

function XFubenModel:GetFubenClientConfig()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FubenClientConfig)
    return configs
end

function XFubenModel:GetStageVoiceTip()
    local configs = self._ConfigUtil:GetByTableKey(TableKey.StageVoiceTip)
    return configs
end

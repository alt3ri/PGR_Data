...
function XFubenAgency:OnRelease()
    -- for hotReload
    XEventManager.RemoveEventListener(XEventId.EVENT_PLAYER_LEVEL_CHANGE, self.InitData, self)
    self._IsListen = false
end
...
function XFubenAgency:InitRpc()
    ...
    XRpc.NotifyIsSetFightCgEnable = handler(self, self.NotifyIsSetFightCgEnable)
end

function XFubenAgency:InitEvent()
    --角色升级
    if self._IsListen then
        XLog.Error("[XFubenAgency] 重复监听XFubenAgency")
    else
...
function XFubenAgency:InitData(checkNewUnlock)
    ...
    -- 发送关卡刷新事件
    XEventManager.DispatchEvent(XEventId.EVENT_FUBEN_REFRESH_STAGE_DATA, checkNewUnlock)
end
...
function XFubenAgency:EnterFight(stage, teamId, isAssist, challengeCount, challengeId, callback, isSkipMovie)
    ...
    if XDataCenter.RoomManager.RoomData then
        ...
        if stage.StageId == 10010101 and stageInfo and not stageInfo.Passed then
            XLuaUiManager.Open("UiFightButtonSettings", enter)
        else
            enter()
        end
    end
...
function XFubenAgency:GetStageType(stageId)
    ...
    if config and XTool.IsNumberValid(config.Type) then
        --增加多一列, 优先读取配置的
        --if XMain.IsEditorDebug then
        --    local stageInfo = self._Model:GetStageInfo(stageId)
        --    if stageInfo then
        --        if stageInfo.Type ~= config.Type then
        --            XLog.Error("[XFubenAgency] stage配置表的type与程序赋值不一致，请修改配置表:", tostring(stageId))
        --        end
        --    end
        --end
        return config.Type
...
function XFubenAgency:CheckStageIsUnlock(stageId)
    local stageCfg = self._Model:GetStageCfg(stageId, false)
    if not stageCfg then
        return false
    end

    local stageType = stageCfg.Type
    local ok, result = XMVCA.XFuben:CallCustomFunc(stageType, ProcessFunc.CheckUnlockByStageId, stageId)
    if ok then
        if result ~= nil then
            return result
        end
    ...
    local preStage = stageCfg.PreStageId
    if preStage then
        for _, preStageId in pairs(preStage) do
            if preStageId > 0 then
                local playerStageData = self:GetStageData(preStageId)
                if not playerStageData or not playerStageData.Passed then
                    return false
                end
            end
        end
...
function XFubenAgency:GetFubenTitle(stageId)
    ...
    elseif stageInfo and stageType == StageType.ShortStory then
        ...
        local stageTitle = XFubenShortStoryChapterConfigs.GetStageTitleByChapterId(chapterId)
        if difficult == self._Model:GetDifficultNormal() then
            diffMsg = CSTextManagerGetText("FubenDifficultyNormal", stageTitle, stageCfg.OrderId)
        elseif difficult == self._Model:GetDifficultHard() then
    ...
    elseif stageInfo and stageType == StageType.Bfrt then
        ...
        local orderId
        if chapterCfg then
            orderId = chapterCfg.OrderId
        else
            --XLog.Error("[XFubenAgency] 缺少BfrtChapter.tab" .. stageId)
            orderId = ""
        end
        res = CSTextManagerGetText("FubenDifficultyNightmare", orderId, stageCfg.OrderId)
    else
        res = stageCfg.Name
    end
    return res
...
function XFubenAgency:GetFubenNames(stageId)
    ...
    elseif curStageType == StageType.Daily then
        ...
        chapterName = XDataCenter.FubenDailyManager.GetChapterName(stageId)
        stageName = stage.Name
    elseif curStageType == StageType.BossSingle then
        local bossSectionId = XMVCA.XFubenBossSingle:GetBossSectionId(stageId)
        chapterName, stageName = XMVCA.XFubenBossSingle:GetBossNameInfo(bossSectionId, stageId)

    elseif curStageType == StageType.Arena then
        chapterName, stageName = XMVCA.XArena:GetEnterAreaStageNameInfo()
    ...
    elseif curStageType == StageType.ShortStory then
        ...
        chapterName = XFubenShortStoryChapterConfigs.GetChapterNameById(chapterMainId)
        stageName = tmpStage.Name
...
function XFubenAgency:GetRequireActionPoint(stageId)
    ...
    if actionPoint == 0 then
        local stageInfo = self._Model:GetStageInfo(stageId)
        local isPassed = self._Model:IsPassed(stageId)
        actionPoint = not isPassed and stageCfg.FirstRequireActionPoint or stageCfg.FinishRequireActionPoint
    end
...
function XFubenAgency:CheckPreFight(stage, challengeCount, autoFight)
    -- 当自动作战时，无需检测自定义按键冲突
    if not autoFight and self:CheckCustomUiConflict() then
        return
    end
    challengeCount = challengeCount or 1
...
function XFubenAgency:DoEnterFight(stage, teamId, isAssist, challengeCount, challengeId, callback, isSkipMovie)
    ...
    if ok then
        ...
        --追加效应Id
        local teamData = XDataCenter.TeamManager.GetXTeam(teamId) or XDataCenter.TeamManager.GetTempTeam(teamId)
        if teamData and teamData.GetCurGeneralSkill then
            preFight.GeneralSkill = teamData:GetCurGeneralSkill()
        end
        --追加入场结算动画角色自选
        if teamData then
            if teamData.GetEnterCgIndex then
                preFight.EnterCgIndex = teamData:GetEnterCgIndex()
            end
            if teamData.GetSettleCgIndex then
                preFight.SettleCgIndex = teamData:GetSettleCgIndex()
            end
        end
    else
    ...
    XDataCenter.TeamManager.ClearTempTeam()
end
...
function XFubenAgency:CtorFightArgs(stageId, roleData)
    ...
    args.FinishCb = function()
        local func = self:GetTempCustomFunc(stageType, ProcessFunc.CallFinishFight) or self.CallFinishFightHandler
        func()
        XEventManager.DispatchEvent(XEventId.EVENT_FIGHT_EXIT_FINAL)
    end
    ...
    local ok, result = self:CallCustomFunc(stageType, ProcessFunc.CheckAutoExitFight, stageId)
    if ok then
...
function XFubenAgency:ChallengeWin(settleData)
    ...
    local endStoryId = self._Model:GetEndStoryId(settleData.StageId)
    local isKeepPlayingStory = stage and self:IsKeepPlayingStory(stage.StageId)
...
function XFubenAgency:RefreshStageInfo(stageList)
    ...
    for _, v in pairs(stageList) do
        ...
        if stageConfig then
            ...
            for i = 1, #nextStageIds do
                local nextStageId = nextStageIds[i]
                if nextStageId and nextStageId > 0 then
                    local nextStageType = self._Model:GetStageType(nextStageId)
    ...
    -- 发送关卡刷新事件
    XEventManager.DispatchEvent(XEventId.EVENT_FUBEN_REFRESH_STAGE_DATA, true)
end
...
function XFubenAgency:EnterColorTableFight(xTeam, stageId)
    ...
    self:NetWorkPreFightRequest(req, function(res)
        ...
        local fightData = res.FightData
        local stageInfo = self._Model:GetStageInfo(fightData.StageId)
        local beginStoryId = self._Model:GetBeginStoryId(fightData.StageId)
        local stage = self:GetStageCfg(stageId)
        if stage and beginStoryId and (not stageInfo or not stageInfo.Passed) then
            -- 播放剧情，进入战斗
            self:EnterRealFight(preFight, fightData, beginStoryId)
        else
            -- 直接进入战斗
...
function XFubenAgency:EnterSkillTeachFight(characterId)
    ...
    local endStoryId = self._Model:GetEndStoryId(stageId)
...
--            local preStageInfo = self._Model:GetStageInfo(preStageId)
...
function XFubenAgency:EnterAwarenessFight(stageId, charIdList, captainPos, startCb, errorCb, firstFightPos, generalSkillId)
    ...
    self:NetWorkPreFightRequest(req, function(res)
        ...
        if stage and beginStoryId and (not stageInfo or not stageInfo.Passed) then
            -- 播放剧情，进入战斗
            self:EnterRealFight(preFight, fightData, beginStoryId, startCb)
        else
            -- 直接进入战斗
...
function XFubenAgency:EnterPracticeBoss(stage, curTeam, simulateTrainInfo)
    ...
    preFight.GeneralSkill = curTeam.SelectedGeneralSkill
    preFight.EnterCgIndex = curTeam.EnterCgIndex or 0
    preFight.SettleCgIndex = curTeam.SettleCgIndex or 0
    for i, charId in pairs(curTeam.TeamData or {}) do
        local isRobot = XEntityHelper.GetIsRobot(charId)
        preFight.RobotIds[i] = isRobot and charId or 0
        preFight.CardIds[i] = isRobot and 0 or charId
    end
    self:NetWorkPreFightRequest({ PreFightData = preFight }, function(res)
        if res.Code ~= XCode.Success then
            XUiManager.TipCode(res.Code)
...
-- 据点战斗
function XFubenAgency:EnterBfrtFight(stageId, team, captainPos, firstFightPos, generalSkillId, enterCgIndex, settleCgIndex)
    local stage = self:GetStageCfg(stageId)
    if not self:CheckPreFight(stage) then
        return
    end
...
function XFubenAgency:DoBattlePrepare(stageId, ...)
    ---@type XTableStage
    local stageCfg = self:GetStageCfg(stageId)
    ...
    if not XTool.IsTableEmpty(stageCfg.RobotId) then
        for i, robotId in pairs(stageCfg.RobotId) do
            ...
            if XTool.IsNumberValid(rebuildNpcId) then
                self:EnterFightByStageId(stageId)
                return
            end
        end
    end
...
function XFubenAgency:EnterMoeWarFight(stage, curTeam, isNewTeam)
    ...
    self:NetWorkPreFightRequest({ PreFightData = preFight }, function(res)
        ...
        local stageInfo = self._Model:GetStageInfo(fightData.StageId)
        local beginStoryId = self._Model:GetBeginStoryId(fightData.StageId)
        local isKeepPlayingStory = stage and self:IsKeepPlayingStory(stage.StageId) and beginStoryId
...
function XFubenAgency:EnterStageWithRobot(stage, curTeam)
    ...
    preFight.FirstFightPos = curTeam.FirstFightPos
    preFight.RobotIds = {}
    for _, v in pairs(curTeam.EntitiyIds or {}) do
...
function XFubenAgency:EnterChessPursuitFight(stage, preFight, callBack)
    self:NetWorkPreFightRequest({ PreFightData = preFight }, function(res)
        ...
        if isKeepPlayingStory or isNotPass then
            -- 播放剧情，进入战斗
            self:EnterRealFight(preFight, fightData, beginStoryId, callBack)
        else
            -- 直接进入战斗
...
function XFubenAgency:EnterBabelTowerFight(stageId, team, captainPos, firstFightPos, generalSkill)
    ...
    self:NetWorkPreFightRequest(rep, function(res)
        if res.Code ~= XCode.Success then
            XUiManager.TipCode(res.Code)
            return
        end
...
-- 边界公约战斗
function XFubenAgency:EnterAssignFight(stageId, charIdList, captainPos, startCb, errorCb, firstFightPos, generalSkillId)
    local stage = self:GetStageCfg(stageId)
    local preFight = {}
    preFight.CardIds = {}
    preFight.StageId = stage.StageId
    ...
    self:NetWorkPreFightRequest(req, function(res)
        ...
        -- end
        local beginStoryId = self._Model:GetBeginStoryId(fightData.StageId)
        if stage and beginStoryId and not self._Model:IsPassed(fightData.StageId) then
            -- 播放剧情，进入战斗
            self:EnterRealFight(preFight, fightData, beginStoryId, startCb)
        else
            -- 直接进入战斗
...
function XFubenAgency:InitOutdateManager()
    ...
    --self:RegisterFubenManager(self.StageType.DoubleTowers, XDataCenter.DoubleTowersManager)s
    ...
    require("XModule/XFuben/XFubenOptimize")
...
function XFubenAgency:CollectAllStageType()
    if XMain.IsWindowsEditor then
        ...
        local stageCount = 0
        local unUseCount = 0
        for id, stageInfo in pairs(self._Model:DebugGetStageInfos()) do
...
function XFubenAgency:GetStageMaxChallengeCount(stageId)
    local stageExCfg = self:GetMultiChallengeStageConfig(stageId)
...
function XFubenAgency:DebugGetStageInfos()
    return self._Model:DebugGetStageInfos()
end
...
function XFubenAgency:EnterRiftFight(xTeam, stageGroupData, index)
    local stageId = XMVCA.XRift:GetStageIdByStageGroup(stageGroupData, index)
    local stage = self:GetStageCfg(stageId)
    if not self:CheckPreFight(stage) then
        return
    end

    local chapterConfig = XMVCA.XRift:GetLayerConfigById(stageGroupData.FightLayerId)
    ...
    preFight.CardIds = characterIds
    preFight.RobotIds = robotIds
    preFight.StageId = isLucky and XMVCA.XRift:GetLuckStageId() or stage.StageId
    preFight.CaptainPos = captainPos
    preFight.FirstFightPos = firstFightPos
    preFight.GeneralSkill = xTeam:GetCurGeneralSkill()
    preFight.EnterCgIndex = xTeam:GetEnterCgIndex()
    preFight.SettleCgIndex = xTeam:GetSettleCgIndex()
    preFight.RiftInfo = {
        ChapterId = isLucky and XMVCA.XRift:GetLuckyNodeChapterId() or chapterConfig.ChapterId,
        LayerId = isLucky and XMVCA.XRift:GetLuckyNodeLayerId() or stageGroupData.FightLayerId,
        -- 节点对应位置, -1表示幸运节点 0表示普通节点
        NodeIdx = isLucky and -1 or 0,
        StageIdx = index,
    }
...
function XFubenAgency:GetActivityChaptersBySort()
    ...
    , XMVCA.XFashionStory:GetActivityChapters()-- 系列涂装剧情活动
...
---是否处于某个类型的副本里面
function XFubenAgency:CheckInStage(stageType)
    if CS.XFight.IsRunning then
        local data = self:GetFightBeginData()
        if data and XTool.IsNumberValid(data.StageId) then
            return self:GetStageType(data.StageId) == stageType
        end
    end
    return false
end
...
---战斗入场结算动画设置埋点
function XFubenAgency:RecordAnimationSet(enterRoleId, settleRoleId)
    local dir = {}
    dir["enter_role_id"] = enterRoleId
    dir["settle_role_id"] = settleRoleId
    CS.XRecord.Record(dir, "900009", "AnimationSetRecord")
end

--region 入场结算角色自选

function XFubenAgency:NotifyIsSetFightCgEnable(data)
    self:SetFightCgEnable(data.IsSetFightCgEnable)
end

function XFubenAgency:SetFightCgEnable(bo)
    self._Model:SetFightCgEnable(bo)
end

function XFubenAgency:IsFightCgEnable()
    return self._Model:IsFightCgEnable()
end

--endregion

function XFubenAgency:Log()
...
function XFubenAgency:NetWorkPreFightRequest(request, ...)
    ...
    local styId = self._Model:GetBeginStoryId(stageId)
    if styId then

...
function XKotodamaActivityAgency:ExOpenMainUi()
    if not XFunctionManager.DetectionFunction(XFunctionManager.FunctionName.Kotodama) then
        return
    end

    -- 判断有没有有效的活动数据
    if XTool.IsNumberValid(self._Model:GetCurActivityId()) then
...
function XKotodamaActivityAgency:ExGetProgressTip()
    ...
    return XUiHelper.FormatText(self._Model:GetClientConfigStringByKey('Process'), passCount, totalCount)
end
...
function XKotodamaActivityAgency:CheckStageIsUnLockById(stageId)
    ...
    if XTool.IsNumberValid(curStageCfg.BranchPreStageId) then
        preIsPass = self:CheckStageIsPassById(curStageCfg.BranchPreStageId)
    end
    
    
    --如果是分支关卡，则还需要判断分支解锁条件
    local branchUnlock = true
    if XTool.IsNumberValid(curStageCfg.IsBranch) then
        --检查前置关卡通关记录中是否有使用需要的句子
        if XTool.IsNumberValid(curStageCfg.BranchPreStageId) and not XTool.IsTableEmpty(curStageCfg.BranchUnLockSentenceIds) then
            local data = self._Model:GetPassStageDataById(curStageCfg.BranchPreStageId)
            if data and not XTool.IsTableEmpty(data.Sentences) then
                branchUnlock = false
                for i, v in ipairs(data.Sentences) do
                    if table.contains(curStageCfg.BranchUnLockSentenceIds, v) then
                        branchUnlock = true
                        break
                    end
                end
    ...
    if not XTool.IsTableEmpty(curStageCfg) then
        ...
        return inTime and preIsPass and branchUnlock, inTime, preIsPass, branchUnlock
    end
end
...
--region 协议请求相关
function XKotodamaActivityAgency:KotodamaSpellSentenceRequest(cb, stageId, sentenceSpell, deleteSentence)
    XNetwork.Call('KotodamaSpellSentenceRequest', { StageId = stageId, SpellSentences = sentenceSpell, DeleteSentence = deleteSentence }, function(result)
...
function XKotodamaActivityAgency:GetKotodamaSpeechKey()
    ...
    return 'KotodamaActivity_Speech_' ..tostring(activityId)..'_'.. XPlayer.Id
end

function XKotodamaActivityAgency:GetKotodamaStageKey()
    local activityId = self:GetCurActivityId()
    return 'KotodamaActivity_Stage_' ..tostring(activityId)..'_'.. XPlayer.Id
end

function XKotodamaActivityAgency:GetKotodamaNotFirstIntoKey()
    local activityId = self:GetCurActivityId()
    return 'KotodamaActivity_NotFirstInto_' ..tostring(activityId)..'_'.. XPlayer.Id
end

function XKotodamaActivityAgency:GetKotodamaNotFirstAllPassKey()
    local activityId = self:GetCurActivityId()
    return 'KotodamaActivity_NotFirstAllPass_' ..tostring(activityId)..'_'.. XPlayer.Id
end
----------private end----------

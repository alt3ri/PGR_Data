...
function XBWMessageData:Ctor(data)
    ...
    self:UpdateStepRecords(data.StepRecordList)
end
...
function XBWMessageData:UpdateStepRecords(stepRecords)
    self.StepIds = {}
    self.StepIdMap = {}

    if not XTool.IsTableEmpty(stepRecords) then
        for _, record in pairs(stepRecords) do
            self:AddStepId(record.LatestStepId)
        end
    end
end

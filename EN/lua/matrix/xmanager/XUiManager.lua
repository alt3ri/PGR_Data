...
function XUiManager.DialogTip(title, content, dialogType, closeCallback, sureCallback, extraData, cancelCallback, isCallbackFirst)
    ...
    XLuaUiManager.Open("UiDialog", title, content, dialogType, closeCallback, sureCallback, extraData, cancelCallback, isCallbackFirst)
end
...
local ControllerTipsList = {}
local ControllerTipsUiName = "UiBigWorldSetControllerTips"
XUiManager.DefaultControlTipsName = "Default"

function XUiManager.AddControllerTips(openUiName)
    if UnityApplication.platform == UnityRuntimePlatform.Android or UnityApplication.platform == UnityRuntimePlatform.IPhonePlayer then return end
    if openUiName == ControllerTipsUiName then return end

    local uiData = CS.XUiManager.Instance:FindUiData(openUiName)
    if uiData and not string.IsNilOrEmpty(uiData.ParentUiName) then return end

    local tipsMap = XSetConfigs.GetControllerTipsMapCfg()
    local config = tipsMap[openUiName] or tipsMap[XUiManager.DefaultControlTipsName]
    if not config.IsShow then return end

    if not XLuaUiManager.IsUiLoad(ControllerTipsUiName) then
        XLuaUiManager.Open(ControllerTipsUiName)
    end
...
function XUiManager.GetShowUIControllerTipsName()
    return ControllerTipsList[#ControllerTipsList]
end

function XUiManager._ShowUiControllerTips()
    XEventManager.DispatchEvent(XEventId.EVENT_CONTROLLER_TIPS_CHNAGE)
end

function XUiManager.RemoveControllerTips(openUiName)
    for i = #ControllerTipsList, 1, -1 do
        if ControllerTipsList[i] == openUiName then
            table.remove(ControllerTipsList, i)
        end
    end
...
function XUiManager.GetOrCreateCanvas(go, isAddGraphicRaycaster)
    local canvasType = typeof(CS.UnityEngine.Canvas)
    local canvas = go:GetComponent(canvasType)
    if XTool.UObjIsNil(canvas) then
        canvas = go:AddComponent(canvasType)
    end
    if isAddGraphicRaycaster then
        local graphicRaycasterType = typeof(CS.UnityEngine.UI.GraphicRaycaster)
        local graphicRaycaster = go:GetComponent(graphicRaycasterType)
        if XTool.UObjIsNil(graphicRaycaster) then
...
function XUiManager.MoveToTop(nodeGo, clickBtn, cb, upOrder)
    local topUiName = XLuaUiManager.GetTopUiName()
    local ui = XUiManager.GetTopUi(topUiName)
    if not ui then return end

    local uiOrder = ui.Ui.SortingOrder
    local upBaseOrder = upOrder or 20
    local upShowOrder = upBaseOrder + 10
    local nodeCanvas = XUiManager.GetOrCreateCanvas(nodeGo, true)
    nodeCanvas.overrideSorting = true
    nodeCanvas.sortingOrder = uiOrder + upShowOrder

    if not clickBtn then return end
    local btnCanvas = XUiManager.GetOrCreateCanvas(clickBtn.gameObject, true)
    btnCanvas.overrideSorting = true
    btnCanvas.sortingOrder = uiOrder + upBaseOrder

    clickBtn.CallBack = function()
        nodeCanvas.overrideSorting = false
        btnCanvas.overrideSorting = false
        if cb then cb() end
    end
end

function XUiManager.MoveToTopAuto(nodeGo, clickBtn, cb, upOrder)
    nodeGo.gameObject:SetActive(true)
    clickBtn.gameObject:SetActive(true)
    XUiManager.MoveToTop(nodeGo, clickBtn, function()
        if cb then cb() end
        nodeGo.gameObject:SetActive(false)

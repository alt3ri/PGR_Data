...
-- path为必传字符串
function XPlatformShareManager.ShareImage(shareId, callback, path, text, showUi, needEnableCheck)
    if string.IsNilOrEmpty(path) then
        XLog.Error("XPlatformShareManager.ShareImage函数错误，image路径不能为空")
        return
    end

    if needEnableCheck then
        if not XHeroSdkManager.SharePlatformIsEnable(shareId) then
...
function XPlatformShareManager.ShareLink(shareId, callback, shareLink, shareLinkTitle, shareLinkDesc, imageLink, showUi, needEnableCheck)
...
-- text为必传字段
function XPlatformShareManager.ShareText(shareId, callback, text, showUi, needEnableCheck)

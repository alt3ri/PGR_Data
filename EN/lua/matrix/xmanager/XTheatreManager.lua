...
XTheatreManagerCreator = function()
    ...
    function XTheatreManager.CallFinishFight()
        ...
        if not res then
            ...
            -- 失败了，更新前的可挑战次数为1，说明结束后挑战次数为0，移除多队伍界面以保证结算逻辑的触发
            if playableCount <= 1 then
    ...
    function XTheatreManager:ExOpenMainUi()
        ...
        if not exCfg or not XTool.IsNumberValid(exCfg.SkipId) then
            if XMain.IsEditorDebug then
                XLog.Error('肉鸽1入口配置SkipId为空，不通过通用跳转进入')
            end

            self:ExOnSkip()
        else
            XExFubenSimulationChallengeManager.ExOpenMainUi(self)
        end
    end
    
    function XTheatreManager.ExOnSkip()
        ...
        return XTheatreManager.CheckAutoPlayStory()
    end

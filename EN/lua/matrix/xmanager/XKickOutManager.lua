XKickOutManagerCreator = function()
    ...
    function XKickOutManager.OnNotifyVersion(data)
        ...
        local isKickOut = data.KickOut
        if version == nil or version == "" then
            return
        end
        if isKickOut == nil or isKickOut == "" then
            return
        end
        XKickOutManager.UpdateVersion(version, isKickOut)
        XKickOutManager.CheckKickOut()
    end
    
    function XKickOutManager.OnReconnect()
        if XFightUtil.IsFighting() then
            return
        end
        XKickOutManager.RequestVersion()
    end
    ...
    function XKickOutManager.Init()
        ...
        if CS.XNetwork.RegisterMaskExclude then
            CS.XNetwork.RegisterMaskExclude("ClientVersionRequest")
        end
        XEventManager.AddEventListener(XEventId.EVENT_NETWORK_RECONNECT, XKickOutManager.OnReconnect)
    end

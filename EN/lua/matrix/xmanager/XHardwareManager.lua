...
function XHardwareManager.CheckGpuMaleoon(tokens)
    local seriesNum = tokens[2]
    -- 暂无其他数据 华为第一款自研GPU 可以直接判定为高
    return XRenderQuality.Highest
end
...
function XHardwareManager.CheckGpuAndroid(gpuName)
    ...
    elseif token1 == "maleoon" then
        return XHardwareManager.CheckGpuMaleoon(tokens)
    end

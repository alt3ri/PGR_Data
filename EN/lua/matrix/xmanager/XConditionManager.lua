...
PlayerCondition = {
    ...
    [15112] = function(condition)
        -- 查询战区是否处于作战期
        local status = condition.Params[1]
        -- 是否对结果取反
        local isReverse = condition.Params[2] == 0
        if isReverse then
            return not XMVCA.XArena:CheckInStatus(status)
        else
            return XMVCA.XArena:CheckInStatus(status)
        end
    end,
    ...
    [20113] = function(condition, skipConfig)
        -- 跳转专用检测分包表SubPackageIntercept.tab
        if not skipConfig then
            return true, condition.Desc
        end

        if not XMVCA.XSubPackage:CheckSubpackage(skipConfig.FunctionalId) then
            return false, condition.Desc
        end
    ...
    -- 判断resId是否已经下载完成
    [20114] = function(condition)
        --分包下载未开启
        if not XMVCA.XSubPackage:IsOpen() then
            return true, condition.Desc
        end
        local resId = condition.Params[1]
    ...
    -- 非 (该期LottoPrimary 玩家是否已获取其Lotto里的所有特殊奖励) = 玩家未获取其Lotto里的所有特殊奖励
    [10340] = function(condition)
        local lottoPrimaryId = condition.Params[1]
        local lottoPrimaryCfg = XLottoConfigs.GetLottoPrimaryCfgById(lottoPrimaryId)
        local lottoIds = lottoPrimaryCfg.LottoIdList
        for _, lottoId in ipairs(lottoIds) do
    ...
    --【v3.5】【肉鸽DLC】新增ConditionType用于判断当前位于的MapId
    [17434] = function(condition)
        local mapId = condition.Params[1]
        return XMVCA.XTheatre4:IsInMap(mapId), condition.Desc
    end,
...
--默认配置0通过
function XConditionManager.CheckConditionAndDefaultPass(id, ...)
    if id == 0 then
        return true, ""
    end
    return XConditionManager.CheckCondition(id, ...)    
end

...
local AccumulateEventConfig = {
    [1] = "Total_Purchase_120",
    [2] = "Total_Purchase_600",
    [3] = "Total_Purchase_2000",
    [4] = "Total_Purchase_10000",
    [5] = "Total_Purchase_18000",
    [6] = "Total_Purchase_32000",
    [7] = "Total_Purchase_50000",
    [8] = "Total_Purchase_80000",
    [9] = "Total_Purchase_120000",
    [10] = "Total_Purchase_160000",
}
...
function XAppEventManager.AppLogEvent(eventName)
    if XOverseaManager.IsOverSeaRegion() then
        CS.XHeroSdkAgent.ApplogEvent(eventName,"")
    end
end

function XAppEventManager.AppLogEventWithParameter(eventName, eventValue)
    if XOverseaManager.IsOverSeaRegion() then
        CS.XHeroSdkAgent.ApplogEvent(eventName, eventValue)
    end
end
...
function XAppEventManager.PayAppLogEvent(amount)
    local orderStr = Json.encode({["af_revenue"] = amount, ["af_currency"] = "JPY"})

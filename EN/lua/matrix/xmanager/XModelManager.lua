...
XModelManager.MODEL_UINAME = {
    ...
    UiBossInshot = "UiBossInshot",
    XUiSimulateTrainBossDetail = "UiSimulateTrainBossDetail",
    XUiSucceedBossChapter = "UiSucceedBossChapter",
}
...
local IsListenerUiDestoryEvent = true
...
function XModelManager.Init()
    ...
    DlcModelTemplates = XTableManager.ReadByStringKey(DLC_MODEL_TABLE_PATH, XTable.XTableModel, "Id")
    ...
    --XModelManager.IsHeXie = XModelHX.GetIsHX()
    CS.XModelHX.Init()
    XModelManager.IsHeXie = CS.XModelHX.GetIsHX()
    if not XModelManager.IsHeXie then
        XLog.Warning("hx closed")
    end
end
...
function XModelManager.GetUiModelPath(modelId)
    ...
    if XModelManager.IsHeXie or string.IsNilOrEmpty(config.HXModelPath) then
        return config.ModelPath
    end
    return config.HXModelPath
end

function XModelManager.GetUiModelHideNodes(modelId)
    local config = GetUiModelConfig(modelId)
    return config.HideNodeName
end

function XModelManager.GetUiModelScriptPartId(modelId)
    local config = GetUiModelConfig(modelId)
    return config.ScriptPartId
end
...
function XModelManager.GetLowModelPath(modelId)
    local config = GetModelConfig(modelId)
    if XModelManager.IsHeXie or string.IsNilOrEmpty(config.LowHXModelPath) then
        return config.LowModelPath
    end
    return config.LowHXModelPath
end
...
function XModelManager.LoadRoleModel(modelId, target, cb)
    ...
    if not XTool.IsTableEmpty(hideNodePaths) then
        for _, nodePath in pairs(hideNodePaths) do
            local node = model:FindTransform(nodePath)
...
-- 设置模型材质球
function XModelManager.LoadModelScriptPart(modelId, model)
    if XTool.UObjIsNil(model) then
        return
    end

    local scriptPartId = XModelManager.GetUiModelScriptPartId(modelId)

    if XTool.IsNumberValid(scriptPartId) then
        local display = model.gameObject:GetComponent(typeof(CS.XCharSkinDisplay))

        if not XTool.UObjIsNil(display) then
            display:Revert(scriptPartId)
            display:ToState(scriptPartId, 1)
        else
            XLog.Error("配置了材质参数但是找不到脚本 CS.XCharSkinDisplay, transDatas.ScriptPartId:" .. scriptPartId)
        end
    end
end
...
function XModelManager.DoEquipModelControl(modelId, uiName, model)
    local hideNodeName = XMVCA.XEquip:GetEquipModelShowHideNodeName(modelId, uiName)
...
--      noRotation 不需要自转
--      usage      武器用途(XEnumConst.EQUIP.WEAPON_USAGE)
--      noSound    不需要声音
function XModelManager.LoadWeaponModel(modelId, target, transformConfig, uiName, cb, param, panelDrag)
    ...
    local usage = param and param.usage or XEnumConst.EQUIP.WEAPON_USAGE.SHOW
    local name = XMVCA.XEquip:GetEquipModelName(modelId, usage)

    local loadPrefabComponent = XUiHelper.TryGetComponent(target.transform, "", "XUiLoadPrefab")
    if loadPrefabComponent and loadPrefabComponent.PrefabAssetUrl ~= name then
        if param.roleModel then
            local RenderingProxy = CS.XNPCRendingUIProxy.GetNPCRendingUIProxy(param.roleModel)
            if RenderingProxy then
                local replaceDummy = CS.UnityEngine.GameObject("WeaponReplaceDummy")
                RenderingProxy:ReplaceWeaponPart(target.name, replaceDummy)
                CS.UnityEngine.GameObject.DestroyImmediate(replaceDummy)
            end
        end
        CS.UnityEngine.GameObject.DestroyImmediate(loadPrefabComponent.PrefabGameObject)
    end
    ...
    if param then
        XModelManager.LoadDisplayWeaponEffect(modelId, model, param, uiName)
    end
    ...
    if cb then
        cb(model, target.name)
    end
end

function XModelManager.LoadDisplayWeaponEffect(modelId, model, param, uiName)
    local usage = param.usage or XEnumConst.EQUIP.WEAPON_USAGE.SHOW
    ...
    for _, config in pairs(configs) do
        ...
        local delay = config.DisplayDelayTime[usage]
        ...
        if not rootName or rootName == XCharacterUiEffectConfig.GetDefaultRootName() then
            ...
            parent = model.gameObject:FindGameObject(rootName)
            if XTool.UObjIsNil(parent) then
                parent = model.gameObject
            end
        end

        -- 由于同个节点下可能加载多个特效 所以为每个特效创建一个节点
        local filename = effectPath:match("^.+/(.+)%..+$")
        ---@type UnityEngine.GameObject
        local effectRoot = parent.gameObject:FindGameObject(filename)
        if XTool.UObjIsNil(effectRoot) then
            effectRoot = CS.UnityEngine.GameObject(filename)
            effectRoot.transform:SetParent(parent.transform, false)
            effectRoot.transform:SetLayerRecursively(parent.layer)
        end

        XModelManager.RemoveEffectTimer(uiName, filename, XEnumConst.ModelDisplayDelay.Weapon)

        -- 支持特效延迟加载
        if XTool.IsNumberValid(delay) then
            effectRoot.gameObject:SetActiveEx(false)
            local timer = XScheduleManager.ScheduleOnce(function()
                if not XTool.UObjIsNil(effectRoot) then
...
function XModelManager.PlayWeaponShowing(target, modelId, uiName, go, param)
    ...
    local animController = XMVCA.XEquip:GetEquipAnimController(modelId, usage)
    local animator
    ...
    XModelManager.ClearAllAudioInfo()
...
function XModelManager.PlayWeaponSound(modelId, go, usage)
    ...
    local animCueId = XMVCA.XEquip:GetEquipUiAnimCueId(modelId, usage)
    if animCueId and animCueId ~= 0 then
        local audioInfo = XLuaAudioManager.PlayAudioByType(XLuaAudioManager.SoundType.SFX, animCueId) -- 音效
...
function XModelManager.LoadRoleWeaponModel(roleModel, equipModelIdList, refName, cb, hideEffect, go, roleModelId, equipUsage)
    ...
    local newCb = function(model, weaponCase)
        --新角色，武器同步骨骼
        ...
        if roleModel then
            local RenderingProxy = CS.XNPCRendingUIProxy.GetNPCRendingUIProxy(roleModel)
            if RenderingProxy then
                RenderingProxy:ReplaceWeaponPart(weaponCase, model)
            end
        end
    ...
    local usage = XEnumConst.EQUIP.WEAPON_USAGE.ROLE
...
function XModelManager.LoadRoleWeaponModelByFight(roleModel, fightNpcData, refName, go, roleModelId, cb)
    ...
    local usage = XEnumConst.EQUIP.WEAPON_USAGE.ROLE
    local idList = XMVCA.XEquip:GetEquipModelIdListByFight(fightNpcData)
    local specialCaseName = XUiHelper.GetClientConfig("UiSpecialWeaponCase", XUiHelper.ClientConfigType.String)
    local specialCase = roleModel.transform:FindTransform(specialCaseName)
    local caseNode = nil
    if not XTool.UObjIsNil(specialCase) then
...
function XModelManager.ClearAllAudioInfo()
    for k, v in pairs(LuaBehaviourDict) do
        v:ClearAudioInfo()
    end
end

function XModelManager.AddEffectTimer(timer, uiName, effectName, delayType)
    if not DisplayDelayTimers then
        DisplayDelayTimers = {}
    end
    if not DisplayDelayTimers[uiName] then
        DisplayDelayTimers[uiName] = {}
    end
    if not DisplayDelayTimers[uiName][delayType] then
        DisplayDelayTimers[uiName][delayType] = {}
    end
    DisplayDelayTimers[uiName][delayType][effectName] = timer
    if IsListenerUiDestoryEvent then
        IsListenerUiDestoryEvent = false
        CS.XGameEventManager.Instance:RegisterEvent(XEventId.EVENT_UI_DESTROY, XModelManager.OnUiDestroy)
    end
end

function XModelManager.OnUiDestroy(evt, ...)
    local args = ...
    local ui = args[0]
    ...
    XModelManager.RemoveEffectTimers(uiName)
end

function XModelManager.RemoveEffectTimers(uiName)
    local datas = DisplayDelayTimers and DisplayDelayTimers[uiName]
    if not XTool.IsTableEmpty(datas) then
        for _, timers in pairs(datas) do
            if not XTool.IsTableEmpty(timers) then
...
function XModelManager.RemoveEffectTimer(uiName, effectName, delayType)
    local timer = DisplayDelayTimers and DisplayDelayTimers[uiName] and DisplayDelayTimers[uiName][delayType] and DisplayDelayTimers[uiName][delayType][effectName]
    if XTool.IsNumberValid(timer) then
        XScheduleManager.UnSchedule(timer)
        DisplayDelayTimers[uiName][delayType][effectName] = nil
    end
end
...
function XModelManager.OpenHeXie()
    XModelManager.IsHeXie = true
end

function XModelManager.CloseHeXie()
    XModelManager.IsHeXie = false
end

function XModelManager.GetHXRes(originalRes)
    -- 如果开启和谐，则直接返回原资源
    if XModelManager.IsHeXie then
        return originalRes
    end
    -- 原本在lua读此配置表，但是战斗中剧情立绘也需要用到，所以把这个表移到c#中
    --if not HXResReplace then
    --    HXResReplace = XTableManager.ReadByStringKey(HX_RES_REPLACE_PATH, XTable.XTableHXResReplace, "Res")
    --end
    --local config = HXResReplace[originalRes]
    --if config then
    --    return config.ResHX
    --end
    local resHX = CS.XModelHX.GetHXRes(originalRes)
    if resHX and resHX ~= "" then
        return resHX
    end
    return originalRes
end
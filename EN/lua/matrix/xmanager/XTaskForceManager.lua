XTaskForceManagerCreator = function()
    ...
    function XTaskForceManager.GetTaskForceConfigById(id)
        if not TaskForceConfig then
            XLog.ErrorTableDataNotFound("XTaskForceManager.GetTaskForceConfigById",
            "TaskForceConfig", "Share/TaskForce/TaskForceConfig.tab", "id", tostring(id))
            return
        end
    ...
    function XTaskForceManager.GetTaskForceSectionConfigById(id)
        if not TaskForceSectionConfig then
            XLog.ErrorTableDataNotFound("XTaskForceManager.GetTaskForceSectionConfigById",
            "TaskForceSectionConfig", "Share/TaskForce/TaskForceSection.tab", "id", tostring(id))
            return
        end
    ...
    function XTaskForceManager.GetLatelyTaskForeInfo()
        ...
        for _, v in ipairs(TaskForceInfo.TaskList) do
    ...
    function XTaskForceManager.AutoChoiceCharacter(taskId)
        ...
        if not conditions or #conditions <= 0 then
            local newCharIds = {}
            for _ = 1, requireMemberCount, 1 do
        ...
        --乱序
        charIds = XTool.RandomBreakTableOrder(charIds)
        ...
        local ret, tmpCharIds = XTaskForceManager.RandomMultContionCalculate(charIds, requireMemberCount, conditions)
    ...
    function XTaskForceManager.CompletedTaskForceTask(taskId)
        ...
        for _, var in ipairs(TaskForceInfo.TaskList) do
    ...
    function XTaskForceManager.AcceptTaskForceRewardRequest(taskId, callback)
        XNetwork.Call(XTaskForceRequest.AcceptTaskForceRewardRequest, { TaskId = taskId }, function(resp)
            ...
            if resp.DropList and #resp.DropList > 0 then
                ...
                for _, var in ipairs(resp.DropList) do
    ...
    local Condition = {
        ...
        [18106] = function() -- 派遣中拥有指定数量达到指定战斗力的构造体
            return true
        end,
    ...
    function XTaskForceManager.CheckCondition(conditionId, characterId)
        ...
        if not template then
            XLog.ErrorTableDataNotFound("XTaskForceManager.CheckCondition", "template",
            "Share/Condition/Condition.tab", "conditionId", tostring(conditionId))
            return false
        end

        local func = Condition[template.Type]
        if not func then
            local tempStr = "XConditionManager.CheckCharacterCondition错误：无法根据Share/Condition/Condition.tab 表中conditionId："
            XLog.Error(tempStr .. conditionId .. " 的type：" .. template.Type .. "找到Condition对应的函数")
            return false
        end
    ...
    function XTaskForceManager.GetConditionRequireMember(conditionId)
        local template = XConditionManager.GetConditionTemplate(conditionId)
        if not template then
            XLog.ErrorTableDataNotFound("XTaskForceManager.GetConditionRequireMember",
            "template", "Share/Condition/Condition.tab", "conditionId", tostring(conditionId))
            return 0
        end

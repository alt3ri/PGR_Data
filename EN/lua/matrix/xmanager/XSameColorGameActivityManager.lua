...
XSameColorGameActivityManagerCreator = function()
    ...
    function XSameColorGameActivityManager.RequestSwapBall(startPos, endPos, callback, failCallBack)
        -- res : { List<XSameColorGameAction> Actions }
    ...
    function XSameColorGameActivityManager.RequestUseItem(skillGroupId, skillId, useItemParam, callback)
        ...
        XNetwork.Call(METHOD_NAME.SameColorGameUseItemRequest, { GroupId = skillGroupId, ItemId = skillId, UseSkillItemParam = serverUseItemParam}, function(res)
            if res.Code ~= XCode.Success then

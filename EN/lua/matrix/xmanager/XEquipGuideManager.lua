XEquipGuideManagerCreator = function()
    ...
    function XEquipGuideManager.IsEquipGuideCharacter(characterId)
        ...
        -- v2.14 修正主界面装备目标入口显示，存在老玩家设置了未拥有角色的装备目标
        return CurrentEquipGuide:GetProperty("_Id") == characterId and XMVCA.XCharacter:IsOwnCharacter(characterId)
    end
    ...
    function XEquipGuideManager.IsSetEquipTarget()
        ...
        -- v2.14 修正主界面装备目标入口显示，存在老玩家设置了未拥有角色的装备目标
        local characterId = CurrentEquipGuide:GetProperty("_Id")
        return XMVCA.XCharacter:IsOwnCharacter(characterId)
    end
    ...
    function XEquipGuideManager.EquipGuideSetTargetRequest(targetId, putOnPosList, cb)
        local validTargetId = XTool.IsNumberValid(targetId)
        --更换目标
        if validTargetId and XEquipGuideManager.IsSetEquipTarget() and not CurrentEquipGuide:IsEquipTarget(targetId) then
    ...
    function XEquipGuideManager.EquipGuideAddOrClearPutOnPosRequest(characterId, equipIds, isAdd)
        local sites = {}
        for i, equipId in ipairs(equipIds or {}) do
            local site = XMVCA.XEquip:GetEquipSiteByEquipId(equipId)
            sites[i] = site
        end 

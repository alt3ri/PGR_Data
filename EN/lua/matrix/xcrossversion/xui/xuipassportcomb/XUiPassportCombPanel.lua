local XUiPassportPanelGrid = require("XCrossVersion/XUi/XUiPassportComb/XUiPassportCombPanelGrid")
local XUiGridCommon = require("XUi/XUiObtain/XUiGridCommon")
local XDynamicTableNormal = require("XUi/XUiCommon/XUiDynamicTable/XDynamicTableNormal")
---@field _Control XPassportCombControl
---@class XUiPassportPanelComb:XUiNode
local XUiPassportPanelComb = XClass(XUiNode, "XUiPassportPanelComb")


--通行证面板
function XUiPassportPanelComb:Ctor(ui, rootUi)
...
function XUiPassportPanelComb:InitRightGrids()
    self.RightGrids = {}
...
function XUiPassportPanelComb:InitData()
...
function XUiPassportPanelComb:InitDynamicList()
...
function XUiPassportPanelComb:AutoAddListener()
    local typeInfoIdList = self._Control:GetPassportActivityIdToTypeInfoIdList()
    for i, typeInfoId in ipairs(typeInfoIdList) do
        ...
        if self["BtnLvUpLeftGrid" .. i] then
            XUiHelper.RegisterClickEvent(
                self,
                self["BtnLvUpLeftGrid" .. i],
                function()
                    self:OnBtnUnlockLeftGridClick(typeInfoId)
                end
            )
...
function XUiPassportPanelComb:Refresh()
    self:UpdateDynamicTable()
    self:UpdateLeftGrid()
end

--遍历DynamicTable的Grid，根据最大等级的LevelId刷新
function XUiPassportPanelComb:UpdateRightGrid()
    ...
    for i, typeInfoId in ipairs(typeInfoIdList) do
        ...
        if passportInfo then
            ...
            if self["RPermit" .. i .. "Lv2"] then
                self["RPermit" .. i .. "Lv2"].gameObject:SetActiveEx(buyTimes == 2)
            end
            if self["RPermit" .. i .. "Lv3"] then
                self["RPermit" .. i .. "Lv3"].gameObject:SetActiveEx(buyTimes == 3)
            end
        end
    end
end

function XUiPassportPanelComb:UpdateLeftGrid()
    local typeInfoIdList = self._Control:GetPassportActivityIdToTypeInfoIdList()
    if not typeInfoIdList then
        return
    end
    local passportInfo
    local isUnLock
    for i, typeInfoId in ipairs(typeInfoIdList) do
        ...
        if passportInfo then
            local buyTimes = passportInfo:GetBuyTimes()
            local buyTimeMax = self._Control:GetPassportTypeInfoRepeatBuyTimes(typeInfoId)
            local reachMax = buyTimes == buyTimeMax
            if self["BtnLvUpLeftGrid" .. i] then
                self["BtnLvUpLeftGrid" .. i].gameObject:SetActiveEx(true)
                self["BtnLvUpLeftGrid" .. i]:SetDisable(reachMax)
            end
            if self["LPermit" .. i .. "Lv2"] then
                self["LPermit" .. i .. "Lv2"].gameObject:SetActiveEx(buyTimes == 2)
            end
            if self["LPermit" .. i .. "Lv3"] then
                self["LPermit" .. i .. "Lv3"].gameObject:SetActiveEx(buyTimes == 3)
            end
        end
    end
end

function XUiPassportPanelComb:UpdateDynamicTable()
...
function XUiPassportPanelComb:OnDynamicTableEvent(event, index, grid)
...
function XUiPassportPanelComb:GetDynamicIndexAndChooseRewardType()
...
function XUiPassportPanelComb:OnBtnUnlockLeftGridClick(typeInfoId)
    XLuaUiManager.Open("UiPassportCardComb", typeInfoId, handler(self, self.Refresh))
end

--一键领取
function XUiPassportPanelComb:OnBtnTongBlackClick()
    self._Control:RequestPassportRecvAllReward(handler(self, self.Refresh))
end

function XUiPassportPanelComb:OnCheckRewardRedPoint(count)
    self.BtnTongBlack:ShowReddot(count >= 0)
end

function XUiPassportPanelComb:Show()
    self:Open()
    self:Refresh()
end

function XUiPassportPanelComb:Hide()
    self:Close()
end

function XUiPassportPanelComb:GridOnClick(passportRewardId)
    local cb = function()
...
-- inf = 无限
function XUiPassportPanelComb:UpdateInfBtnVisible()
    -- 当玩家将列表拉至正常区最后面位置时
...
function XUiPassportPanelComb:InitInfBtn()
...
function XUiPassportPanelComb:OnBtnRewardNormalClick()
    self:SwitchRewardType(XEnumConst.PASSPORT.REWARD_TYPE.NORMAL, true)
end

function XUiPassportPanelComb:OnBtnRewardInfClick()
    self:SwitchRewardType(XEnumConst.PASSPORT.REWARD_TYPE.INFINITE, true)
end

function XUiPassportPanelComb:SwitchRewardType(rewardType, playSwitchAnimation)
...
function XUiPassportPanelComb:GetLevelIdList()

...
function string.Utf8LenCustomOverSea(str)
    local len = stringLen(str)
    local left = len
    local cnt = 0
...
--@return 字符长度
--==============================--
function string.Utf8CharLenCustomOverSea(char)
    if not char then
        return 0
...
function string.Utf8SubCustomOverSea(str, startChar, numChars)
    local startIndex = 1
    while startChar > 1 do
        local char = stringByte(str, startIndex)
        startIndex = startIndex + string.Utf8Size(char)
        startChar = startChar - string.Utf8CharLenCustomOverSea(char)
    end
    ...
    while numChars > 0 and currentIndex <= #str do
        local char = stringByte(str, currentIndex)
        currentIndex = currentIndex + string.Utf8Size(char)
        numChars = numChars - string.Utf8CharLenCustomOverSea(char)
    end
    return str:sub(startIndex, currentIndex - 1)
end

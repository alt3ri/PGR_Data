...
function XNetwork.Call(handler, request, reply, isEncoded, exReply, shieldReply, excludeMask)
    if IsDebug then
        if handler == "" or (string.find(handler, " ")) then
            XLog.Error("发送协议名错误！handler: " .. tostring(handler) .. ", request:", request)
            return
    ...
    excludeMask = excludeMask and true or false

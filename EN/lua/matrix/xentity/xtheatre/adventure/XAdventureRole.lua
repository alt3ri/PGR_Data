...
function XAdventureRole:GetElementList()
    return XMVCA.XCharacter:GetCharacterAllElement(self:GetCharacterId(), true)
end
...
function XAdventureRole:GetWeaponEquip()
    if self:GetIsLocalRole() then
        local equipId = XMVCA.XEquip:GetCharacterWeaponId(self:GetCharacterId())
        return XMVCA.XEquip:GetEquip(equipId)        
    end
...
function XAdventureRole:GetWearingEquipBySite(equipSite)
    if self:GetIsLocalRole() then
        local characterId = self:GetCharacterId()
        return XMVCA.XEquip:GetCharacterEquip(characterId, equipSite)
    end
...
function XAdventureRole:GetSuitMergeActiveDatas()
    ...
    if self:GetIsLocalRole() then
        local characterId = self:GetCharacterId()
        local wearingAwarenessIds = XMVCA.XEquip:GetCharacterAwarenessIds(characterId)
    ...
    if setTwoSuitId then
        table.insert(datas, {
            ...
            Icon = XMVCA.XEquip:GetEquipSuitIconPath(setTwoSuitId)
        })
    end
    return datas
end

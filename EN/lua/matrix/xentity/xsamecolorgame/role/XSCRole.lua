...
function XSCRole:GetIsLock()
    return not self:GetIsInUnlockTime() or not self:IsUnlock()
end
...
function XSCRole:IsUnlock()
    if XTool.IsNumberValid(self.Config.ConditionId) then
        return XConditionManager.CheckCondition(self.Config.ConditionId)
    end
    return true, ""
end

...
function XRLGuildDormRole:LoadModel(root)
    ...
    if not self.LoaderUtil then
        self.LoaderUtil = root:GetLoader()
    end
    local resourceAsset = self.LoaderUtil:Load(assetPath)
    if not resourceAsset then
        XLog.Error(string.format("加载公会宿舍角色资源:%s失败", assetPath))
        return
    end
    self.AssetPath = assetPath
    -- 设置资源
    local model = CS.UnityEngine.Object.Instantiate(resourceAsset)
    -- 重置到根节点上
...
function XRLGuildDormRole:Dispose()
    ...
    -- 清空资源
    if self.LoaderUtil and not string.IsNilOrEmpty(self.AssetPath) then
        self.LoaderUtil:Unload(self.AssetPath)
    end
    self.Animator = nil
    self.CharacterController = nil
    self.TempCopyFollowGo = nil

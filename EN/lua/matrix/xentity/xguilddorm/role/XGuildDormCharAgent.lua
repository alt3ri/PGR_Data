...
-- 设置相机看向坐标和距离
function XGuildDormCharAgent:SetCameraParam(distance, xAngle, yAngle)
    ...
    cameraController.collisionEnabled = false
    cameraController:SetNpcInteract(true, distance)
    cameraController:SetLookAtPos(centralPoint)
    if xAngle and yAngle and xAngle ~= 0 and yAngle ~= 0 then
        cameraController:SetTartAngle(Vector2(xAngle, yAngle))
...
function XGuildDormCharAgent:ResetCameraParam()
    ...
    cameraController.collisionEnabled = true
    cameraController.AllowXAxis = true
    cameraController.AllowYAxis = true
end
...
-- 设置镜头聚焦点(id, gameObjectName) 该方案暂时弃用
function XGuildDormCharAgent:SetCameraByFurnitureGameObjectNode(furnitureId, gameObjectName, distance)
    local playerId = self:GetPlayerId()
    if playerId ~= XPlayer.Id then
        return
    end
    ...
    -- ---@type XGuildDormRoom
    -- local room = XDataCenter.GuildDormManager.GetCurrentRoom()
    -- local furniture = room:GetFurnitureById(furnitureId)
    -- local furnitureTrans = furniture.FurnitureModel.Transform
    -- local tempObject = furnitureTrans:FindTransform(gameObjectName)
    
    -- local cameraController = XDataCenter.GuildDormManager.SceneManager.GetCurrentScene():GetCameraController()
    -- local role = room:GetRoleByPlayerId(XPlayer.Id)
    -- local rolePos = role:GetRLRole():GetTransform().position
    -- cameraController:SetNpcInteract(true, distance or 0)
    -- cameraController:SetLookAt(rolePos)
    -- cameraController.AllowXAxis = false
    -- cameraController.AllowYAxis = false
end

function XGuildDormCharAgent:CheckIsInstrumentPlayingAudio(furnitureId)
    return XMVCA.XInstrumentSimulator:CheckIsInstrumentPlayingAudio(furnitureId)
end
...
-- 打开公会宿舍音乐UI
function XGuildDormCharAgent:OpenMusicUi(furnitureId)
    local playerId = self:GetPlayerId()
    if playerId == XPlayer.Id then
        XMVCA.XInstrumentSimulator:OpenUi(furnitureId)
    end
end

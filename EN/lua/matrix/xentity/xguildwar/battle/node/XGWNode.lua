...
function XGWNode:UpdateWithServerData(data)
    ...
    self.NextReinforcementsBornTime = data.NextReinforcementsBornTime
    self.LastReinforcementId = data.LastReinforcementId
    self.LastAttackedReinforcementId = data.LastAttackedReinforcementId
    self.LastReinforcementAttackedTime = data.LastReinforcementAttackedTime
end
...
function XGWNode:GetBgIcon()
    return self.Config.BgIcon
end
...
function XGWNode:GetIsPlayerNode()
    ...
    if currentNodeId == self:GetId() then
        return true
    end

    if XTool.IsNumberValid(self:GetRootId()) then
        return currentNodeId == self:GetRootId()
    end
...
function XGWNode:GetFightEventDetailConfig()
    local showFightEventIds = self:GetShowFightEventId()
    if not XTool.IsTableEmpty(showFightEventIds) then
...
function XGWNode:CheckIsCanGo(preNodes)
    local currentNodeId = XDataCenter.GuildWarManager.GetBattleManager():GetCurrentNodeId()
    if self:GetIsPlayerNode() then
        return false
    end
    ...
    if preNodes ~= nil then
        if not XTool.IsTableEmpty(preNodes) then
            -- 遍历查找路径是否联通到基地
            local selfId = XTool.IsNumberValid(self:GetRootId()) and self:GetRootId() or self:GetId()
            
            local preNodeId = preNodes[selfId]

            while preNodeId and preNodeId > 0 do
                local preNodeData = XDataCenter.GuildWarManager.GetBattleManager():GetNode(preNodeId)

                if preNodeData:GetIsBaseNode() or preNodeData:GetIsPlayerNode() then
                    return true
                elseif preNodeData:GetIsDead() then
                    preNodeId = preNodes[preNodeId]
                else
                    break
                end
            end
    ...
    for i = 1, #parentNodes do
        ...
        if node:CheckIsCanGo() or node:GetIsBaseNode() then
            return true
        end
    end
    -- false
...
function XGWNode:CheckIsCanGoHome(preNodes)
    if self:GetIsPlayerNode() then
        return false
    end
    ...
    if preNodes ~= nil then
        if not XTool.IsTableEmpty(preNodes) then
            ...
            while preNodeId and preNodeId > 0 do
                ...
                if preNodeData:GetIsBaseNode() then
                    return true
                elseif preNodeData:GetIsDead() then
                    preNodeId = preNodes[preNodeId]
                else
...
--region 援军相关
function XGWNode:CheckIsCreateReinforcements()
    return XTool.IsNumberValid(self.NextReinforcementsBornTime)
end

function XGWNode:GetNextReinforcementBornTime()
    return self.NextReinforcementsBornTime
end

function XGWNode:GetLastReinforcementId()
    return self.LastReinforcementId
end

function XGWNode:GetLastAttackedReinforcementId()
    return self.LastAttackedReinforcementId
end

function XGWNode:GetLastReinforcementAttackedTime()
    return self.LastReinforcementAttackedTime
end
--endregion

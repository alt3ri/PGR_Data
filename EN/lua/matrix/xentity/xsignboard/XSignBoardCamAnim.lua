...
function XSignBoardCamAnim:IsAnimPlaying()
...
function XSignBoardCamAnim:_InitNode(uiNode)
    ...
    if self:CheckCamRootIsHaveCam(self.NearAnimNode) and isSetOffset then
        self.NearCamRoot.localPosition = self.NearCamRoot.localPosition + Vector3(-0.058, 0, -0.006)
    end
    if isSetOffset then
        ---@type UnityEngine.Transform
        ...
        ---@type UnityEngine.Transform
        local EffectParent = uiNode.transform:FindTransform("Effect")

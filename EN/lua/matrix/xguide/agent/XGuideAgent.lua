...
function XGuideAgent:GetUi(uiName)
    ...
    local proxy = (not ui.UiData or not ui.UiData.IsLuaUi) and ui.UiProxy or ui.UiProxy.UiLuaTable
    return proxy
end
...
function XGuideAgent:FocusOnChild(uiName, parentPath, index, eulerAngles, passEvent, sizeDelta, offset, childName, focusStyle, bubbleIndex, bubbleTextId, bubblePosOffset)
    ...
    index = index - 1
    if index >= parent.childCount then
        XLog.Error("节点" .. parentPath .. "子节点数量小于" .. index)
        return
    end
    local target = parent:GetChild(index)
    if not target then
        XLog.Error("节点" .. parentPath .. "不存在子节点" .. index)
        return
    end
    if not string.IsNilOrEmpty(childName) then
        target = target:Find(childName)
        if not target then
            local log = string.format("继续查找子节点出错: UiName:%s\tParent:%s\tIndex:%s\tChild:%s", uiName, parentPath, index, childName)
            XLog.Error(log)
            return
        end
    end

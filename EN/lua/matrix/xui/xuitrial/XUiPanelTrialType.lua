...
local XUiPanelTrialType = XClass(nil, "XUiPanelTrialType")
...
function XUiPanelTrialType:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiPanelTrialType:RegisterClickEvent函数出错, 原因：点击回调函数不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiPanelTrialType:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end
...
function XUiPanelTrialType:OnBtnClickClick()
    if XDataCenter.TrialManager.FinishTrialType() == TrialTypeCfg.TrialBackEnd and XDataCenter.TrialManager.TrialRewardGetedFinish() then

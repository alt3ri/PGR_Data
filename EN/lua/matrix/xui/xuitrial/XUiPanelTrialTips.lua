local XUiPanelTrialTips = XClass(nil, "XUiPanelTrialTips")
...
function XUiPanelTrialTips:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiPanelTrialTips:RegisterClickEvent函数出错, 原因：点击回调函数不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiPanelTrialTips:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end

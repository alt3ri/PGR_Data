...
function XUiDormDressingOverview:SetupDynamicTable()
    ...
    if not XTool.IsTableEmpty(dataList) then
        for i, v in pairs(dataList) do
            if XFurnitureConfigs.CheckFurnitureSuitIsShowById(v.Id) then
                table.insert(validDataList, v)
            end
        end
    end
    
    self.DataList = validDataList
    self.PanelNoneTemplate.gameObject:SetActiveEx(XTool.IsTableEmpty(validDataList))
    self.DynamicTable:SetDataSource(validDataList)

...
function XUiPanelActivityCalendar:RefreshWeekActivity()
    ...
    local mainIds = self._Control:GetWeekMainIds()
...
function XUiPanelActivityCalendar:RefreshTimeLimitActivity()
    local activityInfo, isHasOpenedFunctionalActivity = self._Control:GetTimeLimitActivityIds()
    self.IsFinishTimeLimit = XTool.IsTableEmpty(activityInfo)
    if isHasOpenedFunctionalActivity then
        self.PanelLimitedTimeTitle.gameObject:SetActiveEx(true)
        ...
        self.PanelLimitedTimeTitle.gameObject:SetActiveEx(false)
    end

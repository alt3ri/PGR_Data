...
function XUiSkyGardenShoppingStreetInsideBuildBase:SetBuilding(pos, isInside)
    ...
    self.BtnUpgrade:ShowReddot(shopAreaData:CanShowUpgradeTips())
    self.BtnUpgrade:SetButtonState(isMaxLv and CS.UiButtonState.Disable or CS.UiButtonState.Normal)
end
...
function XUiSkyGardenShoppingStreetInsideBuildBase:OnBtnUpgradeClick()
    local func = function (...)
        XMVCA.XSkyGardenShoppingStreet:AddOpenBlackMaskLoading(nil, function()
            XMVCA.XBigWorldUI:Open("UiSkyGardenShoppingStreetUpgrade", self._ShopUiPos, self._IsInside, true)
        end)
    end
    self.Parent:CheckSaveTipsCallback(func, func)
end

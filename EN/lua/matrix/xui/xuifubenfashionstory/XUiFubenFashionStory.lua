...
function XUiFubenFashionStory:OnBtnBackClick()
    if XLuaUiManager.IsUiShow(FIGHT_DETAIL) or XLuaUiManager.IsUiShow(STORY_DETAIL) then
        ...
        if self.Type ~= XMVCA.XFashionStory.Type.OnlyChapter and self.Mode == XMVCA.XFashionStory.Mode.Chapter then
            self:OnBtnSwitchClick(XMVCA.XFashionStory.Mode.Trial)
        else
            self:Close()
        end
    end
...
function XUiFubenFashionStory:LoadActivity(activityId)
    ...
    local type = XMVCA.XFashionStory:GetType(activityId)
    self:SetType(type)

    local initMode
    if type == XMVCA.XFashionStory.Type.OnlyTrial or type == XMVCA.XFashionStory.Type.Both then
        initMode = self.Mode or XMVCA.XFashionStory.Mode.Trial
    elseif type == XMVCA.XFashionStory.Type.OnlyChapter then
        initMode = self.Mode or XMVCA.XFashionStory.Mode.Chapter
    else
...
function XUiFubenFashionStory:SetType(type)
    self.Type = type
    local isBothMode = type == XMVCA.XFashionStory.Type.Both
    local isOnlyTrial = type == XMVCA.XFashionStory.Type.OnlyTrial
...
function XUiFubenFashionStory:LoadChapter()
    -- 处理跳转按钮
    local skipList = XMVCA.XFashionStory:GetSkipIdList(self.ActivityId)
    ...
    -- 预制体
    local prefabPath = XMVCA.XFashionStory:GetChapterPrefab(self.SingleLineId)
    if prefabPath then
...
function XUiFubenFashionStory:SetMode(mode)
    if (self.Type == XMVCA.XFashionStory.Type.OnlyChapter and mode == XMVCA.XFashionStory.Mode.Trial)
            or (self.Type == XMVCA.XFashionStory.Type.OnlyTrial and mode == XMVCA.XFashionStory.Mode.Chapter) then
...
function XUiFubenFashionStory:ShowPanelByMode()
    local bg
    if self.Mode == XMVCA.XFashionStory.Mode.Chapter then
        self.PanelChapter.gameObject:SetActiveEx(true)
        self.PanelTrial.gameObject:SetActiveEx(false)
        bg = XMVCA.XFashionStory:GetChapterBg(self.ActivityId)
    elseif self.Mode == XMVCA.XFashionStory.Mode.Trial then
        self.PanelChapter.gameObject:SetActiveEx(false)
        self.PanelTrial.gameObject:SetActiveEx(true)
        bg = XMVCA.XFashionStory:GetTrialBg(self.ActivityId)
    else
...
function XUiFubenFashionStory:Refresh(moveToLast)
    if self.Mode == XMVCA.XFashionStory.Mode.Chapter then
        -- 通关进度
        local passNum, totalNum = XMVCA.XFashionStory:GetChapterProgress(self.ActivityId)
...
function XUiFubenFashionStory:RefreshTrial()
    self.DataSource = XMVCA.XFashionStory:GetActiveTrialStage(self.ActivityId)

    if XMVCA.XFashionStory:IsStoryInTime(self.ActivityId) then
        -- 剧情模式处于开放时间，插入模式入口
        table.insert(self.DataSource, 1, XMVCA.XFashionStory.StoryEntranceId)
    end
...
function XUiFubenFashionStory:OnDynamicTableEvent(event, index, grid)
    if event == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_ATINDEX then
        if self.DataSource[index] == XMVCA.XFashionStory.StoryEntranceId then
            grid:Refresh(self.DataSource[index], self.ActivityId, function()
                self:OnBtnSwitchClick(XMVCA.XFashionStory.Mode.Chapter)
...
function XUiFubenFashionStory:RefreshLeftTime()
    local leftTimeStamp = XMVCA.XFashionStory:GetLeftTimeStamp(self.ActivityId)

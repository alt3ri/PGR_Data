...
function XUiPanelSkillDetailsInfo:RefreshEntryBtn()
    if not XTool.IsNumberValid(self.SubSkillId) then
        ...
        self.BtnObservation.gameObject:SetActiveEx(false)
        return
    end

    -- 观察按钮
    local obsCfg = XMVCA.XCharacter:GetModelCharacterObsTriggerMagic()[self.SubSkillId]
    local isShow = false
    if not XTool.IsTableEmpty(obsCfg) and not XTool.IsTableEmpty(obsCfg.Des) then
        for k, v in pairs(obsCfg.Des) do
    ...
    self.BtnObservation.gameObject:SetActiveEx(isShow)
...
function XUiPanelSkillDetailsInfo:RegisterUiEvents()
    ...
    XUiHelper.RegisterClickEvent(self, self.BtnObservation, self.OnBtnObservationClick)
...
function XUiPanelSkillDetailsInfo:OnBtnObservationClick()
    XLuaUiManager.Open("UiSkillObservationMagicInfo", self.SubSkill.SubSkillId)
end

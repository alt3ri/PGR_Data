...
---@class XUiSameColorGameShopItem:XLuaUi
local XUiSameColorGameShopItem = XLuaUiManager.Register(XLuaUi, "UiSameColorGameShopItem")
...
function XUiSameColorGameShopItem:OnStart(shopId, data, cb, shopCanBuyColor, proxy)
    if data then
        self.Data = data
    else
        return
...
function XUiSameColorGameShopItem:InitPanel()
    self.MinCount = 1
    self.Count = 1
    self.Consumes = {}
    self.BuyConsumes = {}
...
function XUiSameColorGameShopItem:SetSelectTextData()
    self.TxtSelect.characterLimit = 4
    self.TxtSelect.contentType = CS.UnityEngine.UI.InputField.ContentType.IntegerNumber
end

function XUiSameColorGameShopItem:RemoveUI()
    XLuaUiManager.Remove(self.Name)
end

function XUiSameColorGameShopItem:BtnAddSelectLongClickCallback(time)
...
function XUiSameColorGameShopItem:BtnMinusSelectLongClickCallback(time)
...
function XUiSameColorGameShopItem:SetCanAddOrMinusBtn()
...
function XUiSameColorGameShopItem:HaveItem()
    -- 五期不显示
...
function XUiSameColorGameShopItem:RefreshCommon()
    ...
    self.Grid.ImgQuality.gameObject:SetActiveEx(false)
end

function XUiSameColorGameShopItem:RefreshPrice()
...
function XUiSameColorGameShopItem:RefreshOnSales(buyCount)
    self.OnSales = {}
...
function XUiSameColorGameShopItem:RefreshConsumes()
...
function XUiSameColorGameShopItem:GetSalesInfo()
    self.OnSales = {}
    XTool.LoopMap(self.Data.OnSales, function(k, sales)
        self.OnSales[k] = sales
    end)
...
function XUiSameColorGameShopItem:GetMaxCount()
    self.Sales = 100
    local sortedKeys = {}
...
function XUiSameColorGameShopItem:SetCanBuyCount()
...
function XUiSameColorGameShopItem:JudgeBuy()
...
function XUiSameColorGameShopItem:ChangeCostColor(bool, index)
    if bool then
...
function XUiSameColorGameShopItem:CheckBuyItemTypeOfWeaponFashion(cb)
...
function XUiSameColorGameShopItem:GetCountProxy(id)
...
function XUiSameColorGameShopItem:TipDialog(cancelCb, confirmCb, TextKey)
...
function XUiSameColorGameShopItem:AddBtnListener()
...
function XUiSameColorGameShopItem:OnBtnMaxClick()
...
function XUiSameColorGameShopItem:OnBtnUseClick()
    if self.HaveNotBuyCount then
    ...
    local func = function()
        XShopManager.BuyShop(self.ShopId, self.Data.Id, self.Count, function()
            if self.CallBack then
                self.CallBack(self.Count)
            end
            XUiManager.TipText("BuySuccess")
            XEventManager.DispatchEvent(XEventId.EVENT_SC_SHOP_BUY)
            self:RemoveUI()
        end)
    end
...
function XUiSameColorGameShopItem:OnBtnAddSelectClick()
    if self.Count + 1 > self.MaxCount then
        XMVCA.XEquip:ShowBoxOverLimitText()
        return
    end
...
function XUiSameColorGameShopItem:OnBtnMinusSelectClick()
    if self.Count - 1 < self.MinCount then
        return
    end
    self.Count = self.Count - 1
...
function XUiSameColorGameShopItem:OnSelectTextChange()
...
function XUiSameColorGameShopItem:OnSelectTextInputEnd()
    if self.TxtSelect.text == nil or self.TxtSelect.text == "" then
        self.TxtSelect.text = 1
        local tmp = tonumber(self.TxtSelect.text)
        self.Count = tmp
...
function XUiSameColorGameShopItem:OnBtnBlockClick()
    self:Close()
end
--endregion

return XUiSameColorGameShopItem
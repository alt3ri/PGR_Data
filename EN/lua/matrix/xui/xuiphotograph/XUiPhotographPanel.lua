...
function XUiPhotographPanel:OnDynamicTableActionEvent(event, index, grid)
    ...
    elseif event == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_TOUCHED then
        if XTool.IsNumberValid(self._LastTouchTime) then
            if CS.XTimerManager.GetRealTime() - self._LastTouchTime < TouchCd then
                return
            end
        end
        
        self._LastTouchTime = CS.XTimerManager.GetRealTime()
...
function XUiPhotographPanel:RefreshBtnSynchronous()
    ...
    self.BtnSynchronous.gameObject:SetActiveEx(self.RootUi:CheckHasChanged() and not self.BtnHide:GetToggleState() and not isRandomBackground)
end

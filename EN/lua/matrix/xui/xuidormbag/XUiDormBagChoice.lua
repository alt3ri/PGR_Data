...
function XUiDormBagChoice:OnStart(selectIds, furnitureTypeId, maxSelectCount, selectCb, filterMap, filterSuitIdMap, 
    ...
    self.FilterSuitIdMap = filterSuitIdMap or {}
...
function XUiDormBagChoice:FilterSuitIdIds(suitIds)
    ...
    if not checkFilter and not self.IsRefit then
        return suitIds
    end
    local ids = {}
    for _, suitId in pairs(suitIds) do
        if self.IsRefit and XFurnitureConfigs.IsIgnoreWhenRefit(suitId) then
            goto continue
        end
        
        if checkFilter and self.FilterSuitIdMap[suitId] then
            goto continue
        end

        if not XFurnitureConfigs.CheckFurnitureSuitIsShowById(suitId) then
            goto continue
        end
...
function XUiDormBagChoice:InitSelectSuitIds()
    ...
    for _, suit in pairs(suitList) do
        if suit.Id ~= XFurnitureConfigs.FURNITURE_SUIT_CATEGORY_ALL_ID then
            ...
            table.insert(self.AllSuitIds, suitId)
            if self.IsRefit and XFurnitureConfigs.IsIgnoreWhenRefit(suitId) then

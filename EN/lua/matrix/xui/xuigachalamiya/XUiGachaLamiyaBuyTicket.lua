---@class XUiGachaLamiyaBuyTicket : XLuaUi
local XUiGachaLamiyaBuyTicket = XLuaUiManager.Register(XLuaUi, "UiGachaLamiyaBuyTicket")

function XUiGachaLamiyaBuyTicket:OnAwake()
...
function XUiGachaLamiyaBuyTicket:OnStart(gachaCfg, itemData1, itemData2, targetItemData, forceBuyCount, closeCb)
...
function XUiGachaLamiyaBuyTicket:OnEnable()
    self:Refresh()
...
function XUiGachaLamiyaBuyTicket:InitItemCardPanel(index)
    local panel = {}
...
function XUiGachaLamiyaBuyTicket:InitButton()
...
function XUiGachaLamiyaBuyTicket:Refresh()
    self:RefreshUiShow()
    self:RefreshBuyAmount(tonumber(self.TxtSelect.text))
end

function XUiGachaLamiyaBuyTicket:GetItemData(index)
    return index == 1 and self._ItemData1 or self._ItemData2
end

function XUiGachaLamiyaBuyTicket:RefreshUiShow()
...
function XUiGachaLamiyaBuyTicket:RefreshBuyAmount(count)
    self._BuyAmount = count
end

function XUiGachaLamiyaBuyTicket:CheckBuyAmountLimit(count)
...
function XUiGachaLamiyaBuyTicket:OnSelfSetBuyCount(count)
...
function XUiGachaLamiyaBuyTicket:OnBtnMaxClick()
...
function XUiGachaLamiyaBuyTicket:OnBtnAddSelectClick()
...
function XUiGachaLamiyaBuyTicket:OnBtnMinusSelectClick()
    local count = tonumber(self.TxtSelect.text)
    local targetCount = count - 1
    if self:CheckBuyAmountLimit(targetCount) then
        return
...
-- text改动的回调
function XUiGachaLamiyaBuyTicket:OnSelectTextChange()
    local count = tonumber(self.TxtSelect.text)
...
function XUiGachaLamiyaBuyTicket:OnImgBtnClick(index)
    local data = XDataCenter.ItemManager.GetItem(self:GetItemData(index).ItemId)
    XLuaUiManager.Open("UiTip", data)
end

function XUiGachaLamiyaBuyTicket:OnBtnBuyClick(index)
    -- CD检测
...
function XUiGachaLamiyaBuyTicket:CheckBuyCD()
...
return XUiGachaLamiyaBuyTicket
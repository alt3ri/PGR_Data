...
function XUiMainLeftBottom:OnEnable()
    ...
    XEventManager.AddEventListener(XEventId.EVENT_MENTOR_TEACHERORSTUDENT_CHANGE, self.OnRefreshChatBtn, self)
    XEventManager.AddEventListener(XEventId.EVENT_GUILD_GIFT_CONTRIBUTE_CHANGED, self.OnRefreshChatBtn, self)
    ...
    if XDataCenter.ChatManager.CheckMainUIEntranceOpen() then
        self:RefreshChatMsg(XDataCenter.ChatManager.GetLatestChatData())
        self.BtnChat.transform.parent.gameObject:SetActiveEx(true)
    else
        self.BtnChat.transform.parent.gameObject:SetActiveEx(false)
    end
...
function XUiMainLeftBottom:OnDisable()
    ...
    XEventManager.RemoveEventListener(XEventId.EVENT_MENTOR_STUDENT_SYSTEMTASK_CHANGE, self.OnRefreshChatBtn, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_MENTOR_TEACHERORSTUDENT_CHANGE, self.OnRefreshChatBtn, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_GUILD_GIFT_CONTRIBUTE_CHANGED, self.OnRefreshChatBtn, self)
end
...
function XUiMainLeftBottom:OnBtnChat()
    if XFunctionManager.DetectionFunction(XFunctionManager.FunctionName.SocialChat) then
        ...
        XUiHelper.OpenUiChatServeMain(true)
    end
end
...
--更新聊天按钮
function XUiMainLeftBottom:OnRefreshChatBtn()
    if XDataCenter.ChatManager.CheckMainUIEntranceOpen() then
        self.BtnChat.transform.parent.gameObject:SetActiveEx(true)
    else
        self.BtnChat.transform.parent.gameObject:SetActiveEx(false)

...
---@class XUiGachaLuciaQuickWear : XLuaUi 使用历程奖励弹框
local XUiGachaLuciaQuickWear = XLuaUiManager.Register(XLuaUi, "UiGachaLuciaQuickWear")

function XUiGachaLuciaQuickWear:OnAwake()
    XUiHelper.RegisterClickEvent(self, self.BtnClose, self.Close)
    XUiHelper.RegisterClickEvent(self, self.BtnWear, self.OnBtnWearClick)
end

function XUiGachaLuciaQuickWear:OnStart(templateId, rewardId, isConvertFrom, rewardName, callBack)
    self._CallBack = callBack
    self._TemplateId = templateId
    self.TxtDesc.text = XUiHelper.GetText(isConvertFrom and "GachaLuciaRewardTitle1" or "GachaLuciaRewardTitle2", rewardName)
...
function XUiGachaLuciaQuickWear:Close()
...
function XUiGachaLuciaQuickWear:OnBtnWearClick()
    ...
    if XDataCenter.HeadPortraitManager.IsHeadPortraitValid(self._TemplateId) then
        ...
        if config.Type == 1 then
            ...
            XDataCenter.HeadPortraitManager.ChangeHeadPortrait(self._TemplateId, function()
                XUiManager.TipText("GachaLuciaSettingSuccess")
                self._IsDone = true
            end)
        else
            -- 更换头像框

---@class XUiGachaLuciaBuyTicket : XLuaUi
local XUiGachaLuciaBuyTicket = XLuaUiManager.Register(XLuaUi, "UiGachaLuciaBuyTicket")

function XUiGachaLuciaBuyTicket:OnAwake()
...
function XUiGachaLuciaBuyTicket:OnStart(gachaCfg, itemData1, itemData2, targetItemData, forceBuyCount, closeCb)
...
function XUiGachaLuciaBuyTicket:OnEnable()
    self:Refresh()
...
function XUiGachaLuciaBuyTicket:InitItemCardPanel(index)
    local panel = {}
...
function XUiGachaLuciaBuyTicket:InitButton()
...
function XUiGachaLuciaBuyTicket:Refresh()
    self:RefreshUiShow()
    self:RefreshBuyAmount(tonumber(self.TxtSelect.text))
end

function XUiGachaLuciaBuyTicket:GetItemData(index)
    return index == 1 and self._ItemData1 or self._ItemData2
end

function XUiGachaLuciaBuyTicket:RefreshUiShow()
...
function XUiGachaLuciaBuyTicket:RefreshBuyAmount(count)
    self._BuyAmount = count
end

function XUiGachaLuciaBuyTicket:CheckBuyAmountLimit(count)
...
function XUiGachaLuciaBuyTicket:OnSelfSetBuyCount(count)
...
function XUiGachaLuciaBuyTicket:OnBtnMaxClick()
...
function XUiGachaLuciaBuyTicket:OnBtnAddSelectClick()
...
function XUiGachaLuciaBuyTicket:OnBtnMinusSelectClick()
    local count = tonumber(self.TxtSelect.text)
    local targetCount = count - 1
    if self:CheckBuyAmountLimit(targetCount) then
        return
...
-- text改动的回调
function XUiGachaLuciaBuyTicket:OnSelectTextChange()
    local count = tonumber(self.TxtSelect.text)
...
function XUiGachaLuciaBuyTicket:OnImgBtnClick(index)
    local data = XDataCenter.ItemManager.GetItem(self:GetItemData(index).ItemId)
    XLuaUiManager.Open("UiTip", data)
end

function XUiGachaLuciaBuyTicket:OnBtnBuyClick(index)
    -- CD检测
...
function XUiGachaLuciaBuyTicket:CheckBuyCD()

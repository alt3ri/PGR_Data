...
---@class XUiGachaLuciaResult : XLuaUi
local XUiGachaLuciaResult = XLuaUiManager.Register(XLuaUi, "UiGachaLuciaResult")

local MODE_LOOP = 1

function XUiGachaLuciaResult:OnAwake()
    self:InitAutoScript()
end

function XUiGachaLuciaResult:OnStart(gachaId, rewardList, backCb, background)
...
function XUiGachaLuciaResult:OnEnable()
    XUiHelper.PopupFirstGet()
end

function XUiGachaLuciaResult:OnDisable()
    XDataCenter.AntiAddictionManager.EndDrawCardAction()
end

function XUiGachaLuciaResult:Update()
...
function XUiGachaLuciaResult:InitAutoScript()
    self._SpecialSoundMap = {}
    self:AutoAddListener()
end

function XUiGachaLuciaResult:GetAutoKey(uiNode, eventName)
    if not uiNode then
        return
    end
...
function XUiGachaLuciaResult:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiGachaLuciaResult:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end
...
function XUiGachaLuciaResult:AutoAddListener()
    self._AutoCreateListeners = {}
    self:RegisterClickEvent(self.BtnBack, self.OnBtnBackClick)
end

function XUiGachaLuciaResult:OnBtnBackClick()
...
function XUiGachaLuciaResult:SetupRewards()
...
function XUiGachaLuciaResult:ShowResult()
...
--已经拥有的角色转换碎片过程
function XUiGachaLuciaResult:ShowTrans()
    local count = 0
    for i = 1, #self._RewardList do
        if self._RewardList[i].ConvertFrom ~= 0 then
            count = count + 1

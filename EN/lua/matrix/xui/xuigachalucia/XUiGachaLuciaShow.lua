---@class XUiGachaLuciaShow : XLuaUi
local XUiGachaLuciaShow = XLuaUiManager.Register(XLuaUi, "UiGachaLuciaShow")
...
function XUiGachaLuciaShow:OnStart(gachaId, rewardList, state, isSkip)
...
function XUiGachaLuciaShow:OnEnable()
...
function XUiGachaLuciaShow:OnDisable()
    self:StopCv()
end

function XUiGachaLuciaShow:OnDestroy()
    self:StopCv()
...
function XUiGachaLuciaShow:RegisterButton()
...
function XUiGachaLuciaShow:InitUi()
    self._FarCameraList = {}
...
function XUiGachaLuciaShow:PlayNext()
    ...
    if not self._LastReward then
        ...
        self._GridRewardList[self._CurrIndex] = require("XUi/XUiGachaLucia/Grid/XUiGridGachaLuciaShowReward").New(self._ModelPanelList[self._CurrIndex], self, self._UiPanelList[self._CurrIndex], self._FarCameraList[self._CurrIndex], self._NearCameraList[self._CurrIndex], self._UiCameraList[self._CurrIndex])
        self._LastReward = self._GridRewardList[self._CurrIndex]
    end
...
function XUiGachaLuciaShow:ShowResult()
    ...
    for i = 1, #self._RewardList do
        ...
        local grid = require("XUi/XUiGachaLucia/Grid/XUiGridGachaLuciaResult").New(obj, self)
...
function XUiGachaLuciaShow:RefreshByState()
...
function XUiGachaLuciaShow:SetDrawEffectGroupId(effectGroupId)
    if effectGroupId > self._MaxEffectGroupId then
        self._MaxEffectGroupId = effectGroupId
    end
end

function XUiGachaLuciaShow:PlayCardEffectSound()
...
function XUiGachaLuciaShow:StopCv()
    if self._CvInfo then
        self._CvInfo:Stop()
        self._CvInfo = nil
    end
end
function XUiGachaLuciaShow:OnBtnSkipClick()
    if self._CurrState == DrawState.Result then
        self:Close()
    else
        self._CurrState = DrawState.Result

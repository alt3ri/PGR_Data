...
function XUiPersonalInfo:GetPanelMsgboard()
    if self.XUiPanelMsgBoard then
        ...
        XLog.Error("XUiPersonalInfo.XUiPanelMsgBoard错误, XUiPanelMsgBoard这个子窗口初始化失败")
    end
end
...
function XUiPersonalInfo:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiPersonalInfo:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end

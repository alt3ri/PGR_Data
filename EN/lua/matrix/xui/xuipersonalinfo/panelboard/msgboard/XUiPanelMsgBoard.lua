...
function XUiPanelMsgBoard:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiPanelMsgBoard:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end
...
function XUiPanelMsgBoard:Refresh()
    ...
    self:ResetPanel()
    XDataCenter.PersonalInfoManager.CreateItems(self.RootUi, self.PanelMsgBoardItem, self.PanelMsgBoardItem.gameObject.transform.parent, 1,nil ,XDataCenter.PersonalInfoManager.ItemPanelType.NorDaily,function()
        self:SetIsShow(true)
        self:MoveToBottom()
    end)
end

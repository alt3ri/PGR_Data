...
function XUiPanelBubbleSkill:ShowWeaponSkill(skillId)
    ...
    self.TxtDetail.text = XUiHelper.ReplaceTextNewLine(self._Control:GetWeaponSkillDesc(skillId))
    self.RImgLegend:SetRawImage(self._Control:GetWeaponSkillMapIcon(skillId))
    local isBoss = self._Control:GetChessEnemy():GetBossInfo(self.RoleId) ~= nil
    local cost = isBoss and 0 or self._Control:GetWeaponSkillCost(self.RoleId, skillId)
...
function XUiPanelBubbleSkill:ShowWeapon(buffId)
    ...
    self.TxtDetail.text = XUiHelper.ReplaceTextNewLine(self._Control:GetWeaponDesc(buffId))
...
function XUiPanelBubbleSkill:ShowCharacterSkill(weaponId)
    ...
    self.TxtDetail.text = XUiHelper.ReplaceTextNewLine(self._Control:GetWeaponDesc(weaponId))

...
XLuaUi = XClass(nil, "XLuaUi", true)

-- require 部分类
XClassPartialRequire("XUi/XUiBase/XLuaUiOverride", "XLuaUi")
...
function XLuaUi:SwitchControlTips(name, isActive)
    if not name then return end

    local key = self.Name .. "/" .. name
    if not self._SwitchTipsCache then
        self._SwitchTipsCache = {}
    end

    if isActive then
        self._SwitchTipsCache[key] = true
        XUiManager.AddControllerTips(key)
    else
        self._SwitchTipsCache[key] = nil
        XUiManager.RemoveControllerTips(key)
    end
end

function XLuaUi:CleanSwitchControlTips()
    if not self._SwitchTipsCache then return end
    for key, value in pairs(self._SwitchTipsCache) do
        XUiManager.RemoveControllerTips(key)
    end
    self._SwitchTipsCache = nil
end
--endregion
...
function XLuaUi:PlayAnimation(animName, callback, beginCallback, wrapMode, disableEvaluateTime)
    if self:IsDestroy() then
        XLog.Error(string.format("界面：%s已经销毁，无法播放动画%s", tostring(self.Name), animName))
        return
    end
    disableEvaluateTime = disableEvaluateTime or false
...
function XLuaUi:PlayAnimationWithMask(animName, callback, beginCallback, wrapMode, disableEvaluateTime)
    if self:IsDestroy() then
        XLog.Error(string.format("界面：%s已经销毁，无法播放动画%s", tostring(self.Name), animName))
        return
    end
...
function XLuaUi:StopAnimation(animName, isTriggerCallBack, isEvaluate)
    if self:IsDestroy() then
        XLog.Error(string.format("界面：%s已经销毁，无法停止动画%s", tostring(self.Name), animName))
        return
    end
...
function XLuaUi:PlayMaskAnimation(animName, callback, beginCallback, wrapMode, disableEvaluateTime)
    if self:IsDestroy() then
        XLog.Error(string.format("界面：%s已经销毁，无法播放动画%s", tostring(self.Name), animName))
        return
    end

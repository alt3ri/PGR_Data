...
-- 2024愚人节显示处理
function XGridCharacterV2P6:AprilFoolShowHandle()
    ...
    if isMainChar then
        ...
        self.TxtLevel.text = aprilFoolsDayMainShowLv
        return
    end
    ...
    if isSubChar then
        ...
        self.TxtLevel.text = aprilFoolsDaySubShowLv
    end
end
...
function XGridCharacterV2P6:UpdateEnergy()
    -- 元素图标
    local elementIcon
    local elementList = XMVCA.XCharacter:GetCharacterAllElement(self.Character.Id, true)
    for i = 1, 3 do
        elementIcon = elementList and elementList[i]

local XUiGridFubenChallengeStage = XClass(XUiNode, "XUiGridFubenChallengeStage")
local XUiPanelStars = require("XUi/XUiFubenMainLineChapter/XUiPanelStars")

function XUiGridFubenChallengeStage:UpdateNode(id, stageId, stageIndex)
    self.Id = id
...
function XUiGridFubenChallengeStage:SetNormalStage()
...
function XUiGridFubenChallengeStage:OnBtnStageClick()
...
--显示选中框
function XUiGridFubenChallengeStage:SetNodeSelect(isSelect)
    if self.IsOpen then
        self.ImageSelected.gameObject:SetActiveEx(isSelect)
    end
end

return XUiGridFubenChallengeStage
...
function XUiPanelTheatre3PropDetail:OnStart()
    ...
    if not self.GridPropUi.RawImage then
        self.GridPropUi.RawImage = XUiHelper.TryGetComponent(self.GridPropUi.Transform, "RawImage", "RawImage")
    end
end
...
function XUiPanelTheatre3PropDetail:RefreshView()
    ...
    if isProp then
        ...
        self.GridPropUi.Lock.gameObject:SetActiveEx(not isUnlock)
        --道具背景
        self.GridPropUi.RawImage.gameObject:SetActiveEx(true)
        self.GridPropUi.RawImage:SetRawImage(self._Control:GetItemBgUrlById(self.Id))
        self.GridPropUi.RawImageRed.gameObject:SetActiveEx(false)
        self.GridPropUi.ImgQualityRed.gameObject:SetActiveEx(false)

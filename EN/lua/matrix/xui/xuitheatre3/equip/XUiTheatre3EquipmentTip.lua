...
function XUiTheatre3EquipmentTip:InitEffectPos()
    if not self._Effect then
        self._Effect = XUiHelper.TryGetComponent(self.Transform, "Effect")
    end
    if self._Effect then
        self._Effect.localPosition = CS.UnityEngine.Vector3(-26, 0, 0)
    end
end

...
---@field Parent XPanelTheatre3EventShop
...
function XGridTheatre3EventShop:Refresh(shopItem)
    ...
    self._IsMoneyShop = self.Parent:GetShopType() == XEnumConst.THEATRE3.ShopType.GiveMoney
    ...
    self.Bg.gameObject:SetActiveEx(not self._IsMoneyShop)
    self.BgSpecial.gameObject:SetActiveEx(self._IsMoneyShop)
...
function XGridTheatre3EventShop:_RefreshItem()
    ...
    if self._IsMoneyShop then
        -- 给钱商店不受折扣影响
        self.TxtCostCount.text = string.format("+%s", math.abs(self._ShopItem:GetOriginalPrice()))
    else
    ...
    self.ImgDiscount.gameObject:SetActiveEx(self._ShopItem:CheckIsHaveDiscount() and not self._IsMoneyShop)
    if self._ShopItem:CheckIsHaveDiscount() then
         local discount = self._ShopItem:GetDiscount()
        if XOverseaManager.IsKRRegion() or XOverseaManager.IsENRegion() and discount > 0 and  discount < 10 then
...
function XGridTheatre3EventShop:_RefreshCost()
    ...
    if self._IsMoneyShop then
        index = 5
    elseif self._Control:IsAdventureALine() then
...
function XGridTheatre3EventShop:OnBtnBuyClick()
    ...
    elseif self._ShopItem:CheckIsBuy() then
        ...
        self._Control:OpenAdventureShopTips(self._IsMoneyShop, self._ShopItem, nil, handler(self, self._OnBtnBuyClick))
    end
end

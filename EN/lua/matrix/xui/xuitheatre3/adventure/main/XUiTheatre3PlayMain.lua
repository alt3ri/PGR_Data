...
function XUiTheatre3PlayMain:RefreshPressure()
    local value = self._Control:GetCurPressureValue()
    if XTool.IsNumberValid(value) then
        self.PanelPressure.gameObject:SetActiveEx(true)
        self.TxtPressure.text = value
        self:SetUiSprite(self.ImgPressure, self._Control:GetPressureIcon(value, true))
    else
        self.PanelPressure.gameObject:SetActiveEx(false)
    end
end
...
function XUiTheatre3PlayMain:AddEventListener()
    ...
    XEventManager.AddEventListener(XEventId.EVENT_THEATRE3_ADD_STEP, self.RefreshUi, self)
end
...
function XUiTheatre3PlayMain:RemoveEventListener()
    ...
    XEventManager.RemoveEventListener(XEventId.EVENT_THEATRE3_ADD_STEP, self.RefreshUi, self)
end
--endregion

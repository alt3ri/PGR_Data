        ...
        for i, group in ipairs(rewardRareLevelList) do
            ...
            for k, v in pairs(group) do
                ...
                item:SetCustomItemTip(function(data, hideSkipBtn, rootUiName, lackNum)
                    XLuaUiManager.Open("UiEpicFashionGachaTip", data, hideSkipBtn, rootUiName, lackNum)
                end)
            end
        end
        ...
        local successCb = function(rewardList, newUnlockGachaId, res)
            ...
            for k, v in pairs(rewardList) do
                if v.TemplateId == self.GachaCfg.TargetTemplateId then
                    ...
                    -- end
                elseif v.RewardType == XRewardManager.XRewardType.Background then
                    backgroundItem = v
                end
            end

            if fashionItem or backgroundItem then
                self.ShowFashionTrigger = function()
                    if fashionItem then
                        XLuaUiManager.Open("UiEpicFashionGachaPassport", fashionItem)
                    end
                    if backgroundItem then
                        XLuaUiManager.Open("UiEpicFashionGachaPassport", backgroundItem)
                    end
                end
            end

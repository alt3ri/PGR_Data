...
function XUiFubenStageDetail:OnStart(stage, cb, onlyEnterBtn)
    if cb then
        self.CallBack = cb
    end
    self.Stage = stage
    self.OnlyEnterBtn = onlyEnterBtn
    self.StarGridList = {}
    self.GridList = {}
...
function XUiFubenStageDetail:AutoInitUi()
    ...
    self.BtnRepeatEnter = self.Transform:Find("SafeAreaContentPane/PanelBottom/BtnRepeatEnter"):GetComponent("Button")
end
...
function XUiFubenStageDetail:AutoAddListener()
    ...
    self:RegisterClickEvent(self.BtnRepeatEnter, self.OnBtnRepeatEnterClick)
...
-- 显示首通还是多重奖励按钮
function XUiFubenStageDetail:OnBtnRepeatEnterClick()
    if self.IsPlaying then
        return
    end

    if self.Stage == nil then
        XLog.Error("OnBtnRepeatEnterClick: Can not find stage!")
        return
    end
    XLuaUiManager.Open("UiRepeatChallengeEnter", self.Stage)
end

function XUiFubenStageDetail:OnBtnAutoFightClick()
...
-- 显示多重挑战
function XUiFubenStageDetail:UpdateRepeatChallenge()
    local stageInfo = XDataCenter.FubenManager.GetStageInfo(self.Stage.StageId)
    local firstDrop = false
    if not stageInfo.Passed then
        local cfg = XDataCenter.FubenManager.GetStageLevelControl(self.Stage.StageId)
    ...
    if self.OnlyEnterBtn then
        self.BtnEnter.gameObject:SetActiveEx(true)
        self.BtnRepeatEnter.gameObject:SetActiveEx(false)
    else
        self.BtnEnter.gameObject:SetActiveEx(firstDrop)
        self.BtnRepeatEnter.gameObject:SetActiveEx(not firstDrop)
    end

    if stageInfo.Passed then
        local stageCfg = XDataCenter.FubenManager.GetStageCfg(self.Stage.StageId)
        self.BtnAutoFight.gameObject:SetActiveEx(stageCfg.AutoFightId > 0)
    else

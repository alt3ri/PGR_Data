...
function XUiFurnitureTypeSelect:OnStart(selectIds, selectSuitIds, isBuild, comfirmCb, isDraft, filterSuitMap)
    ...
    self.PageRecord = self.IsDraft and PanelState.SUIT or PanelState.FURNITURE
...
function XUiFurnitureTypeSelect:InitTabGroup()
    ...
    -- 图纸筛选时不显示页签，只显示标题
    self.FurnitureTypeBtnGroup.gameObject:SetActiveEx(not self.IsDraft)
    self.TitleSuit.gameObject:SetActiveEx(self.IsDraft)
end

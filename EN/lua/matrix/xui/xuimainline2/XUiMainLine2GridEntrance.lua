...
function XUiMainLine2GridEntrance:OnStart(entranceData, chapterId, mainId, parentGo, lineGo, uiName)
    ...
    for _, stageId in ipairs(self.StageIds) do
        ...
        self._Control:CacheStageGroupId(stageId, self.GroupId)
    end
end
...
function XUiMainLine2GridEntrance:OnBtnStageClick()
    ...
    self.Parent.Parent:SetLastClickStageId(self.StageIds[1])
    self:CheckCloseSpecialEffect()
    ...
    or detailType == XEnumConst.MAINLINE2.STAGE_DETAIL_TYPE.CG then
        XLuaUiManager.Open("UiMainLine2DetailStory", self.StageIds, self.ChapterId, self.MainId, function()
            self:CheckReOpenSpecialEffect()
        end)
    ...
    or detailType == XEnumConst.MAINLINE2.STAGE_DETAIL_TYPE.FIGHT_BOSS then
        XLuaUiManager.Open("UiMainLine2DetailBattle", self.StageIds, self.ChapterId, self.MainId, function()
            self:CheckReOpenSpecialEffect()
        end)
    end
end
...
function XUiMainLine2GridEntrance:RefreshAchievements()
    -- 获取所有关卡的成就
    local achieveInfos = self._Control:GetStagesAchievementInfos(self.StageId, false, true)
...
-- 关闭特效
function XUiMainLine2GridEntrance:CheckCloseSpecialEffect()
    ...
    if effectPath and effectPath ~= "" then
        self:ShowSubPrefab("PanelEffect", false)
    end
end

-- 重新打开特效
function XUiMainLine2GridEntrance:CheckReOpenSpecialEffect()
    if self:IsPass() then return end

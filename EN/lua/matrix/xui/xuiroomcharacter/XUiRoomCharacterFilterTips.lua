...
function XUiRoomCharacterFilterTips:OnAwake()
    ...
    self.IsUseHistorySortTag = true
    self:InitComponent()
    self:AddListener()
end

-- hideSortTagDic : { [XRoomCharFilterTipsConfigs.EnumSortTag.xxx] = true } 即为隐藏
function XUiRoomCharacterFilterTips:OnStart(rootUi, filterType, sortType, characterType, hideFilter, hideSort, hideSortTagDic, isUseHistorySortTag)
    if isUseHistorySortTag == nil then isUseHistorySortTag = true end
    self.IsUseHistorySortTag = isUseHistorySortTag
    ...
    if not self.HideSort then
        ...
        for index, sortTabId in ipairs(sortTags) do
            ...
            if tabCharType == self.CharacterType or tabCharType == 0 then
                ...
                if self.IsUseHistorySortTag and XDataCenter.RoomCharFilterTipsManager.CheckSortTagIsSelect(sortTabId, self.CharacterType) then

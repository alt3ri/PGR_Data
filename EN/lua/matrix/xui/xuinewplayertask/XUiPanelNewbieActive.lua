...
function XUiPanelNewbieActive:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiPanelNewbieActive:RegisterClickEvent函数参数错误：参数func不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiPanelNewbieActive:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end
...
function XUiPanelNewbieActive:UpdateNewbieActiveView(currentActiveness, maxActiveness)
    ...
    local tmpNewbieActiveness = XTaskConfig.GetTaskNewbieActivenessTemplate()
    local activeness = tmpNewbieActiveness.Activeness[self.Index or 1]
...
function XUiPanelNewbieActive:OnBtnActiveClick()

...
---@class XUiGridGachaBiankaShowReward : XUiNode
---@field Parent XUiGachaBiankaShow
local XUiGridGachaBiankaShowReward = XClass(XUiNode, "XUiGridGachaBiankaShowReward")
...
function XUiGridGachaBiankaShowReward:OnStart(uiPanel, farCamera, nearCamera, uiCamera)
    self.ModelPanel = self.Transform
...
function XUiGridGachaBiankaShowReward:InitUiObject()
    ...
    self.GridConvert:SetCustomItemTip(function(data, hideSkipBtn, rootUiName, lackNum)
        XLuaUiManager.Open("UiGachaBianka402Tip", data, hideSkipBtn, rootUiName, lackNum)
    end)
    ---@type UnityEngine.RectTransform
...
function XUiGridGachaBiankaShowReward:OnShow(rewardInfo)
    self._Reward = rewardInfo
    ...
    local showTable = XGachaConfigs.GetGachaShowByGroupId(self.Parent._GachaCfg.GachaShowGroupId)[Type]
    ...
    self.Parent.BtnClick.gameObject:SetActiveEx(false)
    -- 播放进入动画
    self.GridModelCaseEnable:PlayTimelineAnimation(function()
        --根据Type创建模型
        self:CreateModel(id, Type)
        self.Parent.BtnClick.gameObject:SetActiveEx(true)
    end)
end

function XUiGridGachaBiankaShowReward:OnShowEnd()
...
function XUiGridGachaBiankaShowReward:CreateModel(id, rewardType)
...
function XUiGridGachaBiankaShowReward:CreateWeaponModel(templateId)
    local modelConfig = XMVCA.XEquip:GetWeaponModelCfg(templateId, self.Parent.Name, 0)
    if modelConfig then
        XModelManager.LoadWeaponModel(modelConfig.ModelId, self.GridModel.gameObject, modelConfig.TransformConfig, self.Parent.Name, function(model)
        ...
        end, { gameObject = self.Parent.GameObject })
    end
end

function XUiGridGachaBiankaShowReward:CreateWeaponFashionModel(templateId)
    local fashionId = XDataCenter.ItemManager.GetWeaponFashionId(templateId)
    local modelConfig = XDataCenter.WeaponFashionManager.GetWeaponModelCfg(fashionId, nil, self.Parent.Name)
    if modelConfig then
        XModelManager.LoadWeaponModel(modelConfig.ModelId, self.GridModel.gameObject, modelConfig.TransformConfig, self.Parent.Name, function(model)
            model.gameObject:SetActiveEx(true)
            self:Load3dLineEffect(model, LineEffectWeapon3d)
...
function XUiGridGachaBiankaShowReward:CreateCharacterModel(templateId, fashionId)
    if not templateId and not fashionId then
        return
    end
    if not self._InitRoleMode then
        self._InitRoleMode = true
        self.RoleModelPanel = require("XUi/XUiCharacter/XUiPanelRoleModel").New(self.GridModel, self.Parent.Name, true, false, false)
    end
    ...
    self.RoleModelPanel:UpdateCharacterModel(curCharacterId, self.GridModel, self.Parent.Name, function(model)
        model.gameObject:SetActiveEx(true)
        ...
        if voiceId then
            self._CvInfo = XLuaAudioManager.PlayAudioByType(XLuaAudioManager.SoundType.Voice, voiceId)
        end
    end, nil, curFashtionId)
end

function XUiGridGachaBiankaShowReward:CreatePartnerModel(templateId)
    if not self._InitPartnerMode then
        self._InitPartnerMode = true
    ...
    XDataCenter.PartnerManager.LoadPartnerStandbyModelWithSToCShow(templateId, self.PartnerModelPanel, self.Parent.Name, function(SModel)
        ...
        -- 战斗模型
        self.PartnerModelPanel:UpdatePartnerModel(modelConfig.CombatModel, self.Parent.Name, nil, function(CModel)
...
function XUiGridGachaBiankaShowReward:CreateItem(iconPath, isSmall, type)
    ...
    ---@type XLoaderUtil
    local loader = root:GetLoader()
    local asset = loader:Load(iconPath)
    if not asset then
        XLog.Error(string.format("加载图片失败：%s", iconPath))
        return
    end
    XTool.LoopArray(renderer, function(v)
        v.material:SetTexture("_MainTex", asset)
        v.trailMaterial = v.material
    end)
end

function XUiGridGachaBiankaShowReward:LoadEffect(id)
    -- 加载底座特效
...
-- 加载3d 通讯线特效
function XUiGridGachaBiankaShowReward:Load3dLineEffect(model, effectKey)
...
function XUiGridGachaBiankaShowReward:OnDestroy()

end

return XUiGridGachaBiankaShowReward
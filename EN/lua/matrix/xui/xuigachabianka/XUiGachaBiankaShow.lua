---@class XUiGachaBiankaShow : XLuaUi
local XUiGachaBiankaShow = XLuaUiManager.Register(XLuaUi, "UiGachaBianka402Show")
...
function XUiGachaBiankaShow:OnStart(gachaId, rewardList, state, isSkip)
...
function XUiGachaBiankaShow:OnEnable()
...
function XUiGachaBiankaShow:OnDisable()
    self:StopCv()
end

function XUiGachaBiankaShow:OnDestroy()
    self:StopCv()
...
function XUiGachaBiankaShow:RegisterButton()
...
function XUiGachaBiankaShow:InitUi()
    self._FarCameraList = {}
...
function XUiGachaBiankaShow:PlayNext()
    ...
    if not self._LastReward then
        ...
        self._GridRewardList[self._CurrIndex] = require("XUi/XUiGachaBianka/Grid/XUiGridGachaBiankaShowReward").New(self._ModelPanelList[self._CurrIndex], self, self._UiPanelList[self._CurrIndex], self._FarCameraList[self._CurrIndex], self._NearCameraList[self._CurrIndex], self._UiCameraList[self._CurrIndex])
        self._LastReward = self._GridRewardList[self._CurrIndex]
    end
...
function XUiGachaBiankaShow:ShowResult()
    ...
    for i = 1, #self._RewardList do
        ...
        local grid = require("XUi/XUiGachaBianka/Grid/XUiGridGachaBiankaResult").New(obj, self)
...
function XUiGachaBiankaShow:RefreshByState()
...
function XUiGachaBiankaShow:SetDrawEffectGroupId(effectGroupId)
    if effectGroupId > self._MaxEffectGroupId then
        self._MaxEffectGroupId = effectGroupId
    end
end

function XUiGachaBiankaShow:PlayCardEffectSound()
...
function XUiGachaBiankaShow:StopCv()
    if self._CvInfo then
        self._CvInfo:Stop()
        self._CvInfo = nil
    end
end
function XUiGachaBiankaShow:OnBtnSkipClick()
    if self._CurrState == DrawState.Result then
        self:Close()
    else
        self._CurrState = DrawState.Result

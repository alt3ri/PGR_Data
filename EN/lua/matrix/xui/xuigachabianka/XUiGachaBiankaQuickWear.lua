...
---@class XUiGachaBiankaQuickWear : XLuaUi 使用历程奖励弹框
local XUiGachaBiankaQuickWear = XLuaUiManager.Register(XLuaUi, "UiGachaBianka402QuickWear")

function XUiGachaBiankaQuickWear:OnAwake()
    XUiHelper.RegisterClickEvent(self, self.BtnClose, self.Close)
    XUiHelper.RegisterClickEvent(self, self.BtnWear, self.OnBtnWearClick)
end

function XUiGachaBiankaQuickWear:OnStart(templateId, rewardId, isConvertFrom, rewardName, callBack)
    self._CallBack = callBack
    self._TemplateId = templateId
    self.TxtDesc.text = XUiHelper.GetText(isConvertFrom and "GachaBiankaRewardTitle1" or "GachaBiankaRewardTitle2", rewardName)
...
function XUiGachaBiankaQuickWear:Close()
...
function XUiGachaBiankaQuickWear:OnBtnWearClick()
    ...
    if XDataCenter.HeadPortraitManager.IsHeadPortraitValid(self._TemplateId) then
        ...
        if config.Type == 1 then
            ...
            XDataCenter.HeadPortraitManager.ChangeHeadPortrait(self._TemplateId, function()
                XUiManager.TipText("GachaBiankaSettingSuccess")
                self._IsDone = true
            end)
        else
            -- 更换头像框
...
return XUiGachaBiankaQuickWear
...
function XUiMail:OnStart()
    ...
    -- 推荐使用XUiRichTextCustomRender组件
    self.HtmlText = self.GridContent:GetComponent("XUiRichTextCustomRender")
    if self.HtmlText then
        self.HtmlText.onLinkClick = function(arg) self:ClickLink(arg) end
    end
    self.HtmlTextNotReward = self.GridContentNotReward:GetComponent("XUiRichTextCustomRender")
    if self.HtmlTextNotReward then
        self.HtmlTextNotReward.onLinkClick = function(arg) self:ClickLink(arg) end
    end
    ...
    if not self.HtmlTextNotReward then
...
function XUiMail:ShowMailInfoNormal(mailInfo)
    ...
    if not mailAgency:IsExpire(mailInfo.Id) then
        if not mailInfo.ExpireTime or mailInfo.ExpireTime <= 0 then
...
function XUiMail:SetRewardBtnStatusFavorite(mailId)
    local cfg = self._Control:GetFavoriteMailCfgById(mailId)

    if cfg then
        if not XTool.IsTableEmpty(cfg.RewardIds) then
            self.BtnGetReward:SetNameByGroup(0, XUiHelper.GetText("MailCanReceive"))
        else
            self.BtnGetReward:SetNameByGroup(0, XUiHelper.GetText("MailCanCollect"))
        end
    else
        -- 如果没有读到配置，要设置回默认的文本
        self.BtnGetReward:SetNameByGroup(0, XUiHelper.GetText("MailCanReceive"))

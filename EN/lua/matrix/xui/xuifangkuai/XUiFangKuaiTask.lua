...
function XUiFangKuaiTask:UpdateDynamicTable()
    ...
    if not XTool.IsTableEmpty(self.ActivityTasks) then
        local allAchieveTasks = {}
        for _, v in pairs(self.ActivityTasks) do
        ...
        if not XTool.IsTableEmpty(allAchieveTasks) then
            table.insert(self.ActivityTasks, 1, { ReceiveAll = true, AllAchieveTaskDatas = allAchieveTasks }) -- 领取全部
        end
    end

...
local XUiGridTurntableRewardTopShow = require("XUi/XUiTurntable/XUiGridTurntableRewardTopShow")
local XUiPanelTurntableRewardShow = require('XUi/XUiTurntable/XUiPanelTurntableRewardShow')
...
function XUiTurntableMain:OnStart()
    ...
    --self:_PlaySpineAnimation('Enable', 'idle')
end
...
function XUiTurntableMain:InitProgress()
    ...
    for i = #rewards, 1, -1 do
        ...
        self._rewards[i]:Init(i, rewards[i][1], rewards[i][2], rewards[i][3])
...
--- 3.6弃用
function XUiTurntableMain:InitSpineObjs()
    local SkeletonAnimationCSArray = self.UiTurntableMainSpine.transform:GetComponentsInChildren(typeof(CS.Spine.Unity.SkeletonAnimation))
    local SkeletonGraphicCSArray = self.UiTurntableMainSpine.transform:GetComponentsInChildren(typeof(CS.Spine.Unity.SkeletonGraphic))
...
function XUiTurntableMain:SetItem(box, cfg, boxRotation, isOver)
    ...
    grid:SetProxyClickFunc(function()
        return self:OnShowItemTip(rewardData)
    end)
...
function XUiTurntableMain:UpdateProgressBar()
    ...
    local percent = 0
    if times > 0 then
        percent = self._Control:GetTurntableMilestonePercentByTurns(times)

        if percent == 0 then
            XLog.Error('抽取次数对应配置不存在, Times: '..tostring(times))
        end
    end
...
function XUiTurntableMain:UpdateProgressTopRewardShow()
    -- 从里程碑最低的开始，遇到的第一个需要置顶的里程碑，执行置顶
    local hasTopShow = false
    local lastCanTopShowIndex = 0
    
    for i = #self._rewards, 1, -1 do
        local rewardCtrl = self._rewards[i]

        local needTopShow = rewardCtrl:CheckNeedTopShow()

        if rewardCtrl:CheckCanTopShow() then
            lastCanTopShowIndex = i
        end

        if needTopShow then
            hasTopShow = true
            self.TopRewardCtrl:Open()
            self.TopRewardCtrl:Init(rewardCtrl._RewardIndex, rewardCtrl._RewardId, rewardCtrl._Progress, rewardCtrl._IsTopShow)
        end
    end

    if not hasTopShow then
        if XTool.IsNumberValid(lastCanTopShowIndex) then
            local rewardCtrl = self._rewards[lastCanTopShowIndex]
            self.TopRewardCtrl:Open()
            self.TopRewardCtrl:Init(rewardCtrl._RewardIndex, rewardCtrl._RewardId, rewardCtrl._Progress, rewardCtrl._IsTopShow)
        else
            self.TopRewardCtrl:Close()
        end
    end
end
...
function XUiTurntableMain:SetIsRewardCanGain()
    ...
    self.TopRewardCtrl:SetForbidGain(self._IsTweening)
end
...
function XUiTurntableMain:ShowRewardTip(records)
    ...
    local panel = isBigTip and self.PanelRewardShow01 or self.PanelRewardShow02
...
function XUiTurntableMain:InitReward(grid, data)
    ...
    if param.Site and param.Site ~= XEnumConst.EQUIP.EQUIP_SITE.WEAPON then
...
function XUiTurntableMain:Close()
    if not self._TurntableEnd then
        return
    end
    
    if self._RewardShowing then
        self:OnBtnObtainClick()
        return
    end
...
function XUiTurntableMain:CountDown()
    ...
    if time > 0 then
        ...
        XLuaUiManager.CloseWithCallback('UiTurntableMain', function()
...
--- 3.6 弃用
---Spine对象组播放动画
function XUiTurntableMain:_PlaySpineAnimation(fromAnim, toAnim)
...
--- 3.6 弃用
---spine对象播放动画
function XUiTurntableMain:_PlaySpineObjAnimation(spineObject, fromAnim, toAnim)

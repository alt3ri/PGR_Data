...
---@class XUiGachaSelenaStageLine : XLuaUi 剧情关界面(用的 festivalActivity 表)
---@field ParentUi XUiGachaSelenaMain
local XUiGachaSelenaStageLine = XLuaUiManager.Register(XLuaUi, "UiGachaSelenaStageLine")

function XUiGachaSelenaStageLine:OnAwake()
...
function XUiGachaSelenaStageLine:OnStart(chapterId, panel3D, isAutoOpen)
...
function XUiGachaSelenaStageLine:OnEnable()
    ...
    self:SetAutoCloseInfo(endTime, function(isClose)
        if not isClose then
            ...
            self.TxtDay.text = XUiHelper.GetText("GachaSelenaTime", XUiHelper.GetTime(time, XUiHelper.TimeFormatType.CHATEMOJITIMER))
        end
    end, nil, 0)

    self.ParentUi:OnAfterStageLineEnable()
end

function XUiGachaSelenaStageLine:OnDisable()
    for _, stage in pairs(self._Stages) do
        stage:Close()
    end
end

function XUiGachaSelenaStageLine:Refresh3DSceneInfo()
...
function XUiGachaSelenaStageLine:InitButton()
...
function XUiGachaSelenaStageLine:SetUiData()
    -- 初始化prefab组件
...
function XUiGachaSelenaStageLine:HandleStages()
    for i = 1, #self._StageIds do
        local itemStage = self.PanelStageContent:Find(string.format("Stage%d", i))
        if not itemStage then
            XLog.Error("XUiGachaSelenaStageLine:HandleStages() 函数错误: 游戏物体PanelStageContent下找不到名字为:" .. string.format("Stage%d", i) .. "的游戏物体")
            return
        end
        -- 组件初始化
        self._StageGroup[i] = itemStage
        if not self._Stages[i] then
            self._Stages[i] = require("XUi/XUiGachaSelena/Grid/XUiGridGachaSelenaStageItem").New(itemStage, self)
        end
        self._Stages[i]:Open()
...
function XUiGachaSelenaStageLine:HandleStageLines()
    ...
    for i = 1, #self._StageIds - 1 do
        ...
        if not itemLine then
            XLog.Error("XUiGachaSelenaStageLine:SetUiData() error: prefab not found a child name:" .. string.format("Line%d", i))
            return
        end
...
-- 背景
function XUiGachaSelenaStageLine:SwitchFestivalBg()
    self.RImgFestivalBg.gameObject:SetActiveEx(false)
end

-- 加载特效
function XUiGachaSelenaStageLine:LoadEffect(effectUrl)
...
-- 更新刷新
function XUiGachaSelenaStageLine:RefreshFestivalNodes()
...
-- 更新节点线条
function XUiGachaSelenaStageLine:UpdateNodeLines()
    if not self._ChapterTemplate or not self._StageIds then
        return
    end
...
function XUiGachaSelenaStageLine:SetStageLineActive(index, isActive)
...
-- 通过Stage调用的接口
function XUiGachaSelenaStageLine:OpenStageDetails(stageId)
    XLuaUiManager.Open("UiGachaSelenaStageDetail", stageId)
end

-- 选中关卡描边效果
function XUiGachaSelenaStageLine:UpdateNodesSelect(stageId)
...
-- 取消选中
function XUiGachaSelenaStageLine:ClearNodesSelect()
...
function XUiGachaSelenaStageLine:SetPanelStageListMovementType(moveMentType)
...
function XUiGachaSelenaStageLine:MoveIntoStage(stageIndex)
...
function XUiGachaSelenaStageLine:OnSceneBtnBackClick()
    self:Close()
    self.ParentUi:Close()
end

function XUiGachaSelenaStageLine:OnGotoGacha()
    self:Close()
    self.ParentUi:OnChildClose()
end

function XUiGachaSelenaStageLine:OnBtnSetClick()
    XLuaUiManager.Open("UiSet")
end

return XUiGachaSelenaStageLine
---@class XUiGachaSelenaBuyTicket : XLuaUi
local XUiGachaSelenaBuyTicket = XLuaUiManager.Register(XLuaUi, "UiGachaSelenaBuyTicket")

function XUiGachaSelenaBuyTicket:OnAwake()
...
function XUiGachaSelenaBuyTicket:OnStart(gachaCfg, itemData1, itemData2, targetItemData, forceBuyCount, closeCb)
...
function XUiGachaSelenaBuyTicket:OnEnable()
    self:Refresh()
...
function XUiGachaSelenaBuyTicket:InitItemCardPanel(index)
    local panel = {}
...
function XUiGachaSelenaBuyTicket:InitButton()
...
function XUiGachaSelenaBuyTicket:Refresh()
    self:RefreshUiShow()
    self:RefreshBuyAmount(tonumber(self.TxtSelect.text))
end

function XUiGachaSelenaBuyTicket:GetItemData(index)
    return index == 1 and self._ItemData1 or self._ItemData2
end

function XUiGachaSelenaBuyTicket:RefreshUiShow()
...
function XUiGachaSelenaBuyTicket:RefreshBuyAmount(count)
    self._BuyAmount = count
end

function XUiGachaSelenaBuyTicket:CheckBuyAmountLimit(count)
...
function XUiGachaSelenaBuyTicket:OnSelfSetBuyCount(count)
...
function XUiGachaSelenaBuyTicket:OnBtnMaxClick()
...
function XUiGachaSelenaBuyTicket:OnBtnAddSelectClick()
...
function XUiGachaSelenaBuyTicket:OnBtnMinusSelectClick()
    local count = tonumber(self.TxtSelect.text)
    local targetCount = count - 1
    if self:CheckBuyAmountLimit(targetCount) then
        return
...
-- text改动的回调
function XUiGachaSelenaBuyTicket:OnSelectTextChange()
    local count = tonumber(self.TxtSelect.text)
...
function XUiGachaSelenaBuyTicket:OnImgBtnClick(index)
    local data = XDataCenter.ItemManager.GetItem(self:GetItemData(index).ItemId)
    XLuaUiManager.Open("UiTip", data)
end

function XUiGachaSelenaBuyTicket:OnBtnBuyClick(index)
    -- CD检测
...
function XUiGachaSelenaBuyTicket:CheckBuyCD()
...
return XUiGachaSelenaBuyTicket
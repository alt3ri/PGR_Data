...
function XUiGridVoteItem:Ctor(ui, currPlayerId, voteId, valueChangeCallback, voteSuccessCallback, rootUi,isFailGroup)
...
function XUiGridVoteItem:InitUiView()
    ...
    if self.RImgGainIcon and self.TxtGainTitle then
        local gainItemId = self.VoteItem:GetGainItemId()
        self.RImgGainIcon:SetRawImage(XDataCenter.ItemManager.GetItemIcon(gainItemId))
        self.TxtGainTitle.text = CS.XTextManager.GetText("MoeWarVoteGainTitle")
    end
...
function XUiGridVoteItem:UpdateLimitText()
    ...
    if self.VoteItem:IsLimitVote() then
        local limitCount = 0
        if self.IsFailGroup then
            limitCount = self.VoteItem:GetDailyLimitFailGroupCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId())
        else
            limitCount = self.VoteItem:GetDailyLimitCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId())
        end
        self.TxtLimit.text = CS.XTextManager.GetText("MoeWarDailyLimitVoteTips", itemName, playerEntity:GetDailyVoteByItemId(self.VoteItem:GetVoteItemId()),limitCount )
    else
...
function XUiGridVoteItem:SetVoteNumber(value)
    ...
    if self.TxtGainNumber then
        self.TxtGainNumber.text = toString(value * self.VoteItem:GetGainCount())
    end
...
function XUiGridVoteItem:OnClickBtnAdd()
    ...
    if self.VoteItem:IsLimitVote() then
        local limitCount = self.IsFailGroup and self.VoteItem:GetDailyLimitFailGroupCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId()) or self.VoteItem:GetDailyLimitCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId())
        local max = limitCount - playerEntity:GetDailyVoteByItemId(self.VoteItem:GetVoteItemId())
        if max < 0 then
            max = 0
        end
...
function XUiGridVoteItem:OnClickBtnMax()
    ...
    if self.VoteItem:IsLimitVote() then
        local limitCount = self.IsFailGroup and self.VoteItem:GetDailyLimitFailGroupCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId()) or self.VoteItem:GetDailyLimitCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId())
        newValue = limitCount - playerEntity:GetDailyVoteByItemId(self.VoteItem:GetVoteItemId())
...
function XUiGridVoteItem:CheckIsCanVote()
    ...
    if self.VoteItem:IsLimitVote() then
        local playerEntity = XDataCenter.MoeWarManager.GetPlayer(self.CurrPlayerId)
        local limitCount = self.IsFailGroup and self.VoteItem:GetDailyLimitFailGroupCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId()) or self.VoteItem:GetDailyLimitCountByMatchId(XDataCenter.MoeWarManager.GetCurMatchId())
        local canVoteCount = limitCount - playerEntity:GetDailyVoteByItemId(self.VoteItem:GetVoteItemId())

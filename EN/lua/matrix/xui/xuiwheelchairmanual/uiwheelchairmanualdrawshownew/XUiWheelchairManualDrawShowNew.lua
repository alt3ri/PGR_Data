local XUiWheelchairManualDrawShowNew = XLuaUiManager.Register(XLuaUi, "UiWheelchairManualDrawShowNew")
...
function XUiWheelchairManualDrawShowNew:OnStart(drawInfo, rewardList, resultCb, state, closeCb, showExtra)
...
function XUiWheelchairManualDrawShowNew:OnEnable()
    if self._JumpRefresh then
        self._JumpRefresh = false
        return
    end
    self:RefreshByState()
end

function XUiWheelchairManualDrawShowNew:OnDisable()
    self:StopCv()
end

function XUiWheelchairManualDrawShowNew:OnDestroy()
    self:StopCv()
...
function XUiWheelchairManualDrawShowNew:RegisterButton()
...
function XUiWheelchairManualDrawShowNew:InitUi()
    self.FarCameraList = {}
    self.NearCameraList = {}
    self.ModelPanelList = {}
    self.UiPanelList = {}
...
function XUiWheelchairManualDrawShowNew:PlayNext()
    if self.LastReward then
        self.LastReward:OnShowEnd()
    end
    ...
    self:TryShowExtra(self.CurrIndex)
    
end

function XUiWheelchairManualDrawShowNew:ShowResult()
    self:HideExtra()
...
function XUiWheelchairManualDrawShowNew:RefreshByState()
...
function XUiWheelchairManualDrawShowNew:SetDrawEffectGroupId(effectGroupId)
    if effectGroupId > self.MaxEffectGroupId then
        self.MaxEffectGroupId = effectGroupId
    end
end

function XUiWheelchairManualDrawShowNew:PlayCardEffectSound()
...
function XUiWheelchairManualDrawShowNew:StopCv()
    if self.CvInfo then
        self.CvInfo:Stop()
        self.CvInfo = nil
    end
...
function XUiWheelchairManualDrawShowNew:OnClose()
    self:Close()
...
function XUiWheelchairManualDrawShowNew:TryShowExtra(index)
    local reward = XTool.IsNumberValid(index) and self.RewardList[index] or self.RewardList[1]

    local isCharacter = reward.RewardType == XRewardManager.XRewardType.Character
    local isWeapon = reward.RewardType == XRewardManager.XRewardType.Equip

    if not isCharacter and XTool.IsNumberValid(reward.ConvertFrom) then
        isCharacter = XTypeManager.GetTypeById(reward.ConvertFrom) == XRewardManager.XRewardType.Character
    end
    
    local showCheckBtn = (isCharacter or isWeapon) and self.ShowExtra
    local showCharacter = isWeapon and self.ShowExtra

    self.BtnCheck.gameObject:SetActiveEx(showCheckBtn)
    self.PanelCharacter.gameObject:SetActiveEx(showCharacter)

    if self.ShowExtra then
        if showCharacter then
            local characterId = XMVCA.XEquip:GetEquipRecommendCharacterId(reward.TemplateId)
            local hasRecommandCharacter = XTool.IsNumberValid(characterId)
            if not hasRecommandCharacter then
                -- 不存在推荐角色，尝试遍历查找到第一个装备适配的角色
                local equipType = XMVCA.XEquip:GetEquipType(reward.TemplateId)
                ...
                if XTool.IsNumberValid(equipType) then
                    ...
                    for _, charCfg in pairs(charCfgs) do
                        if charCfg.EquipType == equipType then
            ...
            if XTool.IsNumberValid(characterId) then
                self.RoleHead:SetRawImage(XMVCA.XCharacter:GetCharSmallHeadIcon(characterId))
                self.TxtRoleName.text = XMVCA.XCharacter:GetCharacterFullNameStr(characterId)
            else
                self.PanelCharacter.gameObject:SetActiveEx(false)
                local equipType = XMVCA.XEquip:GetEquipType(reward.TemplateId)
                XLog.Error('目标装备：'..tostring(reward.TemplateId)..'不存在推荐角色的配置, 也未找到与武器类型：'..tostring(equipType)..'适配的角色，请检查Equip.tab中对应的RecommendCharacterId字段配置')
            end
        end

        if showCheckBtn then
            if not self.BtnCheck.CallBack then
                self.BtnCheck.CallBack = handler(self, self.OnCheckBtnClickEvent)
            end
        end
    end
end

function XUiWheelchairManualDrawShowNew:HideExtra()
    self.BtnCheck.gameObject:SetActiveEx(false)
    self.PanelCharacter.gameObject:SetActiveEx(false)
end

function XUiWheelchairManualDrawShowNew:OnCheckBtnClickEvent()
    local reward = XTool.IsNumberValid(self.CurrIndex) and self.RewardList[self.CurrIndex] or self.RewardList[1]

    local isCharacter = reward.RewardType == XRewardManager.XRewardType.Character
    local characterId = reward.TemplateId
    if not isCharacter and XTool.IsNumberValid(reward.ConvertFrom) then
        isCharacter = XTypeManager.GetTypeById(reward.ConvertFrom) == XRewardManager.XRewardType.Character
        characterId = reward.ConvertFrom
    end
    
    if isCharacter then
        XLuaUiManager.Open('UiCharacterDetail', characterId)
        self._JumpRefresh = true
    elseif reward.RewardType == XRewardManager.XRewardType.Equip then
        XLuaUiManager.Open('UiEquipPreviewV2P6', reward.TemplateId)
        self._JumpRefresh = true
    end
end

return XUiWheelchairManualDrawShowNew
...
---@class XUiSlotMachineRewardPanel: XUiNode
---@field SlotMachineBestRewardScrollTime UnityEngine.UI.ScrollRect
local XUiSlotMachineRewardPanel = XClass(XUiNode, "XUiSlotMachineRewardPanel")

function XUiSlotMachineRewardPanel:OnStart(rootUi)
    self.RootUi = rootUi
    self:Init()
end

function XUiSlotMachineRewardPanel:OnDisable()
    if self._RewardMilestoneTween and self._RewardMilestoneTween:IsActive() then
        self._RewardMilestoneTween:Kill()
        self._RewardMilestoneTween = nil
    end
end
...
function XUiSlotMachineRewardPanel:Init()
    ...
    self.PanelBest.CallBack = handler(self, self.OnBtnBestClickEvent)
end
...
function XUiSlotMachineRewardPanel:OnBtnBestClickEvent()
    local time = CS.XGame.ClientConfig:GetFloat('SlotMachineBestRewardScrollTime')
    
    time = time * (1- self.RewardMilestoneScrollView.verticalNormalizedPosition / 1)

    if self._RewardMilestoneTween then
        if self._RewardMilestoneTween:IsPlaying() then
            return
        end
    end
    
    self._RewardMilestoneTween = CS.DG.Tweening.DOTween.To(function() 
        return self.RewardMilestoneScrollView.verticalNormalizedPosition
    end, function(newVal)
        self.RewardMilestoneScrollView.verticalNormalizedPosition = newVal
    end, 1, time)
    
    self._RewardMilestoneTween.onComplete = function()
        self:CheckPanelBestEnable()
    end
end

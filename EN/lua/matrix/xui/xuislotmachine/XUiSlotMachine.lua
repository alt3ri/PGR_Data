...
function XUiSlotMachine:OnStart()
    self.RewardPanel = XUiSlotMachineRewardPanel.New(self.RewardBg, self, self)
    self.RewardPanel:Open()
...
function XUiSlotMachine:RefreshBg()
    if self.CurMachineEntity then
        local curId = self.CurMachineEntity:GetId()

        local bg = self["BgImage" .. curId]
...
function XUiSlotMachine:DrawCallBack(rockResults, targetRockTimes)
    ...
    RunAsyn(function()
        ...
        asyncPlayAnim("SlotmachineEnable2")
        ...
        for _, info in pairs(rockResults) do
            ...
            if self.MachinePanel:GetIsSkipAnim() then
                self.MachinePanel:ShowIconCardTmp()
                break
            end

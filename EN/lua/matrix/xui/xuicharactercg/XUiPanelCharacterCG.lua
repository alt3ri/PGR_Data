...
function XUiPanelCharacterCG:InitLoadVideoPlayerPrefab()
    ...
    self.VideoPlayer = XLuaVideoManager.LoadVideoPlayerUguiWithPrefab(self.VideoPlayerRoot)
    -- 目前CG的制作规格
    if not XTool.UObjIsNil(self.VideoPlayer) then
...
function XUiPanelCharacterCG:ChangeCGState(pause)
    if self:IsCGPlaying() then
        if pause then
            if not self:IsCGPause() then
                self.VideoPlayer:Pause()
            end

        else
            if self:IsCGPause() then
...
function XUiPanelCharacterCG:ReplayCG()
    if not XTool.IsNumberValid(self.LastPlayId) then
        return
    end

    if not self:IsCGShow() then
        self:PlayCG(self.LastPlayId)
        return
    end

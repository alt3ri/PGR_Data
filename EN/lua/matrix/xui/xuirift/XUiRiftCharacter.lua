...
function XUiRiftCharacter:OnStart(isMultiTeam, xTeam, teamPos, hideTeamPefabBtn)
    ...
    self.XTeam = xTeam or self._Control:GetSingleTeamData()
    ...
    if xTeam and teamPos then
        ...
        local xRole = self._Control:GetEntityRoleById(roleId)
        if xRole then
...
function XUiRiftCharacter:InitFilter()
    ...
    local onSeleCb = function(character, index, grid, isFirstSelect)
        ...
        local xRole = self._Control:GetEntityRoleById(character.Id)
        self:OnRoleSelected(xRole)
    end
...
function XUiRiftCharacter:RefresFilter()
    ...
    appendArray(list, self._Control:GetRobot())
...
function XUiRiftCharacter:UpdateRightCharacterInfo()
    ...
    local elementIcons = XMVCA.XCharacter:GetCharacterAllElement(characterId, true)
    self:RefreshTemplateGrids(self.RImgCharElement, elementIcons, self.BtnElementDetail.transform, nil, "UiRiftCharacterElement", function(grid, data)
        local icon = XMVCA.XCharacter:GetCharElement(data).Icon
        grid.RImgCharElement:SetRawImage(icon)
    end)

    -- 负载信息
    self.TxtLoadNum.text = CS.XTextManager.GetText("RiftPluginLoad", self._Control:GetCurrentLoad(self.CurrSelectRole), self._Control:GetMaxLoad())
    self.ImgLoadProgress.fillAmount = self._Control:GetCurrentLoad(self.CurrSelectRole) / self._Control:GetMaxLoad()
    ...
    local pluginIds = self._Control:GetRolePlugInIdList(self.CurrSelectRole)
    if pluginIds then
        for k, pluginId in pairs(pluginIds) do
            ...
            if not grid then
                ...
                grid = XUiRiftPluginGrid.New(uiGo, self)
                grid:Init(function ()
                    self:OnBtnAddClick()
                end) -- 加号和格子点击功能一致
...
function XUiRiftCharacter:CheckLimitBeforeChangeTeam(roleId, pos)
    ...
    local isInTeam, seleRoleTeam, seleRolePos = self._Control:CheckRoleInTeam(roleId)
    if self.XTeam ~= self._Control:GetSingleTeamData() then -- 多队伍需要进行的检测
        -- 多队伍压制锁定
        local isMultiEditLock = self._Control:CheckRoleInMultiTeamLock(seleRoleTeam)
        ...
        for k, xTeam in pairs(self._Control:GetMultiTeamData()) do
    ...
    if self.IsMultiTeam and isInTeam and seleRoleTeam ~= self.XTeam then
        ...
        local characterName = XMVCA.XCharacter:GetCharacterName(self._Control:GetEntityRoleById(targetSeleRoleId):GetCharacterId())
        ...
        XUiManager.DialogTip(
                ...
                function ()
                    TipCount = TipCount - 1
                    -- 在别的队伍中，可以交换
                    self._Control:SwapMultiTeamMember(seleRoleTeam:GetId(), seleRolePos, self.XTeam:GetId(), pos)
                    CloseTeamPrefabCb()
                end
        )
    end

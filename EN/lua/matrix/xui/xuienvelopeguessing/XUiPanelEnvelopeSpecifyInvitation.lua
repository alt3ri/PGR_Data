...
---@class XUiGridEnvelopeSpecifyInvitation : XUiNode
---@field private _Control XEnvelopeGuessingControl
local XUiGridEnvelopeSpecifyInvitation = XClass(XUiNode, "XUiGridEnvelopeSpecifyInvitation")

function XUiGridEnvelopeSpecifyInvitation:SetData(data, onClicked, selectedRole)
    self._Data = data
    self.BtnRole:AddEventListener(function() onClicked(data) end)
...
---@class XUiPanelEnvelopeSpecifyInvitation : XUiNode
---@field private _Control XEnvelopeGuessingControl
---@field Parent XUiEnvelopeGuessingInvitation
local XUiPanelEnvelopeSpecifyInvitation = XClass(XUiNode, "XUiPanelEnvelopeSpecifyInvitation")

function XUiPanelEnvelopeSpecifyInvitation:OnStart(setupConfirmButton, closeParent)
    ...
    self._Card = {}
    XTool.InitUiObjectByInstance(self.UiEnvelopeGuessingSpecifyCard, self._Card)
end

function XUiPanelEnvelopeSpecifyInvitation:OnEnable()
    self._Characters = XTool.FilterList(self._Control:GetAllCharacterConfigs(), function(conf)
        return not self._Control:IsCharacterOpened(conf.Id)
    end)
...
function XUiPanelEnvelopeSpecifyInvitation:_RefreshCard()
    ...
    self._Card.TxtSpecifyCardTip.text = CS.XTextManager.GetText("EnvelopeGuessingCharacterCardSpecifyCardTip", data.CharacterName)
    self._Card.RImgPackagHead:SetRawImage(data.HeadIcon)
end

---@param grid XUiGridEnvelopeSpecifyInvitation
function XUiPanelEnvelopeSpecifyInvitation:OnDynamicTableEvent(evt, index, grid)
    if evt == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_ATINDEX then
        grid:SetData(self._Characters[index], handler(self, self._SelectRole), self._SelectedRole)
    end
end
...
function XUiPanelEnvelopeSpecifyInvitation:_OnConfirmClicked(fastOpen)
    ...
    local characterId = self._SelectedRole.Id
    self._Control:EnvelopeSelectOpenRequest(characterId, function()
        self.Parent:ReportOpenCard(self.Parent.ReportOpenType.Specify, characterId, fastOpen, 0)
        XLuaUiManager.Open("UiEnvelopeGuessingOpenPackage", XUiEnvelopeGuessingOpenPackage.OpenType.Specify, self._SelectedRole, fastOpen)
    end)
end

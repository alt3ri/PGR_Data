...
---@class XUiGridEnvelopeGuessingTaskType : XUiNode
---@field private _Control XEnvelopeGuessingControl
local XUiGridEnvelopeGuessingTaskType = XClass(XUiNode, "XUiGridEnvelopeGuessingTaskType")

---@class XUiGridEnvelopeGuessingTask : XUiNode
---@field private _Control XEnvelopeGuessingControl
local XUiGridEnvelopeGuessingTask = XClass(XUiNode, "XUiGridEnvelopeGuessingTask")

function XUiGridEnvelopeGuessingTask:OnStart()
    self.BtnSkip:AddEventListener(handler(self, self.OnBtnSkipClick))
end

function XUiGridEnvelopeGuessingTask:OnBtnSkipClick()
...
function XUiGridEnvelopeGuessingTask:SetData(data, args)
    ...
    self.BtnReceive:AddEventListener(args.OnReceiveClick)
    ...
    if data.State == XDataCenter.TaskManager.TaskState.Achieved then
        ...
        self.BtnReceive:SetName(CS.XTextManager.GetText("EnvelopeGuessingTaskButtonFinish"))
    elseif data.State == XDataCenter.TaskManager.TaskState.Finish then
        self.BtnReceive.gameObject:SetActiveEx(true)
        self.BtnReceive:SetDisable(true, false)
        self.BtnReceive:SetName(CS.XTextManager.GetText(args.TaskType.TaskButtonArchivedText))
    else
        ...
        if cfg.SkipId and cfg.SkipId ~= 0 then
            ...
            self.BtnSkip:SetName(CS.XTextManager.GetText("EnvelopeGuessingTaskButtonSkip"))
        else
            self.BtnSkip:SetButtonState(CS.UiButtonState.Disable)
            self.BtnSkip:SetName(CS.XTextManager.GetText("EnvelopeGuessingTaskButtonNotFinished"))
        end
    end
end
...
function XUiGridEnvelopeGuessingTaskType:SetData(taskType, onReceiveClick)
    ...
    self._GridArg = { TaskType = taskType, OnReceiveClick = onReceiveClick }
end

...
function XUiPanelRogueSimRoundSettlement:RefreshEvent()
    ...
    for i, id in ipairs(eventIds) do
        ...
        local isDurEvent = self._Control.MapSubControl:CheckIsEventDurationById(id)
        local remainDur = self._Control.MapSubControl:GetEventRemainingDuration(id)
        local uiObject = btn.transform:GetComponent("UiObject")
        uiObject:GetObject("ImgNormalNum").gameObject:SetActiveEx(isDurEvent)
        uiObject:GetObject("TxtNormalNum").text = tostring(remainDur)
        uiObject:GetObject("ImgPressNum").gameObject:SetActiveEx(isDurEvent)
        uiObject:GetObject("TxtPressNum").text = tostring(remainDur)
    ...
    for _, reward in pairs(rewards) do
        ...
        local uiObject = btn.transform:GetComponent("UiObject")
        uiObject:GetObject("ImgNormalNum").gameObject:SetActiveEx(false)
        uiObject:GetObject("ImgPressNum").gameObject:SetActiveEx(false)
        btn.gameObject:SetActiveEx(true)
        btn.transform:SetAsLastSibling()

...
function XUiExhibitionOverclockingV2P6:OnEnable()
    ...
    self:UpdateView()
    self.BtnQuickOverclocking.gameObject:SetActiveEx(equip.CharacterId == self.CharacterId)
end
...
function XUiExhibitionOverclockingV2P6:SetButtonCallBack()
    ...
    self:RegisterClickEvent(self.BtnQuickOverclocking, self.OnBtnQuickOverclockingClick)
end
...
function XUiExhibitionOverclockingV2P6:RefreshByPos(pos)
    ...
    local characterId = XMVCA.XEquip:GetResonanceBindCharacterId(self.EquipId, pos)
    ...
    local descList = XMVCA.XEquip:GetAwakeSkillDesList(self.EquipId, pos)
    ...
    local skillInfo = XMVCA.XEquip:GetResonanceSkillInfo(self.EquipId, pos)
    ...
    -- 公约加成
    local harmStr = XMVCA.XEquip:GetEquipAwarenessOccupyHarmDesc(self.EquipId, 1)
...
function XUiExhibitionOverclockingV2P6:OnBtnAwakeClick(pos)
    if not XMVCA.XEquip:CheckEquipCanAwake(self.EquipId, pos) then
...
function XUiExhibitionOverclockingV2P6:OnBtnQuickOverclockingClick()
    XLuaUiManager.Open("UiEquipResonanceQuick", self.CharacterId, true)
end

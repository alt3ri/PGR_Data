...
---@field PnlTxtLayout UnityEngine.RectTransform 文案布局容器（设文本后强制刷新布局，防文字长度变化后布局滞后）
...
function XUiPanelPunishaarGuideTips:Show(hudCfg)
    ...
    -- 设文本后强制刷新布局（防文字长度变化后 LayoutGroup 滞后，导致气泡尺寸/位置不跟随）
    if self.PnlTxtLayout then
        CS.UnityEngine.UI.LayoutRebuilder.ForceRebuildLayoutImmediate(self.PnlTxtLayout)
    end
    self:Open()

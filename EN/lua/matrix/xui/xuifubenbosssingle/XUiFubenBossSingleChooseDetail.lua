...
function XUiFubenBossSingleChooseDetail:OnPanelTabClick(index)
    ...
    local bossIcon = self._Control:GetBossIcon(self._BossList[index])
    local sectionInfo = self._Control:GetBossSectionInfoByBossId(self._BossList[index])

    if bossIcon then
        self.ImgBoss:SetRawImage(bossIcon)
...
function XUiFubenBossSingleChooseDetail:_Init(bossBtnList)
    for i = 1, #self._BossList do
        local bossName = self._Control:GetBossName(self._BossList[i])

        bossBtnList[i]:SetNameByGroup(0, bossName)
    end
end

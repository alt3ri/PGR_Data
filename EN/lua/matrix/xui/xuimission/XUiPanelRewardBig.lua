local XUiPanelRewardBig = XClass(nil, "XUiPanelRewardBig")
...
function XUiPanelRewardBig:SetupReward(result)
    ...
    local start = 0
    if rewardBig then
        for _, item in ipairs(rewardBig) do
            start = start + 1
            local grid
    ...
    if rewardExtra then
        for _, item in ipairs(rewardExtra) do
            start = start + 1
            local grid
            if self.GridList[start] then
                grid = self.GridList[start]
...
function XUiPanelRewardBig:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiPanelRewardBig:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end

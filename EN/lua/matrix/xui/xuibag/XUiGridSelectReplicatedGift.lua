...
function XUiGridSelectReplicatedGift:RefreshSelf(NeedDefulatQulity, isSmallIcon, notCommonBg)
    ...
    if template.RewardType == XRewardManager.XRewardType.Partner then
        quality = XMVCA.XPartner.Util.GetGoodsQualityByPartnerQuality(quality)
    end
...
function XUiGridSelectReplicatedGift:SetSelectLongClickCallBack(callback)
    self.SelectLongClickCallBack = callback
end

function XUiGridSelectReplicatedGift:BtnAddSelectLongClickCallback(time)
    local count = 0
    if self.SelectLongClickCallBack then
        count = self.SelectLongClickCallBack(time)
    end

...
-- 新手任务二期
local XUiNewbieSelectGift = XLuaUiManager.Register(XLuaUi, "UiNewbieSelectGift")
local CsGetText = CS.XTextManager.GetText

function XUiNewbieSelectGift:OnAwake()
    self:RegisterUiEvents()
    self.RewardItems = {}
end

function XUiNewbieSelectGift:OnStart(id, closeCb)
    self.ItemId = id
    ...
    self:PlayAnimation("AnimSelectGiftEnable")
end

function XUiNewbieSelectGift:SelectRewardGrid(gridData, grid)
...
function XUiNewbieSelectGift:OpenDetailUi(data)
...
function XUiNewbieSelectGift:HandleConfirmClick(itemId ,rewardIds, rewardHasOwnTypeList)
...
function XUiNewbieSelectGift:RegisterUiEvents()
    XUiHelper.RegisterClickEvent(self, self.BtnClose, self.OnBtnBackClick)
    XUiHelper.RegisterClickEvent(self, self.BtnCloseAllScreen, self.OnBtnBackClick)
...
function XUiNewbieSelectGift:OnBtnBackClick()
    self:Close()
...
function XUiNewbieSelectGift:OnBtnConfirmClick()
...
function XUiNewbieSelectGift:OnBtnClearSelectionClick()
    for _, index in pairs(self.SelectGridIndexs)do

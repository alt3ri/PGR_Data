...
function XUiPacMan2PopupStageDetail:PlayAnimationTarget()
    for i = 1, #self._GridsTarget do
        local grid = self._GridsTarget[i]
        grid.GameObject:SetActiveEx(false)
        grid:Tween(0.2 * i, function()
            grid.GameObject:SetActiveEx(true)
            grid:PlayAnimation("GridTargetEnbale")
        end)
    end
end

...
---@field BtnWaitingEnter XUiComponent.XUiButton
...
function XUiDlcCasualplayerRoomCute:OnAwake()
    ...
    self._BtnGroup = {
        ...
        [ButtonState.WaitingEnter] = self.BtnWaitingEnter.gameObject,
    }
    self:_Init()
...
function XUiDlcCasualplayerRoomCute:_RegisterButtonClicks()
    ...
    self:RegisterClickEvent(self.BtnWaitingEnter, self.OnBtnWaitingEnterClick, true)
...
function XUiDlcCasualplayerRoomCute:_RegisterListeners()
    ...
    XEventManager.AddEventListener(XEventId.EVENT_DLC_ROOM_STATE_CHANGE, self.OnRoomStateChange, self)
    XEventManager.AddEventListener(XEventId.EVENT_CHAT_RECEIVE_ROOM_MSG, self.RefreshChatMsg, self)
    XEventManager.AddEventListener(XEventId.EVENT_DLC_ROOM_WORLD_CHANGE, self.OnRoomWorldChange, self)
end
...
function XUiDlcCasualplayerRoomCute:_RemoveListeners()
    ...
    XEventManager.RemoveEventListener(XEventId.EVENT_DLC_ROOM_STATE_CHANGE, self.OnRoomStateChange, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_CHAT_RECEIVE_ROOM_MSG, self.RefreshChatMsg, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_DLC_ROOM_WORLD_CHANGE, self.OnRoomWorldChange, self)
end
...
function XUiDlcCasualplayerRoomCute:_RefreshButtonStatus()
    ...
    if member and member:IsLeader() then
        if self._Team:IsAllReady() then
            if self._Team:IsEnough() then
                self:_SwitchButtonState(ButtonState.Fight)
            else
                self:_SwitchButtonState(ButtonState.WaitingEnter)
            end
        else
...
function XUiDlcCasualplayerRoomCute:OnBtnWaitingEnterClick()
    XUiManager.TipMsg(string.format(XUiHelper.GetText("OnlineRoomLeastPlayer"), self._Team:GetMinMemeberNumber()))
end
...
function XUiDlcCasualplayerRoomCute:OnRoomInfoChange(roomData, changeFlags)
    if changeFlags.IsWorldIdChange then
        ...
        local msgText = self._Control:GetChapterNameByWorldId(worldId)

        self:_RefreshWorldMode(not isDifficulty)
        self._Control:SetDifficultyMode(isDifficulty)
        XUiManager.TipText("DlcCasualPlayerRoomChangeWorld", nil, true, msgText)
    end
...
function XUiDlcCasualplayerRoomCute:OnRoomWorldChange(worldId, levelId)
    local isDifficulty = self._Control:CheckDifficultyWorld(worldId)
    local msgText = self._Control:GetChapterNameByWorldId(worldId)
...
function XUiDlcCasualplayerRoomCute:OnRoomStateChange()
    self:_CheckLeaderCountDown()
end

...
function XPracticeConfigs.GetPracticeChapterById(id)
    ...
    if not XTool.IsTableEmpty(currentChapter.Groups) then
        ...
        for k, charId in pairs(currentChapter.Groups) do
            local config = XMVCA.XFavorability:GetModelGetCharacterCollaboration(charId)
            if config and not config.IsNewCharacter and not XMVCA.XCharacter:IsOwnCharacter(charId) then

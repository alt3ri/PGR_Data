...
local QualityColor = {
    CS.XGame.ClientConfig:GetString("QualityColor1"),
    CS.XGame.ClientConfig:GetString("QualityColor2"),
    CS.XGame.ClientConfig:GetString("QualityColor3"),
    CS.XGame.ClientConfig:GetString("QualityColor4"),
    CS.XGame.ClientConfig:GetString("QualityColor5"),
    CS.XGame.ClientConfig:GetString("QualityColor6"),
    CS.XGame.ClientConfig:GetString("QualityColor7"),
}
...
function XArrangeConfigs.GeQualityPath(quality)
    if not quality then
        XLog.Error("XArrangeConfigs.GeQualityPath 函数错误: 参数quality不能为空")
        return
    end
    return QualityPath[quality]
end

function XArrangeConfigs.GeQualityColor(quality)
    if not quality then
        XLog.Error("XArrangeConfigs.GeQualityColor 函数错误: 参数quality不能为空")
        return
    end
    return XUiHelper.Hexcolor2Color(QualityColor[quality])
end

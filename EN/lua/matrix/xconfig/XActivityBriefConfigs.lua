...
---根据主副面板Id获取标题变换条件Id(0表示不启用)，副标题与特效标题共用此条件
---@param panelType integer
---@return integer
function XActivityBriefConfigs.GetTitleDiffCondition(panelType)
    local config = ActivityTemplates[panelType]
    if XTool.IsTableEmpty(config) then return 0 end
    return config.TitleDiffCondition or 0
end
...
function XActivityBriefConfigs.GetActivityShopByShopId(shopId)
    for i, v in pairs(ActivityShopTemplates) do
...
function XActivityBriefConfigs.GetActivityBriefGroupRedParam(groupId)
    local config = XActivityBriefConfigs.GetActivityGroupConfig(groupId)
    return config and config.RedParam or nil
end

---V4.2 白龙战斗 简单模式
local XLevelScript9002 = XDlcScriptManager.RegLevelLogicScript(9002, "XLevelLogicScript9002") --注册脚本类到管理器（逻辑脚本注册
...
---@param phase number
function XLevelScript9002:SetPhase(phase)    --跳转关卡阶段
...
function XLevelScript9002:OnEnterPhase(phase)
...
function XLevelScript9002:OnExitPhase(phase)
    --退出一个关卡阶段时需要做的事情在这里实现（最好不要在这里跳转关卡阶段

end
function XLevelScript9002:OnPhaseChanged(lastPhase, nextPhase)
    --关卡阶段改变时需要执行的逻辑，一般用于通知外部
end
function XLevelScript9002:HandlePhaseEvent(eventType, eventArgs)
...
function XLevelScript9002:LevelEnd(isPlayerWin)

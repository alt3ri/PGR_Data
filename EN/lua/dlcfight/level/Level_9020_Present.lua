--relink 木桩关
local XLevelScript9020 = XDlcScriptManager.RegLevelPresentScript(9020, "XLevelPresentScript9020")
local Timer = require("Level/Common/XTaskScheduler")


-- 脚本构造函数
---@param proxy XDlcCSharpFuncs
function XLevelScript9020:Ctor(proxy)
    self._proxy = proxy
    self._timer = Timer.New()
end

-- 初始化
function XLevelScript9020:Init()
...
function XLevelScript9020:HandleEvent(eventType, eventArgs)
end

-- 每帧执行
---@param dt number @ delta time
function XLevelScript9020:Update(dt)
...
-- 脚本终止
function XLevelScript9020:Terminate()

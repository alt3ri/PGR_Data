...
---@class XBuffScript1015558 : XBuffBase
local XBuffScript1015558 = XDlcScriptManager.RegBuffScript(1015558, "XBuffScript1015558", Base)
--效果说明：每间隔10秒，触发一次雷伤+30%的效果，持续5秒;
--1015592,释放技能后，1015550的触发时间提前1秒
--1015590，触发两次1015550后，基础触发时间缩短至5秒
function XBuffScript1015558:Init() --初始化
    Base.Init(self)
    ------------配置------------
    self.BuffTimer =  0 --初始计时器
    self.BuffId = 1015559   --持续5秒的雷伤+30%的buffid
...
function XBuffScript1015558:Update(dt) --每帧执行
    Base.Update(self, dt)
    ------------执行------------
...
function XBuffScript1015558:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEvent(EWorldEvent.NpcCastActionAfter)         -- OnNpcCastSkillEvent
end

function XBuffScript1015558:OnNpcCastActionAfterEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort)
    --未持有技能减cd的buff直接返回
...
function XBuffScript1015558:HandleEvent(eventType, eventArgs)
    Base.HandleEvent(self, eventType, eventArgs)
end

function XBuffScript1015558:Terminate()
    Base.Terminate(self)
end

return XBuffScript1015558

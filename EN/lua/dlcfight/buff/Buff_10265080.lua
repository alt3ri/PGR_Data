...
---@class XBuffScript10265080 : XTheatre6SkillBase
local XBuffScript10265080 = XDlcScriptManager.RegBuffScript(10265080, "XBuffScript10265080", XTheatre6SkillBase)

--效果说明：
--· 每使用过一次白色技能，本技能伤害提升10%攻击。

function XBuffScript10265080:ScriptInit(isGainControl) --初始化
    self.TargetSkill = self._skillId
    --self:LogError(".....初始化完成")
    self.Count = 0
    self._critController = self:GetNpc():GetCritController()
    self._damageMagicId = 1026816 --注册伤害id
...
function XBuffScript10265080:OnLuaSkillStart(eventArgs)
    ------------执行------------
...
function XBuffScript10265080:OnLuaSkillEnd(eventArgs)
    if not self.isCount then return end
    self.Count = self.Count + 1
    self.isCount = false
end

function XBuffScript10265080:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEventByTarget(EWorldEvent.NpcCalcDamageBefore, self._npcUUID)
end

function XBuffScript10265080:ChangeDamageBeforeCalc(eventArgs)

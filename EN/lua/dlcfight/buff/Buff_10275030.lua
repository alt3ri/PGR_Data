...
---@class XBuffScript10275030 : XTheatre6SkillBase
local XBuffScript10275030 = XDlcScriptManager.RegBuffScript(10275030, "XBuffScript10275030", XTheatre6SkillBase)

--效果说明：
--· 【体力】属性>120点时，伤害额外提高20/25/30%攻击。

function XBuffScript10275030:ScriptInit(isGainControl) --初始化
    self.TargetSkill = self._skillId
    self.TargetTL = 120
    --self:LogError(".....初始化完成")
    self._damageMagicId = 10270209 --注册伤害id
    --self._proxy:Theatre6ChangeStaminaValue(self._npcUUID, -30, 0)
    if self._skillId == 10275031 then self._exDamageRate = 2000
    else if self._skillId == 10275032 then self._exDamageRate = 2500
...
function XBuffScript10275030:OnLuaSkillStart(eventArgs)
    ------------执行------------
...
function XBuffScript10275030:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEventByTarget(EWorldEvent.NpcCalcDamageBefore, self._npcUUID)
end

function XBuffScript10275030:ChangeDamageBeforeCalc(eventArgs)

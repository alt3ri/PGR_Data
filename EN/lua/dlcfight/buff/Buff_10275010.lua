...
---@class XBuffScript10275010 : XTheatre6SkillBase
local XBuffScript10275010 = XDlcScriptManager.RegBuffScript(10275010, "XBuffScript10275010", XTheatre6SkillBase)

--效果说明：
--· 【拼刀】属性>230点时，伤害额外提高20/25/30%攻击。

function XBuffScript10275010:ScriptInit(isGainControl) --初始化
    self.TargetSkill = self._skillId
    self.TargetPD = 230
    --self:LogError(".....初始化完成")
    self._damageMagicId = 10270201 --注册超算成功技1伤害id
    --self._proxy:Theatre6ChangeStaminaValue(self._npcUUID, -30, 0)
    self._exDamageRate = 2000
    if self._skillId == 10275011 then self._exDamageRate = 2000
    else if self._skillId == 10275012 then self._exDamageRate = 2500
...
function XBuffScript10275010:OnLuaSkillStart(eventArgs)
    ------------执行------------
...
function XBuffScript10275010:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEventByTarget(EWorldEvent.NpcCalcDamageBefore, self._npcUUID)
end

function XBuffScript10275010:ChangeDamageBeforeCalc(eventArgs)

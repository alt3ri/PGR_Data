...
function XBuffScript8060001:Update(dt)
    --每帧执行
    Base.Update(self, dt)
    ------------执行------------
    ...
    if self.hasLevel==false then
        self.hasLevel,self.magicLevel=self._proxy:TryQueryBuffLevel(self._uuid,8060001)--获取自身的BUFF等级
    end

    local hpRate =self._proxy :GetNpcAttribRate(self._uuid,0) --血量比例
    if hpRate>self.hpRate01 and self.openBuff==true then --血量大于就移除BUFF
...
function XBuffScript8060001:InitEventCallBackRegister()
    --按需求解除注释进行注册
end
...
function XBuffScript8060001:HandleEvent(eventType, eventArgs)
    Base.HandleEvent(self, eventType, eventArgs)
end

function XBuffScript8060001:Terminate()
    Base.Terminate(self)
end

return XBuffScript8060001

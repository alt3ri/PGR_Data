local XLevelScriptXXX = XDlcScriptManager.RegLevelLogicScript(0000) --注册脚本类到管理器（逻辑脚本注册
--local XLevelScriptXXX = XDlcScriptManager.RegLevelPresentScript(0000) --表现脚本注册

---@param proxy XDlcCSharpFuncs
function XLevelScriptXXX:Ctor(proxy) --构造函数，用于执行与外部无关的内部构造逻辑（例如：创建内部变量等）
    --脚本代理对象，通过它来调用战斗程序开放的函数接口。
...
function XLevelScriptXXX:SetPhase(phase) --跳转关卡阶段
    ...
    self:OnPhaseChanged(self._lastPhase, self._currentPhase)
end
...
---@param dt number @ delta time
function XLevelScriptXXX:OnUpdatePhase(dt) --当前关卡阶段需要一直执行的逻辑在这里实现（一般在这里跳转关卡阶段

...
---肉鸽6囚徒脚本
---@class XChar8227:XTheatre6CharBase
local XChar8227 = XDlcScriptManager.RegCharScript(8227, "XChar8227", XTheatre6CharBase)

---@class XChar8227.State:XTheatre6State
---@field _owner XChar8227
---@field _stateMachine XChar8227.StateMachine

---@class XChar8227.StateMachine:XTheatre6StateMachine

--注册状态机
local StateMachine, States = XTheatre6CharBase:CreateClasses("XChar8227")

XChar8227.StateMachine = StateMachine --[[@as XChar8227.StateMachine]]
XChar8227.States = States --[[@as table<string|integer, XChar8227.State>]]

--注册拼刀、超算技能
-- States.Wrestle.WrestleSkillIdLeft = 1025001 -- 拼刀start动作 左 (fighter1 怪物不需要
States.Wrestle.WrestleSkillIdRight = 8227009 -- 拼刀start动作 右 (fighter2
-- States.Wrestle.WrestleSkillIdLeftCountinue = 1025006 -- 拼刀僵持动作 左 (fighter1 怪物不需要
States.Wrestle.WrestleSkillIdRightCountinue = 8227010 -- 拼刀僵持动作 右 (fighter2
-- States.Wrestle.PindaoStart2LCamera = 10250102 -- 拼刀start冲刺特写镜头动画buff 左(fighter1 怪物不需要
States.Wrestle.PindaoStart2RCamera = 8227021 -- 拼刀start冲刺特写镜头动画buff 右(fighter2
States.Wrestle.SucceedActionId = 8227011 -- 拼刀成功动作
States.Wrestle.SecondWrestleReset = 8227014 -- 二次拼刀位置重置动作
States.Dodge.DodgeSkillId = 8227012  -- 超算受身动作
States.Dodge.SucceedActionId = 8227013 --超算受身成功反击
-- States.Block.ActionId = 1025009 -- 格挡动作 (怪物不需要

function XChar8227:_BaseInit()
    XTheatre6CharBase._BaseInit(self)
    self._skillTimer = 0 
    self._insertSkillTriggerTimer = 6 + math.random(10)
end
--注册脚本事件
function XChar8227:InitEventCallBackRegister()
    --囚徒独特注册脚本
...
function XChar8227:HandleEvent(eventType, eventArgs)
...
-- function XChar8227:OnNpcCastActionAfterEvent(skillActionId, launcherId, targetId, targetSceneObjId, isAbort)

...
---首席指挥官角色脚本
---@class XChar8092 : XRelinkCharBase
local XChar8092 = XDlcScriptManager.RegCharScript(8092, "XChar8092", Base)

--[[function XChar8092:Init() -- 初始化
    Base.Init(self)
    self.Move = true
    self.Guangjian = true
...
function XChar8092:ScriptInit(isGainControl)
    self.Move = true
    self.Guangjian = true
...
function XChar8092:InitEventCallBackRegister()
...
--[[function XChar8092:AfterDamageCalc(eventArgs)   -- 计算伤害后
...
function XChar8092:OnNpcCastActionBeforeEvent(SkillId, LauncherId, TargetId, TargetSceneObjId, IsAbort)


end

function XChar8092:OnNpcCastActionAfterEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort)
...
function XChar8092:ChouShouCheck() --获取左右触手uuid
...
function XChar8092:ClassCheck() --按职业区分目标
    XLog.Warning("开始职业检测")
    self.Tuuid = nil
    self.Cuuid = nil
    self.Huuid = nil
...
--------------------------------技能组逻辑---------------------------------------------
function XChar8092:JiGuangSkill3() --触手炼狱启动
...
function XChar8092:JiGuangSkill1() --激光技能组1
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8092:JiGuangSkill2() --激光技能组2
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8092:NormalSkill1() --常规技能组1
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8092:ODSkill1() --OD技能组
...
function XChar8092:ODJiHuo() --OD激活
    local Target = self.PlayUUID[1]
    self._proxy:CastActionToTarget(self._uuid,805323,Target) -- OD激活技能
end


function XChar8092:HeiQiuSkill1() --黑球技能组
...
function XChar8092:XiShou() --千子吸收
...
function XChar8092:ChuShouSkill1() --左触手拍打
...
function XChar8092:ChuShouSkill2() --左触手横扫
...
function XChar8092:ChuShouSkill3() --左触手突刺
...
function XChar8092:ChuShouSkill4() --触手投掷
...
--[[function XChar8092:ChuShouZhaoHuan() --召唤触手1
    local Target = self.PlayUUID[1]
...
function XChar8092:ChuShouLianYu() --触手炼狱
    local Target = self.PlayUUID[1]
    for i = 1,#self.PlayUUID, 1 do
...
function XChar8092:HandleLuaEvent(eventType, eventArgs)
    -- 响应AI开启和停止
...
function XChar8092:OnNpcExitActionEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort) -- 技能释放完成时
    if launcherId ~= self._uuid then
        return
    end
...
function XChar8092:OnNpcAddBuffEvent(casterNpcUUID, npcUUID, buffId, buffKinds, buffUUId)
...
function XChar8092:OnNpcSkillActionKeyframeSendEvent(launcher, eventName, skillActionId, keyFrameId, skillId)
...
function XChar8092:OnNpcOverDriveFull(targetUUID) -- OD已满事件
...
function XChar8092:OnNpcODBreakBefore(targetUUID)  -- Break前事件
    if targetUUID ~= self._uuid then
        return
    end
    local Target = self.PlayUUID[1]
...
function XChar8092:OnNpcODExitBreakAfter(targetUUID) -- 退出Break后
    if targetUUID ~= self._uuid then
        return
    end
    local Target = self.PlayUUID[1]
...
function XChar8092:OnNpcDamageEvent(launcherId, targetId, magicId, kind, physicalDamage, elementDamage, elementType, realDamage, isCritical, skillActionId, magicTags)
...
function XChar8092:OnNpcBrokenAfter(launcherUUID, targetUUID, magicId)  --破韧
    local Target = self.PlayUUID[1]
    XLog.Warning("破韧成功")

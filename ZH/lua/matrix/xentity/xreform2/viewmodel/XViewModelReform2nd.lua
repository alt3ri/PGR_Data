...
function XViewModelReform2nd:InitCurrentStageIndex()
    ...
    for i = 1, #stageIds do
        ...
        if stage and stage:GetIsSelect() then
...
function XViewModelReform2nd:GetTaskDataByIndex(index)
    ...
    for i = 1, self.ChapterTotalNumber do
        ...
        nowStar = nowStar + self._Model:GetChapterStarNumber(chapter, true, true)
                + self._Model:GetChapterStarNumber(chapter, true, false)
    end
...
function XViewModelReform2nd:GetCurrentStage()
    local chapter = self._Model:GetChapterByIndex(self.CurrentChapterIndex)
    -- 由于关卡内去掉了难度选择的逻辑, 所以在传入stage的时候, 需要传入正确的stage
    --local stageIds = self._Model:GetChapterStageIdById(chapter:GetId())
    --local stageId = stageIds[self.CurrentStageIndex]
    --local stage = self._Model:GetStage(stageId)
    local stage = chapter:GetStageByDifficulty(self._Model, self._Model:GetIsSelectHardMode())
...
function XViewModelReform2nd:GetChapterLockedTipByIndex(index)
    ...
    local timeId = self._Model:GetChapterOpenTimeByChapter(chapter, self._Model:GetIsSelectHardMode())
...
function XViewModelReform2nd:GetHardChapterLockedTipByIndex(index)
    local chapter = self._Model:GetChapterByIndex(index)
    if chapter:IsHasStageHard(self._Model) then
        if not chapter:IsShowToggleHard(self._Model) then
            return XUiHelper.GetText("ReformLockDifficulty")
        end
    end
    return self:GetChapterLockedTipByIndex(index)
end

function XViewModelReform2nd:GetChapterIsUnlockedByIndex(index)
    local chapter = self._Model:GetChapterByIndex(index)
    return self._Model:IsChapterUnlocked(chapter, self._Model:GetIsSelectHardMode())
end

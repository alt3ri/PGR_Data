...
function XReform2ndMobGroup:IsMobSelected(mob)
...
function XReform2ndMobGroup:GetMobCanSelectIndex(mob)
    if not mob then
        return false
    end
    local mobCanSelect = self:GetMobCanSelect()
    for i = 1, #mobCanSelect do
        if mobCanSelect[i]:GetId() == mob:GetId() then
            return i
        end
    end
    return false

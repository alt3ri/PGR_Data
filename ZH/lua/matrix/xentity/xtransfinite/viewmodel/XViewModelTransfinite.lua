...
function XViewModelTransfinite:Ctor()
    self.Data = {
        ...
        TextStartStageProgress = "", -- 开始关卡进度
        StageGroupIsAllPass = false, -- 关卡组是否全部通关
        TextPassRecordTime = "",     -- 通关记录时间
        ImgPassRecord = "",          -- 通关记录图标
    }
...
function XViewModelTransfinite:Update()
    ...
    data.TextStartStageProgress = stageGroup:GetNextStartProgress()
    data.StageGroupIsAllPass = stageGroup:CheckStageGroupIsPass()
    if data.StageGroupIsAllPass then
        local clearTime = stageGroup:GetBestClearTime()
        data.TextPassRecordTime = XUiHelper.GetTime(clearTime)
        local modelId = XTransfiniteConfigs.GetMedalIdByTime(clearTime, stageGroup:GetStartStageProgressId())
        data.ImgPassRecord = XTransfiniteConfigs.GetMedalIcon(modelId)
    end
end
...
function XViewModelTransfinite:UpdateIsland()
    ...
    local islandImages = XTransfiniteConfigs.GetIslandImage(self._Region:GetIslandId())
...
function XViewModelTransfinite:GetMedal(stageGroup)
    ...
    medal:SetTime(stageGroup:GetBestClearTime(), stageGroup:GetStartStageProgressId())
    return medal
end

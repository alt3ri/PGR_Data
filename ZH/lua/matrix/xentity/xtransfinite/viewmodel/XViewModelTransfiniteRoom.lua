    ...
    if stageGroup:IsHaveTeam() then
        ...
        for i = 1, XTeamConfig.MEMBER_AMOUNT do
            ...
            if member and member:IsValid() then
                ...
                local dataMember = {
                    Index = i,
                    Icon = XMVCA.XCharacter:GetCharLiberationAndDefaultHeadIcon(member:GetId()),
                    Hp = hp,
                    Sp = member:GetSp() / 100,
...
function XViewModelTransfiniteRoom:OnEnable()
    if self._IsOpenRoom then
        ...
        stageGroupTeam:SetEnterCgIndex(team:GetEnterCgIndex())
        stageGroupTeam:SetSettleCgIndex(team:GetSettleCgIndex())
        stageGroupTeam:UpdateSelectGeneralSkill(team:GetCurGeneralSkill())
        stageGroupTeam:Save()
        self._IsOpenRoom = false
    end
end
...
function XViewModelTransfiniteRoom:_Fight()
    ...
    if self._StageGroup:IsStageLock(stage) then
        XUiManager.TipMsg(self.Data.TxtStageLock)
        return
    end

...
local XGuildDormInteractInfo = require('XEntity/XGuildDorm/Datas/XGuildDormInteractInfo')
...
function XGDNpcManagerComponent:CreateRLNpc(npc)
    ...
    info.Id = npc:GetEntityId()
    info.ButtonType = XGuildDormConfig.FurnitureButtonType.Npc
    info.ButtonId = npc:GetEntityId()
    info.ShowButtonName = refreshConfig.InteractName
    info.InitPos = initPos
    info.BehaviorType = refreshConfig.TalkBehaviorId
    
    rlRole:AddInteractInfo(info)
    -- 添加组件
    -- 设置显示

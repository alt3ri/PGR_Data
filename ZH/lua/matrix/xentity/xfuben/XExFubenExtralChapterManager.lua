...
function XExFubenExtralChapterManager:ExOpenChapterUi(viewModel, difficulty)
    ...
    if isUnlock then
        ...
        if id and not XMVCA.XSubPackage:CheckSubpackage(XFunctionManager.FunctionName.ExtralChapter)then
            return
        end
...
function XExFubenExtralChapterManager:ExOpenChapterUiByStageId(stageId)
    local mainId = XFubenExtraChapterConfigs.GetStageMainId(stageId)
    ...
    if subChapterId ~= nil and subChapterId > 0 then
        result = CreateAnonClassInstance({
            ...
            CheckHasRedPoint = function(proxy)
                return XRedPointConditions.Check(XRedPointConditions.Types.CONDITION_EXTRA_CHAPTER_REWARD, proxy:GetId())
            end,

...
local function PayPC(self, productKey, cpOrderId, goodsId, cb)
    XHeroSdkManager.Pay(productKey, cpOrderId, goodsId, function(err)
        if err then
            TipPayFail()
            return
...
function XPayHeroAgent:Ctor()
    ...
    elseif Platform == RuntimePlatform.WindowsPlayer then 
        -- XLog.Error("XPayHeroAgent Ctor: RuntimePlatform.WindowsPlayer")
        XHeroSdkManager.RegisterPCCallback(function(err, orderId)
            self:OnPCPayCallback(err, orderId)
        end)
        self.Pay = PayPC
...
function XPayHeroAgent:OnPCPayCallback(err, orderId)
    if err then
        XLog.Error("PC Pay err:" .. tostring(err))
        TipPayFail()
        return
    end

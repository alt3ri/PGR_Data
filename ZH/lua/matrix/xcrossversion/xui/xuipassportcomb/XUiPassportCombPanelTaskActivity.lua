...
---@class XUiPassportCombPanelTaskActivity:XUiNode
local XUiPassportCombPanelTaskActivity = XClass(XUiNode, "XUiPassportCombPanelTaskActivity")

--活动任务
function XUiPassportCombPanelTaskActivity:Ctor(ui, rootUi)
    self.RootUi = rootUi
    ...
    self:AddRedPointEvent(self.BtnTongBlack, self.OnCheckTaskRedPoint, self, { XRedPointConditions.Types.CONDITION_PASSPORT_COMB_TASK_ACTIVITY_RED })
end

function XUiPassportCombPanelTaskActivity:Refresh()
...
function XUiPassportCombPanelTaskActivity:OnDynamicTableEvent(event, index, grid)
...
--一键领取
function XUiPassportCombPanelTaskActivity:OnBtnTongBlackClick()
    self._Control:FinishMultiTaskRequest(XEnumConst.PASSPORT.TASK_TYPE.ACTIVITY)
end

function XUiPassportCombPanelTaskActivity:OnCheckTaskRedPoint(count)
    self.BtnTongBlack:ShowReddot(count >= 0)
end

function XUiPassportCombPanelTaskActivity:Show()
    self.GameObject:SetActiveEx(true)
    self:Refresh()
end

function XUiPassportCombPanelTaskActivity:Hide()
    self.GameObject:SetActiveEx(false)
end

function XUiPassportCombPanelTaskActivity:IsShow()
...
return XUiPassportCombPanelTaskActivity
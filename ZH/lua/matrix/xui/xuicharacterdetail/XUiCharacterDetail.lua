...
function XUiCharacterDetail:OnStart(CharacterId)
    ...
    XEventManager.AddEventListener(XEventId.EVENT_CHARACTER_SKILL_UNLOCK, self.UpdateStateView, self)
    XEventManager.AddEventListener(XEventId.EVENT_CHARACTER_SKILL_UP, self.UpdateStateView, self)
    XEventManager.AddEventListener(XEventId.EVENT_CHARACTER_ENHANCESKILL_UNLOCK, self.UpdateStateView, self)
    XEventManager.AddEventListener(XEventId.EVENT_CHARACTER_ENHANCESKILL_UP, self.UpdateStateView, self)
end

function XUiCharacterDetail:OnDestroy()
    XEventManager.RemoveEventListener(XEventId.EVENT_CHARACTER_SKILL_UNLOCK, self.UpdateStateView, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_CHARACTER_SKILL_UP, self.UpdateStateView, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_CHARACTER_ENHANCESKILL_UNLOCK, self.UpdateStateView, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_CHARACTER_ENHANCESKILL_UP, self.UpdateStateView, self)
end
...
function XUiCharacterDetail:UpdateRightElementView()
    ...
    local elementList = XMVCA.XCharacter:GetCharacterAllElement(self.CharacterId)
    for i = 1, 3 do
        ...
        if elementList and elementList[i] then
            ...
            if elementValueList[i] then
                btn:SetNameByGroup(1, elementValueList[i] .. "%")
            else
                btn:SetNameByGroup(1, "")
            end
            local isElementActive = XMVCA.XCharacter:IsElementActive(self.CharacterId, elementList[i])
            btn:SetButtonState(isElementActive and CS.UiButtonState.Normal or CS.UiButtonState.Disable)
        else
    ...
    for i = 1, self.ListGeneralSkillDetail.childCount, 1 do
        ...
        self["BtnGeneralSkill"..i].gameObject:SetActiveEx(XTool.IsNumberValid(id))
        if id then
            ...
            local isActive = XMVCA.XCharacter:GetGeneralSkillIsActive(self.CharacterId, id)
            local isAddGeneral = XMVCA.XCharacter:GetCharacterIsAddGeneral(self.CharacterId)[i]
            if isActive or not isAddGeneral then -- 2.14表现上未配置isAddGeneral字段，默认不加锁(不涉及功能判断)一般该技能随角色直接解锁的
                self["BtnGeneralSkill"..i]:SetButtonState(CS.UiButtonState.Normal)
            else
                self["BtnGeneralSkill"..i]:SetButtonState(CS.UiButtonState.Disable)
            end
        end
    end

...
function XUiPanelComeAcrossReward:AutoInitUi()
    self.BtnRewardBg = self.Transform:Find("BtnRewardBg"):GetComponent(typeof(CS.UnityEngine.UI.Button))
    ...
    self.ImgRoleA = self.Transform:Find("GameObject/PanelRewardInfo/PanelRole/ImgRole"):GetComponent(typeof(CS.UnityEngine.UI.Image))
    self.PanelFavor = self.Transform:Find("GameObject/PanelRewardInfo/PanelFavor")
    self.TxtFavorAdd = self.Transform:Find("GameObject/PanelRewardInfo/PanelFavor/TxtFavorAdd"):GetComponent(typeof(CS.UnityEngine.UI.Text))
...
function XUiPanelComeAcrossReward:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiPanelComeAcrossReward:RegisterClickEvent函数参数错误：参数func不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiPanelComeAcrossReward:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end
...
-- auto
function XUiPanelComeAcrossReward:OnBtnRewardBgClick()
    self.Parent:Close()
end

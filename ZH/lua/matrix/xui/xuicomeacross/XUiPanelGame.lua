local XUiPanelGame = XClass(nil, "XUiPanelGame")
...
--等待下一关
function XUiPanelGame:OnPlayWaitForNext(IsWait, countDown)
...
function XUiPanelGame:OnPlayClick(success, removeList, step)
    ...
    for i = 1, step, 1 do
        if self.QuestionGirds[i] then
            for _, var in ipairs(self.QuestionGirds[i]) do
                var:SetGray()
            end
        end
    end
...
function XUiPanelGame:Play()
    ...
    self.RootUi:PlayAnimation("UiComeAcrossGame1Begin", function()
...
function XUiPanelGame:AutoInitUi()
    ...
    self.TxtResult = self.Transform:Find("PanelScore/PanelResult/TxtResult"):GetComponent(typeof(CS.UnityEngine.UI.Text))
    self.TxtResultDesc = self.Transform:Find("PanelScore/PanelResult/TxtResultDesc"):GetComponent(typeof(CS.UnityEngine.UI.Text))
    ...
    self.TxtCountDown = self.Transform:Find("PanelReady/TxtCountDown"):GetComponent(typeof(CS.UnityEngine.UI.Text))
end

function XUiPanelGame:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiPanelGame:RegisterClickEvent函数参数错误：参数func不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiPanelGame:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end

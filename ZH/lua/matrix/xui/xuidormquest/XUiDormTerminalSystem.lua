...
function XUiDormTerminalSystem:OnEnable()
    ...
    self:RefreshQuestAutoAcceptBtnName()
...
function XUiDormTerminalSystem:AddEventListener()
    XEventManager.AddEventListener(XEventId.EVENT_DORM_QUEST_AUTO_ACCEPT, self.OnQuestAutoAccept, self)
    XEventManager.AddEventListener(XEventId.EVENT_DORM_QUEST_FINISH, self.RefreshQuestAutoAcceptBtnName, self)
end

function XUiDormTerminalSystem:RemoveEventListener()
    XEventManager.RemoveEventListener(XEventId.EVENT_DORM_QUEST_AUTO_ACCEPT, self.OnQuestAutoAccept, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_DORM_QUEST_FINISH, self.RefreshQuestAutoAcceptBtnName, self)
end
...
function XUiDormTerminalSystem:UpdateTerminalBtn()
    ...
    -- 终端按钮状态
    self.BtnTerminalSysNormal.gameObject:SetActiveEx(not isUpgrade)
    self.BtnTerminalSysPress.gameObject:SetActiveEx(not isUpgrade)
    self.BtnTerminalSysCanUpNormal.gameObject:SetActiveEx(isUpgrade)
    self.BtnTerminalSysCanUpPress.gameObject:SetActiveEx(isUpgrade)
end
...
function XUiDormTerminalSystem:RefreshQuestAutoAcceptBtnName(isClientFinishQuest)
    isClientFinishQuest = isClientFinishQuest or false
    if isClientFinishQuest or XDataCenter.DormQuestManager.CheckHasCanClaimRewardQuest() then
        self:SetBtnQuestState(BtnQuestAcceptState.isCanReceive)
    else
        if XDataCenter.DormQuestManager.CheckCanAcceptQuestAndHasNewPos() then
            self:SetBtnQuestState(BtnQuestAcceptState.CanAccept)
        else
            self:SetBtnQuestState(BtnQuestAcceptState.Normal)
        end
    end
end

function XUiDormTerminalSystem:SetBtnQuestState(btnQuestAcceptState)
    self.BtnQuestAcceptNormal.gameObject:SetActiveEx(BtnQuestAcceptState.Normal == btnQuestAcceptState)
    self.BtnQuestAcceptPress.gameObject:SetActiveEx(BtnQuestAcceptState.Normal == btnQuestAcceptState)
    self.BtnQuestReceiveNormal.gameObject:SetActiveEx(BtnQuestAcceptState.isCanReceive == btnQuestAcceptState)
    self.BtnQuestReceivePress.gameObject:SetActiveEx(BtnQuestAcceptState.isCanReceive == btnQuestAcceptState)
    self.BtnQuestCanAcceptNormal.gameObject:SetActiveEx(BtnQuestAcceptState.CanAccept == btnQuestAcceptState)
    self.BtnQuestCanAcceptPress.gameObject:SetActiveEx(BtnQuestAcceptState.CanAccept == btnQuestAcceptState)
end
...
-- 委托完成领取奖励
function XUiDormTerminalSystem:QuestFinishReceiveReward(finishQuestInfos, isMergeData)
    local asynOpenCompleteDetail = asynTask(function(finishQuestInfo, cb)
        XLuaUiManager.Open("UiDormTerminalCompleteDetail", finishQuestInfo, cb, isMergeData)
    end)
    RunAsyn(function()
        if isMergeData then
            asynOpenCompleteDetail(finishQuestInfos)
        else
...
function XUiDormTerminalSystem:OnQuestAutoAccept()
    -- 刷新委托面板和队伍面板
    self:RefreshTerminalQuest()
    self:SetupDynamicTable()
    self:RefreshQuestAutoAcceptBtnName()
    self.TerminalTips:ShowAutoQuestSuccess()
end
...
function XUiDormTerminalSystem:RegisterUiEvents()
    ...
    XUiHelper.RegisterClickEvent(self, self.BtnQuestAutoAccept, self.OnBtnQuestAutoAcceptClick, nil, true)
...
function XUiDormTerminalSystem:OnBtnQuestAutoAcceptClick()
    if XDataCenter.DormQuestManager.CheckHasCanClaimRewardQuest() then
        XDataCenter.DormQuestManager.QuestGetAllRewardRequest(function(finishQuestInfos)
            self:QuestFinishReceiveReward(finishQuestInfos, true)
        end)
    else
        XDataCenter.DormQuestManager.QuestAutoAcceptReq()
    end
end
...
function XUiDormTerminalSystem:OnShowShopTip()
    ...
    self.PanelRewardEnable:PlayTimelineAnimation(function()
        self.PanelReward.gameObject:SetActiveEx(false)
    end)
end

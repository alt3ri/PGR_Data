...
function XUiRiftHandbookBuff:InitCompnent()
    ...
    for star = 3, 6 do
        ...
        local cur, all = self._Control:GetPluginCount(star)
        title.text = XUiHelper.GetText("RiftHandbookTitle", star)
        progress.text = string.format("%s/%s", cur, all)

        local effects = self._Control:GetHandbookEffect(star)
        ...
        for _, data in ipairs(effects) do
            ...
            local attr = self._Control:GetTeamAttributeEffectConfigById(cfg.Attr)
    ...
    -- 加成
    local index = 1
    local datas = self._Control:GetHandbookTakeEffectList()
    ...
    self:RefreshTemplateGrids(self.GridBuff, datas, self.GridBuff.parent, nil, "UiRiftHandbookBuffEffect", function(grid, data)
        local attr = self._Control:GetTeamAttributeEffectConfigById(data.AttrId)

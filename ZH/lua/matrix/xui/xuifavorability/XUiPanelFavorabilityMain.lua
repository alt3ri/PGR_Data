...
function XUiPanelFavorabilityMain:SetCurShowFuncType(funcType)
    self.Parent:SetCurShowFuncType(funcType)
end
...
function XUiPanelFavorabilityMain:UpdateCvName()
    ...
    local currentCharacterId = self.Parent:GetCurrFavorabilityCharacter()
    local isCollaborationCharacter = self._Control:GetModelGetCharacterCollaboration(currentCharacterId)
    ...
    if isCollaborationCharacter then
        ...
        for _, v in pairs(XEnumConst.CV_TYPE) do
            if cvTextList[v] and cvTextList[v].text then
                cvTextList[v].text = self._Control:GetCharacterCvByIdAndType(currentCharacterId, v)
                self.DropMaskList[v] = XUiTextScrolling.New(cvTextList[v], cvNameMaskList[v])
            end
        end
...
function XUiPanelFavorabilityMain:UpdateMainInfo(doAnim)
    ...
    local isCollaborationCharacter = self._Control:GetModelGetCharacterCollaboration(characterId)
    ...
    if not doAnim then
        local targetExp = tonumber(curFavorabilityTableData.Exp)
        self.ImgExp.fillAmount = XTool.IsNumberValid(targetExp) and curExp / targetExp or 0
...
function XUiPanelFavorabilityMain:UpdatePreviewExp(args)
    ...
    self.ImgExpUp.fillAmount = XTool.IsNumberValid(levelExp) and leftExp / levelExp or 0
    self.TxtLevel.text = trustLv
    self.Parent:SetUiSprite(self.ImgHeart, self._Control:GetTrustLevelIconByLevel(trustLv))

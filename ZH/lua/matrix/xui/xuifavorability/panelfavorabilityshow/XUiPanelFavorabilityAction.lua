...
---@class XUiPanelFavorabilityAction:XUiNode
---@field Parent XUiPanelFavorabilityMain
---@field UiRoot XUiFavorabilityNew
...
function XUiPanelFavorabilityAction:OnDisable()
    ...
    if self.DynamicTableAction then
...
function XUiPanelFavorabilityAction:OnDynamicTableEvent(event, index, grid)
    ...
    elseif event == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_TOUCHED then
        -- 动画过渡需要时间 所以不能让玩家快速切换动作
        local nowTime = CS.XTimerManager.Ticks
        if self.PlayAnimCd and (nowTime - self.PlayAnimCd) / CS.System.TimeSpan.TicksPerSecond < ClickCd then
            return
        end
        self.PlayAnimCd = nowTime
...
function XUiPanelFavorabilityAction:OnActionClick(clickAction, grid, index)
    ...
    if self._lastClickTime == nil then
        self._lastClickTime = CS.UnityEngine.Time.realtimeSinceStartup
    elseif CS.UnityEngine.Time.realtimeSinceStartup - self._lastClickTime < 0.1 then
        self._lastClickTime = CS.UnityEngine.Time.realtimeSinceStartup
        return
    else
        self._lastClickTime = CS.UnityEngine.Time.realtimeSinceStartup
    ...
    if isUnlock or canUnlock then
        ...
        self._ActionClickTimer = XScheduleManager.ScheduleNextFrame(function()
...
function XUiPanelFavorabilityAction:UnScheduleAction(playEffect, force)
    ...
    if self._ActionClickTimer then
        XScheduleManager.UnSchedule(self._ActionClickTimer)
        self._ActionClickTimer = nil
    end

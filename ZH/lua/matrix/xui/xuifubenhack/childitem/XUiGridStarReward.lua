...
function XUiGridStarReward:Ctor(uiRoot, ui)
    self.UiRoot = uiRoot
...
function XUiGridStarReward:OnBtnReceiveClick()
    ...
    XDataCenter.FubenHackManager.GetStarReward(self.Data.Id, function(reward)
...
function XUiGridStarReward:Refresh(data)
    self.Data = data
    local ownStars = XDataCenter.FubenHackManager.GetStarProgress()
    ...
    if data.IsFinish then
        ...
        local isGet = XDataCenter.FubenHackManager.CheckStarRewardGet(data.Id)
        if isGet then
            self:SetBtnAlreadyReceive()
        else
            self:SetBtnActive()

...
---@field BtnQuit XUiComponent.XUiButton
...
function XUiBigWorldPopupQuitShow:OnAwake()
    if not self.BtnQuit then
        self.BtnQuit = self.BtnClose
    end
...
function XUiBigWorldPopupQuitShow:OnBtnQuitClick()
...
function XUiBigWorldPopupQuitShow:_RefreshClick()
    ...
    if not string.IsNilOrEmpty(cancelText) then
        self.BtnQuit:SetNameByGroup(0, cancelText)
    end
    ...
    self.BtnQuit.gameObject:SetActiveEx(data.CancelClickData.IsActive)

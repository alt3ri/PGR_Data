local XUiBWBtnKeyItem = XClass(XUiNode, "XUiBWBtnKeyItem")

local XCSInputManager = CS.XInputManager
local ToInt32 = CS.System.Convert.ToInt32

function XUiBWBtnKeyItem:OnStart()
...
function XUiBWBtnKeyItem:OnBtnClearClick()
    if not self.DefaultKeyMapTable then
        return
    end
    XCSInputManager.InputMapper:ClearKeySetting(self.DefaultKeyMapTable.OperationKey, ToInt32(self._KeySetType), self.CurInputMapId, self.CurOperationType)
    self:Refresh()
end

function XUiBWBtnKeyItem:SetKeySetType(keySetType)
    self._KeySetType = keySetType
end

function XUiBWBtnKeyItem:IsKeyboard()
    return self._KeySetType == CS.InputDeviceType.Keyboard
end

function XUiBWBtnKeyItem:SetData(data, cb, curInputMapId, curOperationType)
...
function XUiBWBtnKeyItem:Refresh(data, cb, resetTextOnly, curInputMapId, curOperationType)
    ...
    if operationKey and self._KeySetType then
        ...
        local curOperationTypeEnum = XCSInputManager.XOperationType.__CastFrom(self.CurOperationType)
        local name = keyCodeType == XSetConfigs.KeyCodeType.NotCustomIgnoreCheck and self.DefaultKeyMapTable.KeyboardKeyCode or XCSInputManager.GetKeyCodeString(self._KeySetType, curInputMapIdEnum, operationKey, curOperationTypeEnum)
        ...
        if isKeyboard then
            ...
            local icons = keyCodeType == XSetConfigs.KeyCodeType.NotCustomIgnoreCheck and XCSInputManager.GetKeyCodeIcon(self._KeySetType, self.DefaultKeyMapTable.Id)
                    or XCSInputManager.GetKeyCodeIcon(self._KeySetType, curInputMapIdEnum, operationKey, CS.PressKeyIndex.End, curOperationTypeEnum)
...
function XUiBWBtnKeyItem:SetRecommendText(operationKey)
    local recommendKey = XCSInputManager.GetRecommendKeyIcoPath(self._KeySetType, operationKey)
...
function XUiBWBtnKeyItem:GetCurOperationType()
    return self.CurOperationType
end

function XUiBWBtnKeyItem:GetCurInputMapId()
    return self.CurInputMapId
end

function XUiBWBtnKeyItem:GetDataId()
    return self.Data and self.Data.Id
end

return XUiBWBtnKeyItem
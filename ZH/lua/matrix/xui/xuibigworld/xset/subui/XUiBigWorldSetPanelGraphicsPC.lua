...
function XUiBigWorldSetPanelGraphicsPC:OnTGroupFullscreenClick(index)
    if index == 1 then
        self._Setting:SetFullScreenValue(true)
        self.DrdSort.interactable = false
    else
        self._Setting:SetFullScreenValue(false)
        self.DrdSort.interactable = true
    end
end
...
function XUiBigWorldSetPanelGraphicsPC:_InitTogGroup()
    ...
    self._TogFullscreenGroup = {
        self.TogFullscreen_0,
        self.TogFullscreen_1,
    }
end
...
function XUiBigWorldSetPanelGraphicsPC:_RegisterButtonClicks()
    ...
    self:_RegisterTGroupFullscreen()
...
function XUiBigWorldSetPanelGraphicsPC:_RegisterTGroupFullscreen()
    self.TGroupFullscreen:Init(self._TogFullscreenGroup, Handler(self, self.OnTGroupFullscreenClick))
end
...
function XUiBigWorldSetPanelGraphicsPC:_RefreshToggleFullScreen()
    ...
    if isFullScreen then
        self.TGroupFullscreen:SelectIndex(1, false)
    else
        self.TGroupFullscreen:SelectIndex(2, false)
    end
...
function XUiBigWorldSetPanelGraphicsPC:_RefreshDropdownResolution()
    ...
    self.DrdSort:SetValueWithoutNotify(currentIndex - 1)
    self.DrdSort.interactable = not isFullScreen
    self.ImgMask.gameObject:SetActiveEx(isFullScreen)
end

...
function XUiBigWorldSetPanelOtherPC:OnTogCursorGroupClick(index)
    self._Setting:SetCursorSizeValue(index - 1)
end

function XUiBigWorldSetPanelOtherPC:_RegisterButtonClicks()
    self.Super._RegisterButtonClicks(self)

    self.TGroupCursor:Init({
        self.TogCursor_1,
        self.TogCursor_2,
        self.TogCursor_3,
    }, Handler(self, self.OnTogCursorGroupClick))
...
function XUiBigWorldSetPanelOtherPC:_Refresh()
    ...
    self.TGroupCursor:SelectIndex(self._Setting:GetCursorSizeValue() + 1, false)
    -- self.TogCursor_1:SetIsOnWithoutNotify(self._Setting:GetCursorSizeValue() == XEnumConst.BWSetting.CursorSize.Small)
    -- self.TogCursor_2:SetIsOnWithoutNotify(self._Setting:GetCursorSizeValue() == XEnumConst.BWSetting.CursorSize.Medium)
    -- self.TogCursor_3:SetIsOnWithoutNotify(self._Setting:GetCursorSizeValue() == XEnumConst.BWSetting.CursorSize.Large)

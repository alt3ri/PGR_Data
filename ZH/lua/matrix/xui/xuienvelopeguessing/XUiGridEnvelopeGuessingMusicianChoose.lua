local XUiGridEnvelopeGuessingMusicianChoose =
    XClass(XUiNode, "XUiGridEnvelopeGuessingMusicianChoose")

function XUiGridEnvelopeGuessingMusicianChoose:OnStart()
    self.BtnSelf.CallBack = handler(self, self._OnClick)
end

function XUiGridEnvelopeGuessingMusicianChoose:SetData(
    characterId,
    instrument,
    onClick)
    ...
    self.ImgTag.gameObject:SetActiveEx(not not
        self._Control:GetInstrumentByBoundCharacterId(self._CharacterId))
end

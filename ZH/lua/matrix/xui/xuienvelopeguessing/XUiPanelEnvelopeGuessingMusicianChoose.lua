...
local XUiPanelEnvelopeGuessingMusicianChoose =
    XClass(XUiNode, "XUiPanelEnvelopeGuessingMusicianChoose")

function XUiPanelEnvelopeGuessingMusicianChoose:OnStart(
    refreshInstrument,
    closeChoosePanel)
    ...
    self._DynTable:SetProxy(
        require("XUi/XUiEnvelopeGuessing/XUiGridEnvelopeGuessingMusicianChoose"),
        self)
...
function XUiPanelEnvelopeGuessingMusicianChoose:OnDynamicTableEvent(evt, index, grid)
    if evt == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_ATINDEX then
        grid:SetData(
            self._AllOpenedCharacterIds[index],
            self._Instrument,
            self._OnCharacterClickedHandler)
    end
end

function XUiPanelEnvelopeGuessingMusicianChoose:_OnCharacterClicked(
    instConfig, charId)
    ...
    if charId and self._Control:GetInstrumentBinding(self._Instrument.Id) == charId then
        self._Control:SetInstrumentBinding(instConfig.Id, nil, function(data)
            self:_OnInstrumentBindingFinished(
                data, instConfig, nil, nil, nil, charId)
        end)
    else
        self._Control:SetInstrumentBinding(
            instConfig.Id,
            charId,
            function(data, anotherInst, anotherChar)
                self:_OnInstrumentBindingFinished(
                    data, instConfig, anotherInst, anotherChar, charId, nil)
            end)
    end
end

function XUiPanelEnvelopeGuessingMusicianChoose:_OnInstrumentBindingFinished(
    data, instConfig, anotherInstId, anotherCharId, charId, prevCharId)
    ...
    if not charId then
        ...
        msg = CS.XTextManager.GetText(
            "EnvelopeGuessingChoosePanelUnbindingSuccess",
            self._Control:GetCharacterConfig(prevCharId).CharacterName)
    elseif anotherInstId and anotherCharId then
        msg = CS.XTextManager.GetText(
            "EnvelopeGuessingChoosePanelSwapSuccess",
            self._Control:GetCharacterConfig(charId).CharacterName,
            self._Control:GetCharacterConfig(anotherCharId).CharacterName)
    elseif anotherInstId and not anotherCharId then
        msg = CS.XTextManager.GetText(
            "EnvelopeGuessingChoosePanelMoveSuccess",
            self._Control:GetCharacterConfig(charId).CharacterName)
    else
        refreshType = XUiPanelEnvelopeGuessingInstrument.RefreshType.Enter
        msg = CS.XTextManager.GetText(
            "EnvelopeGuessingChoosePanelBindingSuccess",
            self._Control:GetCharacterConfig(charId).CharacterName)
    end

    if anotherInstId then self._RefreshInstrument(anotherInstId) end

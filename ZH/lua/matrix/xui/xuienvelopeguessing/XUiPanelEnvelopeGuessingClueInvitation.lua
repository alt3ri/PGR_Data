...
local XDynamicTableCurve =
    require("XUi/XUiCommon/XUiDynamicTable/XDynamicTableCurve")

local XUiPanelEnvelopeGuessingClueInvitation =
    XClass(XUiNode, "XUiPanelEnvelopeGuessingClueInvitation")

local XUiPanelEnvelopeGuessingEnvelopeCard =
    require("XUi/XUiEnvelopeGuessing/XUiPanelEnvelopeGuessingEnvelopeCard")

function XUiPanelEnvelopeGuessingClueInvitation:OnStart(
    setupConfirmButton,
    closeParent)
...
function XUiPanelEnvelopeGuessingClueInvitation:OnEnable()
    ...
    self._EnvelopesNotOpened = XTool.FilterList(
        allEnvelopes,
        function(e) return not self._Control:IsCharacterOpened(e.CharacterId) end)
    ...
    if prevOpenedEnvelopeId then
        ...
        if slicePoint then
            local right = XTool.ArraySliceCopy(
                self._EnvelopesNotOpened, 1, slicePoint - 1)

            local left = XTool.ArraySliceCopy(
                self._EnvelopesNotOpened, slicePoint, #self._EnvelopesNotOpened)
...
function XUiPanelEnvelopeGuessingClueInvitation:_GetPrevOpenedEnvelopeStorageKey()
    return
        "XUiPanelEnvelopeGuessingClueInvitation._GetPrevOpenedEnvelopeStorageKey_"
        .. XPlayer.Id
        .. "_"
        .. XMVCA.XEnvelopeGuessing:GetCurrentActivity().Id
end

function XUiPanelEnvelopeGuessingClueInvitation:_GetPrevOpenedEnvelopeId()
    local envelopeId = XSaveTool.GetData(self:_GetPrevOpenedEnvelopeStorageKey())
    local allEnvelopes = self._Control:GetAllEnvelopes()
    if allEnvelopes[envelopeId] then return envelopeId end
    return nil
end
...
function XUiPanelEnvelopeGuessingClueInvitation:_OpenEnvelope(fastOpen)
    ...
    if selected then
        ...
        self._Control:EnvelopeOpenRequest({ envelope.Id }, fastOpen, function(data)
            ...
            XLuaUiManager.Open(
                ...
                XUiEnvelopeGuessingOpenPackage.OpenType.Clue,
                envelope,
                fastOpen)

            XSaveTool.SaveData(
                self:_GetPrevOpenedEnvelopeStorageKey(),
                envelope.Id)
        end)
    end
end

...
---@class XUiGachaLuciaStageLine : XLuaUi 剧情关界面(用的 festivalActivity 表)
---@field ParentUi XUiGachaLuciaMain
local XUiGachaLuciaStageLine = XLuaUiManager.Register(XLuaUi, "UiGachaLuciaStageLine")

function XUiGachaLuciaStageLine:OnAwake()
...
function XUiGachaLuciaStageLine:OnStart(chapterId, panel3D, isAutoOpen)
...
function XUiGachaLuciaStageLine:OnEnable()
    ...
    self:SetAutoCloseInfo(endTime, function(isClose)
        if not isClose then
            ...
            self.TxtDay.text = XUiHelper.GetText("GachaLuciaTime", XUiHelper.GetTime(time, XUiHelper.TimeFormatType.CHATEMOJITIMER))
        end
    end, nil, 0)
...
function XUiGachaLuciaStageLine:OnDisable()
    for _, stage in pairs(self._Stages) do
        stage:Close()
    end
end

function XUiGachaLuciaStageLine:Refresh3DSceneInfo()
...
function XUiGachaLuciaStageLine:InitButton()
...
function XUiGachaLuciaStageLine:SetUiData()
    -- 初始化prefab组件
...
function XUiGachaLuciaStageLine:HandleStages()
    for i = 1, #self._StageIds do
        local itemStage = self.PanelStageContent:Find(string.format("Stage%d", i))
        if not itemStage then
            XLog.Error("XUiGachaLuciaStageLine:HandleStages() 函数错误: 游戏物体PanelStageContent下找不到名字为:" .. string.format("Stage%d", i) .. "的游戏物体")
            return
        end
        -- 组件初始化
        self._StageGroup[i] = itemStage
        if not self._Stages[i] then
            self._Stages[i] = require("XUi/XUiGachaLucia/Grid/XUiGridGachaLuciaStageItem").New(itemStage, self)
        end
        self._Stages[i]:Open()
...
function XUiGachaLuciaStageLine:HandleStageLines()
    ...
    for i = 1, #self._StageIds - 1 do
        ...
        if not itemLine then
            XLog.Error("XUiGachaLuciaStageLine:SetUiData() error: prefab not found a child name:" .. string.format("Line%d", i))
            return
        end
...
-- 背景
function XUiGachaLuciaStageLine:SwitchFestivalBg()
    self.RImgFestivalBg.gameObject:SetActiveEx(false)
end

-- 加载特效
function XUiGachaLuciaStageLine:LoadEffect(effectUrl)
...
-- 更新刷新
function XUiGachaLuciaStageLine:RefreshFestivalNodes()
...
-- 更新节点线条
function XUiGachaLuciaStageLine:UpdateNodeLines()
    if not self._ChapterTemplate or not self._StageIds then
        return
    end
...
function XUiGachaLuciaStageLine:SetStageLineActive(index, isActive)
...
-- 通过Stage调用的接口
function XUiGachaLuciaStageLine:OpenStageDetails(stageId)
    XLuaUiManager.Open("UiGachaLuciaStageDetail", stageId)
end

-- 选中关卡描边效果
function XUiGachaLuciaStageLine:UpdateNodesSelect(stageId)
...
-- 取消选中
function XUiGachaLuciaStageLine:ClearNodesSelect()
...
function XUiGachaLuciaStageLine:SetPanelStageListMovementType(moveMentType)
...
function XUiGachaLuciaStageLine:MoveIntoStage(stageIndex)
...
function XUiGachaLuciaStageLine:OnSceneBtnBackClick()
    self:Close()
    self.ParentUi:Close()
end

function XUiGachaLuciaStageLine:OnGotoGacha()
    self:Close()
    self.ParentUi:OnChildClose()
end

function XUiGachaLuciaStageLine:OnBtnSetClick()
    XLuaUiManager.Open("UiSet")
end

return XUiGachaLuciaStageLine
...
function XUiFashionStoryStageTrialDetail:Refresh(activityId, stageId)
    ...
    -- 图标
    self.RImgNandu:SetRawImage(XMVCA.XFashionStory:GetTrialDetailHeadIcon(stageId))

    -- 名称
    self.TxtTitle.text = XFubenConfigs.GetStageName(stageId)

    -- 推荐等级
    self.TxtRecommendLevel.text = XMVCA.XFashionStory:GetTrialDetailRecommendLevel(stageId)

    -- 背景
    self.ImgFullScreen.gameObject:SetActiveEx(true)
    self.PanelSpine.gameObject:SetActiveEx(false)
    local spine = XMVCA.XFashionStory:GetTrialDetailSpine(stageId)
    if spine then
        self.PanelSpine.gameObject:SetActiveEx(true)
        self.PanelSpine.gameObject:LoadSpinePrefab(spine)
    else
        self.ImgFullScreen.gameObject:SetActiveEx(true)
        self.ImgFullScreen:SetRawImage(XMVCA.XFashionStory:GetTrialDetailBg(stageId))
    end

    -- 描述
    self.TxtDes.text = string.gsub(XMVCA.XFashionStory:GetTrialDetailDesc(stageId), "\\n", "\n")

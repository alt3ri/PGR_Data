...
function XUiGridFashionStoryTrial:Refresh(stageId, activityId, entranceCb,singleLineId)
    if stageId == XMVCA.XFashionStory.StoryEntranceId and activityId and singleLineId then
    ...
    self.BtnSummer:SetRawImage(self.IsStoryEntrance and XMVCA.XFashionStory:GetStoryEntranceBg(singleLineId) or XFubenConfigs:GetStageIcon(stageId))
    self.ImageStoryProcess:SetSprite(self.IsStoryEntrance and XMVCA.XFashionStory:GetStoryEntranceFinishTag(singleLineId) or XMVCA.XFashionStory:GetTrialFinishTag(stageId))
...
function XUiGridFashionStoryTrial:RefreshLeftTime()
    local leftTime
    if self.IsStoryEntrance then
        leftTime = XMVCA.XFashionStory:GetStoryTimeStamp(self.ActivityId)
    else
        leftTime = XMVCA.XFashionStory:GetTrialStageLeftTimeStamp(self.StageId)
    end

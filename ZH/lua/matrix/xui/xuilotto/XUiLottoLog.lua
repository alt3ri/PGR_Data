...
function XUiLottoLog:OnStart(data, selectIndex, panelType)
    ...
    for _,rewardData in pairs(rewardDataList) do
        if rewardData:GetRareLevel() == rareLevel then
            ...
            if not reward then
                ...
                local fashionDesc = nil
                local weaponFashionId = nil
                if self.PanelType then
                    local fashionDescValue = XLottoConfigs.LottoFashionDesc[self.PanelType]
                    if fashionDescValue then
                        fashionDesc = XUiHelper.GetText(fashionDescValue)
                    end

                    local weaponFashionIdValue = XLottoConfigs.WeaponFashionId[self.PanelType]
                    if weaponFashionIdValue then
                        weaponFashionId = XLottoConfigs.GetLottoClientConfigNumber(weaponFashionIdValue)
                    end
                end
                reward:SetCustomWeaopnFashionId(weaponFashionId, fashionDesc)

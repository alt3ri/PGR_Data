--- 和研发动画一样，Lotto通用的动画，不需要单独做抽卡动画时可考虑使用
---@class XUiLottoCommonDrawShow: XLuaUi
...
function XUiLottoCommonDrawShow:OnEnable()
...
function XUiLottoCommonDrawShow:InitSceneObject()
...
function XUiLottoCommonDrawShow:OnDrawEnd()
    if self.IsSkip then
        return
    end
    XLuaUiManager.Remove("UiLottoCommonDrawShow")
...
function XUiLottoCommonDrawShow:RefreshItem()
    ...
    for i = 1,#self.RewardList do
        ...
        if XTool.IsNumberValid(item.ShowQuality) then
            quality = item.ShowQuality
        end

        if not XTool.IsNumberValid(quality) then
            quality = self:GetQuality(i)
        end
...
function XUiLottoCommonDrawShow:GetQuality(showIndex)

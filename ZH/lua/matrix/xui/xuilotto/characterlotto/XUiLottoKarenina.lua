...
function XUiLottoKarenina:_RefreshTimeText(endTime)
...
function XUiLottoKarenina:RefreshReward()
    self._PanelLottoPreview:UpdateTwoLevelPanel(XEnumConst.Lotto.Karenina)
end
--endregion
...
function XUiLottoKarenina:ShowRewardDialog()
    self._DrawControl:ShowRewardDialog(XEnumConst.Lotto.Karenina)
end
...
function XUiLottoKarenina:InitSceneVideo()
    ...
    if not sceneVideoCfg then
        XLog.Error(string.format("[XUiLottoKarenina] LottoSceneVideo 缺少 LottoId=%s 的配置", tostring(lottoId)))
        return
    end
...
function XUiLottoKarenina:RefreshBtnChange()
...
function XUiLottoKarenina:OnBtnChangeClick()
...
function XUiLottoKarenina:OnBtnRewardDetailClick()
    XLuaUiManager.Open("UiLottoLog", self._LottoGroupData, 1, XEnumConst.Lotto.Karenina)
end

function XUiLottoKarenina:OnBtnDrawRuleClick()
    XLuaUiManager.Open("UiLottoLog", self._LottoGroupData, 2, XEnumConst.Lotto.Karenina)
end

...
function XUiActivityBriefRefreshButton:RefreshNormal()
    if not XTool.IsNumberValid(self.ActivityGroupId) then
        return
    end
...
function XUiActivityBriefRefreshButton:RefreshAllActivityPrequel()
    --全部间章
...
function XUiActivityBriefRefreshButton:RefreshActivityRogueLike()
    -- 爬塔
...
function XUiActivityBriefRefreshButton:RefreshFubenActivityTrial()
    ...
    local activityBrieButton = self:GetActivityBrieButton(activityGroupId)
    local tagCondition = XActivityBriefConfigs.GetActivityBriefGroupTagCondition(activityGroupId)
    local tagParam = XActivityBriefConfigs.GetActivityBriefGroupTagParam(activityGroupId)
    local experimentLevelId = nil
    if tagParam and tagParam ~= "" then
        experimentLevelId = tonumber(tagParam) -- 转换成ExperimentLevel.tab的Id
    end

    local isShowTag = XRedPointConditions.Check(tagCondition, experimentLevelId)
    activityBrieButton:ShowTag(isShowTag)
...
function XUiActivityBriefRefreshButton:RefreshReward(activityGroupId, reOpen)
    ...
    if activityRewardIds and #activityRewardIds > 0 then
        local rewardId
        for i = 1, #config.RewardShowTimeIds do
            local timeId = config.RewardShowTimeIds[1]
            if XFunctionManager.CheckInTimeByTimeId(timeId) then
                rewardId = activityRewardIds[i]
                break
            end
        end
        if rewardId then
            if panelReward then
                ...
                local rewardGrid = require('XUi/XUiActivityBrief/XUiPanelActivityBriefShowGooods').New(btn.PanelReward, rewardId)

...
function XUiDormFieldGuide:InitEnterCfg()
    ...
    local check = function(template)
        if not string.IsNilOrEmpty(template.ShowTimeStr) then
            -- 如果有时间限制，不显示时间未到的套装
            if now < XTime.ParseToTimestamp(template.ShowTimeStr) then
                return false
            end
        end
...
function XUiDormFieldGuide:CheckFurnitureFilter(template)
    ...
    --剔除未到显示时间的家具
    if not string.IsNilOrEmpty(template.ShowTimeStr) and XTime.GetServerNowTimestamp() < XTime.ParseToTimestamp(template.ShowTimeStr) then
        return false
    end

...
function XUiFubenBossSingleModeBattleRoleRoom:AOPOnRefreshRecommendGeneralSkillBefore(rootUi, skillUi)
    if XTool.IsNumberValid(self._FeatureId) then
        if skillUi then
            local generalSkill = XUiPanelRecommendGeneralSkill.New(skillUi, rootUi)
            
            if XMVCA.XFubenBossSingle:CheckShowRecommend(self._FeatureId) then
                generalSkill:Open()
                generalSkill:RefreshGeneralSkillIds(XMVCA.XFubenBossSingle:GetShowRecommendIds(self._FeatureId))
            else
                generalSkill:Close()
            end
            rootUi.XUiPanelRecommendGeneralSkill = generalSkill
        end

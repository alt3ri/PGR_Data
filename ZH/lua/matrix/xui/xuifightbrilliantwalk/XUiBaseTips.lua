...
function XUiBaseTips:Update()
    ...
    if not self.FollowNpc.IsActivate then
        ...
        if self.CSXRLManagerCamera:CheckInView(pos) then
            self.RectTransform.localPosition = self:GetObjPosToUguiPos(self.ParentRectTransform, self.FollowNode.position)
            self.GameObject:SetActiveEx(true)
        else
            self.GameObject:SetActiveEx(false)
        end
...
function XUiBaseTips:GetObjPosToUguiPos(canvasRect, fromObjPos)
    local v2 = XUiHelper.ObjPosToUguiPos(canvasRect, fromObjPos, CS.XRLManager.Camera.Camera)
    V3.x, V3.y = v2.x, v2.y
    return V3
end

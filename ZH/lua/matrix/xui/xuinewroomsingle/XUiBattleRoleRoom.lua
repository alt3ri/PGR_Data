...
function XUiBattleRoleRoom:RefreshRoleModels()
    ...
    for pos = 1, MAX_ROLE_COUNT do
        ...
        if skillList then
            for i, v in ipairs(skillList) do
                ...
                for index, skillId2 in ipairs(groupSkillIds) do
                    skillExchangeConfig = XMVCA.XCharacter:GetCharacterSkillExchangeDesBySkillIdAndLevel(skillId2, skillLevel)
        ...
        -- 设置技能UiType为2，则不打显示切换模式
        if XTool.IsNumberValidEx(entityId) and isShowCurPanelChangeMode then
            local _, exchangeDesConfig = XMVCA.XCharacter:GetSkillExchangeDesSkillIdAndConfigByCharacterId(entityId)
            if exchangeDesConfig and exchangeDesConfig.UiType == XEnumConst.CHARACTER.SKILL_EXCHANGE_UI_TYPE.FORM then
                isShowCurPanelChangeMode = false
            end
        end

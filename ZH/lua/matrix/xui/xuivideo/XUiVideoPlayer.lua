...
function XUiVideoPlayer:OnAwake()
    ...
    self.VideoPlayerUgui.ActionError = handler(self, self.ActionErrorCb)
    self.IsPause = false
    self.BtnAuto.CallBack = function() self:Pause() end
    self.BtnSkip.CallBack = function()
        XLuaVideoManager.RecordVideoSkip(self.VideoId)
        self:Close() 
    end
...
function XUiVideoPlayer:OnStart(data, closeCb, needAuto, needSkip, defaultHideButtons)
    ...
    if type(data) == "table" then
        ...
        local config = XLuaVideoManager.GetVideoConfigById(movieId)
        width = config.Width
...
function XUiVideoPlayer:OnDestroy()
    -- 记录结束埋点
    XLuaVideoManager.RecordVideoEnd(self.VideoId)
...
function XUiVideoPlayer:ActionErrorCb()
    XLog.Warning()
    self:Close()
end

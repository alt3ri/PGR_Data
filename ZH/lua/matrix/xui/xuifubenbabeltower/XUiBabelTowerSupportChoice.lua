...
function XUiBabelTowerSupportChoice:SetItemData(itemData)
    ...
    self:UpdateGridChoiceState(self.UiRoot:GetAvailableSupportPoint())
end
...
function XUiBabelTowerSupportChoice:OnSupportChoiceItemClick(index)
    ...
    if self.BuffGoupDatas.SelectedBuffId == currentSelectBuffId then
        ...
        if curbuffTemplate.PointSub > self.UiRoot:GetAvailableSupportPoint() + lastPointSub then return end
...
function XUiBabelTowerSupportChoice:IsBuffListOverCount()
    if not self.BuffGoupDatas then return false end
    return self.BuffGoupDatas.IsOverCount
end

-- 更新未选中buff可选状态
function XUiBabelTowerSupportChoice:UpdateGridChoiceState(availableSupportPoint)
    if self.BuffGoupDatas.SelectedBuffId then
        ...
        availableSupportPoint = availableSupportPoint + lastBuffTemplate.PointSub
    end
    self.GridContent.CurSelectId = self.BuffGoupDatas.CurSelectId
    for i=1, #self.BuffGroupTemplate.BuffId do
        local buffId = self.BuffGroupTemplate.BuffId[i]
        ...
        if not self:GetBuffSelectStatus(buffId) and self.SupportBtnCompList[i] then
            local btnStatus = (availableSupportPoint >= buffTemplate.PointSub) and UiButtonState.Normal or UiButtonState.Disable
            self.SupportBtnCompList[i]:SetButtonState(btnStatus)
            self.SupportItemList[i]:ShowOverCount(false)
        else
            self.SupportItemList[i]:ShowOverCount(self:IsBuffListOverCount())
        end
    end
end

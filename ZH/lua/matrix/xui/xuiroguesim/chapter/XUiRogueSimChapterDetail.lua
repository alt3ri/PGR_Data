...
function XUiRogueSimChapterDetail:RefreshView()
    -- 名称
    local name = self._Control:GetRogueSimStageName(self.StageId)
    self.RImgStageName.text = name
    -- 描述
...
function XUiRogueSimChapterDetail:RefreshStarReward()
    ...
    for i, conditionId in ipairs(conditions) do
        ...
        local desc = self._Control.ConditionSubControl:GetConditionDesc(conditionId)
        local isPass = map[i]
...
function XUiRogueSimChapterDetail:RegisterUiEvents()
    ...
    XUiHelper.RegisterClickEvent(self, self.BtnEnter, self.OnBtnEnterClick, nil, true)
end

...
function XUiStrongholdRoomCharacterV2P6:InitFilter()
    ...
    if isAssitant then
        ...
        if XTool.IsNumberValid(self.GroupId) then
            local index = XDataCenter.StrongholdManager.GetTeamIndexByProp(self.GroupId, self.TeamId)
            tempList = XDataCenter.StrongholdManager.GetAssistantPlayerIds(self.GroupId, index, self.TeamList)
        else
            tempList = XDataCenter.StrongholdManager.GetAssistantPlayerIds(self.GroupId, self.TeamId, self.TeamList)
        end
...
function XUiStrongholdRoomCharacterV2P6:OnSelectCharacter(characterId)
    ...
    --别人的角色信息
    local checkId = characterId
    if self:CheckIsOtherPlayer() then
        -- 检查得用真正的角色id
        checkId = XDataCenter.StrongholdManager.GetAssistantPlayerCharacterId(characterId)
    end

    XRedPointManager.CheckOnceByButton(self.BtnTeaching, { XRedPointConditions.Types.CONDITION_CELICA_TEACH }, checkId) 
end

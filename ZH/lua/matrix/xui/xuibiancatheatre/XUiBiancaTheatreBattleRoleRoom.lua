...
function XUiBiancaTheatreBattleRoleRoom:CheckTeam(team)
    ...
    for teamPos, entityId in ipairs(team:GetEntityIds()) do
        if XEntityHelper.GetIsRobot(entityId) then
            ...
            local targetId = adventureRobotRole:GetId()
            if adventureRobotRole and targetId ~= entityId then
                team:UpdateEntityTeamPos(targetId, teamPos, true)
            end
        end
    end
end
...
-- 检查是否开启效应选择
---@overload
function XUiBiancaTheatreBattleRoleRoom:CheckIsEnableGeneralSkillSelection()
    return false
end

-- 获取有效角色进行剔除检查
---@overload
function XUiBiancaTheatreBattleRoleRoom:GetValidEntityIdList(stageId, team)
    -- 这里屏蔽掉这个功能
    return nil
end

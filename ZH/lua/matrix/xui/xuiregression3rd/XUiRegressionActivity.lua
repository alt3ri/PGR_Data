...
function XUiRegressionActivity:InitTab()
    ...
    for idx, tabData in ipairs(tabDataList or {}) do
        ...
        if ( tabData.ActivityType == XRegression3rdConfigs.ActivityType.Activity or tabData.ActivityType == XRegression3rdConfigs.ActivityType.Questionnaire ) and not open then
            goto continue
        end
        ...
        table.insert(activeTabData, tabData)
        ::continue::
    end

    self.TabData = activeTabData
...
function XUiRegressionActivity:GetSubPanel()
    ...
    if not panel then
        ...
        panel = require(modulePath).New(go, self)

...
function XUiActivityChapter:TimeUpdate()
    ...
    local managerRecordDir = {}
    local viewModelRecordDir = {}
    for _, data in pairs(dataList) do
        if data.ExConfig then
            managerRecordDir[data.ExConfig.Id] = true
        else
            viewModelRecordDir[data:GetId()] = true
        end
    end
    for _, data in pairs(curDataList) do
        if data.ExConfig then
            if not managerRecordDir[data.ExConfig.Id] then
                self:RefreshActivityList(index)
                return
            else
                managerRecordDir[data.ExConfig.Id] = nil
            end
        else
            if not viewModelRecordDir[data:GetId()] then
                self:RefreshActivityList(index)
                return
            else
                viewModelRecordDir[data:GetId()] = nil
            end
        end
    end
    if next(managerRecordDir) or next(viewModelRecordDir) then
        self:RefreshActivityList(index)
    end
end

function XUiActivityChapter:CheckActivityIsOpen(data)
    if self.FubenManagerEx.IsFubenBase(data) then
        return not data:ExGetIsLocked()
    elseif CheckClassSuper(data, XChapterViewModel) then
        return not data:GetIsLocked()
    end
...
---@return table XExFubenActivityManager|XChapterViewModel
function XUiActivityChapter:GetActivityData()
    ...
    table.sort(dataList, function(dataA, dataB)
        local isActivityAOpened = self:CheckActivityIsOpen(dataA)
        local isActivityBOpened = self:CheckActivityIsOpen(dataB)

        if isActivityAOpened == isActivityBOpened then
            return self:GetDataOrder(dataA) < self:GetDataOrder(dataB)
        else
            return isActivityAOpened
        end
    end)
...
function XUiActivityChapter:OnDynamicTableEvent(event, index, grid)
    ...
    elseif event == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_TOUCHED then
        ...
        local doOpenFun = function()
            if self.FubenManagerEx.IsFubenBase(data) then
                if data:ExOpenMainUi() then
                    -- 判断是否埋点
                    ---@type XTableFubenActivity
                    local config = data:ExGetConfig()
                    if config and config.IsRecord then
                        XFunctionManager.RecordSkip(config.Id, XFunctionManager.SkipTypeForRecord.FubenActivitySkip)
                    end
                end
            elseif CheckClassSuper(data, XChapterViewModel) then

---@class XUiGachaSelenaPassport : XLuaUi 使用皮肤和场景弹框
local XUiGachaSelenaPassport = XLuaUiManager.Register(XLuaUi, "UiGachaSelenaPassport")

function XUiGachaSelenaPassport:OnAwake()
...
function XUiGachaSelenaPassport:OnStart(reward, callBack)
    ...
    self.BtnWear:SetNameByGroup(0, XUiHelper.GetText(self._IsBackGround and "GachaSelenaBgBtn" or "GachaSelenaFashionBtn"))
...
function XUiGachaSelenaPassport:Close()
...
function XUiGachaSelenaPassport:OnWear()
...
function XUiGachaSelenaPassport:OnSetAssistant()
    self.BtnSetAssistant.gameObject:SetActiveEx(false)

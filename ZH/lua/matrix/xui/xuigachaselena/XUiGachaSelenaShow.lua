---@class XUiGachaSelenaShow : XLuaUi
local XUiGachaSelenaShow = XLuaUiManager.Register(XLuaUi, "UiGachaSelenaShow")
...
function XUiGachaSelenaShow:OnStart(gachaId, rewardList, state, isSkip)
...
function XUiGachaSelenaShow:OnEnable()
...
function XUiGachaSelenaShow:OnDisable()
    self:StopCv()
end

function XUiGachaSelenaShow:OnDestroy()
    self:StopCv()
...
function XUiGachaSelenaShow:RegisterButton()
...
function XUiGachaSelenaShow:InitUi()
    self._FarCameraList = {}
...
function XUiGachaSelenaShow:PlayNext()
    ...
    if not self._LastReward then
        ...
        self._GridRewardList[self._CurrIndex] = require("XUi/XUiGachaSelena/Grid/XUiGridGachaSelenaShowReward").New(self._ModelPanelList[self._CurrIndex], self, self._UiPanelList[self._CurrIndex], self._FarCameraList[self._CurrIndex], self._NearCameraList[self._CurrIndex], self._UiCameraList[self._CurrIndex])
        self._LastReward = self._GridRewardList[self._CurrIndex]
    end
...
function XUiGachaSelenaShow:ShowResult()
    ...
    for i = 1, #self._RewardList do
        ...
        local grid = require("XUi/XUiGachaSelena/Grid/XUiGridGachaSelenaResult").New(obj, self)
...
function XUiGachaSelenaShow:RefreshByState()
...
function XUiGachaSelenaShow:SetDrawEffectGroupId(effectGroupId)
    if effectGroupId > self._MaxEffectGroupId then
        self._MaxEffectGroupId = effectGroupId
    end
end

function XUiGachaSelenaShow:PlayCardEffectSound()
...
function XUiGachaSelenaShow:StopCv()
    if self._CvInfo then
        self._CvInfo:Stop()
        self._CvInfo = nil
    end
end
function XUiGachaSelenaShow:OnBtnSkipClick()
    if self._CurrState == DrawState.Result then
        self:Close()
    else
        self._CurrState = DrawState.Result

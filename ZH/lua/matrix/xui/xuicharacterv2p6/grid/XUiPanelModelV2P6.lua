local XDynamicTableFixed3D = require("XUi/XUiCommon/XUiDynamicTable/XDynamicTableFixed3D")
-- 整个成员系统的镜头model管理
...
function XUiPanelModelV2P6:SetCamera(targetIndex)
    for i, cameraTrans in pairs(self.CameraFars) do
        if cameraTrans and not XTool.UObjIsNil(cameraTrans) then
...
function XUiPanelModelV2P6:RefreshSingleBigBall(isActiveNode, isEvo)
    ...
    for i = 1, XEnumConst.CHARACTER.MAX_QUALITY_STAR do
        ...
        if not isAttrEmpty then
            ...
            -- 名字
            local qualityDesc = XMVCA.XCharacter:GetCharQualityDesc(self.SeleQuality, XModelHX.QualityStyle.Chinese)
    ...
    for i = 1, XEnumConst.CHARACTER.MAX_QUALITY do
        ...
        --region 品阶
        local qualityDesc = XMVCA.XCharacter:GetCharQualityDesc(i, XModelHX.QualityStyle.Chinese)
        btnDrop:SetNameByGroup(0, qualityDesc)
    end
...
function XUiPanelModelV2P6:RefreshBigBallEffect(quality, isActive, isEvo)
    ...
    self:LoadQualityEffectIcon(obj.EffectBallBig, quality)
end
...
function XUiPanelModelV2P6:LoadQualityEffectIcon(transform, quality)
    -- 因为PrefabLoader，是在start以后才加载完成，所以需要注册callback，等待完成
    local prefabLoader = XUiHelper.TryGetComponent(transform, "", "XPrefabLoader")
    if not prefabLoader then
        XLog.Error("[XUiPanelModelV2P6] 找不到品质图标的PrefabLoader")
        return
    end

    -- 兼容新增c#接口
    if prefabLoader.RegisterCallback then
        prefabLoader:RegisterCallback(function()
            local icon = XMVCA.XCharacter:GetCharacterQualityEffectIcon(quality)
            local resource = self._Control:GetLoader():Load(icon)
            if resource then
                local child = transform:GetChild(0)
                ---@type UnityEngine.MeshRenderer
                local meshRenderer = XUiHelper.TryGetComponent(child, "FX/01", "MeshRenderer")

                if meshRenderer then
                    meshRenderer.sharedMaterial:SetTexture("_MainTex", resource)
                end
            end
        end)
    end

...
function XUiPanelGeneralSkill:Refresh(characterId, generalSkillIndex, robotId)
...
function XUiPanelGeneralSkill:RefreshCurInfo()
        ...
        if active then
            ...
            XUiHelper.RefreshCustomizedList(gridTempalte.parent, gridTempalte, gridCount, function (index, grid)
                ...
                if curIndexInTotal <= #relatedSkillIds then
                    ...
                    if XTool.IsNumberValid(self.RobotId) then -- 如果是机器人
                        local xRobot = XRobotManager.GetRobotById(self.RobotId)
                        local robotSkillDic = xRobot:GetSkillLevelDic() 
                        level = robotSkillDic[skillId] or 0
                        uiObj:GetObject("TxtLevel").text = level
                    ...
                    local values = XMVCA.XCharacter:GetModelSkillIdGeneralSkillIdsDic()[skillId]
                    ...
                    if XTool.IsNumberValid(self.RobotId) then
                        local xRobot = XRobotManager.GetRobotById(self.RobotId)
                        local enhanceSkillList = xRobot:GenarateRobotEnhanceSkillList()
                        level = 0
                        for k, data in pairs(enhanceSkillList) do
    ...
    -- 相关角色
    if self.ImgGeneralSkill then
        self.ImgGeneralSkill:SetRawImage(curGenralSkillConfig.Icon)
    end
...
function XUiPanelGeneralSkill:OnGridRelatedSkillClick(config, skillLv, isSkillLock)
    XLuaUiManager.Open("UiSkillDetailsForGeneralSkillTips", config, skillLv, isSkillLock)
end
...
function XUiPanelGeneralSkill:OnGridPlayerInfoCharacterClick(charId)
    ...
    -- XLuaUiManager.Open("UiCharacterAttributeDetail", charId, XEnumConst.UiCharacterAttributeDetail.BtnTab.GeneralSkill)
    self.Parent:RefreshByCharacterId(charId, XEnumConst.UiCharacterAttributeDetail.BtnTab.GeneralSkill)
end

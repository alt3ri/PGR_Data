...
function XUiDormComponent:OnAwake()
    ...
    XEventManager.AddEventListener(XEventId.EVENT_HOME_OBJ_SHOW_DIALOG_BOX, self.OnShowDialogBox, self)
    XEventManager.AddEventListener(XEventId.EVENT_HOME_OBJ_HIDE_DIALOG_BOX, self.OnHideDialogBox, self)
    XEventManager.AddEventListener(XEventId.EVENT_HOME_OBJ_SHOW_3DUI_OBJ, self.OnShow3DObj, self)
    XEventManager.AddEventListener(XEventId.EVENT_HOME_OBJ_HIDE_3DUI_OBJ, self.OnHide3DObj, self)
...
function XUiDormComponent:OnDestroy()
    ...
    XEventManager.RemoveEventListener(XEventId.EVENT_HOME_OBJ_SHOW_DIALOG_BOX, self.OnShowDialogBox, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_HOME_OBJ_HIDE_DIALOG_BOX, self.OnHideDialogBox, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_HOME_OBJ_SHOW_3DUI_OBJ, self.OnShow3DObj, self)
    XEventManager.RemoveEventListener(XEventId.EVENT_HOME_OBJ_HIDE_3DUI_OBJ, self.OnHide3DObj, self)
...
----------------------- 外部调用 ------------------------------------
-- 对话气泡显示
function XUiDormComponent:OnShowDialogBox(id, contextId, transform, cb, isPet)
    -- 处理已经在显示中
    if self.DialogGridsDir[id] then
        self.DialogGridsDir[id]:RefreshContext(contextId)
        return
    end
    ...
    if #self.DialogGridsList > 0 then
        ...
        temp:Show(self.CurRoomId, id, contextId, transform, cb, isPet)
        self.DialogGridsDir[id] = temp
        return
    end
    ...
    gridBox:Show(self.CurRoomId, id, contextId, transform, cb, isPet)
    grid.transform:SetParent(self.DialogContainer, false)
    self.DialogGridsDir[id] = gridBox
end

-- 对话气泡隐藏
function XUiDormComponent:OnHideDialogBox(id)
    if not self.DialogGridsDir[id] then
        return
    end

    self.DialogGridsDir[id]:Hide()
    table.insert(self.DialogGridsList, self.DialogGridsDir[id])
    self.DialogGridsDir[id] = nil
end

-- 3DUI显示
function XUiDormComponent:OnShow3DObj(id, effectId, transform, bindWorldPos, renderUIProxy, headTransform, isPet)
    -- 处理已经在显示中
    if self.Obj3DGridsDir[id] then
        self.Obj3DGridsDir[id]:RefreshEffect(effectId, bindWorldPos, renderUIProxy)
        return
    end
    ...
    if #self.Obj3DGridsList >= 5 then
        ...
        temp:Show(id, effectId, transform, bindWorldPos, renderUIProxy, headTransform, isPet)
        self.Obj3DGridsDir[id] = temp
        return
    end
    ...
    gridBox:Show(id, effectId, transform, bindWorldPos, renderUIProxy, headTransform, isPet)
    self.Obj3DGridsDir[id] = gridBox
end

-- 3DUI隐藏
function XUiDormComponent:OnHide3DObj(id)
    if not self.Obj3DGridsDir[id] then
        return
    end

    self.Obj3DGridsDir[id]:Hide()
    table.insert(self.Obj3DGridsList, self.Obj3DGridsDir[id])
    self.Obj3DGridsDir[id].Transform:SetParent(self.Obje3DContainer, false)
    self.Obj3DGridsDir[id] = nil
end

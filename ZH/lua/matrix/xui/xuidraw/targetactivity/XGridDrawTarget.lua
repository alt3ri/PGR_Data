...
function XGridDrawTarget:Ctor(ui, parent)
    ...
    local path1 = "BtnInfo/Normal/PanelProbability/TxtProbability"
    local textPercentA = XUiHelper.TryGetComponent(self.Transform, path1, "Text")
    if textPercentA then
        textPercentA.text = XUiHelper.GetText("PercentSText")
    end

    local path2 = "BtnInfo/Select/PanelProbability/TxtProbability"
    local textPercentB = XUiHelper.TryGetComponent(self.Transform, path2, "Text")
    if textPercentB then
        textPercentB.text = XUiHelper.GetText("PercentSText")
    end
end
...
function XGridDrawTarget:SetData(index, templateId, goodsType, isSelect)
    ...
    self.AssignTagCfg = XDrawConfigs.GetDrawNewPlayerAssignTagCfg(templateId)
    ...
    -- 推荐页签
    self.PanelRecommend.gameObject:SetActiveEx(self.AssignTagCfg and self.AssignTagCfg.IsRecommend and not XMVCA.XCharacter:IsOwnCharacter(self.CharId))
...
function XGridDrawTarget:SetCharacterData()
    ...
    for _, stateName in ipairs(StateList) do
        ...
        local txtTips = uiObj:GetObject("TxtTips")
        if self.AssignTagCfg then
            txtTips.gameObject:SetActiveEx(true)
            txtTips.text = self.AssignTagCfg.TagText
        else
            txtTips.gameObject:SetActiveEx(false)
        end

...
function XUiEquipResonanceAwakeV2P6:OnNotify(evt, ...)
    if evt == XEventId.EVENT_EQUIP_AWAKE_NOTYFY then
        ...
        self.Parent:OnOverClockingSuccess(self.Pos)
...
-- 刷新名称
function XUiEquipResonanceAwakeV2P6:UpdateEquipName()
    ...
    local key = "AwarenessAwakePos" .. tostring(self.Pos)
...
function XUiEquipResonanceAwakeV2P6:UpdateResonanceSkill()
    ...
    -- 刷新意识套装的图
    local charId = XMVCA.XEquip:GetResonanceBindCharacterId(self.EquipId, self.Pos)
...
function XUiEquipResonanceAwakeV2P6:GetAwakeConsumeCoin()
    return self._Control:GetAwakeConsumeCrystalCoin(self.EquipId)
end

function XUiEquipResonanceAwakeV2P6:GetAwakeConsumeItemList()
    return self._Control:GetAwakeConsumeItemCrystalList(self.EquipId)
end
...
function XUiEquipResonanceAwakeV2P6:OnBtnAwakeClick()
    ...
    local bindCharacterId = XMVCA.XEquip:GetResonanceBindCharacterId(equipId, pos)
    ...
    XUiManager.DialogTip(title, content, XUiManager.DialogType.Normal, nil, function()
        XMVCA.XEquip:Awake(equipId, pos, XEnumConst.EQUIP.AWAKE_CRYSTAL_MONEY)
    end)
end

...
---@class XUiNode
XUiNode = XClass(nil, "XUiNode", true)

XClassPartialRequire("XUi/XUiBase/XUiNodeOverride", "XUiNode")
...
function XUiNode:SetVisible(enable)
    if enable then
        self:Open()
    else
        self:Close()
...
function XUiNode:DelayCall(func, delayTime)
    if self._TweenAnimationAgency then
        self._TweenAnimationAgency:DelayCall(func, delayTime)
    end
end

function XUiNode:DelayCallRaw(func, delayTime)
    if self._TweenAnimationAgency then
        self._TweenAnimationAgency:DelayCallRaw(func, delayTime)
    end
end

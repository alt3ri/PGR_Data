...
---@class XUiGachaAlphaMain : XLuaUi 拉弥亚卡池
local XUiGachaAlphaMain = XLuaUiManager.Register(XLuaUi, "UiGachaAlphaMain")

function XUiGachaAlphaMain:OnAwake()
    ...
    self._HasBeenKey = XGachaConfigs.GetSkipAnimCacheKey("GachaAlphaHasBeenKey")
    self._SkipBtnKey = XGachaConfigs.GetSkipAnimCacheKey("UiGachaAlpha")
...
function XUiGachaAlphaMain:OnStart(gachaId, autoOpenStory)
...
function XUiGachaAlphaMain:OnDisable()
    -- 本地缓存skip按钮状态
...
-- 记录战斗前后数据
function XUiGachaAlphaMain:OnReleaseInst()
    return {
        IsGoFight = true
    }
end

function XUiGachaAlphaMain:OnResume(data)
    data = data or {}
    self._IsGoFight = data.IsGoFight
end

function XUiGachaAlphaMain:SetSelfActive(flag)
...
function XUiGachaAlphaMain:InitButton()
    ...
    self:RegisterClickEvent(self.BtnAward, function()
        XLuaUiManager.Open("UiGachaAlphaLog", self._GachaCfg, 1)
    end)
    self:RegisterClickEvent(self.BtnHelp, function()
        XLuaUiManager.Open("UiGachaAlphaLog", self._GachaCfg)
    end)
...
function XUiGachaAlphaMain:Init3DSceneInfo()
    ...
    ---- 阴影要放在武器模型加载完之后
    if not XTool.UObjIsNil(self.Panel3D.ModelAlpha) then
        CS.XShadowHelper.AddShadow(self.Panel3D.ModelAlpha.gameObject, true)
    end
...
function XUiGachaAlphaMain:OnChildClose()
...
function XUiGachaAlphaMain:AutoOpenChild()
...
-- 开启/关闭角色的视线跟随
function XUiGachaAlphaMain:SetXPostFaicalControllerActive(flag)
    if XTool.UObjIsNil(self.Panel3D.ModelAlpha) then
        return
    end
    local targetComponent = self.Panel3D.ModelAlpha:GetComponent(typeof(CS.XPostFaicalController))
...
function XUiGachaAlphaMain:SetXPostFaicalControllerEnable(flag)
    if XTool.UObjIsNil(self.Panel3D.ModelAlpha) then
        return
    end
    local targetComponent = self.Panel3D.ModelAlpha:GetComponent(typeof(CS.XPostFaicalController))
...
function XUiGachaAlphaMain:PlayEnableAnim(cb)
...
function XUiGachaAlphaMain:PlayLongEnableAnim(cb)
    self.AnimEnableLong:PlayTimelineAnimation()
    if CS.UnityEngine.Application.platform == CS.UnityEngine.RuntimePlatform.Android then
        self._CG:PlayCG(1015301)
    else
        self._CG:PlayCG(10153)
    end
...
function XUiGachaAlphaMain:PlayShortEnableAnim(cb)
...
function XUiGachaAlphaMain:RefreshUiShow()
    -- 资源栏
...
function XUiGachaAlphaMain:RefreshReddot()
    -- 红点
...
-- 打开gacha道具购买界面
function XUiGachaAlphaMain:OpenGachaItemShop(openCb, gachaCount)
    -- 购买上限检测
    local gachaBuyTicketRuleConfig = XGachaConfigs.GetGachaItemExchangeCfgById(self._GachaCfg.ExchangeId)
    ...
    XLuaUiManager.Open("UiGachaAlphaBuyTicket", self._GachaCfg, itemData1, itemData2, targetData, gachaCount, function()
        self:RefreshUiShow()
    end)
...
function XUiGachaAlphaMain:CheckIsCanGacha(gachaCount)
    ...
    if not self["IsCanGacha" .. gachaCount] then
        if gachaCount == 10 and not self.IsGachaTimesEnd then
            XUiManager.TipText("GachaAlphaItemNoEnough")
        end
        return
    end
...
-- 抽卡流程
function XUiGachaAlphaMain:DoGacha(gachaCount, isSkipToShow)
    ...
    if self._IsCanGacha then
        ...
        local successCb = function(rewardList, newUnlockGachaId, res)
            ...
            self._FinishCbTrigger = function(isSkipToShow2)
                ...
                self:StopAnime()
                XLuaUiManager.Open("UiGachaAlphaShow", self._GachaId, self.RewardList, nil, isSkipToShow and gachaCount > 1) -- 单抽不能跳过奖励展示
...
function XUiGachaAlphaMain:PlayGachaAnime(quality)
    self.PlayAnime = true
...
-- 关闭所有特效
function XUiGachaAlphaMain:StopAnime()
    self.BtnSkipGacha.gameObject:SetActiveEx(false)
end

-- 传入要抽多少抽
function XUiGachaAlphaMain:OnBtnGachaClick(gachaCount)
...
-- 跳过抽卡演出,直接展示所有奖励
function XUiGachaAlphaMain:OnBtnSkipGachaClick()
    self.PlayAnime = false
    if self._FinishCbTrigger then
...
function XUiGachaAlphaMain:OnBtnStoryLineClick(isAutoOpen)
    ...
    self.Panel3D.UiFarCamStory.gameObject:SetActiveEx(true)
    self.Panel3D.UiNearCamStory.gameObject:SetActiveEx(true)
    self:OpenOneChildUi("UiGachaAlphaStageLine", self._GachaCfg.FestivalActivityId, self.Panel3D, isAutoOpen)
end

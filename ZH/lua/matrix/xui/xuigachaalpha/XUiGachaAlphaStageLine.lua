...
function XUiGachaAlphaStageLine:OnAwake()
...
function XUiGachaAlphaStageLine:OnStart(chapterId, panel3D, isAutoOpen)
...
function XUiGachaAlphaStageLine:OnEnable()
...
function XUiGachaAlphaStageLine:OnDisable()
    for _, stage in pairs(self._Stages) do
        stage:Close()
    end
end

function XUiGachaAlphaStageLine:Refresh3DSceneInfo()
...
function XUiGachaAlphaStageLine:InitButton()
...
function XUiGachaAlphaStageLine:HandleStages()
    for i = 1, #self._StageIds do
        local itemStage = self.PanelStageContent:Find(string.format("Stage%d", i))
        if not itemStage then
            XLog.Error("XUiGachaAlphaStageLine:HandleStages() 函数错误: 游戏物体PanelStageContent下找不到名字为:" .. string.format("Stage%d", i) .. "的游戏物体")
            return
        end
        -- 组件初始化
        self._StageGroup[i] = itemStage
        if not self._Stages[i] then
            self._Stages[i] = require("XUi/XUiGachaAlpha/Grid/XUiGridGachaAlphaStageItem").New(itemStage, self)
        end
        self._Stages[i]:Open()
...
function XUiGachaAlphaStageLine:HandleStageLines()
    ...
    for i = 1, #self._StageIds - 1 do
        ...
        if not itemLine then
            XLog.Error("XUiGachaAlphaStageLine:SetUiData() error: prefab not found a child name:" .. string.format("Line%d", i))
            return
        end
...
-- 背景
function XUiGachaAlphaStageLine:SwitchFestivalBg()
    self.RImgFestivalBg.gameObject:SetActiveEx(false)
end

-- 加载特效
function XUiGachaAlphaStageLine:LoadEffect(effectUrl)
...
-- 更新刷新
function XUiGachaAlphaStageLine:RefreshFestivalNodes()
...
-- 更新节点线条
function XUiGachaAlphaStageLine:UpdateNodeLines()
    if not self._ChapterTemplate or not self._StageIds then
        return
    end
...
function XUiGachaAlphaStageLine:SetStageLineActive(index, isActive)
...
-- 通过Stage调用的接口
function XUiGachaAlphaStageLine:OpenStageDetails(stageId)
    XLuaUiManager.Open("UiGachaAlphaStageDetail", stageId)
end

-- 选中关卡描边效果
function XUiGachaAlphaStageLine:UpdateNodesSelect(stageId)
...
-- 取消选中
function XUiGachaAlphaStageLine:ClearNodesSelect()
...
function XUiGachaAlphaStageLine:SetPanelStageListMovementType(moveMentType)
...
function XUiGachaAlphaStageLine:MoveIntoStage(stageIndex)
...
function XUiGachaAlphaStageLine:OnSceneBtnBackClick()
    self:Close()
    self.ParentUi:Close()
end

function XUiGachaAlphaStageLine:OnGotoGacha()
    self:Close()
    self.ParentUi:OnChildClose()
end

return XUiGachaAlphaStageLine
...
function XUiGuildWarStageDetail:OnBtnAutoFightClicked()
    ...
    if self.IsMonsterStatus then -- 敌人节点
        local monster = self:GetNodeMonster()
        if monster == nil then
            --当玩家扫荡击杀怪物后 Action的Notify可能回比MonsterUpdate晚 会导致这里为空
            return
    ...
    elseif self._CanSweepByDragonRageRule then -- 周目扫荡
        damage = getRoundingValue(self.Node:GetLastRoundMaxDamage() / self.Node:GetMaxHP() * 100, 2)
    else
        damage = getRoundingValue((self.Node:GetMaxDamage() / self.Node:GetMaxHP()) * 100, 2)
    end
    ...
    local sureSweepFunc = function()
        -- 补充判断_Control是否存在，处理线上引用丢失 
        -- 如果是龙怒玩法的周目扫荡则要单独提示
        local isHideSweepTips = not self._Control and true or self._Control.DragonRageControl:GetIsHideSweepTips()

        if self._CanSweepByDragonRageRule and not self._CanSweepByBaseRule and not isHideSweepTips then
            local hintInfo = {
                HintText = XGuildWarConfig.GetClientConfigValue('DragonRageRoundNoTips'),
                Status = isHideSweepTips,
                SetHintCb = function()
                    self._Control.DragonRageControl:SetHideSweepTips()
                end,
            }

            XUiManager.DialogHintTip(XUiHelper.GetText('TipTitle'), XGuildWarConfig.GetClientConfigValue('DragonRageSweepTips'), nil, nil, doSweepFunc, hintInfo)
        else
            doSweepFunc()
        end
    end
...
function XUiGuildWarStageDetail:RefreshButtonStatus()
    ...
    if self.Node:GetNodeType() ~= XGuildWarConfig.NodeType.Resource then
        ...
        self.MoveCostUnit = cost or math.maxinteger
    end
    ...
    local isPlayerNode = node:GetIsPlayerNode()
    local isShowAutoFight = node:CheckCanSweep() and isPlayerNode
    ...
    self._CanSweepByDragonRageRule = self._Control.DragonRageControl:CheckCanSweepByDragonRageRule(node:GetUID(), node:GetNodeType(), not node:GetIsDead())
    self._CanSweepByBaseRule = isShowAutoFight
    
    self.BtnAutoFight.gameObject:SetActiveEx(isShowAutoFight or (self._CanSweepByDragonRageRule and isPlayerNode))

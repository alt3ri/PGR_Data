...
function XUiGridSGFurniturePhotoOp:OnBtnCancelClick()
    ...
    self.Parent:RevertSingle(self._Index, self._Id, self._Slot and self._Slot:IsAlbumPhoto() or false)
    self.Parent:ExitEditMode()
end
...
---@field _FocusSlot XDormitory.XFurnitureSlot
...
function XUiPanelSGPhotoWallOp:InitCb()
    ...
    self._RemoveFunc = {
        [SgFurnitureType.SystemPhoto] = function(id, cfgId) self:OnRemoveSystemPhoto(id, cfgId) end,
        [SgFurnitureType.AlbumPhoto] = function(id, cfgId) self:OnRemoveAlbumPhoto(id, cfgId) end,
...
function XUiPanelSGPhotoWallOp:CreateFurniture(index, id, visible, ignoreUpdate, isAlbumPhoto)
    local fightData = self._Control:GetFightFurnitureData(id, isAlbumPhoto)
...
function XUiPanelSGPhotoWallOp:RemoveFurniture(index, id)
    ...
    local isAlbumPhoto = slot and slot:IsAlbumPhoto() or false
    local majorType, cfgId
    if isAlbumPhoto then
        cfgId = 0
        majorType = self._Control:GetMajorType(self._Control:GetAlbumPhotoTypeId())
    else
        cfgId = self._Control:GetFurnitureConfigIdById(id)
    ...
    self:GetPutWallContainerData():RemoveFurniture(id, isAlbumPhoto)
    
    self:RefreshAllGridOp()
    self:FullUpdateView()
end

function XUiPanelSGPhotoWallOp:RevertSingle(index, id, isAlbumPhoto)
    if not id or id <= 0 then
        return
    end
    ...
    if isAlbumPhoto then
        ...
        isPhoto = majorType == XMVCA.XSkyGardenDorm.XSgFurnitureType.SystemPhoto
    end
    
    XMVCA.X3CProxy:Send(CS.X3CCommand.CMD_DORMITORY_SET_PHOTO_OR_ADORN_TRANSFORM, {
        Id = (not isAlbumPhoto) and id or 0,
        PhotoId = isAlbumPhoto and id or 0,
        IsPhoto = isPhoto,
        IsDefault = isDefault,
        X = x,
        Y = y,
...
function XUiPanelSGPhotoWallOp:OnRemoveSystemPhoto(id, cfgId)
...
function XUiPanelSGPhotoWallOp:OnRemoveAlbumPhoto(id, cfgId)
    XMVCA.X3CProxy:Send(X3C_CMD.CMD_DORMITORY_DESTROY_PHOTO, {
        Id = 0,
        PhotoId = id
    })
...
function XUiPanelSGPhotoWallOp:SetTopLayer(index, id)
    ...
    if majorType == SgFurnitureType.SystemPhoto 
            or majorType == SgFurnitureType.AlbumPhoto then
    ...
    if cmd then
        ...
        local f = self:GetPutWallContainerData():GetFurniture(id, isAlbumPhoto)
...
function XUiPanelSGPhotoWallOp:RotateByHandle(z)
    if not self._FocusSlot then
        return
    end
    self._FocusSlot:SetFurnitureRotationByDirection(z)
end

...
function XUiGridSGCardItem:InitView()
    self._CheckNewUiName = "UiSkyGardenCafeHandBook"
...
function XUiGridSGCardItem:RefreshSmall(card)
    ...
    self._CardId = cardId
    self.RImgHead:SetRawImage(self._Control:GetCustomerSmallIcon(cardId))
    ...
    self.Effect.gameObject:SetActiveEx(false)
    if not self._ClickSmallCb then
...
function XUiGridSGCardItem:Update(card, index)
    self._Index = index
    self:RefreshSmall(card)
end
...
function XUiGridSGCardItem:RefreshBig(card, useCount)
    ...
    local lockDetail = self._Control:GetCustomerUnlockDesc(cardId)
    unlock = unlock or string.IsNilOrEmpty(lockDetail)
    --local unlock = true
...
function XUiGridSGCardItem:RefreshNewMark()
    if not self:IsCheckNewCard() then
        return
    end
    local v = self._Control:CheckCardNewMark(self._CardId)
    self.ImgNew.gameObject:SetActiveEx(v)
end
...
function XUiGridSGCardItem:OnBtnCheckClick()
    ...
    if self:IsCheckNewCard() then
        self.ImgNew.gameObject:SetActiveEx(false)
        self._Control:MarkNewCard(self._CardId)
        
        self._Control:SaveNewMark()
    end
...
function XUiGridSGCardItem:IsCheckNewCard()
    if not self.ImgNew then
        return false
    end
    local check = self.Parent and self.Parent.Name == self._CheckNewUiName and self._Control:CheckCardUnlock(self._CardId)
    if not check then
        self.ImgNew.gameObject:SetActiveEx(false)
    end
    return check

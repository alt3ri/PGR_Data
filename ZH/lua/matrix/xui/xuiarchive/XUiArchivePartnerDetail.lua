...
function XUiArchivePartnerDetail:UpdatePartnerInfo()
    ...
    self:UpdateHideChangeBtn4PartnerLink()
end
...
--region 辅助机 -> 异界装备
-- 在异界装备下, 隐藏异界装备的切换按钮
function XUiArchivePartnerDetail:UpdateHideChangeBtn4PartnerLink()
    local partnerId = self._PartnerId
    local partnerType = XPartnerConfigs.GetPartnerType(partnerId)
    if partnerType == XPartnerConfigs.PartnerType.Link then
        self.BtnStandby.transform.parent.gameObject:SetActiveEx(false)
        self:SetCameraType(XPartnerConfigs.CameraType.Combat)
    else
        self.BtnStandby.transform.parent.gameObject:SetActiveEx(true)
    end
end
--endregion 辅助机 -> 异界装备
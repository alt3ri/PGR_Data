...
---@class XUiGachaBiankaResult : XLuaUi
local XUiGachaBiankaResult = XLuaUiManager.Register(XLuaUi, "UiGachaBianka402Result")

local MODE_LOOP = 1

function XUiGachaBiankaResult:OnAwake()
    self:InitAutoScript()
end

function XUiGachaBiankaResult:OnStart(gachaId, rewardList, backCb, background)
...
function XUiGachaBiankaResult:OnEnable()
    XUiHelper.PopupFirstGet()
end

function XUiGachaBiankaResult:OnDisable()
    XDataCenter.AntiAddictionManager.EndDrawCardAction()
end

function XUiGachaBiankaResult:Update()
...
function XUiGachaBiankaResult:InitAutoScript()
    self._SpecialSoundMap = {}
    self:AutoAddListener()
end

function XUiGachaBiankaResult:GetAutoKey(uiNode, eventName)
    if not uiNode then
        return
    end
...
function XUiGachaBiankaResult:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiGachaBiankaResult:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end
...
function XUiGachaBiankaResult:AutoAddListener()
    self._AutoCreateListeners = {}
    self:RegisterClickEvent(self.BtnBack, self.OnBtnBackClick)
end

function XUiGachaBiankaResult:OnBtnBackClick()
...
function XUiGachaBiankaResult:SetupRewards()
...
function XUiGachaBiankaResult:ShowResult()
...
--已经拥有的角色转换碎片过程
function XUiGachaBiankaResult:ShowTrans()
    local count = 0
    for i = 1, #self._RewardList do
        if self._RewardList[i].ConvertFrom ~= 0 then
            count = count + 1

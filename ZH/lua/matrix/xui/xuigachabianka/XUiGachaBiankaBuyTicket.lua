---@class XUiGachaBiankaBuyTicket : XLuaUi
local XUiGachaBiankaBuyTicket = XLuaUiManager.Register(XLuaUi, "UiGachaBianka402BuyTicket")

function XUiGachaBiankaBuyTicket:OnAwake()
...
function XUiGachaBiankaBuyTicket:OnStart(gachaCfg, itemData1, itemData2, targetItemData, forceBuyCount, closeCb)
...
function XUiGachaBiankaBuyTicket:OnEnable()
    self:Refresh()
...
function XUiGachaBiankaBuyTicket:InitItemCardPanel(index)
    local panel = {}
...
function XUiGachaBiankaBuyTicket:InitButton()
...
function XUiGachaBiankaBuyTicket:Refresh()
    self:RefreshUiShow()
    self:RefreshBuyAmount(tonumber(self.TxtSelect.text))
end

function XUiGachaBiankaBuyTicket:GetItemData(index)
    return index == 1 and self._ItemData1 or self._ItemData2
end

function XUiGachaBiankaBuyTicket:RefreshUiShow()
...
function XUiGachaBiankaBuyTicket:RefreshBuyAmount(count)
    self._BuyAmount = count
end

function XUiGachaBiankaBuyTicket:CheckBuyAmountLimit(count)
...
function XUiGachaBiankaBuyTicket:OnSelfSetBuyCount(count)
...
function XUiGachaBiankaBuyTicket:OnBtnMaxClick()
...
function XUiGachaBiankaBuyTicket:OnBtnAddSelectClick()
...
function XUiGachaBiankaBuyTicket:OnBtnMinusSelectClick()
    local count = tonumber(self.TxtSelect.text)
    local targetCount = count - 1
    if self:CheckBuyAmountLimit(targetCount) then
        return
...
-- text改动的回调
function XUiGachaBiankaBuyTicket:OnSelectTextChange()
    local count = tonumber(self.TxtSelect.text)
...
function XUiGachaBiankaBuyTicket:OnImgBtnClick(index)
    local data = XDataCenter.ItemManager.GetItem(self:GetItemData(index).ItemId)
    XLuaUiManager.Open("UiTip", data)
end

function XUiGachaBiankaBuyTicket:OnBtnBuyClick(index)
    -- CD检测
...
function XUiGachaBiankaBuyTicket:CheckBuyCD()
...
return XUiGachaBiankaBuyTicket
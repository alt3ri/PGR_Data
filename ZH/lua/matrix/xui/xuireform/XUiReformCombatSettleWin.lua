...
function XUiReformCombatSettleWin:OnStart(winData, isUnlockHardMode)
    ...
    if self._Control:IsHardStage(stage) then
        isHardMode = true
    end
    local starMax = XMVCA.XReform:GetStarMax(stage, isHardMode)
    ...
    self._GoalDesc = XMVCA.XReform:GetGoalDesc(stage)
    ...
    --end
    self._StarMax = XMVCA.XReform:GetStarMax(stage, isHardMode)
    self._Stage = stage
end

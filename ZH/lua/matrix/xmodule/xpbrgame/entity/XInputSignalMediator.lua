--- 输入信号中介者，用于统筹和中转输入信号
---@class XInputSignalMediator
local XInputSignalMediator = XClass(nil, 'XInputSignalMediator')

function XInputSignalMediator:Ctor(signalMap)
    self.SignalTypeList = {}
...
function XInputSignalMediator:ReceiveInputSignal(signalType, signalData)
...
--- 清空所有输入信号记录
function XInputSignalMediator:ClearAllInputSignalRecord()
...
--- 注册信号处理函数
function XInputSignalMediator:RegisterSignalHandler(signalType, handlerFunc)
...
function XInputSignalMediator:StartInputSignalUpdateTimer()
...
function XInputSignalMediator:StopInputSignalUpdateTimer()
...
function XInputSignalMediator:Update()

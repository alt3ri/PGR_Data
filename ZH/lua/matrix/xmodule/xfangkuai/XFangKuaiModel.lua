...
local TableKey = {
    ...
    FangKuaiScoreRate = {},
    FangKuaiItem = { CacheType = XConfigUtil.CacheType.Normal },
...
function XFangKuaiModel:CheckStageGroupRedPoint(StageGroupId)
    ...
    return not self:IsStageGroupEnterOnce(StageGroupId)
end
...
function XFangKuaiModel:GetItemIdByBlockOnCreate(blockKind)
    if not self._ItemIdBlockDict then
        self._ItemIdBlockDict = {}
        ---@type XTableFangKuaiItem[]
        local itemConfigs = self._ConfigUtil:GetByTableKey(TableKey.FangKuaiItem)
        for _, config in pairs(itemConfigs) do
            for _, blockType in pairs(config.BlockTypes) do
                if self._ItemIdBlockDict[blockType] then
                    XLog.Error(string.format("道具配置错误：方块类型%s拥有多个道具", blockType))
                else
                    self._ItemIdBlockDict[blockType] = config.Id
                end
            end
        end
    end
    return self._ItemIdBlockDict[blockKind]
end
...
---@return XTableFangKuaiBrickFace
...
function XFangKuaiModel:GetArchieveConfig(blockType)
    ---@type XTableFangKuaiArchieve[]
    local configs = self._ConfigUtil:GetByTableKey(TableKey.FangKuaiArchieve)
    for _, v in pairs(configs) do
        if v.Type == blockType then
            return v
        end
    end
    return nil
...
function XFangKuaiModel:GetArchieveByBlockTypes(stageId)
    local typeDict = {}
    local archieves = {}
    local templates = self:GetBlockTemplates()
    for _, blocks in pairs(templates[stageId]) do
        for _, block in pairs(blocks) do
            local blockType = block.Type
            if not typeDict[blockType] then
                typeDict[blockType] = true
                local config = self:GetArchieveConfig(blockType)
                if config then
                    table.insert(archieves, config)
                end
            end
        end
    end
...
function XFangKuaiModel:RecordInitRound()
    ---@type CollectionRecordData
    self._CollectionRecordData = {}
    self._CollectionRecordData.InitRound = self.ActivityData:GetAllMaxRound()
    self._CollectionRecordData.IsNeedShowCollection = self.ActivityData:IsNoStagePassed() -- 第一次通关时显示获得藏品弹框
end

function XFangKuaiModel:RecordFinalRound(round)
    if self._CollectionRecordData then
        if not self._CollectionRecordData.FinalRound or self._CollectionRecordData.FinalRound < round then
            self._CollectionRecordData.FinalRound = round
        end
    end
end

function XFangKuaiModel:GetCollectionRecordData()
    return self._CollectionRecordData
end

function XFangKuaiModel:ClearCollection()
    self._CollectionRecordData = nil
end

---@class CollectionRecordData
---@field InitRound number
---@field FinalRound number
---@field IsNeedShowCollection boolean

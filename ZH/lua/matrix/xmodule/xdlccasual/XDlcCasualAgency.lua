...
function XDlcCasualAgency:DlcGetWorldType()
    return XEnumConst.DlcWorld.WorldType.Cube
end

function XDlcCasualAgency:DlcCheckActivityInTime()
    return self:ExCheckInTime()
end

function XDlcCasualAgency:DlcGetAttribIdByNpcId(npcId)
    local activityId = self._Model:GetActivityId()
    local characterPoolId = self._Model:GetActivityCharacterPoolIdById(activityId)
    local characterIds = self._Model:GetCharacterPoolListById(characterPoolId)

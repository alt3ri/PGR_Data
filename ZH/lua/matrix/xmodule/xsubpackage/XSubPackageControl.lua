...
function XSubPackageControl:GetGroupNameShort(groupId)
    local template = self._Model:GetGroupTemplate(groupId)
    return template and template.NameShort or "???"
end
...
function XSubPackageControl:GetSubpackageDownloadTaskId(subpackageId)
    return self._Model:GetSubpackageDownloadTaskId(subpackageId)
end

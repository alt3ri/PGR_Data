...
function XSgDormData:Reset()
    ...
    self._LocalPhotoList = nil
    self._AreaType2LayoutData = nil
    self._AreaType2LayoutId = nil
end

function XSgDormData:NotifySgPhotoData(dataList)
    if XTool.IsTableEmpty(dataList) then
        for i = #self._LocalPhotoList, 1, -1 do
            table.remove(self._LocalPhotoList, i)
        end
        return
    end
    local list = self._LocalPhotoList
    local cnt = #list
    for i, data in pairs(dataList) do
        ...
        if not temp then
            temp = {
                Id = data.Id,
                CheckSalt = data.CheckSalt,
                CreateTime = data.CreateTime,
            }
            list[i] = temp
        else
            temp.Id = data.Id
            temp.CheckSalt = data.CheckSalt
            temp.CreateTime = data.CreateTime
        end
    end
    if #dataList < cnt then
        for i = cnt, #dataList + 1, -1 do
            table.remove(list, i)
        end
    end
end
...
function XSgDormData:NotifySgDormLayoutChanged(data)
    if not data then
        return
    end
    local layoutList = data.LayoutList
    self:UpdateLayoutList(layoutList)
end
...
function XSgDormData:UpdateOwnDormFurnitureList(furnitureList)
    ...
    XMVCA.XSkyGardenDorm:Notify(XMVCA.XSkyGardenDorm.XEventId.EVENT_DORM_FURNITURE_REFRESH)
end
...
function XSgDormData:GetFurnitureCount(cfgId)
    if not self:CheckFurnitureUnlockByConfigId(cfgId) then
        return
    end
    local cnt = 0
    for _, furniture in pairs(self._OwnFurnitureDict) do
...
---@return number[]
function XSgDormData:GetNotPutFurnitureIdList(cfgId, containerFurnitureData, isAlbumPhoto)
    ...
    for _, id in pairs(list) do
        ...
        if container:GetId() ~= id and not containerFurnitureData:CheckContainFurnitureById(id, isAlbumPhoto) then
            temp[#temp + 1] = id
        end
    end
    return temp
...
function XSgDormData:GetAlbumPhotoList()
    return self._LocalPhotoList
end

function XSgDormData:GetAlbumPhotoSalt(id)
    local salt = 0
    if not XTool.IsTableEmpty(self._LocalPhotoList) then
        for _, data in pairs(self._LocalPhotoList) do
            if data.Id == id then
                salt = data.CheckSalt
                break
            end
...
function XSgDormData:CheckContainFurnitureById(areaType, layoutId, id, isAlbumPhoto)
    local containerFurnitureData = self:GetContainerFurnitureData(areaType, layoutId)
    if not containerFurnitureData then
        return false
    end
    return containerFurnitureData:CheckContainFurnitureById(id, isAlbumPhoto)
end

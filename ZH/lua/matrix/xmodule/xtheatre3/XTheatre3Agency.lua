...
function XTheatre3Agency:InitRpc()
    ...
    XRpc.NotifyTheatre3SwitchLine = handler(self, self.NotifyTheatre3SwitchLine)
    XRpc.NotifyTheatre3PassFightNode = handler(self, self.NotifyTheatre3PassFightNode)
end
...
function XTheatre3Agency:CheckItemPressure(value)
    if self._Model.ActivityData then
        return value <= self._Model.ActivityData:GetFireItemCount()
    end
    return false
end
--endregion
...
function XTheatre3Agency:NotifyTheatre3AddStep(data)
    ...
    XEventManager.DispatchEvent(XEventId.EVENT_THEATRE3_ADD_STEP)
end
...
function XTheatre3Agency:NotifyTheatre3SwitchLine(data)
    self._Model.ActivityData:UpdateCurChapterId(data.ChapterId)
    XEventManager.DispatchEvent(XEventId.EVENT_THEATRE3_CHAPTER_CHANGE, self._Model:IsAdventureALine())
end

function XTheatre3Agency:NotifyTheatre3PassFightNode(data)
    self._Model.ActivityData:UpdatePassEventFightNodes(data.PassEventFightNodes)
end
--endregion
...
function XTheatre3Agency:PreFight(stage, teamId, isAssist, challengeCount)
    ...
    preFight.GeneralSkill = self._Model:GetTeamGeneralSkillId()
    preFight.EnterCgIndex = self._Model.ActivityData:GetEnterCgIndex()
    preFight.SettleCgIndex = self._Model.ActivityData:GetSettleCgIndex()
    return preFight
end
...
function XTheatre3Agency:FinishFight(settle)
    if settle.IsWin then
        ...
        if self:CheckCurNodeIsUnDefeatNode() then
            if self._Model:CheckChapterIsALine(self._Model.ActivityData:GetCurChapterId()) then
                XLuaUiManager.Open("UiTheatre3PlayMain")
            else
                XLuaUiManager.Open("UiTheatre3QuantumPlayMain")
            end
            return
        end
        if not self:CheckAndOpenSettle() then
...
function XTheatre3Agency:CheckCurNodeIsUnDefeatNode()
    if self._Model.ActivityData then
        local chapterDb = self._Model.ActivityData:GetCurChapterDb()
        if chapterDb then
            local nodeSlot, _ = chapterDb:GetLastNodeSlot()
            local eventId = nodeSlot:GetEventId()
            if nodeSlot and XTool.IsNumberValid(eventId) then
                local nodeCfg = self._Model:GetEventNodeCfgById(eventId)
                if nodeCfg then
                    return nodeCfg.UnDefeatNode == 1
                end
            end
        end
    end
...
function XTheatre3Agency:IsCurNodeMoneyShop()
    if self._Model.ActivityData then
        local chapterDb = self._Model.ActivityData:GetCurChapterDb()
        if chapterDb then
            local nodeSlot, _ = chapterDb:GetLastNodeSlot()
            if nodeSlot then
                local shopId = nodeSlot:GetShopId()
                if XTool.IsNumberValid(shopId) then
                    local shopCfg = self._Model:GetShopNodeCfgById(shopId)
                    return shopCfg and shopCfg.ShopType == XEnumConst.THEATRE3.ShopType.GiveMoney
                end
            end
        end
    end
...
function XTheatre3Agency:GetEventStepType(eventId, stepId)
    return self._Model:GetEventCfgByIdAndStep(eventId, stepId)
end

function XTheatre3Agency:CheckPassEventFightNode(nodeId)
    if not self._Model.ActivityData then
        return false
    end
    local nodes = self._Model.ActivityData:GetPassEventFightNodes()
    return table.indexof(nodes, nodeId)
end
--endregion
...
function XTheatre3Agency:RemoveStepView()
    ...
    XLuaUiManager.Remove("UiTheatre3PropChoose")
end
...
function XTheatre3Agency:CheckAndOpenSettle()
    ...
    local storyId = self:GetEndingStoryId(settle.EndId)
    if storyId then
        self:OpenBlackView()
        --结局剧情
        XDataCenter.MovieManager.PlayMovie(storyId, function()
...
function XTheatre3Agency:GetEndingStoryId(endId)
    local endConfig = self._Model:GetEndingById(endId)
    if not XTool.IsTableEmpty(endConfig.StoryId) then
        for i, v in ipairs(endConfig.StoryId) do
            local condition = endConfig.StoryCondition[i]
...
function XTheatre3Agency:ClearGeneralSkillId()
    self._Model:SetTeamGeneralSkillId()

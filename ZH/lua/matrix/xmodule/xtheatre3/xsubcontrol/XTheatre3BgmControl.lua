...
function XTheatre3BgmControl:_InitBgmNode()
    ...
    local quantumBgmNodeAsset = self:GetLoader():Load(url)
    if not quantumBgmNodeAsset then
        return
    end
    ---@type UnityEngine.Transform
    local audioManagerTran = CS.XAudioManager.AtomSource.transform
    if audioManagerTran then
        self._QuantumBgmNode = XUiHelper.Instantiate(quantumBgmNodeAsset, audioManagerTran)
    else
        self._QuantumBgmNode = XUiHelper.Instantiate(quantumBgmNodeAsset)
    end

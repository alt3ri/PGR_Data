...
function XTheatre3Activity:NotifyTheatre3Activity(data)
    ...
    self:_UpdateEndingRecord(data.EndingRecord)
    self:UpdateGetRewardIdData(data.GetRewardIds)
    self:UpdateFireItemCount(data.FireItemCount)
    ...
    self:UpdatePassEventFightNodes(data.PassEventFightNodes)
end
...
--region Record EndingRecord
function XTheatre3Activity:_InitEndingRecordParams()
    self._EndingRecord = {}
end

function XTheatre3Activity:_UpdateEndingRecord(data)
    if XTool.IsTableEmpty(data) then
        return
    end
    self._EndingRecord = data
end
...
function XTheatre3Activity:CheckEndingIsPass(endingId)
    ...
    if not XTool.IsTableEmpty(self._EndingRecord) then
        for _, recordEndingId in ipairs(self._EndingRecord) do
            if recordEndingId == endingId then
                return true
            end
        end
    end
    if not XTool.IsTableEmpty(self.PassDifficultyRecords) then
...
function XTheatre3Activity:_UpdateTeamData(data)
    ...
    if XTool.IsTableEmpty(data) then
        ...
        XMVCA.XTheatre3:ClearGeneralSkillId()
        return
    end
    ...
    self.CurTeamData:SetEnterCgIndex(data.EnterCgIndex)
    self.CurTeamData:SetSettleCgIndex(data.SettleCgIndex)
end
...
function XTheatre3Activity:GetEnterCgIndex()
    return self.CurTeamData and self.CurTeamData:GetEnterCgIndex() or 0
end

function XTheatre3Activity:GetSettleCgIndex()
    return self.CurTeamData and self.CurTeamData:GetSettleCgIndex() or 0
end
...
--- 效应选择逻辑依赖XTeam接口，需要获取XTeam对象
function XTheatre3Activity:GetTeamData()
...
function XTheatre3Activity:UpdateFireItemCount(count)
    self._FireItemCount = count or 0
end

function XTheatre3Activity:GetFireItemCount()
    return self._FireItemCount
end

function XTheatre3Activity:UpdatePassEventFightNodes(nodes)
    self._PassEventFightNodes = nodes
end

function XTheatre3Activity:GetPassEventFightNodes()
    return self._PassEventFightNodes
end

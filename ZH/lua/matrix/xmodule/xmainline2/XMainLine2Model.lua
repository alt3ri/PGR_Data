...
function XMainLine2Model:IsClientConfigExit(key)
    local cfgs = self._ConfigUtil:GetByTableKey(TableKey.MainLine2ClientConfig)
    return cfgs and cfgs[key] ~= nil
end
...
-- 获取章节上一次的解锁入口下标
function XMainLine2Model:GetChapterLastUnlockEntranceIndex(chapterId)
    local key = "ChapterUnlockEntranceIndex" .. tostring(chapterId)
    return self._SaveUtil:GetData(key)
end

-- 设置章节上一次的解锁入口下标
function XMainLine2Model:SetChapterLastUnlockEntranceIndex(chapterId, entranceIndex)
    local key = "ChapterUnlockEntranceIndex" .. tostring(chapterId)
...
function XMainLine2Model:IsSetPlayerGender()
    ...
    return genderType ~= nil
end

-- 获取主线指挥官性别
function XMainLine2Model:GetPlayerGender()
    -- 默认使用男指挥官，男指挥官没有配置时使用女指挥官
    local genderType = self._SaveUtil:GetData("GenderType") or XEnumConst.PLAYER.GENDER_TYPE.MAN
    return genderType
end

-- 设置主线指挥官性别，注意：主线进关卡的指挥官性别与剧情的指挥官性别是分开设置

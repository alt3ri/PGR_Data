...
function XFavorabilityAgency:HasStroyToBeUnlock(characterId)
    ...
    for _, story in pairs(storys) do
        ...
        local canStoryUnlock = self._Model:IsStorySatisfyUnlock(characterId, story.Id)
...
--[是否需要显示新解锁的标签：当缓存中未标记已解锁、且满足解锁条件时]
function XFavorabilityAgency:IsStoryShowNewTag(characterId, storyId)
    local storys = self:GetCharacterStoryById(characterId)
    if storys == nil then
        return false 
    end
    ...
    local isStoryUnlock = favorabilityDatas.UnlockStory[storyId]

    local canStoryUnlock = self._Model:IsStorySatisfyUnlock(characterId, storyId)
    if (not isStoryUnlock) and canStoryUnlock then
        return true
    end
...
function XFavorabilityAgency:HasStroyTaskCanFinish(characterId)
    ...
    if not XTool.IsTableEmpty(storys) then
        for _, story in pairs(storys) do
            if XTool.IsNumberValid(story.TaskId) then
                if XDataCenter.TaskManager.CheckTaskAchieved(story.TaskId) then
                    return true
                end
            end
        end
...
-- [好感度剧情是否满足好感等级的解锁条件]
function XFavorabilityAgency:CheckStoryIsSatisfyUnlockCondition(characterId, Id)
    return self._Model:IsStorySatisfyUnlock(characterId, Id)
end

---@param entranceType @入口类型：埋点约定1看板交流界面，2试玩关，3支线剧情
---@return number @打开结果：0成功，-1分包问题， -2角色传参问题
function XFavorabilityAgency:OpenUiStory(characterId, entranceType)
    if not XMVCA.XSubPackage:CheckSubpackage() then
        return -1
    end

    if not XTool.IsNumberValid(characterId) then
        XLog.Error('跳转角色好感度剧情关卡界面，角色Id无效：'..tostring(characterId))
        return -2
    end
    
    XLuaUiManager.Open("UiFavorabilityStory", characterId)

    if XTool.IsNumberValid(entranceType) then
        local dict = {}
        dict["entrance_type"] = entranceType
        dict["event_time"] = XTime.GetServerNowTimestamp()
        dict["character_id"] = characterId
        CS.XRecord.Record(dict, "900010", "FavorabilityStoryEnterRecord")
    end
...
-- 检查动作条件是否解锁/互动面板展示是否解锁
function XFavorabilityAgency:CheckCharacterActionUnlock(actionData, trustLv)
    local isUnlock = true
    
    local actionCfg = actionData.config and actionData.config or actionData
    
    for k, conditionId in pairs(actionCfg.UnlockCondition) do
        if not XConditionManager.CheckCondition(conditionId, actionCfg.CharacterId) then
            isUnlock = false
            break
        end
    end
    return actionCfg.UnlockLv <= trustLv and isUnlock
end

-- [试穿][检查动作是否满足解锁条件]/拍照试穿时用假的涂装id检测
--- func desc
...
function XFavorabilityAgency:CheckTryCharacterActionUnlock(actionData, trustLv, tryFashionId, trySceneId)
    ...
    for k, conditionId in pairs(actionCfg.UnlockCondition) do
        if not XConditionManager.CheckCondition(conditionId, actionCfg.CharacterId, tryFashionId, trySceneId) then
            isUnlock = false
            break
        end
    end
...
-- 真实穿戴检测，主界面看板互动
---@param config XTableSignBoardFeedback
function XFavorabilityAgency:CheckTryCharacterActionUnlockByConfig(config, roleId, tryFashionId, trySceneId)
    if config and roleId and tryFashionId and trySceneId then
        for _, conditionId in pairs(config.UnlockCondition) do
            if not XConditionManager.CheckCondition(conditionId, roleId, tryFashionId, trySceneId) then
                return false
            end
        end
    end
...
function XFavorabilityAgency:CheckCharacterActionUnlockBySignBoardActionId(signBoardActionId)
    ...
    local characterId = actionCfg.CharacterId
    local characterData = XMVCA.XCharacter:GetCharacter(characterId)
    local trustLv = 1
    if characterData then
        trustLv = characterData.TrustLv or 1
    end
    return self:CheckCharacterActionUnlock(actionCfg, trustLv) , actionCfg.ConditionDescript
...
function XFavorabilityAgency:PlayCvByType(characterId, soundType)
    ...
    local voices = self._Model:GetCharacterVoiceMapById(characterId)
    if not voices then
    ...
    for _, voice in pairs(voices) do
        if voice.config.SoundType == soundType then
            ...
            self._Model._PlayingCvInfo = XLuaAudioManager.PlayAudioByType(XLuaAudioManager.SoundType.Voice, voice.config.CvId, function()
...
function XFavorabilityAgency:GetCharacterVoiceById(characterId)
    ...
    return self._Model:GetCharacterShowVoiceCfgsById(characterId)
end
...
function XFavorabilityAgency:GetCharacterActionById(characterId)
    ...
    local action = self._Model:GetCharacterActionById(characterId)
    return action
end
    ...
    if conditionId == XEnumConst.Favorability.XSignBoardEventType.CLICK then
        for _, var in ipairs(configs) do
            if var.ConditionParam < 0 or var.ConditionParam == param then
                if self:CheckTryCharacterActionUnlockByConfig(var, roleId, tryFashionId, trySceneId) then
...
-- 是否使用自己的Ui动画，但是不弹窗通用关闭
function XFavorabilityAgency:CheckIsUseNoneUiAnim(signBoardid, uiName)
    local signBoard = self._Model:GetIsUseSelfUiAnim(signBoardid, uiName)
    return signBoard == XEnumConst.Favorability.XSignBoardUiAnimType.None
end
...
--是不是联动角色
function XFavorabilityAgency:GetModelGetCharacterCollaboration(characterId)
    return self._Model:GetCharacterCollaboration()[characterId]
end

--是否有至少一个联动角色
function XFavorabilityAgency:IsHasCollaborationCharacter()
    local allConfig = self._Model:GetCharacterCollaboration()
    for id, v in pairs(allConfig) do
        local collabConfig = allConfig[id]
        if XMVCA.XCharacter:IsOwnCharacter(id) or (collabConfig and collabConfig.IsNewCharacter) then --或者是联动new角色
            return true
        end
    end
    return false
...
function XFavorabilityAgency:LoadSceneAnim(rootNode, farCam, nearCam, uiFarCam, uiNearCam, sceneId, signBoardId, ui, isUnloadAnim)
    ...
    if not self._Model._sceneAnim:CheckIsSameAnim(sceneId, signBoardId, rootNode) or isUnloadAnim then
        self:UnLoadAnim()
...
---@param self XFavorabilityAgency
InitCharacterData= function(self)
    ...
    for id, v in pairs(allStory) do
        local trustLv = self:GetCurrCharacterFavorabilityLevel(id)
        ---@param var XTableCharacterStory
        for _, var in ipairs(v) do
            local isUnLock = self._Model:IsStorySatisfyUnlock(id, var.Id)
    ...
    -- 获取该角色的语音列表
    local allvoices = self._Model:GetAllCharacterShowVoiceCfgs()
    for characterId, characterIdVoiceGroup in pairs(allvoices) do
        local trustLv = XMVCA.XFavorability:GetCurrCharacterFavorabilityLevel(characterId)
        for _, voice in ipairs(characterIdVoiceGroup) do

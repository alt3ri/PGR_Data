...
function XBigWorldTeachModel:GetTeachContentMap()
    if not self._TeachContentMap then
        ...
        for id, config in pairs(contentConfigs) do
            if self._TeachContentMap[config.HelpCourseId] == nil then
                self._TeachContentMap[config.HelpCourseId] = {}
            end

            self._TeachContentMap[config.HelpCourseId][config.Index] = config
        end
    end

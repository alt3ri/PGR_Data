...
function XFubenBaseAgency:ExCheckShowCondition()
    local showCondition = self:ExGetConfig().ShowConditionId
    if not XTool.IsNumberValid(showCondition) then
        return true
    end
    return XConditionManager.CheckCondition(showCondition)
end
...
function XFubenBaseAgency:ExOpenMainUi(...)
    XFunctionManager.SkipInterface(self:ExGetConfig().SkipId, nil, ...)
end
...
function XFubenBaseAgency:ExCheckIsShowRedPoint(...)
    if self:ExGetConfig().RedPointConditions and XRedPointManager.CheckConditions(self:ExGetConfig().RedPointConditions, self:ExGetConfig().RedParam) then
        return true
    end

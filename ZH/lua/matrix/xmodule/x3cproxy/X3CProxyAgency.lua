...
function X3CProxyAgency:OnInit()
    ...
    self._On3CError = handler(self, self.On3CError)
...
function X3CProxyAgency:On3CError(error)
    XLog.Error("X3C逻辑执行异常：", error)
end
...
function X3CProxyAgency:Send(cmd, data)
    local ok, receive = xpcall(self._X3CProxy.RequestCSharp, self._On3CError, self._X3CProxy, cmd, data)
    if not ok then
...
function X3CProxyAgency:Receive(cmd, data)
    ...
    local ok, receive = xpcall(handler[1], self._On3CError, handler[2], data)
    if not ok then
        XLog.Error(string.format("x3c receive error. cmd = %s, error = %s", cmd, receive))
        return nil
    end

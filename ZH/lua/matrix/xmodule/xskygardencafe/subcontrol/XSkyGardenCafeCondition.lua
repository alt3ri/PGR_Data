...
function XSkyGardenCafeCondition:OnInit()
    self._Func = {
        ...
        [10011] = function(template, ...) --检测使用了指定稀有度的卡牌
            return self:CheckDealHasQuality(template, ...)
        end,
        [10012] = function(template, ...) --检测出牌区下一张牌是否满足条件
            return self:CheckDealNextSeat(template, ...)
        end,
        [10013] = function(template, ...) --检测某张卡是否未使用过
            return self:CheckCardIsNotUsed(template, ...)
        end,
    }
end
...
---@param template XTableSGCafeCondition
function XSkyGardenCafeCondition:CheckDealCount(template, card, isPreview, argList)
    ...
    if isPreview then
        --预览时多一张
        count = math.min(self._Model:GetBattleInfo():GetDealLimit(), count + 1)
    end
    return count >= template.Params[1], template.Desc
end

---@param template XTableSGCafeCondition
function XSkyGardenCafeCondition:CheckDealHasTarget(template, card, isPreview, argList)
    local cardEntities = self._MainControl:GetBattle():GetRoundEntity():GetDealCardEntities()
    if not XTool.IsTableEmpty(cardEntities) then
        local targetId = template.Params[1]
        local isEqual = template.Params[2] == 1
        if isEqual then
            if isPreview and card and card:GetCardId() == targetId then
                return true, template.Desc
            end
...
function XSkyGardenCafeCondition:CheckDealHasQuality(template, card, isPreview, argList)
    local cardEntities = self._MainControl:GetBattle():GetRoundEntity():GetDealCardEntities()
    if not XTool.IsTableEmpty(cardEntities) then
        local quality = template.Params[1]
        local count = template.Params[2] or 1
        local sum = 0
        if isPreview and card and card:GetCardQuality() == quality then
            sum = sum + 1
        end
        for _, c in pairs(cardEntities) do
            if c:GetCardQuality() == quality then
                sum = sum + 1
            end
        end
...
---@param template XTableSGCafeCondition
---@param card XSkyGardenCafeCardEntity
function XSkyGardenCafeCondition:CheckDealNextSeat(template, card, isPreview, argList)
    if not card then
        return false, template.Desc
    end
    local roundEntity = self._MainControl:GetBattle():GetRoundEntity()
    local index = roundEntity:GetDealCardIndexWithCard(card)
    --不在出牌区
    if not index or index <= 0 then
        return false, template.Desc
    end
    local cardEntities = roundEntity:GetDealCardEntities()
    local nextCard = cardEntities[index + 1]
    if not nextCard then
        return false, template.Desc
    end
    local type, value = template.Params[1], template.Params[2]
    if type == 1 then --类型
        return nextCard:GetCardType() == value, template.Desc
    elseif type == 2 then --稀有度
        return nextCard:GetCardQuality() == value, template.Desc
    elseif type == 3 then --Id
        return nextCard:GetCardId() == value, template.Desc
    end
    return false, template.Desc
end

---@param template XTableSGCafeCondition
function XSkyGardenCafeCondition:CheckRoundState(template, card, isPreview, argList)
...
---@param template XTableSGCafeCondition
---@param card XSkyGardenCafeCardEntity
function XSkyGardenCafeCondition:CheckDealIndex(template, card, isPreview, argList)
    local cardId = template.Params[1]
    local index
    if isPreview then
        index = self._MainControl:GetBattle():GetRoundEntity():GetDealCardIndexWithCard(card)
        if index <= 0 then
        ...
        if cardId <= 0 then
            ...
            index = self._MainControl:GetBattle():GetRoundEntity():GetDealCardIndexWithCardId(cardId)
        end
    end
...
---@param template XTableSGCafeCondition
---@param card XSkyGardenCafeCardEntity
function XSkyGardenCafeCondition:CheckCardResourceChange(template, card, isPreview, argList)
...
---@param template XTableSGCafeCondition
function XSkyGardenCafeCondition:CheckCardResourceSatisfy(template, card, isPreview, argList)
    local checkType = template.Params[1]
    local param2 = template.Params[2]
...
---@param template XTableSGCafeCondition
function XSkyGardenCafeCondition:CheckDeckCount(template, card, isPreview, argList)
...
---@param template XTableSGCafeCondition
function XSkyGardenCafeCondition:CheckRoundEqual(template, card, isPreview, argList)
...
function XSkyGardenCafeCondition:CheckDrawCard(template, card, isPreview, argList)
    local drawCardType = argList[1]
    local cards = argList[2]
...
---@param template XTableSGCafeCondition
---@param card XSkyGardenCafeCardEntity
function XSkyGardenCafeCondition:CheckCardBuffEffect(template, card, isPreview, argsList)
    if not card then
        return false, template.Desc
    end
    local buffId, count = template.Params[1], template.Params[2]
    return card:IsBuffApply(buffId, count, isPreview), template.Desc
end

---@param template XTableSGCafeCondition
---@param card XSkyGardenCafeCardEntity
function XSkyGardenCafeCondition:CheckCardIsNotUsed(template, card, isPreview, argsList)
    local cardId = template.Params[1]
    if not cardId or cardId then
        return false, template.Desc
    end
    local useCount = self._Model:GetBattleInfo():GetCardUseCount(cardId)
    return useCount <= 0, template.Desc
end

...
function XCharacterAgency:GetSkillExchangeDesConfigBySkillId(skillId)
    local groupSkillIds = self:GetGroupSkillIds(skillId)
    for index, skillId in ipairs(groupSkillIds) do
        local skillLevel = self:GetSkillLevel(skillId)
        local skillExchangeConfig = self:GetCharacterSkillExchangeDesBySkillIdAndLevel(skillId, skillLevel)
        if skillExchangeConfig ~= nil then
...
function XCharacterAgency:GetModelCharacterQualityIconInitColor(quality)
    local template = self:GetModelCharacterQualityIcon(quality)
    --region 品阶图标开关
    if XModelHX.GetIconStyle() == XModelHX.QualityStyle.Chinese then
        return template.InitColor2
    end
    --endregion 品阶图标开关
    return template.InitColor
end

function XCharacterAgency:GetModelCharacterQualityIconInit(quality)
    local template = self:GetModelCharacterQualityIcon(quality)
    --region 品阶图标开关
    if XModelHX.GetIconStyle() == XModelHX.QualityStyle.Chinese then
        return template.IconCharacterInit2
    end
    --endregion 品阶图标开关
    return template.IconCharacterInit
end
...
function XCharacterAgency:GetEnhanceSkillLevelEffectBySkillIdAndLevel(skillId, level)
    ...
    local config = self._Model:GetEnhanceSkillLevelEffect()[targetId]
    if not config then
        XLog.Error("EnhanceSkillLevelEffect.tab not found skillId, level", skillId, level, targetId)
        return
    end
...
-- 抽卡界面用的品质图标
function XCharacterAgency:GetCharQualityIconDraw(quality)
    ...
    --region 品阶图标开关
    if XModelHX.GetIconStyle() == XModelHX.QualityStyle.Chinese then
        return template.IconDraw2
    end
    --endregion 品阶图标开关
    return template.IconDraw
end
...
function XCharacterAgency:GetCharacterQualityIcon(quality)
    ...
    --region 品阶图标开关
    if XModelHX.GetIconStyle() == XModelHX.QualityStyle.Chinese then
        return template.IconCharacter2
    end
    --endregion 品阶图标开关
...
-- 获取进化界面, 特效里的品质图标
function XCharacterAgency:GetCharacterQualityEffectIcon(quality)
    if not quality or quality < 1 then
        XLog.Error("XCharacterAgency:GetCharacterQualityIcon函数参数不规范，参数是quality：", quality)
        return
    end
    ...
    --region 品阶图标开关
    if XModelHX.GetIconStyle() == XModelHX.QualityStyle.Chinese then
        return template.IconEffect2
    end
    --endregion 品阶图标开关
    return template.IconEffect
end
...
function XCharacterAgency:GetCharQualityIconGoods(quality)
    ...
    --region 品阶图标开关
    if XModelHX.GetIconStyle() == XModelHX.QualityStyle.Chinese then
        return template.IconGoods2
    end
    --endregion
    return template.IconGoods
end

function XCharacterAgency:GetCharQualityDesc(quality, iconStyle)
    if not quality or quality < 1 then
    ...
    local template = self:GetModelCharacterQualityIcon(quality)
    --region 品阶图标开关
    iconStyle = iconStyle or XModelHX.GetIconStyle()
    if iconStyle == XModelHX.QualityStyle.Chinese then
        return template.Desc2
    end
    --endregion
    return template.Desc
...
function XCharacterAgency:GetCharactersListByGeneralSkillId(generalSkillId)
    ...
    for charId, config in pairs(charDetailConfigs) do
        ...
        local collabconfig = XMVCA.XFavorability:GetModelGetCharacterCollaboration(charId) -- 若是联动角色则只需要:当期开放或已拥有
        local collabCheck = (not collabconfig) or (collabconfig and (collabconfig.IsNewCharacter or self:IsOwnCharacter(charId)))
        if table.contains(generalSkillIds, generalSkillId) and collabCheck then

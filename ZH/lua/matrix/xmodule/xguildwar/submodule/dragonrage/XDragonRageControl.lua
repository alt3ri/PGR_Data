...
--- 龙怒玩法路径消耗修正
function XDragonRageControl:FixCostPath(preNodes, currentNodeId, targetId, originPath)
    -- 经过废墟节点不需要消耗
    if not XTool.IsTableEmpty(preNodes) then
        local preNodeId = targetId

        local gameThroughId = self:GetGameThroughCfgId()
        ---@type XTableGuildWarPlayThrough
        local gameThroughCfg = self._Model:GetDragonRagePlayThroughCfgById(gameThroughId)

        if not gameThroughCfg then
            return originPath
        end

        while preNodeId > 0 and preNodeId ~= currentNodeId do
            -- 如果根节点属于周目变更节点，那么说明当前该节点实际显示的是废墟节点，经过无需消耗物资
            if table.contains(gameThroughCfg.ChangeNodeIds, preNodeId) then
    ...
    originPath = math.max(0, originPath)
    
    return originPath
end

--- 获取已播放过龙怒开启的缓存
function XDragonRageControl:GetDragonRageOpenIsMark()
    return self._Model:GetDragonRageOpenIsMark()
end

--  标记已播放过龙怒开启动画
function XDragonRageControl:SetDragonRageOpenMark()
    self._Model:SetDragonRageOpenMark(true)
end

-- 周目扫荡-判断是否可以扫荡
function XDragonRageControl:CheckCanSweepByDragonRageRule(nodeUid, nodeType, nodeIsAlive)
    if nodeType ~= XGuildWarConfig.NodeType.Normal or not nodeIsAlive then
        return false
    end
    ...
    if battleManager then
        local damage = battleManager:GetLastRoundMaxDamageByUID(nodeUid, XGuildWarConfig.FightRecordAliveType.Alive)
        
        return XTool.IsNumberValid(damage)
    end
...
-- 周目扫荡提示
function XDragonRageControl:GetIsHideSweepTips()
    local curRoundId = XDataCenter.GuildWarManager.GetCurrentRoundId()
    
    return self._Model:GetIsDragonRageHideSweepTips(curRoundId, self._Model:GetDragonRageData():GetDragonRageLevel())
end

function XDragonRageControl:SetHideSweepTips()
    local curRoundId = XDataCenter.GuildWarManager.GetCurrentRoundId()

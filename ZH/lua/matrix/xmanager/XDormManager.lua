...
XDormManagerCreator = function()
    ...
    local CharacterMoodCache = {} -- 构造体心情临时缓存(用于一键处理事件，缓存处理前心情)
    ...
    local DormitoryRequest = {
        ...
        DormCharacterFinishAllEventReq = "DormCharacterFinishAllEventRequest", -- 一键完成全部构造体事件
    }
        ...
        if dormDataType == XDormConfig.DormDataType.Template then
            ...
            if idList and #idList > 0 then
                for _, id in ipairs(idList) do
                    ...
                    if isShow then
                        local showTimeStr = XDormConfig.GetDormTemplateShowTimeStrById(data:GetRoomId())
                ...
                for _, v in pairs(TempDormitoryData) do
                    if v:GetRoomDataType() == XDormConfig.DormDataType.Template then
                        ...
                        if isShow then
                            local showTimeStr = XDormConfig.GetDormTemplateShowTimeStrById(v:GetRoomId())
        ...
        if not XTool.IsNumberValid(templateDormType) then
            for _, template in pairs(TempDormitoryData) do
                ...
                elseif roomType == XDormConfig.DormDataType.Template then
                    ...
                    if isShow then
                        local showTimeStr = XDormConfig.GetDormTemplateShowTimeStrById(template:GetRoomId())
    ...
    -- 判断全部宿舍是否存在事件
    function XDormManager.IsHaveAllDormCharactersEvent()
        for _, v in pairs(DormitoryData) do
            if v:WhetherRoomUnlock() then
                if XDormManager.IsHaveDormCharactersEvent(v:GetRoomId()) then
                    return true
                end
            end
        end
    ...
    -- 本地方法 根据服务器发回的心情百分值更新成员心情并返回心情改变值和加之前的值
    local function UpdateCharacterMoodByChangeValuePercent(character, changeMoodValue)
        local changeValue = XDormManager.GetCharacterMoodByChangeValuePercent(character, changeMoodValue)
        character.Mood = character.Mood + changeValue
        return changeValue
    end
    
    -- 只获取变化值，不更新数据
    function XDormManager.GetCharacterMoodByChangeValuePercent(character, changeMoodValue)
        local changeValue = XDormManager.GetCharacterChangeMoodByChangeValuePercent(character.Mood, changeMoodValue)
        return changeValue
    end
    
    -- 根据传的Mood值获取变化值
    function XDormManager.GetCharacterChangeMoodByChangeValuePercent(characterMood, changeMoodValue)
        local changeValue = math.floor(changeMoodValue / 100)
        if characterMood + changeValue > XDormConfig.DORM_MOOD_MAX_VALUE then
            changeValue = XDormConfig.DORM_MOOD_MAX_VALUE - characterMood
        end
    ...
    function XDormManager.RequestDormitoryCharacterOperate(charId, dormitoryId, eventId, operateType, cb)
        XNetwork.Call(DormitoryRequest.DormCharacterOperateReq, { CharacterId = charId, EventId = eventId, OperateType = operateType }, function(res)
            ...
            -- 处理回复弹条
            local changeValue = UpdateCharacterMoodByChangeValuePercent(character, res.MoodValue)
    ...
    function XDormManager.QuickDormDoneCallBack(workposList)
        if CurrentDormId == -1 then
            return
        end --若记录停留的宿舍为空说明没有进入宿舍页面，不用重新放入小人
    ...
    function XDormManager.NotifyDormName(data)
        ...
        if dorm then
            dorm:SetRoomName(data.DormName)
        end
    end
    ...
    --一键完成全部构造体事件
    --================
    function XDormManager.DormCharacterFinishAllEventReq()
        if not XDataCenter.DormManager.IsHaveAllDormCharactersEvent() then
            XUiManager.TipText("DormEventAutoHasNotDormEvent")
            return
        end

        -- 请求前先缓存构造体心情值
        for charId, data in pairs(CharacterData) do
            CharacterMoodCache[charId] = data.Mood
        end
        
        XNetwork.Call(DormitoryRequest.DormCharacterFinishAllEventReq, {}, function(res)
            if res.Code ~= XCode.Success then
                XUiManager.TipCode(res.Code)
                CharacterMoodCache = {}
                return
            ...
            local moodChangeDic = res.MoodChange
            local rewardGoodsList = res.RewardGoods

            local characterMoodsChangeList = {}
            for characterId, moodChangeData in pairs(moodChangeDic) do
                local character = XDataCenter.DormManager.GetCharacterDataByCharId(characterId)
                local characterAfterMood = CharacterMoodCache[characterId]
                if character then
                    character.EventList = {}
                end

                if characterAfterMood then
                    if moodChangeData > 0 then
                        local moodChangeValue = XDormManager.GetCharacterChangeMoodByChangeValuePercent(characterAfterMood, moodChangeData)
                        table.insert(characterMoodsChangeList, { CharacterId = character.CharacterId, MoodChangeValue = moodChangeValue, AfterMoodValue = characterAfterMood })
                    end
                end
            end
            CharacterMoodCache = {}
            
            local rewardGoodsMergeDic = {}
            for _, rewardGood in ipairs(rewardGoodsList) do
                ...
                if not rewardGoodsMergeDic[rewardGood.Id] then
                    rewardGoodsMergeDic[rewardGood.Id] = rewardInfo
                else
                    rewardGoodsMergeDic[rewardGood.Id].Count = rewardGoodsMergeDic[rewardGood.Id].Count + rewardGood.Count
                end
            end

            XEventManager.DispatchEvent(XEventId.EVENT_DORM_CHARACTER_FINISH_ALL_EVENT, characterMoodsChangeList, rewardGoodsMergeDic)
        end)
    end
    ---------------------end net---------------------

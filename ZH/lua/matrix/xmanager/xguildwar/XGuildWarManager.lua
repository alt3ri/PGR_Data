...
XGuildWarManagerCreator = function()
    ...
    function XGuildWarManager.OnNotifyActivityData(data)
        ...
        if XDataCenter.GuildWarManager.CheckActivityIsInTime() and XDataCenter.GuildWarManager.GetActivityLeftTime() < seconds then
            if XFunctionManager.DetectionFunction(XFunctionManager.FunctionName.ShopCommon,false,true) then
                local shopId = tonumber(XGuildWarConfig.GetServerConfigValue('GuildWarShopId','Int')) or 0
    ...
    function XGuildWarManager.RefreshActivityData(activityData, isRealTime, type, id)
        ...
        for _, settleData in pairs(activityData.SettleDatas or {}) do
            ...
            local round = XMVCA.XGuildWar:GetRoundByRoundId(settleData.RoundId)
            if round then
    ...
    function XGuildWarManager.RefreshMyRoundData(myRoundData)
        if myRoundData then
            for _, myData in ipairs(myRoundData) do
                local round = XMVCA.XGuildWar:GetRoundByRoundId(myData.RoundId)
                if round then
    ...
    function XGuildWarManager.GetCurrentRound()
        return XMVCA.XGuildWar:GetRoundByRoundId(CurrentRoundId)
    end
    ...
    function XGuildWarManager.GetPreRound()
        ...
        return XMVCA.XGuildWar:GetRoundByRoundId(CurrentRoundId - 1)
    end
    --endregion
    ...
    function XGuildWarManager.CheckIsGuildSkipRound(roundId)
        local round = XMVCA.XGuildWar:GetRoundByRoundId(roundId or CurrentRoundId)
    ...
    function XGuildWarManager.CheckIsInGuildByRound(roundId)
        ...
        local round = XMVCA.XGuildWar:GetRoundByRoundId(roundId)
        if not round then
            return false
        end
    ...
    function XGuildWarManager.GetDifficultyPreSelected()
        ...
        for i = CurrentRoundId - 1, 1, -1 do
            local round = XMVCA.XGuildWar:GetRoundByRoundId(i)
    ...
    function XGuildWarManager.IsActivationNotRecommend(difficulty)
        local preActive = 0
        if CurrentRoundId > 1 then
            local preRound = XMVCA.XGuildWar:GetRoundByRoundId(CurrentRoundId - 1)
    ...
    function XGuildWarManager.GetAllShowedTaskTypeList()
        ...
        while true do
            ...
            if index == 0 then
                ...
                local round = XMVCA.XGuildWar:GetRoundByRoundId(index)
    ...
    function XGuildWarManager.GetTaskList(taskType)
        ...
        local tempTaskDatas = {}
        local difficultyId = XMVCA.XGuildWar:GetRoundByRoundId(taskType):GetMyDifficulty()
    ...
    --获取节点实例
    ---@return XGWNode
    local function GetNode(nodeId, noTips)
        local manager = XGuildWarManager.GetBattleManager()
        if not manager then
            return nil
        end
        return manager:GetNode(nodeId, noTips)
    end
    function XGuildWarManager.GetNode(nodeId, noTips)
        return GetNode(nodeId, noTips)
    end
    ...
    function XGuildWarManager.ShowNodeDestroyed(actionGroup, showOverCallback)
        ...
        -- 找到有龙怒的，并且uid最大的action进行更新
        local dragonRageAction
        
        for _, action in pairs(actionGroup or {}) do
            if not action.NotShow then
                table.insert(nodeIdList, action.NodeId)
            end

            if action.IsRealTime and type(action.DragonRageValue) == 'number' then
                if dragonRageAction then
                    if action.ActionId > dragonRageAction.ActionId then
                        dragonRageAction = action
                    end
                else
                    dragonRageAction = action    
                end
        ...
        if dragonRageAction then
            XMVCA.XGuildWar.DragonRageCom:UpdateDragonRageValue(dragonRageAction.DragonRageValue)
        end
        ...
        local callBackCheck = function()
            ...
            if not XMVCA.XGuildWar.DragonRageCom:IsOpenDragonRageSystem() then
                -- 非龙怒周目玩法需要检查是否击破Boss
    ...
    function XGuildWarManager.ReceiveSupportSupply()
        XNetwork.CallWithAutoHandleErrorCode(METHOD_NAME.ReceiveSupportSupply, {}, function(res)
            ...
            if res.TotalSupply > 0 then
                local rewards = {}
                table.insert(rewards, { TemplateId = tonumber(XGuildWarConfig.GetServerConfigValue('RewardItemId')), Count = res.TotalSupply })
    ...
    --@param cb: function 回调
    --===============
    function XGuildWarManager.RequestPlayerMove(targetNodeId, cb, moveCostUnit)
        local cost = tonumber(XGuildWarConfig.GetServerConfigValue("MoveCostEnergy")) * moveCostUnit
    ...
    function XGuildWarManager.OpenUiGuildWarMain(openDifficultyFunc)
        ...
        if XGuildWarManager.IsLastRound() and XDataCenter.GuildDormManager.GetGuildCreateTime() > XGuildWarManager.GetRoundEndTime() then
    ...
    function XGuildWarManager.GetNextAttackedTime()
        ...
        return garrisonData:GetNextAttackedTime()
    end
    
    function XGuildWarManager.MarkNearestAttackAnimIsPlayed()
        ---@type XGuildWarGarrisonData
        local garrisonData = XMVCA.XGuildWar:GetGarrisonData()
        XSaveTool.SaveData(XGuildWarManager.GetGuildWarAttackTimeLocKey(), garrisonData:GetAttackedTimes())
    end
    ...
    function XGuildWarManager.FilterPlayerStayListByDefendId(rankList,defendId,cb)
        ...
        ---@type XGuildWarGarrisonData
        local garrisonData = XMVCA.XGuildWar:GetGarrisonData()
        local defendDict = garrisonData:GetDefendDict()
    ...
    --region 2.15 援军
    local ReinforcementSupportCD = 10
    local LastReinforcementSupportTime = 0
    
    function XGuildWarManager.RequestGuildWarSupportReinforcement(uid, cb)
        local now = XTime.GetServerNowTimestamp()

        if now - LastReinforcementSupportTime < ReinforcementSupportCD then
            XUiManager.TipMsg(XGuildWarConfig.GetClientConfigValues('ReinforcementSupportInCDTips')[1])
            return
        end

        LastReinforcementSupportTime = now
        
        XNetwork.Call("GuildWarSupportReinforcementRequest", { ReinforcementUid = uid }, function(res)
            if res.Code ~= XCode.Success then
                XUiManager.TipCode(res.Code)
                return
            end

            local GBattleManager = XGuildWarManager.GetBattleManager()
            GBattleManager:UpdateReinforcementsData(res.ReinforcementData)

            if cb then
                cb(GBattleManager:GetReinforcementById(res.ReinforcementData.Uid))
            end
        end)
    end
    
    function XGuildWarManager.RequestGuildWarCancelSupportReinforcement(uid, cb)
        local now = XTime.GetServerNowTimestamp()
        ...
        XNetwork.Call("GuildWarCancelSupportReinforcementRequest", { ReinforcementUid = uid }, function(res)
            if res.Code ~= XCode.Success then
                XUiManager.TipCode(res.Code)
                return
            end
    ...
    function XGuildWarManager.UpdateExtraActionPointData(data)
        ExtraActionPointData = data
    end
    
    function XGuildWarManager.TryShowExtraActionPointReward()
        if not XTool.IsTableEmpty(ExtraActionPointData) then
            XUiManager.OpenUiObtain(ExtraActionPointData)
            ExtraActionPointData = nil
        end
    end
    --endregion

...
XFunctionalSkipManagerCreator = function()
    ...
    -- 前往宿舍主界面
    ---@param skipCfg XTableSkipFunctional
    function XFunctionalSkipManager.SkipDormMain(skipCfg)
        ...
        if not XHomeDormManager.InDormScene() then
            XHomeDormManager.EnterDorm(XPlayer.Id, nil, false, function()
                XFunctionManager.RecordSkip(skipCfg.SkipId)
            end)
            return false
    ...
    function XFunctionalSkipManager.OnOpenUiArena(skipDatas)
        return XMVCA.XArena:OpenMainUiFromCommonSkip(skipDatas.SkipId)
    end
    ...
    -- 公会
    ---@param skipCfg XTableSkipFunctional
    function XFunctionalSkipManager.OnOpenGuild(skipCfg)
    ...
    -- 公会宿舍
    ---@param skipCfg XTableSkipFunctional
    function XFunctionalSkipManager.OnOpenGuildDorm(skipCfg)
        if not XFunctionManager.DetectionFunction(XFunctionManager.FunctionName.Guild) then
            return false
        end
    ...
    function XFunctionalSkipManager.OnOpenMaintainerAction()
        ...
        local result = XDataCenter.MaintainerActionManager.Open()
        return result
    end
    ...
    function XFunctionalSkipManager.SkipToPokemonMainUi()
        XLog.Error("口袋战双主界面已经被屏蔽, 有问题请联系程序")
    end
    ...
    function XFunctionalSkipManager.SkipToFashionStory(skipData)
        XMVCA.XFashionStory:OpenFashionStoryMain(skipData.CustomParams[1])
    end
    ...
    function XFunctionalSkipManager.SkipToTheatre()
        ...
        return XDataCenter.TheatreManager.ExOnSkip()
    end
    ...
    function XFunctionalSkipManager.SkipToBiancaTheatre()
        return XDataCenter.BiancaTheatreManager:OpenMainUi()
    end
    ...
    function XFunctionalSkipManager.SkipToUiCharacterTowerChapter(list)
        ...
        return XDataCenter.CharacterTowerManager.OpenChapterUi(param2, closeLastStage)
    end
    ...
    function XFunctionalSkipManager.OnOpenCerberusGame()
        ...
        return XMVCA.XCerberusGame:OpenMainUi()
    end
    ...
    function XFunctionalSkipManager.SkipToBrilliantWalkMain(list)
        XLog.Error("光辉同行已经被删除，如果有疑问请联系程序")
    end
    ...
    function XFunctionalSkipManager:SkipToUiNewbieTaskMain()
        return XDataCenter.NewbieTaskManager.OpenMainUi()
    end
    ...
    function XFunctionalSkipManager.SkipToWheelchairManualMainUi(list)
        ...
        return XMVCA.XWheelchairManual:OpenMainUi(activityId, tabIndex)
    end

    -- 跳转到新手任务合集
    function XFunctionalSkipManager.SkipToNewPlayerTaskCollection(skipCfg)
        local skipId = XMVCA.XUiMain:GetFirstCanSkipNewPlayerTaskCollectionSKipId()
        if XTool.IsNumberValid(skipId) then
            return XFunctionManager.SkipInterface(skipId)
        end
    end
    ...
    function XFunctionalSkipManager.SkipToSelfChoiceLotto()
        ...
        if not XTool.IsNumberValid(selectedLottoId) then
            ...
            XLuaUiManager.Open("UiLottoFashionSelfChoiceEntrance", not forceNormalMode)
            return
        end
    ...
    --- 打开道具购买界面
    --- 跳转自定义参数1：道具Id，参数2：指定购买数量，参数3：购买一次后是否立刻关闭界面（0/1)
    ---@param skipCfg XTableSkipFunctional
    function XFunctionalSkipManager.SkipToBuyAssetPanel(skipCfg)
        XLuaUiManager.Open("UiBuyAsset", skipCfg.CustomParams[1], nil, nil, skipCfg.CustomParams[2], XTool.IsNumberValid(skipCfg.CustomParams[3]))
        return true
    end

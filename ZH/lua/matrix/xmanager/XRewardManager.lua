...
local CreateGoodsFunc = {
    ...
    [XRewardType.BWDIYPart] = function(templateId, count)
        return {
            RewardType = XRewardType.BWDIYPart,
            TemplateId = templateId,
            Count = count and count or 1,
        }
    end,
    [XRewardType.SgDormFashion] = function(templateId, count)
        return {
            RewardType = XRewardType.SgDormFashion,
            TemplateId = templateId,
            Count = count and count or 1,
        }
    end,
...
local SortBWDIYPart = function(a, b)
    local priorityA = XMVCA.XBigWorldCommanderDIY:GetPartPriority(a.TemplateId)
    local priorityB = XMVCA.XBigWorldCommanderDIY:GetPartPriority(b.TemplateId)
...
function XRewardManager.GetRewardSubIds(id)
    ...
    if XMVCA.XBigWorldGamePlay:IsBigWorldOpen() then
        rewardSubIds = XMVCA.XBigWorldService:GetDlcRewardTemplate(id, true)
        if not rewardSubIds then

...
XFubenManagerExCreator = function()
    ...
    function FubenManagerEx.RegisterAutoAgency()
        ...
        for _, config in ipairs(AgencyAutoCreator) do
            ...
            if targetAgency then
                ...
                XLog.Error("[XFubenManagerEx] 配置了副本代码，但是没有找到对应的代码" .. tostring(config.ManagerName))
            end
        end
        AgencyAutoCreator = {}
    end
    ...
    function FubenManagerEx.GetActivityManagers()
        ...
        for _, manager in ipairs(ActivityManagers or {}) do
            if manager:ExCheckInTime() then
                if not XTool.IsTableEmpty(manager:ExGetConfig()) then
                    if manager:ExCheckShowCondition() then
                        -- 在继承重载时, 程序经常忘记Super.ExCheckInTime, 故改为默认检查FubenActivity表的timeId
                        if not manager.ExCheckInTimeFubenActivityConfig or manager:ExCheckInTimeFubenActivityConfig() then
    ...
    function FubenManagerEx.GetShowOnMainWeeklyManagers(isNotSort)
        ...
        local ShowConditionList = XFubenConfigs.GetChallengeGridShowConditionList()
        local managers = {}
        local indexRecord = {}
        for index, chapterType in ipairs(managerChapterList) do
            local manager = FubenManagerEx.GetManager(chapterType)
            if manager:ExGetIsLocked() and XTool.IsNumberValid(ShowConditionList[index])
                    and not XConditionManager.CheckCondition(ShowConditionList[index]) then
            else
    ...
    function FubenManagerEx.GetCurrentRecordChapterAndManager()
        ...
        -- 如果没有【限时开放】，开始执行下列代码
        local configs = XMVCA.XFuben:GetFubenStoryLine()
        local normalList = {}
        ...
        if currActivityViewModel and currActivityViewModel:GetIsLocked() and cuurChapterViewModel and cuurChapterViewModel:GetIsLocked() then
            ...
            if currViewModelFirstStage and activityCondition then
                ...
                if conditionTemplate and conditionTemplate.Type == 10101 and stageRequireLevel then
                    -- 如果2者都是等级限制条件
                    if conditionTemplate.Params[1] <= stageRequireLevel then
                        -- 且如果限时条件副本所需等级 <= 正常进度副本所需等级 ，则优先显示限时的
    ...
    function FubenManagerEx.GetMainUiTabConfigs()
        return XMVCA.XFuben:GetFubenTabConfig()
    end

    -- 获取主界面活动提示
    function FubenManagerEx.GetActivityMainUiTips()
        local configs = XMVCA.XFuben:GetFubenActivityTimeTips()
    ...
    function FubenManagerEx.GetTagConfigByChapterType(chapterType)
        ...
        local allSecondTagConfigs = XMVCA.XFuben:GetFubenSecondTag()
    ...
    function FubenManagerEx.CheckHasOpenByFirstTagId(firstTagId)
        if not firstTagId then
            return
        end
        local allSecondTag = XFubenConfigs.GetSecondTagConfigsByFirstTagId(firstTagId) -- 拿到该模式下所有的二级标签
        if not allSecondTag or not next(allSecondTag) then
            --没用二级标签配置的特殊标签都不会上锁(主线和战斗面板都没有配置，主线是自动生成的)
            return true
        end

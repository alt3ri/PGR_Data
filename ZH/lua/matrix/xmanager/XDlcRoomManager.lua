...
XDlcRoomManagerCreator = function()
        ...
        local cb = function(success)
            ...
            if success then
                ...
                XFightNetwork.Call(RequestProto.JoinWorldRequest, { WorldNo = response.WorldNo, PlayerId = XPlayer.Id, Token = response.Token, IsRejoin = isRejoin }, function(res)
                    XDlcRoomManager.OnJoinWorldResponse(res, response.IpAddress, isRejoin)
                end)
            else
                -- 网络错误
                disconnectCb()
    ...
    function XDlcRoomManager.OnJoinWorldResponse(response, ip, isRejion)
        ...
        XDlcRoomManager.OnConnectWorld(response.WorldData, response.JoinWorldData)
    ...
    function XDlcRoomManager.OnEnterWorld(worldData)
        ...
        fightArgs.CloseLoadingCb = function()
        ...
        fightArgs.CheckLuaSettleCb = function()
    ...
    function XDlcRoomManager.OnConnectWorld(worldData, connectData)
        XLuaUiManager.Remove("UiDialog")
        local playerData
        for i = 1, #worldData.Players do
            if worldData.Players[i].Id == XPlayer.Id then

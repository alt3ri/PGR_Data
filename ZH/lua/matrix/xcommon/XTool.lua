...
XTool.MathGetRoundingValue = function(value, digit)
    ...
    local digitValue = XTool.MathPow(10, digit)
    return XMath.FixFloor(value * digitValue) / digitValue
end
...
---@param gapFrames number@ 每隔多少帧load一次
function XTool.UpdateDynamicItemLazy(gridArray, dataArray, uiObject, class, parent, loadPerFrame, gapFrames)
    ...
    gapFrames = gapFrames or 1
    local gap = 0
    parent._TimerDelayInit = parent:Tween(20, function()
        if gap < gapFrames then
            gap = gap + 1
            return
        end
        gap = 0
        ...
        if step < dataCount then
            XLog.Error("[XTool] 延迟加载，20秒还没加载完？是不是有问题")
        end
        parent._TimerDelayInit = nil
    end)
...
function XTool.UpdateDynamicItemByUiCache(gridArray, dataArray, uiParentTr, class, parent, defaultIndex)
    ...
    for i = 1, dataCount do
        ...
        if not grid then
            if not baseGo then
                baseGo = uiParentTr:GetChild(defaultIndex or 0).gameObject
            end
            local go = CS.UnityEngine.Object.Instantiate(baseGo, baseGo.transform.parent)
            grid = class.New(go, parent)
            gridArray[i] = grid
        end
        grid:Open()
        grid:Update(dataArray[i], i)
...
-- 不离开战斗打开带场景的UI前处理一些逻辑
function XTool.NoExitFightPreOpen3DUi()
    XLuaAudioManager.StopAll();

    local rlManager = CS.XRLManager
    local camera = rlManager.Camera
    if camera then

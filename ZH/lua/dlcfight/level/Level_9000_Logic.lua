local XLevelScript9000 = XDlcScriptManager.RegLevelLogicScript(9012, "XLevelLogicScript9012") --注册脚本类到管理器（逻辑脚本注册


---@param proxy XDlcCSharpFuncs
function XLevelScript9000:Ctor(proxy) --构造函数，用于执行与外部无关的内部构造逻辑（例如：创建内部变量等）
    self._proxy = proxy --脚本代理对象，通过它来调用战斗程序开放的函数接口。
end

function XLevelScript9000:Init() --初始化逻辑
    --事件注册
...
function XLevelScript9000:Update(dt) --每帧更新逻辑
    self._levelTime = self._levelTime + dt       --记录关卡已进行时间
    self:OnUpdatePhase(dt)
end
function XLevelScript9000:InitPhase()
    --初始化关卡各个阶段的相关变量
end
---@param phase number
function XLevelScript9000:SetPhase(phase)    --跳转关卡阶段
...
function XLevelScript9000:OnEnterPhase(phase)
...
---@param dt number @ delta time
function XLevelScript9000:OnUpdatePhase(dt)
    --当前关卡阶段需要一直执行的逻辑在这里实现（一般在这里跳转关卡阶段
    self._proxy:SetNpcPosition(self._localNpc, {x=123,y=23,z=97})                 --传送玩家1位置
...
function XLevelScript9000:OnExitPhase(phase)
    --退出一个关卡阶段时需要做的事情在这里实现（最好不要在这里跳转关卡阶段

end
function XLevelScript9000:OnPhaseChanged(lastPhase, nextPhase)
    --关卡阶段改变时需要执行的逻辑，一般用于通知外部
end
function XLevelScript9000:HandlePhaseEvent(eventType, eventArgs)
...
function XLevelScript9000:HandleEvent(eventType, eventArgs) --事件响应逻辑
    
end

function XLevelScript9000:CheckAllPlayerDead() --检查是否所有玩家都死亡了
    self._playerNpcList = self._proxy:GetPlayerNpcList() --获取玩家列表
...
function XLevelScript9000:CheckLevelEnd() --检查关卡结束
    ----胜利结算检测-----------------
...
function XLevelScript9000:LevelEnd(isPlayerWin)
...
function XLevelScript9000:Terminate() --脚本结束逻辑（脚本被卸载、Npc死亡、关卡结束......）

end

return XLevelScript9000
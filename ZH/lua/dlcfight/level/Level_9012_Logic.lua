---@class XLevelScript9012_logic
local XLevelScript9012 = XDlcScriptManager.RegLevelLogicScript(9012, "XLevelLogicScript9012") --注册脚本类到管理器（逻辑脚本注册

---@param proxy XDlcCSharpFuncs
function XLevelScript9012:Ctor(proxy) --构造函数，用于执行与外部无关的内部构造逻辑（例如：创建内部变量等）
    self._proxy = proxy               --脚本代理对象，通过它来调用战斗程序开放的函数接口。
end

function XLevelScript9012:Init() --初始化逻辑
    --事件注册
...
function XLevelScript9012:Update(dt)       --每帧更新逻辑
    self._levelTime = self._levelTime + dt --记录关卡已进行时间
    self:OnUpdatePhase(dt)
end

function XLevelScript9012:InitPhase()
    --初始化关卡各个阶段的相关变量
end

---@param phase number
function XLevelScript9012:SetPhase(phase) --跳转关卡阶段
...
function XLevelScript9012:OnEnterPhase(phase)
    ...
    if phase == Phase.Show then --一开始默认在Start，所以只有Show阶段开始才会有Enter流程
        ...
        self._proxy:DispatchLuaEvent(ELuaEventTarget.Npc, EFightLuaEvent.RelinkSetAIActivate,
            { NpcUUid = self.monster_UUID, IsActivated = false }) --关闭白龙AI，做木桩
...
function XLevelScript9012:OnUpdatePhase(dt)
    ...
    elseif self._currentPhase == Phase.Battle then
        ...
        -- self.resetDamageTime = 10 - math.floor( self._levelTime - self._finalDamageTime )   --伤害统计
...
function XLevelScript9012:OnExitPhase(phase)
    --退出一个关卡阶段时需要做的事情在这里实现（最好不要在这里跳转关卡阶段
end

function XLevelScript9012:OnPhaseChanged(lastPhase, nextPhase)
    --关卡阶段改变时需要执行的逻辑，一般用于通知外部
end

function XLevelScript9012:HandlePhaseEvent(eventType, eventArgs)
...
function XLevelScript9012:HandleEvent(eventType, eventArgs) --事件响应逻辑
    ...
    if eventType == EWorldEvent.NpcAddBuff then
        if eventArgs.BuffTableId == 9012001 then                 --开启怪物AI
            XLog.Debug("开启怪物AI")
            self._proxy:DispatchLuaEvent(ELuaEventTarget.Npc,EFightLuaEvent.RelinkSetAIActivate, {NpcUUid=self.monster_UUID,IsActivated=true})  --开启白龙AI，做木桩
        end
...
function XLevelScript9012:CheckLevelEnd() --检查关卡结束
    ----胜利结算检测-----------------
...
function XLevelScript9012:Terminate() --脚本结束逻辑（脚本被卸载、Npc死亡、关卡结束......）

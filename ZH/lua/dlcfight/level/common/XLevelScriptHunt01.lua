...
function XLevelScriptHunt01:Update(dt)
    ...
    if self._currentLevelId == 0013 or self._currentLevelId == 0014 or self._currentLevelId == 0016 then
        if not self._jumpFunStarted then
            if self._proxy:CheckBuffByKind(self._npc, self._jumpFunStartSign) then
                ...
                --        self._proxy:RegisterEvent(EWorldEvent.ActorTrigger) -- tower.placeId, 3
    ...
    if not self._FightUIActive then
        if not self._proxy:CheckBuffByKind(self._npc, self._closeFightUISign) then
            Tool.SetFightUiActive(self._proxy, true)
    ...
    elseif self._FightUIActive then
        if self._proxy:CheckBuffByKind(self._npc, self._closeFightUISign) then
            Tool.SetFightUiActive(self._proxy, false)
...
function XLevelScriptHunt01:HandleEvent(eventType, eventArgs)
    ...
    --    print("XLevelBossFight1 ActorTriggerEvent:"
    ...
    if eventType == EWorldEvent.ActorTrigger and eventArgs.EnteredActorUUID == self._npc then
        print("0013 ClientScript handle trigger event:" .. tostring(eventArgs.EnteredActorUUID))
        if eventArgs.HostSceneObjectPlaceId == self._bossLockTrigger and eventArgs.TriggerId == 1 then --boss停留在场地中央等待的区域检测
        ...
        elseif self:_IsTower(eventArgs.HostSceneObjectPlaceId) and eventArgs.TriggerId == 3 then -- NpcLandTrigger 站上塔的触发器
            if eventArgs.TriggerState == ETriggerState.Enter then
                XLog.Debug("<color=#F0D800>[黑龙BOSS战本地模块]</color>玩家进入塔" .. eventArgs.HostSceneObjectPlaceId)
                self._playerLandTower = eventArgs.HostSceneObjectPlaceId
            elseif eventArgs.TriggerState == ETriggerState.Exit then
                XLog.Debug("<color=#F0D800>[黑龙BOSS战本地模块]</color>玩家退出塔" .. eventArgs.HostSceneObjectPlaceId)
                self._playerLandTower = nil
            end
        end
...
function XLevelScriptHunt01:GetNearestTower(towerIdSet)
    ...
    for i, id in pairs(towerIdSet) do
        local position = self._proxy:GetSceneObjectPositionByPlaceId(id)
...
function XLevelScriptHunt01:FakeLevelFinish()
    ...
    self._proxy:ApplyMagic(self._npc, self._npc, 200007, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200008, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200010, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200012, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200013, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200014, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200015, 1)
    self._proxy:ApplyMagic(self._npc, self._npc, 200016, 1)
    Tool.SetFightUiActive(self._proxy, false)
    self._timer:Schedule(8, nil, function() Tool.SetFightUiActive(self._proxy, true)  end)

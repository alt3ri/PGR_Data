--relink 木桩关
local XLevelScript9019 = XDlcScriptManager.RegLevelPresentScript(9019, "XLevelPresentScript9019")
local Timer = require("Level/Common/XTaskScheduler")


-- 脚本构造函数
---@param proxy XDlcCSharpFuncs
function XLevelScript9019:Ctor(proxy)
    self._proxy = proxy
    self._timer = Timer.New()
end

-- 初始化
function XLevelScript9019:Init()
...
function XLevelScript9019:HandleEvent(eventType, eventArgs)
end

-- 每帧执行
---@param dt number @ delta time
function XLevelScript9019:Update(dt)
...
-- 脚本终止
function XLevelScript9019:Terminate()

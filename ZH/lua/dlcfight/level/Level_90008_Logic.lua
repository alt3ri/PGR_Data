local XLevelScript90008 = XDlcScriptManager.RegLevelLogicScript(90008, "XLevelLogicScript90008") --注册脚本类到管理器（逻辑脚本注册
local XPlayerNpcContainer = require("Level/Common/XPlayerNpcContainer")
local Timer = require("Level/Common/XTaskScheduler")

---@param proxy XDlcCSharpFuncs
function XLevelScript90008:Ctor(proxy) --构造函数，用于执行与外部无关的内部构造逻辑（例如：创建内部变量等）
    self._proxy = proxy --脚本代理对象，通过它来调用战斗程序开放的函数接口。
end

function XLevelScript90008:Init() --初始化逻辑
    --事件注册
...
function XLevelScript90008:Update(dt) --每帧更新逻辑
    self._levelTime = self._levelTime + dt       --记录关卡已进行时间
    self:OnUpdatePhase(dt)
end
function XLevelScript90008:InitPhase()
    --初始化关卡各个阶段的相关变量
end
---@param phase number
function XLevelScript90008:SetPhase(phase)    --跳转关卡阶段
...
function XLevelScript90008:OnEnterPhase(phase)
    --进入一个关卡阶段时需要做的事情在这里实现（最好不要在这里跳转关卡阶段
    if phase == Phase.Show then                                                         --一开始默认在Start，所以只有Show阶段开始才会有Enter流程
        XLog.Debug("进入Show阶段")
        self._proxy:SetBBBoolean(1, self.monster_UUID, 8005001, false)                  --关闭白龙UI
...
---@param dt number @ delta time
function XLevelScript90008:OnUpdatePhase(dt)
...
function XLevelScript90008:OnExitPhase(phase)
    --退出一个关卡阶段时需要做的事情在这里实现（最好不要在这里跳转关卡阶段

end
function XLevelScript90008:OnPhaseChanged(lastPhase, nextPhase)
    --关卡阶段改变时需要执行的逻辑，一般用于通知外部
end
function XLevelScript90008:HandlePhaseEvent(eventType, eventArgs)
...
function XLevelScript90008:HandleEvent(eventType, eventArgs) --事件响应逻辑
  
end

function XLevelScript90008:CheckAllPlayerDead() --检查是否所有玩家都死亡了
    self._playerNpcList = self._proxy:GetPlayerNpcList() --获取玩家列表
...
function XLevelScript90008:CheckLevelEnd() --检查关卡结束
    ----胜利结算检测-----------------
...
function XLevelScript90008:LevelEnd(isPlayerWin)
...
function XLevelScript90008:Terminate() --脚本结束逻辑（脚本被卸载、Npc死亡、关卡结束......）

end

return XLevelScript90008
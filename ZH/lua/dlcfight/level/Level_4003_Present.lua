local XLevelScript4003 = XDlcScriptManager.RegLevelLogicScript(4003) --注册脚本类到管理器（逻辑脚本注册
...
function XLevelScript4003:Init() --初始化逻辑
    self._tempLevelSwitcherUUID = self._proxy:GetSceneObjectUUID(2)
    self._tempOperatePhotoWallUUID = self._proxy:GetSceneObjectUUID(1)
    self._tempOperateFrameGoodsUUID = self._proxy:GetSceneObjectUUID(5)
    self._playerCustomAppearanceEntryUUID = self._proxy:GetSceneObjectUUID(8)
    self._proxy:RegisterEvent(EWorldEvent.NpcInteractStart)
    self._lxyObjSwitchUUID = 400008                   --进入剧情副本入口1
    self._lxyObjSwitch1UUID = 400009                   --进入剧情副本入口2
end

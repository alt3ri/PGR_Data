...
---@class XBuffScript1015574 : XBuffBase
local XBuffScript1015574 = XDlcScriptManager.RegBuffScript(1015574, "XBuffScript1015574", Base)
--效果说明：每间隔10秒，触发一次护盾+30%的效果，持续5秒;
--1015592,释放技能后，1015550的触发时间提前1秒
--1015590，触发两次1015550后，基础触发时间缩短至5秒
function XBuffScript1015574:Init() --初始化
    Base.Init(self)
    ------------配置------------
    self.BuffTimer =  0 --初始计时器
    self.BuffId = 1015575   --持续5秒的护盾+30%的buffid
    ...
    self.ReduceTimeBuffid = 1015591 --和概率共用的叠层Buff
    ...
    ------------执行------------
    self.runeId = self.BuffId - 1015000 + 20000 - 1     --增伤宝珠Id
...
function XBuffScript1015574:Update(dt) --每帧执行
    Base.Update(self, dt)
    ------------执行------------
    ...
    if self.BuffTimer >= self.FireTime then    --达到触发时间触发
...
function XBuffScript1015574:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEvent(EWorldEvent.NpcCastActionAfter)         -- OnNpcCastSkillEvent
end

function XBuffScript1015574:OnNpcCastActionAfterEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort)
    --未持有技能减cd的buff直接返回
    ...
    --如果不是技能起手就返回
    local skillTypeTemp = self._proxy:GetActionType(skillId)
...
function XBuffScript1015574:HandleEvent(eventType, eventArgs)
    Base.HandleEvent(self, eventType, eventArgs)
end

function XBuffScript1015574:Terminate()
    Base.Terminate(self)
end

return XBuffScript1015574

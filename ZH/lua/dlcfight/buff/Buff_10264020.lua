...
function XBuff10264020:ScriptInit(isGainControl) --初始化
    ...
    self.wrestlePoint = 100                      --每x点拼刀获得1层坚毅
...
function XBuff10264020:OnLuaSpecialHit(eventArgs)
    --释放特殊命中时，获得坚毅、扣除对手体力
    if eventArgs._skillId ~= self._skillId then return end
    if eventArgs._launcherUUID ~= self._npcUUID then return end
    --获得坚毅
    local blockStacks = math.floor(self._proxy:GetNpcAttribValue(self._npcUUID, ETheatre6AttribType.WrestlePoint) /
    self.wrestlePoint)

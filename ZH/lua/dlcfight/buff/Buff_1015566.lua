...
---@class XBuffScript1015566 : XBuffBase
local XBuffScript1015566 = XDlcScriptManager.RegBuffScript(1015566, "XBuffScript1015566", Base)
--效果说明：每间隔10秒，触发一次冰伤+30%的效果，持续5秒;
--1015592,释放技能后，1015550的触发时间提前1秒
--1015590，触发两次1015550后，基础触发时间缩短至5秒
function XBuffScript1015566:Init() --初始化
    Base.Init(self)
    ------------配置------------
    self.BuffTimer =  0 --初始计时器
    self.BuffId = 1015567   --持续5秒的冰伤+30%的buffid
...
function XBuffScript1015566:Update(dt) --每帧执行
    Base.Update(self, dt)
    ------------执行------------
...
function XBuffScript1015566:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEvent(EWorldEvent.NpcCastActionAfter)         -- OnNpcCastSkillEvent
end

function XBuffScript1015566:OnNpcCastActionAfterEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort)
    --未持有技能减cd的buff直接返回
...
function XBuffScript1015566:HandleEvent(eventType, eventArgs)
    Base.HandleEvent(self, eventType, eventArgs)
end

function XBuffScript1015566:Terminate()
    Base.Terminate(self)
end

return XBuffScript1015566

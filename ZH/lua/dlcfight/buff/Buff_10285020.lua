...
---@class XBuffScript10285020 : XTheatre6SkillBase
local XBuffScript10285020 = XDlcScriptManager.RegBuffScript(10285020, "XBuffScript10285020", XTheatre6SkillBase)

--效果说明：
--· 【超算】属性>230点时，伤害额外提高20/25/30%攻击。

function XBuffScript10285020:ScriptInit(isGainControl) --初始化
    self.TargetSkill = self._skillId
    self.TargetCS = 230
    --self:LogError(".....初始化完成")
    self._damageMagicId = 10280036 --注册超算成功技1伤害id
    --self._proxy:Theatre6ChangeStaminaValue(self._npcUUID, -30, 0)
    if self._skillId == 10285021 then self._exDamageRate = 2000
    else if self._skillId == 10285022 then self._exDamageRate = 2500
...
function XBuffScript10285020:OnLuaSkillStart(eventArgs)
    ------------执行------------
...
function XBuffScript10285020:InitEventCallBackRegister()
    --按需求解除注释进行注册
    self._proxy:RegisterEventByTarget(EWorldEvent.NpcCalcDamageBefore, self._npcUUID)
end

function XBuffScript10285020:ChangeDamageBeforeCalc(eventArgs)

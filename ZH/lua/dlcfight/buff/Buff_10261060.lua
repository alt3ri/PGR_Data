...
function XBuffScript10261060:ScriptInit(isGainControl) --初始化
    self.isFollowHitDown = false                       --注册标记，是否跟随击倒
...
function XBuffScript10261060:OnLuaAffixHitDown(eventArgs)
    ...
    --如果开关已经打开，直接返回
    if self.isFollowHitDown then return end
    self.isFollowHitDown = true
end
...
function XBuffScript10261060:OnLuaSkillStart(eventArgs)
    ...
    if eventArgs._skillId == self._skillId then
        --1. 是否跟随击倒，获得怒火
        if self.isFollowHitDown then self._AngerController:CastStackBuff(self.angerRecover, self._npcUUID) end
        --2. 判断是否处于狂暴状态，额外造成击倒
        local isBurst = self._proxy:GetBuffStacks(self._npcUUID, self.burstBuffId) >= 1
        if isBurst then self._HitDownController:AddSkillCount(self.hitDownStack) 
        end
    end
    --无论是否是自己释放，都关闭跟随击倒的标记
    self.isFollowHitDown = false
end

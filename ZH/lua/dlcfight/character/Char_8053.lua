...
---首席指挥官角色脚本
---@class XChar8053 : XRelinkCharBase
local XChar8053 = XDlcScriptManager.RegCharScript(8053, "XChar8053", Base)

--[[function XChar8053:Init() -- 初始化
    Base.Init(self)
    self.Move = true
    self.Guangjian = true
...
function XChar8053:ScriptInit(isGainControl)
    self.Move = true
    self.Guangjian = true
    self.AISwitch = true -- AI总开关
    self.SkillSwitch = true  -- 技能释放开关
...
function XChar8053:InitEventCallBackRegister()
...
function XChar8053:HandleEvent(eventType, eventArgs)
    Base.HandleEvent(self, eventType, eventArgs)
end

function XChar8053:Update(dt)
    Base.Update(self, dt)
    ...
    if self.SkillSwitch == true  and self._proxy:CheckBuffByKind(self._uuid, 8053046) then -- 技能开关开启后执行以下
        self.SkillSwitch = false
        self.SkillSwitch2 = false
        self:XiShou()
    end

    if self.SkillSwitch == true and self._proxy:CheckBuffByKind(self._uuid, 8053045) then -- 技能开关开启后执行以下
        self.SkillSwitch = false
        self.SkillSwitch2 = false
        self:JiGuangSkill3()
    end
...
function XChar8053:OnNpcCastActionBeforeEvent(SkillId, LauncherId, TargetId, TargetSceneObjId, IsAbort)


end

function XChar8053:OnNpcCastActionAfterEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort)
...
--[[    if skillId == 805303 then
    ...
    if skillId == 805313 then
        ...
        self._proxy:AddTimerTask(4.3, function()--延迟5秒后，释放影牌技能
            self._proxy:ApplyMagic(self._uuid, self._uuid, 8053015, 1)
            if not self._proxy:CheckBuffByKind(self.GuangJian_UUID, 8053010) then
                self._proxy:LaunchMissile(self._uuid, self.GuangJian_UUID, 80531204, 80530512,1)
            end
        ...
        self._proxy:AddTimerTask(7.3, function()--延迟5秒后，释放影牌技能
            if not self._proxy:CheckBuffByKind(self.GuangJian_UUID, 8053010) then
                self._proxy:LaunchMissile(self._uuid, self.GuangJian_UUID, 80531204, 80530512,1)
            end
            self._proxy:ApplyMagic(self._uuid, self._uuid, 8053015, 1)
...
function XChar8053:ChouShouCheck() --获取左右触手uuid
...
function XChar8053:ClassCheck() --按职业区分目标
    XLog.Warning("开始职业检测")
    self.Tuuid = nil
    self.Cuuid = nil
    self.Huuid = nil
...
--------------------------------技能组逻辑---------------------------------------------
function XChar8053:JiGuangSkill3() --最终激光
    ...
    self.JiGangSkill1index3 = self.JiGangSkill1index3 + 1 -- 每次释放技能加一次序号
    ...
    if self.JiGangSkill1index3 > 3 then --序号大于3则返回1
...
function XChar8053:JiGuangSkill1() --激光技能组1
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8053:JiGuangSkill2() --激光技能组2
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
    ...
    if self.JiGangSkill1index2 > 4 then --序号大于2则返回1
...
function XChar8053:NormalSkill1() --常规技能组1
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
    ...
    elseif self.Skill1index1 == 4 then
        local juli = self._proxy:CheckNpcDistance(self.ChuShouLuuid,self.Target,15)
        if juli == true then
            self:ChuShouSkill2()
        else
...
function XChar8053:ODSkill1() --OD技能组
...
function XChar8053:ODJiHuo() --OD激活
    local Target = self.PlayUUID[1]
    self._proxy:CastActionToTarget(self._uuid,805323,Target) -- OD激活技能
end


function XChar8053:HeiQiuSkill1() --黑球技能组
...
function XChar8053:FuHua() --千子孵化
    ...
    self._proxy:AddTimerTask(1.5, function()--延迟4秒后
        ...
        self.QianZi_UUID = self._proxy:GenerateNpc(8056, QianZiCamp, QianZiPos, QianZiRota)
    end)
    self._proxy:AddTimerTask(13, function()--延迟13秒后
...
function XChar8053:XiShou() --千子吸收
    ...
    self._proxy:AddTimerTask(2, function()--延迟4秒后
        ...
        self._proxy:NpcDie(self.ChuShouLuuid)
        self._proxy:NpcDie(self.ChuShouRuuid)
    end)
...
function XChar8053:ChuShouSkill1() --左触手拍打
...
function XChar8053:ChuShouSkill2() --左触手横扫
...
function XChar8053:ChuShouSkill3() --左触手突刺
...
function XChar8053:ChuShouSkill4() --触手投掷
...
function XChar8053:ChuShouZhaoHuan() --召唤触手1
    local Target = self.PlayUUID[1]
    local monsterCamp= ENpcCampType.Camp2
    local ChuShouBornPos = {x = 26.73, y = 8.749, z = 22}
    local ChuShouBornPos2 = {x = 27, y = 8.749, z = 65}
    local ChuShouBornPos3 = {x = 35, y = 8.749, z = 18}
...
function XChar8053:OnNpcExitActionEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort) -- 技能释放完成时
    if launcherId ~= self._uuid then
        return
    end
...
function XChar8053:OnNpcAddBuffEvent(casterNpcUUID, npcUUID, buffId, buffKinds, buffUUId)
...
function XChar8053:OnNpcSkillActionKeyframeSendEvent(launcher, eventName, skillActionId, keyFrameId, skillId)
...
--[[    if (eventName == "HeBao1") then
        local poissonDiskPoints1 = self._proxy:PoissonDiscPoints(11,45,6)
        for xIndex1 = 1, #poissonDiskPoints1, 2 do
            local yIndex1 = xIndex1 + 1
            local Points1 = {x = poissonDiskPoints1[xIndex1]+23, y = 9.8, z = poissonDiskPoints1[yIndex1]+19}
            local ZiDanIndex1 = (math.random(5))
            if ZiDanIndex1 == 1 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531501,Points1, Points1,1)
           ]]--[[     XLog.Warning(string.format("采样坐标:{%f, %f}",
                        poissonDiskPoints[xIndex],
                        poissonDiskPoints[yIndex]))]]--[[
            elseif ZiDanIndex1 == 2 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531509,Points1, Points1,1)
            elseif ZiDanIndex1 == 3 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531510,Points1, Points1,1)
            elseif ZiDanIndex1 == 4 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531511,Points1, Points1,1)
            elseif ZiDanIndex1 == 5 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531512,Points1, Points1,1)
            end
        end
    end

    if (eventName == "HeBao2") then
        local poissonDiskPoints1 = self._proxy:PoissonDiscPoints(11,45,6)
        for xIndex1 = 1, #poissonDiskPoints1, 2 do
            local yIndex1 = xIndex1 + 1
            local Points1 = {x = poissonDiskPoints1[xIndex1]+36, y = 9.8, z = poissonDiskPoints1[yIndex1]+19}
            local ZiDanIndex1 = (math.random(5))
            if ZiDanIndex1 == 1 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531501,Points1, Points1,1)
                ]]--[[     XLog.Warning(string.format("采样坐标:{%f, %f}",
    ...
    if (eventName == "HeBao3") then
        local poissonDiskPoints1 = self._proxy:PoissonDiscPoints(11,45,6)
        for xIndex1 = 1, #poissonDiskPoints1, 2 do
            local yIndex1 = xIndex1 + 1
            local Points1 = {x = poissonDiskPoints1[xIndex1]+48, y = 9.8, z = poissonDiskPoints1[yIndex1]+19}
            local ZiDanIndex1 = (math.random(5))
            if ZiDanIndex1 == 1 then
                self._proxy:LaunchMissileFromPosToPos(self._uuid, 80530115, 80531501,Points1, Points1,1)
                ]]--[[     XLog.Warning(string.format("采样坐标:{%f, %f}",
...
function XChar8053:OnNpcOverDriveFull(targetUUID) -- OD已满事件
...
function XChar8053:OnNpcODBreakBefore(targetUUID)  -- Break前事件
    if targetUUID ~= self._uuid then
        return
    end
    local Target = self.PlayUUID[1]
    ...
    self.AISwitch = false
    self._proxy:AbortAction(self._uuid, true)
    self._proxy:CastActionToTarget(self._uuid,805324,Target)
...
function XChar8053:OnNpcODExitBreakAfter(targetUUID) -- 退出Break后
    if targetUUID ~= self._uuid then
        return
    end
    local Target = self.PlayUUID[1]
...
function XChar8053:OnNpcDamageEvent(launcherId, targetId, magicId, kind, physicalDamage, elementDamage, elementType, realDamage, isCritical, skillActionId, magicTags)
...
function XChar8053:OnNpcBrokenAfter(launcherUUID, targetUUID, magicId)
    local Target = self.PlayUUID[1]
    XLog.Warning("破韧成功")

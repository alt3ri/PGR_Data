...
---首席指挥官角色脚本
---@class XChar8101 : XRelinkCharBase
local XChar8101 = XDlcScriptManager.RegCharScript(8101, "XChar8101", Base)

--[[function XChar8101:Init() -- 初始化
    Base.Init(self)
    self.Move = true
    self.Guangjian = true
...
function XChar8101:ScriptInit(isGainControl)
    self.Move = true
    self.Guangjian = true
...
function XChar8101:InitEventCallBackRegister()
...
--[[function XChar8101:AfterDamageCalc(eventArgs)   -- 计算伤害后
...
function XChar8101:OnNpcCastActionBeforeEvent(SkillId, LauncherId, TargetId, TargetSceneObjId, IsAbort)


end

function XChar8101:OnNpcCastActionAfterEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort)
...
function XChar8101:ChouShouCheck() --获取左右触手uuid
...
function XChar8101:ClassCheck() --按职业区分目标
    XLog.Warning("开始职业检测")
    self.Tuuid = nil
    self.Cuuid = nil
    self.Huuid = nil
...
--------------------------------技能组逻辑---------------------------------------------
function XChar8101:JiGuangSkill3() --触手炼狱启动
...
function XChar8101:JiGuangSkill1() --激光技能组1
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8101:JiGuangSkill2() --激光技能组2
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8101:NormalSkill1() --常规技能组1
    local MaxThreatTarget = self._proxy:GetMaxThreatNpc(self._uuid)  -- 获取最大仇恨目标
...
function XChar8101:ODSkill1() --OD技能组
...
function XChar8101:ODJiHuo() --OD激活
    local Target = self.PlayUUID[1]
    self._proxy:CastActionToTarget(self._uuid,805323,Target) -- OD激活技能
end


function XChar8101:HeiQiuSkill1() --黑球技能组
...
function XChar8101:XiShou() --千子吸收
...
function XChar8101:ChuShouSkill1() --左触手拍打
...
function XChar8101:ChuShouSkill2() --左触手横扫
...
function XChar8101:ChuShouSkill3() --左触手突刺
...
function XChar8101:ChuShouSkill4() --触手投掷
...
--[[function XChar8101:ChuShouZhaoHuan() --召唤触手1
    local Target = self.PlayUUID[1]
...
function XChar8101:ChuShouLianYu() --触手炼狱
    local Target = self.PlayUUID[1]
    for i = 1,#self.PlayUUID, 1 do
...
function XChar8101:HandleLuaEvent(eventType, eventArgs)
    -- 响应AI开启和停止
...
function XChar8101:OnNpcExitActionEvent(skillId, launcherId, targetId, targetSceneObjId, isAbort) -- 技能释放完成时
    if launcherId ~= self._uuid then
        return
    end
...
function XChar8101:OnNpcAddBuffEvent(casterNpcUUID, npcUUID, buffId, buffKinds, buffUUId)
...
function XChar8101:OnNpcSkillActionKeyframeSendEvent(launcher, eventName, skillActionId, keyFrameId, skillId)
...
function XChar8101:OnNpcOverDriveFull(targetUUID) -- OD已满事件
...
function XChar8101:OnNpcODBreakBefore(targetUUID)  -- Break前事件
    if targetUUID ~= self._uuid then
        return
    end
    local Target = self.PlayUUID[1]
...
function XChar8101:OnNpcODExitBreakAfter(targetUUID) -- 退出Break后
    if targetUUID ~= self._uuid then
        return
    end
    local Target = self.PlayUUID[1]
...
function XChar8101:OnNpcDamageEvent(launcherId, targetId, magicId, kind, physicalDamage, elementDamage, elementType, realDamage, isCritical, skillActionId, magicTags)
...
function XChar8101:OnNpcBrokenAfter(launcherUUID, targetUUID, magicId)  --破韧
    local Target = self.PlayUUID[1]
    XLog.Warning("破韧成功")

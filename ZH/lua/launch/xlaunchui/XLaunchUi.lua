local Creator = function()
    ...
    function XUiLaunchUi:OnAwake()
        ...
        self.UiPanelDownloadTips = XUiPanelDownloadTips.New(self.UiDownloadTips.gameObject, self)
        self.UiPanelDownloadTips.GameObject.transform:SetSiblingIndex(10)
        self.UiPanelDownloadTips:RegisterGridClickCb(function(removeResSize) self:RefreshSizeShow(removeResSize) end)

        self.UiLoading:SetActiveEx(false)
        self.UiDownload:SetActiveEx(false)
        self.UiVideoPlay:SetActiveEx(false)
        self.UiPanelDownloadTips:SetActiveEx(false)
    ...
    function XUiLaunchUi:OnSelectBasic()
        ...
        self.UiPanelDownloadTips:SetAllGridDownloadFlag(false)
        self.UiPanelDownloadTips:SetAllGridClickLock(true)
    ...
    function XUiLaunchUi:OnSelectAllDlc()
        ...
        self.UiPanelDownloadTips:SetAllGridClickLock(false)
    ...
    function XUiLaunchUi:OnConfirmSelect()
        ...
        self.UiPanelDownloadTips:SetActiveEx(false)
        CS.XGameEventManager.Instance:Notify(CS.XEventId.EVENT_LAUNCH_DONE_DOWNLOAD_SELECT, isFullDownload, removeResIdList)
    ...
    function XUiLaunchUi:SetupDownloadSelect(args)
        ...
        self.UiPanelDownloadTips:SetActiveEx(true)
        ...
        self.BaseUpdateSize = baseUpdateSize
        self.AllUpdateSize = allUpdateSize
        self:RefreshSizeShow()
    ...
    function XUiLaunchUi:RefreshSizeShow(removeResSize)
        local targetAllSize = self.AllUpdateSize
        if removeResSize then
            targetAllSize = targetAllSize - removeResSize
        end
        
        local baseSize, baseUnit = GetSizeAndUnit(self.BaseUpdateSize)
        local allSize, allUnit = GetSizeAndUnit(targetAllSize)

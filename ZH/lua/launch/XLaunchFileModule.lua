...
local module_creator = function()
    ...
    function XLaunchFileModule.GetResSizeDic()
        return ResSizeDic
    end
            ...
            CsTool.WaitCoroutinePerFrame(downloader:Send(), function(isComplete)
                if not isComplete then
                    ...
                    if downloader.State ~= CS.XDownloaderState.Success then
                        ...
                        XLaunchConst.ShowStartErrorDialog("FileManagerInitFileTableDownloadError", CsApplication.Exit, function()
                            Loop()
    ...
    CheckIndexFile = function()
        ...
        CsTool.WaitCoroutine(downloader:Send(), function()
            if downloader.State ~= CS.XDownloaderState.Success then
                XLaunchConst.ShowStartErrorDialog("FileManagerInitVersionDownLoadError")
            else
                if ResFileType == RES_FILE_TYPE.LAUNCH_MODULE then
            ...
            if ResFileType == RES_FILE_TYPE.MATRIX_FILE and DlcIndexTable then
                ...
                for resId, dlcTable in pairs(DlcIndexTable) do
                    ...
                    for asset, info in pairs(dlcTable) do
                        ...
                        ResSizeDic[resId] = ResSizeDic[resId] + info[3]
                    end
                    DlcNeedFileMap[resId] = fileMap
                end
    ...
    ResolveResIndex = function()
        ...
        for _, info in pairs(CurrentFileTable) do
            if UpdateTable[info[1]] then
                ...
                --XLaunchConst.ShowStartErrorDialog("FileManagerInitFileTableUpdateTableError")
                --return
            else
    ...
    local InitFullDownloadForRemoveResId = function(removeResIds)
        UpdateTable = AllUpdateTable
        UpdateSize = AllUpdateSize
        ...
        local removeSize = 0
        if DlcIndexTable then
            for _, resId in pairs(removeResIds) do
                local dlcTable = DlcIndexTable[resId]
                if dlcTable then
                    for asset, info in pairs(dlcTable) do
        ...
        UpdateSize = UpdateSize - removeSize
    end
    ...
    local OnDoneSelect = function(isFullDownload, removeResIds)
        ...
        XLaunchDlcManager.SetRemoveResIdsRecord(isRemoveResIdsHasValue and removeResIds or "") -- 防止版本更新时切换与上一次下载选项不同的情况错误读取到removeResIds
        if isFullDownload then
            if isRemoveResIdsHasValue then
                InitFullDownloadForRemoveResId(removeResIds)
            else
                InitFullDownload()
            end
        else
    ...
    local AndroidBackgroundDownload = function()
        ...
        updateInfoCb = function()
            ...
            elseif state == DownloadState.FAIL or (state == DownloadState.NONE and waitTimeCnt > 20) then
                ...
                XLaunchConst.ShowStartErrorDialog("FileManagerInitFileTableDownloadError", exitCb, function()
                    RemoveEvents()
                    CheckIndexFile()
            ...
            CsTool.WaitCoroutinePerFrame(downloader:Send(), function(isComplete)
                if not isComplete then
                    ...
                    if downloader.State ~= CS.XDownloaderState.Success then
                        ...
                        XLaunchConst.ShowStartErrorDialog(errorCode, exitCb, function()
                            Loop()
                        end, CsApplication.GetText("Retry")) -- 重试
                        return
                    end
